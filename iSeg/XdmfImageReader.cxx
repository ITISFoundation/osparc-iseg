					...
					const std::string &tissue_dname, tissues_size_t **TissueSlices)
{
	...
	if (ReadContiguousMemory) {
		...
		try {
			cerr << "N = " << N
					 << ", bufferFloat.max_size() = " << bufferFloat.max_size() << endl;
		...
		if (reader.exists(tissue_dname)) {
			...
			if (type.compare("unsigned char") == 0) {
				...
				try {
					...
					cerr << "N = " << N
							 << ", bufferUChar.max_size() = " << bufferUChar.max_size()
							 << endl;
				...
				try {
					...
					// vector<float> bufferFloat(N);
					cerr << "N = " << N
							 << ", bufferUShort.max_size() = " << bufferUShort.max_size()
							 << endl;
				...
				try {
					...
					// vector<float> bufferFloat(N);
					cerr << "N = " << N
							 << ", bufferUInt.max_size() = " << bufferUInt.max_size() << endl;
					bufferUInt.resize(N);
				}
				catch (length_error &le) {
		...
		if (reader.exists(source_dname)) {
			...
			for (int k = 0; k < NumberOfSlices; k++, offset += slice_size) {
				ok =
						ok && reader.read(ImageSlices[k], offset, slice_size, source_dname);
			}
		}
		if (reader.exists(target_dname)) {
			bool ok = true;
		...
		if (reader.exists(tissue_dname)) {
			...
			for (int k = 0; k < NumberOfSlices; k++, offset += slice_size) {
				ok = ok &&
						 reader.read(TissueSlices[k], offset, slice_size, tissue_dname);
			}
		}
	}
	...
	while (!n.isNull()) {
		...
		if (e.tagName() == "Grid") {
			...
			if (type != QString("Uniform")) {
				cerr << "error, unsupported grid type: " << type.toAscii().data()
						 << endl;
				return 0;
			}
			...
			if (geometry.attribute(QString("Type")) != QString("ORIGIN_DXDYDZ") &&
			...
			if (topology.attribute(QString("Type")) != QString("3DCORECTMesh") &&
...
{
	const size_t N =
			(size_t)this->Width * (size_t)this->Height * (size_t)this->NumberOfSlices;
...
std::shared_ptr<ColorLookupTable> HDFImageReader::ReadColorLookup() const
{
	...
	if (reader.exists("/Lut")) {
		...
		if (!colors.empty()) {
			...
			if (version > 1) {
				std::cerr << "Error: could not load color lookup table. The file "
