...
namespace
{
	int _Read(HDF5Reader& reader, size_t NumberOfSlices, size_t Width, size_t Height, bool ReadContiguousMemory,
		...
		const std::string& tissue_dname, tissues_size_t** TissueSlices)
	{
		...
		if (ReadContiguousMemory)
		{
			...
			try
			{
				cerr << "N = " << N << ", bufferFloat.max_size() = " << bufferFloat.max_size() << endl;
			...
			if (reader.exists(tissue_dname))
			{
				...
				if (type.compare("unsigned char") == 0)
				{
					...
					try
					{
						...
						cerr << "N = " << N << ", bufferUChar.max_size() = " << bufferUChar.max_size() << endl;
					...
					for (int k = 0; k < NumberOfSlices; k++)
					{
						...
						for (int j = 0; j < Height; j++)
						{
							for (int i = 0; i < Width; i++, pos++)
							{
								TissueSlices[k][pos] = (tissues_size_t)bufferUChar[n];
								n++;
							}
						}
					}
				...
				else if (type.compare("unsigned short") == 0)
				{
					...
					try
					{
						// vector throws a length_error if resized above max_size
						// vector<float> bufferFloat(N);
						cerr << "N = " << N << ", bufferUShort.max_size() = " << bufferUShort.max_size() << endl;
					...
					for (int k = 0; k < NumberOfSlices; k++)
					{
						...
						for (int j = 0; j < Height; j++)
						{
							for (int i = 0; i < Width; i++, pos++)
							{
								TissueSlices[k][pos] = (tissues_size_t)bufferUShort[n];
								n++;
							}
						}
					}
				...
				else if (type.compare("unsigned int") == 0)
				{
					...
					try
					{
						// vector throws a length_error if resized above max_size
						// vector<float> bufferFloat(N);
						cerr << "N = " << N << ", bufferUInt.max_size() = " << bufferUInt.max_size() << endl;
						bufferUInt.resize(N);
					}
					catch (length_error &le)
					{
					...
					for (int k = 0; k < NumberOfSlices; k++)
					{
						...
						for (int j = 0; j < Height; j++)
						{
							for (int i = 0; i < Width; i++, pos++)
							{
								TissueSlices[k][pos] = (tissues_size_t)bufferUInt[n];
								n++;
							}
						}
					}
...
int XdmfImageReader::ParseXML()
{
	...
	QString msg;
	if (!inputDocument.setContent(inputContent, false, &msg)) // This function is not reentrant.
	{
		cerr << "error assigning content of " << FileName << ": " << msg.toAscii().data() << endl;
	}

	QDomElement root = inputDocument.documentElement();
	if (root.tagName() != "Xdmf")
	{
		cerr << "invalid root element: " << root.tagName().toAscii().data() << endl;
	}
	...
	while (!n.isNull())
	{
		...
		if (e.tagName() == "Grid")
		{
			...
			{
				cerr << "error, unsupported grid type: " << type.toAscii().data() << endl;
				return 0;
			}

			// Geometry
			QDomElement geometry = e.firstChildElement(QString("Geometry"));
			if (geometry.attribute(QString("Type")) != QString("ORIGIN_DXDYDZ") && geometry.attribute(QString("GeometryType")) != QString("ORIGIN_DXDYDZ"))
			{
			...
			if (topology.attribute(QString("Type")) != QString("3DCORECTMesh") && topology.attribute(QString("TopologyType")) != QString("3DCORECTMesh"))
			{
				cerr << "error, unsupported topology type..." << endl;
				return 0;
			}
			QStringList textList = topology.attribute(QString("Dimensions")).split(QString(" "));
			if (textList.size() != 3)
			{
			...
			for (int i2 = 0; i2 < attributeList.size(); ++i2)
			{
				...
				QDomElement dataitem = scalar.firstChildElement(QString("DataItem"));
				QString text = dataitem.text().trimmed();
				int idx = text.indexOf(":");
				QString datasetname = text.remove(0, idx + 1);
				cerr << "mapping " << name.toAscii().data() << " to " << datasetname.toAscii().data() << endl;
...
int XdmfImageReader::Read()
{
	...
	cerr << "restored current folder " << QDir::current().absolutePath().toAscii().data() << endl;
...
int HDFImageReader::ParseHDF()
{
	...
	dimensions.resize(3);
	ok = ok && (reader.read(dimensions, "/dimensions") != 0) && (dimensions.size() == 3);
	this->Width = dimensions[0];
	this->Height = dimensions[1];
	this->NumberOfSlices = dimensions[2];

	vector<float> pixelsize(3);
	ok = ok && (reader.read(pixelsize, "/pixelsize") != 0) && (pixelsize.size() == 3);
	for (unsigned short i = 0; i < 3; i++) PixelSize[i] = pixelsize[i];
	...
	if (reader.exists("/rotation"))
	{
		...
		ok = ok && (reader.read(rotation, "/rotation") != 0) && (rotation.size() == 9);
...
std::shared_ptr<ColorLookupTable> HDFImageReader::ReadColorLookup() const
{
	...
	if (reader.exists("/Lut"))
	{
		...
		if (!colors.empty())
		{
			...
			if (version > 1)
			{
				std::cerr << "Error: could not load color lookup table. The file format is newer than this iSEG.\n";
				return nullptr;
			}
			...
			for (auto name : colors)
			{
				if (ok && name.find("color") != std::string::npos)
				{
					...
					ok = ok && (reader.read(&index, folder_name + "/index") != 0);
					ok = ok && (reader.read(float_rgb, folder_name + "/rgb") != 0);
