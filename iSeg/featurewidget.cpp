  ...
  : QWidget1( parent, name, wFlags ),handler3D(hand3D)
{
	setToolTip(Format("Obtain information about the gray value and tissue distribution."
		"<br>"
		"A rectangular area is marked by pressing down the left mouse button and moving the mouse."));
	...
	vbox3->setFixedSize(3*vbox3->sizeHint().width(), vbox3->sizeHint().height());
...
void featurewidget::pt_moved(Point p)
{
	...
	lb_pt_value->setText(QString::number(handler3D->return_width()-1-p.px)+QString(":")+QString::number(p.py));
	lb_work_pt_value->setText(QString("(")+QString::number((handler3D->return_width()-1-p.px)*psize.high)+QString(":")+QString::number(p.py*psize.low)+QString(" mm)"));
	lb_grey_value->setText(QString::number(bmphand->bmp_pt(p)));
	lb_work_grey_value->setText(QString::number(bmphand->work_pt(p)));
	tissues_size_t tnr=bmphand->tissues_pt(handler3D->get_active_tissuelayer(), p);
	if(tnr==0)
		lb_tissuename->setText("- (0)");
	else
		lb_tissuename->setText(TissueInfos::GetTissueName(tnr)+QString(" (")+QString::number((int)tnr)+QString(")"));
