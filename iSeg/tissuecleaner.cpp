...
{
	if(c==map[c]) return c;
	else return map[c]=base_connection(map[c]);
}
...
bool TissueCleaner::Allocate()
{
	...
	volume=(int *)malloc(sizeof(int)*(unsigned)(width)*(unsigned)(height)*(unsigned)(n));
...
void TissueCleaner::ConnectedComponents()
{
	...
	for(unsigned short i=1;i<n;i++){
		i2=0;
		if(slices[i][i2]==slices[i-1][i2]) volume[i1]=base_connection(volume[i1-offset[2]]);
		else {
			map.push_back(newest);
	...
	for(unsigned long long i=0;i<offset[3];i++) volume[i]=base_connection(volume[i]);
...
{
	for(unsigned i=0;i<TISSUES_SIZE_MAX+1;i++) totvolumes[i]=0;
	...
	for(size_t i=0;i<map.size();i++) {
		if(map[i]==i) totvolumes[tissuemap[i]]+=volumes[i];
	}
}
...
void TissueCleaner::Clean(float ratio, unsigned minsize)
{
	...
	for(size_t i=0;i<map.size();i++) {
		if(volumes[i]<minsize&&volumes[i]<ratio*totvolumes[tissuemap[i]]) erasemap[i]=true;
	}
