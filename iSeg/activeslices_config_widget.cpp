...
ActiveSlicesConfig::ActiveSlicesConfig(SlicesHandler *hand3D, QWidget *parent, const char *name, Qt::WindowFlags wFlags)
: QDialog( parent, name,TRUE, wFlags ),handler3D(hand3D)
{
	...
	QObject::connect(sb_start,SIGNAL(valueChanged(int)),this,SLOT(startslice_changed(int)));
	QObject::connect(sb_end,SIGNAL(valueChanged(int)),this,SLOT(endslice_changed(int)));
