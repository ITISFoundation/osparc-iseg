...
AtlasViewer::AtlasViewer(float *bmpbits1, tissues_size_t *tissue1, unsigned char orient1, 
		unsigned short dimx1, unsigned short dimy1, unsigned short dimz1, float dx1, float dy1, float dz1, 
		std::vector<float> *r, std::vector<float> *g, std::vector<float> *b, QWidget *parent, const char *name, Qt::WindowFlags wFlags )
...
void AtlasViewer::update(QRect rect) {
	...
	if(newwidth!=width||newheight!=height||newdx!=pixelsize.high||newdy!=pixelsize.low){
		width=newwidth;
		height=newheight;
...
{
	if(orient==0) {
		if(slicenr>=dimx) slicenr=dimx-1;
		unsigned pos=0;
		...
		if(slicenr>=dimy) slicenr=dimy-1;
		unsigned pos=0;
		unsigned pos1=unsigned(slicenr)*width;
		for(unsigned short i=0;i<height;i++,pos1+=unsigned(dimy-1)*dimx) {
		...
		if(slicenr>=dimz) slicenr=dimz-1;
		unsigned pos=0;
...
void AtlasViewer::reload_bits()
{
	...
	for(int y=height-1;y>=0;y--){
		for(int x=0;x<width;x++){
			f=(int)max(0.0f,min(255.0f,scaleoffset+scalefactor*(current_bmpbits)[pos]));
			if(current_tissue[pos]==0)
				image.setPixel(x,y,qRgb(int(f),int(f),int(f)));
			else
				image.setPixel(x,y,qRgb(int(f+tissueopac*(255.0f*((*color_r)[current_tissue[pos]-1])-f)),int(f+tissueopac*(255.0f*((*color_g)[current_tissue[pos]-1])-f)),int(f+tissueopac*(255.0f*((*color_b)[current_tissue[pos]-1])-f))));
			pos++;
		}
	}
...
void AtlasViewer::zoom_in()
{
	set_zoom(2*zoom);
}

void AtlasViewer::zoom_out()
{
	set_zoom(0.5*zoom);
}

void AtlasViewer::unzoom()
{
	set_zoom(1.0);
}

double AtlasViewer::return_zoom()
{
	return zoom;
}
...
{
	slicenr=slicenr1;
	if(orient==0&&slicenr>=dimx) slicenr=dimx-1;
	else if(orient==1&&slicenr>=dimy) slicenr=dimy-1;
	else if(orient==2&&slicenr>=dimz) slicenr=dimz-1;
	update();
}
