...
void GPUMC::renderScene(int windowWidth, int windowHeight, int xRotw, int yRotw,
												int zRotw, float zoom, float red, float green,
												float blue, int op)
{
	if (extractSurfaceOnEveryFrame || extractSurface)
	{
		...
		*/
		...
		{
			implementation->queue.enqueueReadImage(
					implementation->images[implementation->images.size() - 1], CL_FALSE,
					implementation->origin, implementation->region, 0, 0, sum);
		}
		else
		{
			implementation->queue.enqueueReadBuffer(
					implementation->buffers[implementation->buffers.size() - 1], CL_FALSE,
					0, sizeof(int) * 8, sum);
		}
		...
		glBufferData(GL_ARRAY_BUFFER, totalSum * 18 * sizeof(cl_float), NULL,
								 GL_STATIC_DRAW);
	...
	glScalef(zoom * implementation->scalingFactor.x,
					 zoom * implementation->scalingFactor.y,
					 zoom * implementation->scalingFactor.z);
	glTranslatef(implementation->translation.x, implementation->translation.y,
							 implementation->translation.z);
...
void GPUMC::setupOpenGL(int size, int sizeX, int sizeY, int sizeZ,
...
void GPUMC::setupOpenGL(int *argc, char **argv, int size, int sizeX, int sizeY,
												int sizeZ, float spacingX, float spacingY,
												float spacingZ)
{
...
int GPUMC::prepareDataset(uchar **voxels, int sizeX, int sizeY, int sizeZ)
{
	...
	for (int x = 0; x < sizeX; x++)
	{
		for (int y = 0; y < sizeY; y++)
		{
			...
			{
				newVoxels[x + y * size + z * size * size] =
						voxels[0][x + y * sizeX + z * sizeX * sizeY];
			}
		}
	}
	delete[] voxels[0];
...
bool GPUMC::setupOpenCL(uchar *voxels, int size)
{
	...
	try
	{
		...
		std::vector<Device> devices =
				implementation->context.getInfo<CL_CONTEXT_DEVICES>();
		...
		if ((int)devices[0].getInfo<CL_DEVICE_EXTENSIONS>().find(
		...
		{
			std::cout << "Writing to 3D textures is not supported on this device. "
									 "Writing to regular buffers instead."
								<< std::endl;
		...
		catch (Error error)
		{
			...
			{
				std::cout << "Build log:\t"
									<< implementation->program.getBuildInfo<CL_PROGRAM_BUILD_LOG>(
												 devices[0])
									<< std::endl;
			}
			throw error;
		...
		if (writingTo3DTextures)
		{
			...
			implementation->images.push_back(
					Image3D(implementation->context, CL_MEM_READ_WRITE,
									ImageFormat(CL_RGBA, CL_UNSIGNED_INT8), bufferSize,
									bufferSize, bufferSize));
			bufferSize /= 2;
			implementation->images.push_back(
					Image3D(implementation->context, CL_MEM_READ_WRITE,
									ImageFormat(CL_R, CL_UNSIGNED_INT8), bufferSize, bufferSize,
									bufferSize));
			bufferSize /= 2;
			// And the third, fourth and fifth INT16
			implementation->images.push_back(
					Image3D(implementation->context, CL_MEM_READ_WRITE,
									ImageFormat(CL_R, CL_UNSIGNED_INT16), bufferSize, bufferSize,
									bufferSize));
			bufferSize /= 2;
			implementation->images.push_back(
					Image3D(implementation->context, CL_MEM_READ_WRITE,
			...
			for (int i = 5; i < (log2((float)contour->m_Size)); i++)
			{
				...
				implementation->images.push_back(
						Image3D(implementation->context, CL_MEM_READ_WRITE,
										ImageFormat(CL_R, CL_UNSIGNED_INT32), bufferSize,
										bufferSize, bufferSize));
				bufferSize /= 2;
			}
		...
		else
		{
			...
			implementation->buffers.push_back(Buffer(implementation->context,
																							 CL_MEM_READ_WRITE,
			...
			implementation->cubeIndexesBuffer = Buffer(
					implementation->context, CL_MEM_WRITE_ONLY,
					sizeof(char) * contour->m_Size * contour->m_Size * contour->m_Size);
			implementation->cubeIndexesImage =
					Image3D(implementation->context, CL_MEM_READ_ONLY, //CL_MEM_READ_ONLY
									ImageFormat(CL_R, CL_UNSIGNED_INT8), contour->m_Size,
									contour->m_Size, contour->m_Size);
		}

		// Transfer dataset to device
		implementation->rawData =
				Image3D(implementation->context,
								CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, //CL_MEM_READ_ONLY,//
								ImageFormat(CL_R, CL_UNSIGNED_INT8), contour->m_Size,
								contour->m_Size, contour->m_Size, 0, 0, voxels);
		delete[] voxels;

		// Make kernels
		implementation->constructHPLevelKernel =
				Kernel(implementation->program, "constructHPLevel");
		implementation->classifyCubesKernel =
				Kernel(implementation->program, "classifyCubes");
		implementation->traverseHPKernel =
				Kernel(implementation->program, "traverseHP");

		if (!writingTo3DTextures)
		{
			implementation->constructHPLevelCharCharKernel =
					Kernel(implementation->program, "constructHPLevelCharChar");
			implementation->constructHPLevelCharShortKernel =
					Kernel(implementation->program, "constructHPLevelCharShort");
			implementation->constructHPLevelShortShortKernel =
					Kernel(implementation->program, "constructHPLevelShortShort");
			implementation->constructHPLevelShortIntKernel =
					Kernel(implementation->program, "constructHPLevelShortInt");
		}
	}
	catch (Error error)
	{
...
void GPUMC::histoPyramidConstruction()
{
	...
	if (writingTo3DTextures)
	{
		...
		implementation->queue.enqueueNDRangeKernel(
				implementation->constructHPLevelKernel, NullRange,
		...
		{
			implementation->constructHPLevelKernel.setArg(0,
																										implementation->images[i]);
			implementation->constructHPLevelKernel.setArg(
					1, implementation->images[i + 1]);
			previous /= 2;
			implementation->queue.enqueueNDRangeKernel(
					implementation->constructHPLevelKernel, NullRange,
					NDRange(previous, previous, previous), NullRange);
		}
	}
	else
	{
		// Run base to first level
		implementation->constructHPLevelCharCharKernel.setArg(
				0, implementation->buffers[0]);
		implementation->constructHPLevelCharCharKernel.setArg(
				1, implementation->buffers[1]);

		implementation->queue.enqueueNDRangeKernel(
				implementation->constructHPLevelCharCharKernel, NullRange,
				NDRange(contour->m_Size / 2, contour->m_Size / 2, contour->m_Size / 2),
				NullRange);

		int previous = contour->m_Size / 2;

		implementation->constructHPLevelCharShortKernel.setArg(
				0, implementation->buffers[1]);
		implementation->constructHPLevelCharShortKernel.setArg(
				1, implementation->buffers[2]);

		implementation->queue.enqueueNDRangeKernel(
				implementation->constructHPLevelCharShortKernel, NullRange,
				NDRange(previous / 2, previous / 2, previous / 2), NullRange);

		previous /= 2;

		implementation->constructHPLevelShortShortKernel.setArg(
				0, implementation->buffers[2]);
		implementation->constructHPLevelShortShortKernel.setArg(
				1, implementation->buffers[3]);

		implementation->queue.enqueueNDRangeKernel(
				implementation->constructHPLevelShortShortKernel, NullRange,
				NDRange(previous / 2, previous / 2, previous / 2), NullRange);

		previous /= 2;

		implementation->constructHPLevelShortShortKernel.setArg(
				0, implementation->buffers[3]);
		implementation->constructHPLevelShortShortKernel.setArg(
				1, implementation->buffers[4]);
		...
		implementation->constructHPLevelShortIntKernel.setArg(
				0, implementation->buffers[4]);
		implementation->constructHPLevelShortIntKernel.setArg(
				1, implementation->buffers[5]);

		implementation->queue.enqueueNDRangeKernel(
				implementation->constructHPLevelShortIntKernel, NullRange,
				NDRange(previous / 2, previous / 2, previous / 2), NullRange);
		...
		{
			implementation->constructHPLevelKernel.setArg(0,
																										implementation->buffers[i]);
			implementation->constructHPLevelKernel.setArg(
					1, implementation->buffers[i + 1]);
			previous /= 2;
			implementation->queue.enqueueNDRangeKernel(
					implementation->constructHPLevelKernel, NullRange,
					NDRange(previous, previous, previous), NullRange);
...
void GPUMC::updateScalarField()
{
	if (writingTo3DTextures)
	{
		...
		implementation->queue.enqueueNDRangeKernel(
				implementation->classifyCubesKernel, NullRange,
				NDRange(contour->m_Size, contour->m_Size, contour->m_Size), NullRange);
	}
	else
	{
		implementation->classifyCubesKernel.setArg(0, implementation->buffers[0]);
		implementation->classifyCubesKernel.setArg(
				1, implementation->cubeIndexesBuffer);
		...
		implementation->queue.enqueueCopyBufferToImage(
				implementation->cubeIndexesBuffer, implementation->cubeIndexesImage, 0,
				offset, region);
	}
}
...
void GPUMC::histoPyramidTraversal(int sum)
{
	...
	else
	{
		...
		implementation->traverseHPKernel.setArg(1,
																						implementation->cubeIndexesImage);
		for (i = 0; i < implementation->buffers.size(); i++)
		{
			implementation->traverseHPKernel.setArg(i + 2,
																							implementation->buffers[i]);
		}
		i += 2;
	}
	...
	implementation->queue.enqueueNDRangeKernel(
			implementation->traverseHPKernel, NullRange, NDRange(global_work_size),
			NDRange(64));
...
void GPUMC::setIso(int iso) { contour->isolevel = iso; }
