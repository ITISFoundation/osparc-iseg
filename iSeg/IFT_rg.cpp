...
IFTrg_widget::IFTrg_widget(SlicesHandler *hand3D, QWidget *parent, const char *name, Qt::WindowFlags wFlags)
  : QWidget1( parent, name, wFlags ),handler3D(hand3D)
{
	setToolTip(Format("Segment multiple tissues by drawing lines in the current slice based on the Image Foresting Transform. "
		"These lines are drawn with the color of the currently selected tissue. Multiple lines of different colours can be drawn "
		"and they are subsequently used as seeds to grow regions based on a local homogeneity criterion. Through competitive growing the best boundaries "
		"between regions grown from lines with different colours are identified."
		"<br>"
		"The result is stored in the Target. To assign a segmented region to a tissue the 'Adder' must be used."));
	...
	pushremove->setToolTip(Format("Remove Line followed by a click on a line deletes "
		"this line and automatically updates the segmentation. If Remove Line has "
		"been pressed accidentally, a second press will deactivate the function again."));
	...
	QObject::connect(sl_thresh,SIGNAL(sliderMoved(int)),this,SLOT(slider_changed(int)));
	QObject::connect(sl_thresh,SIGNAL(sliderPressed()),this,SLOT(slider_pressed()));
	QObject::connect(sl_thresh,SIGNAL(sliderReleased()),this,SLOT(slider_released()));
}
...
IFTrg_widget::~IFTrg_widget()
{
	...
	if(IFTrg!=NULL) delete IFTrg;
	if(lbmap!=NULL) delete lbmap;
	return;
}
...
void IFTrg_widget::execute()
{
	...
	for(unsigned i=0;i<area;i++) {
		if(f2[i]<thresh) work_bits[i]=f1[i]*d;
		else work_bits[i]=0;
	}
	sl_thresh->setEnabled(true);
...
void IFTrg_widget::getrange()
{
	...
	if(thresh>maxthresh||thresh==0) thresh=maxthresh;
	if(maxthresh==0) maxthresh=thresh=1;
	sl_thresh->setValue(min(int(thresh*100/maxthresh),100));
	
	return;
}

QSize IFTrg_widget::sizeHint() const
{
...
void IFTrg_widget::slider_released()
{
	emit end_datachange(this);
}
...
{
	if(version>=2) {
		QObject::disconnect(sl_thresh,SIGNAL(sliderMoved(int)),this,SLOT(slider_changed(int)));
