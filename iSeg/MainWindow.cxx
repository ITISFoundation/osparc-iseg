...
bool read_grouptissues(const char *filename, vector<tissues_size_t> &olds, vector<tissues_size_t> &news)
{
	FILE *fp;
...
bool read_grouptissuescapped(const char *filename, vector<tissues_size_t> &olds, vector<tissues_size_t> &news, bool fail_on_unknown_tissue)
{
	FILE *fp;
	if ((fp = fopen(filename, "r")) == NULL)
	{
	...
	else
	{
		...
		else
		{
			...
			{
				ok = false;
				if (type1 == 0 || type1 > tissueCount) std::cerr << "old: " << name1 << " not in tissue list\n";
...
MainWindow::MainWindow(SlicesHandler *hand3D, QString locationstring, QDir picpath, QDir tmppath, bool editingmode, QWidget *parent, const char *name, Qt::WindowFlags wFlags, char **argv)
	: QMainWindow(parent, name, wFlags)
{
	...
	setCaption(QString(" iSeg ") + QString::number(iSegVersion) + QString(".") + QString::number(iSegSubversion) + QString(" B") + QString::number(build) + QString(" - No Filename"));
	...
	bmp_show =
	    new bmptissuemarklineshower(this, "new window", Qt::WDestructiveClose | Qt::WResizeNoErase);
	...
	modifyTissueFolder->setToolTip(Format("Edit the selected tissue or folder properties."));
	//xxxb	modifyTissue->setFixedWidth(110);
	removeTissueFolder = new QPushButton("Del. Tissue/Folder", this);
	removeTissueFolder->setToolTip(Format("Remove the selected tissues or folders."));
	...
	tissue3Dopt->setToolTip(Format("'Get Tissue' is applied in 3D or current slice only."));
	getTissue = new QPushButton("Get Tissue", this);
	getTissue->setToolTip(Format("Get tissue creates a mask in the Target from the currently selected Tissue."));
	getTissueAll = new QPushButton("All", this);
	getTissueAll->setFixedWidth(30);
	getTissueAll->setToolTip(Format("Get all tissue creates a mask in the Target from all Tissue excluding the background."));
	//xxxb	getTissue->setFixedWidth(110);
	clearTissue = new QPushButton("Clear Tissue", this);
	clearTissue->setToolTip(Format("Clears the currently selected tissue (use '3D' option to clear whole tissue or just the current slice)."));
	//xxxb	clearTissue->setFixedWidth(110);
	clearTissues = new QPushButton("All", this);
	clearTissues->setFixedWidth(30);
	clearTissues->setToolTip(Format("Clears all tissues (use '3D' option to clear the entire segmentation or just the current slice)."));
	//xxxb	clearTissues->setFixedWidth(110);

	cb_addsub3d = new QCheckBox("3D", this);
	cb_addsub3d->setToolTip(Format("Apply add/remove actions in 3D or current slice only."));
	cb_addsuboverride = new QCheckBox("Override", this);
	cb_addsuboverride->setToolTip(Format("If override is off, Tissue voxels which are already assigned will not be modified. Override allows to change these voxels, unless they are locked."));
	cb_addsubconn = new QCheckBox("Connected", this);
	cb_addsubconn->setToolTip(Format("Only the connected image region is added/removed."));
	//	vboxtissueadder2->addStretch();
	pb_add = new QPushButton("+", this);
	pb_add->setToggleButton(true);
	pb_add->setToolTip(Format("Adds next selected/picked Target image region to current tissue."));
	pb_sub = new QPushButton("-", this);
	pb_sub->setToggleButton(true);
	pb_sub->setToolTip(Format("Removes next selected/picked Target image region from current tissue."));
	pb_addhold = new QPushButton("++", this);
	pb_addhold->setToggleButton(true);
	pb_addhold->setToolTip(Format("Adds selected/picked Target image regions to current tissue."));
	...
	pb_subhold->setToolTip(Format("Removes selected/picked Target image regions from current tissue."));
	...
	scb_slicenr = new QScrollBar(1, (int)handler3D->return_nrslices(), 1, 5, 1, Qt::Horizontal, this);
	...
	lb_slicenr = new QLabel(QString(" of ") +
	                        QString::number((int)handler3D->return_nrslices()), this);
	...
	threshold_widget = new thresh_widget(handler3D, this, "new window", Qt::WDestructiveClose | Qt::WResizeNoErase);
	tabwidgets.push_back(threshold_widget);
	hyst_widget = new hyster_widget(handler3D, this, "new window", Qt::WDestructiveClose | Qt::WResizeNoErase);
	tabwidgets.push_back(hyst_widget);
	lw_widget = new livewire_widget(handler3D, this, "new window", Qt::WDestructiveClose | Qt::WResizeNoErase);
	tabwidgets.push_back(lw_widget);
	iftrg_widget = new IFTrg_widget(handler3D, this, "new window", Qt::WDestructiveClose | Qt::WResizeNoErase);
	tabwidgets.push_back(iftrg_widget);
	FMF_widget = new FastMarchFuzzy_widget(handler3D, this, "new window", Qt::WDestructiveClose | Qt::WResizeNoErase);
	tabwidgets.push_back(FMF_widget);
	wshed_widget = new watershed_widget(handler3D, this, "new window", Qt::WDestructiveClose | Qt::WResizeNoErase);
	tabwidgets.push_back(wshed_widget);
	OutlineCorrect_widget = new OutlineCorr_widget(handler3D, this, "new window", Qt::WDestructiveClose | Qt::WResizeNoErase);
	tabwidgets.push_back(OutlineCorrect_widget);
	interpolwidget = new interpol_widget(handler3D, this, "new window", Qt::WDestructiveClose | Qt::WResizeNoErase);
	tabwidgets.push_back(interpolwidget);
	smoothing_widget = new smooth_widget(handler3D, this, "new window", Qt::WDestructiveClose | Qt::WResizeNoErase);
	tabwidgets.push_back(smoothing_widget);
	morph_widget = new morpho_widget(handler3D, this, "new window", Qt::WDestructiveClose | Qt::WResizeNoErase);
	tabwidgets.push_back(morph_widget);
	edge_widg = new edge_widget(handler3D, this, "new window", Qt::WDestructiveClose | Qt::WResizeNoErase);
	tabwidgets.push_back(edge_widg);
	feature_widget = new featurewidget(handler3D, this, "new window", Qt::WDestructiveClose | Qt::WResizeNoErase);
	tabwidgets.push_back(feature_widget);
	measurement_widget = new measure_widget(handler3D, this, "new window", Qt::WDestructiveClose | Qt::WResizeNoErase);
	tabwidgets.push_back(measurement_widget);
	vesselextr_widget = new vessel_widget(handler3D, this, "new window", Qt::WDestructiveClose | Qt::WResizeNoErase);
	tabwidgets.push_back(vesselextr_widget);
	pickerwidget = new picker_widget(handler3D, this, "new window", Qt::WDestructiveClose | Qt::WResizeNoErase);
	tabwidgets.push_back(pickerwidget);
	transfrmWidget = new TransformWidget(handler3D, this, "new window", Qt::WDestructiveClose | Qt::WResizeNoErase);
	...
	for (auto a : addons)
	{
		...
		tabwidgets.push_back(a->CreateWidget(this, "new window", Qt::WDestructiveClose | Qt::WResizeNoErase));
	}
...
//		methodTab->addTab(tabwidgets[i],tabwidgets[i]->GetName().c_str());
...

	{
		...
		while (posi < nrtabbuttons && !showpb_tab[posi]) posi++;
		if (posi < nrtabbuttons) methodTab->raiseWidget(tabwidgets[posi]);
	}
...
//	qsmax.setWidth(0);
	...
	scalewidget = new ScaleWork(handler3D, m_picpath, this, "new window", Qt::WDestructiveClose | Qt::WResizeNoErase);
	imagemathwidget = new ImageMath(handler3D, this, "new window", Qt::WDestructiveClose | Qt::WResizeNoErase);
	imageoverlaywidget = new ImageOverlay(handler3D, this, "new window", Qt::WDestructiveClose | Qt::WResizeNoErase);
	...
	m_MultiDataset_widget = new MultiDataset_widget(handler3D, this, "multi dataset window", Qt::WDestructiveClose | Qt::WResizeNoErase);
	...
	if (handler3D->return_startslice() >= slicenr || handler3D->return_endslice() + 1 <= slicenr)
	{
...

	{
		...
		if (vboxtabs2w->sizeHint().width() > widthsuggest) widthsuggest = vboxtabs2w->sizeHint().width();
	...
	tabswdock->setAllowedAreas(Qt::TopDockWidgetArea |
	...
	notesdock->setAllowedAreas(Qt::TopDockWidgetArea |
	                           Qt::BottomDockWidgetArea);
	...
	zoomdock->setAllowedAreas(Qt::LeftDockWidgetArea |
	...
	tissueadddock->setToolTip(Format("The tissue adder provides functionality "
		"to add/remove an object selected/picked in the Target to/from the selected tissue."
	...
	if (!m_editingmode)
	{
		file->insertItem(QIcon(m_picpath.absFilePath(QString("filenew.png")).ascii()), "&New...", this, SLOT(execute_new()));
	...
	reloadmenu = new Q3PopupMenu(this, "reloadmenu");//xxxa add reload function for s4llink data later
	...
	if (!m_editingmode)
	{
		...
		connect(importS4LLink, SIGNAL(triggered()), this, SLOT(execute_loads4llivelink()));
		importS4LLink->setToolTip("Loads a Sim4Life live link file (the h5 file, which is part of an iSEG project).");
	...
	importSurface->setToolTip("Some data must be opened first to load the surface on top of the existing project.");
	...
	file->insertItem(QIcon(m_picpath.absFilePath(QString("filesave.png")).ascii()), "Save Pro&ject", this, SLOT(execute_saveproj()), QKeySequence("Ctrl+S"));
	file->insertItem("Save Active Slices...", this, SLOT(execute_saveactiveslicesas()));
	if (!m_editingmode)
		file->insertItem(QIcon(m_picpath.absFilePath(QString("fileopen.png")).ascii()), "Open P&roject...", this, SLOT(execute_loadproj()));
	file->insertSeparator();
	...
	file->insertItem("Export Surface Generation Tool XML...", this, SLOT(execute_exportsurfacegenerationtoolxml()));
#endif
	file->insertSeparator();
	if (!m_editingmode)
	{
	...
	imagemenu->insertItem(QIcon(m_picpath.absFilePath(QString("histo.png")).ascii()), "&Histogram...", this, SLOT(execute_histo()));
	...
	imagemenu->insertItem("3D surface view", this, SLOT(execute_3Dsurfaceviewer()));
	imagemenu->insertItem("3D isosurface view", this, SLOT(execute_3Dsurfaceviewerbmp()));
	imagemenu->insertItem("3D volume view source", this, SLOT(execute_3Dvolumeviewerbmp()));
	imagemenu->insertItem("3D volume view tissue", this, SLOT(execute_3Dvolumeviewertissue()));
	if (!m_editingmode)
	{
		//xxxa;
		imagemenu->insertSeparator();
	...
	undonr = editmenu->insertItem(QIcon(m_picpath.absFilePath(QString("undo.png")).ascii()), "&Undo", this, SLOT(execute_undo()));
	redonr = editmenu->insertItem(QIcon(m_picpath.absFilePath(QString("redo.png")).ascii()), "Redo", this, SLOT(execute_redo()));
	editmenu->insertSeparator();
	editmenu->insertItem("&Configure Undo...", this, SLOT(execute_undoconf()));
	editmenu->insertItem("&Active Slices...", this, SLOT(execute_activeslicesconf()));
	...
	m_atlasfilename.atlasnr[0] = atlasmenu->insertItem("", this, SLOT(execute_loadatlas0()));
	m_atlasfilename.atlasnr[1] = atlasmenu->insertItem("", this, SLOT(execute_loadatlas1()));
	m_atlasfilename.atlasnr[2] = atlasmenu->insertItem("", this, SLOT(execute_loadatlas2()));
	m_atlasfilename.atlasnr[3] = atlasmenu->insertItem("", this, SLOT(execute_loadatlas3()));
	m_atlasfilename.atlasnr[4] = atlasmenu->insertItem("", this, SLOT(execute_loadatlas4()));
	m_atlasfilename.atlasnr[5] = atlasmenu->insertItem("", this, SLOT(execute_loadatlas5()));
	m_atlasfilename.atlasnr[6] = atlasmenu->insertItem("", this, SLOT(execute_loadatlas6()));
	m_atlasfilename.atlasnr[7] = atlasmenu->insertItem("", this, SLOT(execute_loadatlas7()));
	m_atlasfilename.atlasnr[8] = atlasmenu->insertItem("", this, SLOT(execute_loadatlas8()));
	m_atlasfilename.atlasnr[9] = atlasmenu->insertItem("", this, SLOT(execute_loadatlas9()));
	m_atlasfilename.atlasnr[10] = atlasmenu->insertItem("", this, SLOT(execute_loadatlas10()));
	m_atlasfilename.atlasnr[11] = atlasmenu->insertItem("", this, SLOT(execute_loadatlas11()));
	m_atlasfilename.atlasnr[12] = atlasmenu->insertItem("", this, SLOT(execute_loadatlas12()));
	m_atlasfilename.atlasnr[13] = atlasmenu->insertItem("", this, SLOT(execute_loadatlas13()));
	m_atlasfilename.atlasnr[14] = atlasmenu->insertItem("", this, SLOT(execute_loadatlas14()));
	m_atlasfilename.atlasnr[15] = atlasmenu->insertItem("", this, SLOT(execute_loadatlas15()));
	m_atlasfilename.atlasnr[16] = atlasmenu->insertItem("", this, SLOT(execute_loadatlas16()));
	...
	m_atlasfilename.atlasnr[19] = atlasmenu->insertItem("", this, SLOT(execute_loadatlas19()));
	...
	helpmenu->insertItem(QIcon(m_picpath.absFilePath(QString("help.png")).ascii()), "About", this, SLOT(execute_about()));

	QObject::connect(toworkBtn, SIGNAL(clicked()), this, SLOT(execute_bmp2work()));
	QObject::connect(tobmpBtn, SIGNAL(clicked()), this, SLOT(execute_work2bmp()));
	QObject::connect(swapBtn, SIGNAL(clicked()), this, SLOT(execute_swap_bmpwork()));
	QObject::connect(swapAllBtn, SIGNAL(clicked()), this, SLOT(execute_swap_bmpworkall()));
	...
	QObject::connect(this, SIGNAL(work_changed()), bmp_show, SLOT(workborder_changed()));
	QObject::connect(this, SIGNAL(marks_changed()), bmp_show, SLOT(mark_changed()));
	QObject::connect(this, SIGNAL(marks_changed()), work_show, SLOT(mark_changed()));
	QObject::connect(this, SIGNAL(tissues_changed()), this, SLOT(update_tissue()));
	QObject::connect(bmp_show, SIGNAL(addmark_sign(Point)), this, SLOT(add_mark(Point)));
	QObject::connect(bmp_show, SIGNAL(addlabel_sign(Point, std::string)), this, SLOT(add_label(Point, std::string)));
	QObject::connect(bmp_show, SIGNAL(clearmarks_sign()), this, SLOT(clear_marks()));
	QObject::connect(bmp_show, SIGNAL(removemark_sign(Point)), this, SLOT(remove_mark(Point)));
	QObject::connect(bmp_show, SIGNAL(addtissue_sign(Point)), this, SLOT(add_tissue(Point)));
	QObject::connect(bmp_show, SIGNAL(addtissueconnected_sign(Point)), this, SLOT(add_tissue_connected(Point)));
	QObject::connect(bmp_show, SIGNAL(subtissue_sign(Point)), this, SLOT(subtract_tissue(Point)));
	QObject::connect(bmp_show, SIGNAL(addtissue3D_sign(Point)), this, SLOT(add_tissue_3D(Point)));
	QObject::connect(bmp_show, SIGNAL(addtissuelarger_sign(Point)), this, SLOT(add_tissuelarger(Point)));
	QObject::connect(bmp_show, SIGNAL(selecttissue_sign(Point)), this, SLOT(select_tissue(Point)));
	QObject::connect(work_show, SIGNAL(addmark_sign(Point)), this, SLOT(add_mark(Point)));
	QObject::connect(work_show, SIGNAL(addlabel_sign(Point, std::string)), this, SLOT(add_label(Point, std::string)));
	QObject::connect(work_show, SIGNAL(clearmarks_sign()), this, SLOT(clear_marks()));
	QObject::connect(work_show, SIGNAL(removemark_sign(Point)), this, SLOT(remove_mark(Point)));
	QObject::connect(work_show, SIGNAL(addtissue_sign(Point)), this, SLOT(add_tissue(Point)));
	QObject::connect(work_show, SIGNAL(addtissueconnected_sign(Point)), this, SLOT(add_tissue_connected(Point)));
	QObject::connect(work_show, SIGNAL(subtissue_sign(Point)), this, SLOT(subtract_tissue(Point)));
	QObject::connect(work_show, SIGNAL(addtissue3D_sign(Point)), this, SLOT(add_tissue_3D(Point)));
	QObject::connect(work_show, SIGNAL(addtissuelarger_sign(Point)), this, SLOT(add_tissuelarger(Point)));
	QObject::connect(work_show, SIGNAL(selecttissue_sign(Point)), this, SLOT(select_tissue(Point)));
	QObject::connect(tissueFilter, SIGNAL(textChanged(const QString &)), this, SLOT(tissueFilterChanged(const QString &)));
	QObject::connect(lockTissues, SIGNAL(clicked()), this, SLOT(lockAllTissues()));
	QObject::connect(addTissue, SIGNAL(clicked()), this, SLOT(newTissuePressed()));
	QObject::connect(addFolder, SIGNAL(clicked()), this, SLOT(newFolderPressed()));
	QObject::connect(modifyTissueFolder, SIGNAL(clicked()), this, SLOT(modifTissueFolderPressed()));
	QObject::connect(removeTissueFolder, SIGNAL(clicked()), this, SLOT(removeTissueFolderPressed()));
	QObject::connect(removeTissueFolderAll, SIGNAL(clicked()), this, SLOT(removeTissueFolderAllPressed()));
	QObject::connect(getTissue, SIGNAL(clicked()), this, SLOT(selectedtissue2work()));
	QObject::connect(getTissueAll, SIGNAL(clicked()), this, SLOT(tissue2workall()));
	...
	QObject::connect(methodTab, SIGNAL(aboutToShow(QWidget *)), this, SLOT(tab_changed(QWidget *)));

	tissueTreeWidget->setSelectionMode(QAbstractItemView::SelectionMode::ExtendedSelection);

	QObject::connect(tissueTreeWidget, SIGNAL(itemSelectionChanged()), this, SLOT(tissue_selection_changed()));
	QObject::connect(tissueTreeWidget, SIGNAL(itemDoubleClicked(QTreeWidgetItem *, int)), this, SLOT(tree_widget_doubleclicked(QTreeWidgetItem *, int)));
	QObject::connect(tissueTreeWidget, SIGNAL(customContextMenuRequested(const QPoint &)), this, SLOT(tree_widget_contextmenu(const QPoint &)));
	...
	QObject::connect(cb_bmptissuevisible, SIGNAL(clicked()), this, SLOT(bmptissuevisible_changed()));
	QObject::connect(cb_bmpcrosshairvisible, SIGNAL(clicked()), this, SLOT(bmpcrosshairvisible_changed()));
	QObject::connect(cb_bmpoutlinevisible, SIGNAL(clicked()), this, SLOT(bmpoutlinevisible_changed()));
	QObject::connect(cb_worktissuevisible, SIGNAL(clicked()), this, SLOT(worktissuevisible_changed()));
	QObject::connect(cb_workcrosshairvisible, SIGNAL(clicked()), this, SLOT(workcrosshairvisible_changed()));
	QObject::connect(cb_workpicturevisible, SIGNAL(clicked()), this, SLOT(workpicturevisible_changed()));
	...
	QObject::connect(le_slicethick, SIGNAL(textChanged(const QString &)), this, SLOT(slicethickness_changed()));
	...
	QObject::connect(bmp_scroller, SIGNAL(contentsMoving(int, int)), this, SLOT(setWorkContentsPos(int, int)));
	QObject::connect(work_scroller, SIGNAL(contentsMoving(int, int)), this, SLOT(setBmpContentsPos(int, int)));
	QObject::connect(bmp_show, SIGNAL(setcenter_sign(int, int)), bmp_scroller, SLOT(center(int, int)));
	QObject::connect(work_show, SIGNAL(setcenter_sign(int, int)), work_show, SLOT(center(int, int)));
	tomove_scroller = true;

	interpolwidget->tissuenr_changed(currTissueType - 1);

	QObject::connect(zoom_widget, SIGNAL(set_zoom(double)), bmp_show, SLOT(set_zoom(double)));
	QObject::connect(zoom_widget, SIGNAL(set_zoom(double)), work_show, SLOT(set_zoom(double)));

	QObject::connect(this, SIGNAL(begin_dataexport(common::DataSelection &, QWidget *)), this, SLOT(handle_begin_dataexport(common::DataSelection &, QWidget *)));
	QObject::connect(this, SIGNAL(end_dataexport(QWidget *)), this, SLOT(handle_end_dataexport(QWidget *)));

	QObject::connect(this, SIGNAL(begin_datachange(common::DataSelection &, QWidget *, bool)), this, SLOT(handle_begin_datachange(common::DataSelection &, QWidget *, bool)));
	QObject::connect(this, SIGNAL(end_datachange(QWidget *, common::EndUndoAction)), this, SLOT(handle_end_datachange(QWidget *, common::EndUndoAction)));
	QObject::connect(this, SIGNAL(end_datachange(QRect, QWidget *, common::EndUndoAction)), this, SLOT(handle_end_datachange(QRect, QWidget *, common::EndUndoAction)));
	QObject::connect(threshold_widget, SIGNAL(begin_datachange(common::DataSelection &, QWidget *, bool)), this, SLOT(handle_begin_datachange(common::DataSelection &, QWidget *, bool)));
	QObject::connect(threshold_widget, SIGNAL(end_datachange(QWidget *, common::EndUndoAction)), this, SLOT(handle_end_datachange(QWidget *, common::EndUndoAction)));
	QObject::connect(OutlineCorrect_widget, SIGNAL(begin_datachange(common::DataSelection &, QWidget *, bool)), this, SLOT(handle_begin_datachange(common::DataSelection &, QWidget *, bool)));
	QObject::connect(OutlineCorrect_widget, SIGNAL(end_datachange(QWidget *, common::EndUndoAction)), this, SLOT(handle_end_datachange(QWidget *, common::EndUndoAction)));
	QObject::connect(OutlineCorrect_widget, SIGNAL(end_datachange(QRect, QWidget *, common::EndUndoAction)), this, SLOT(handle_end_datachange(QRect, QWidget *, common::EndUndoAction)));
	QObject::connect(OutlineCorrect_widget, SIGNAL(signal_request_selected_tissue_TS()), this, SLOT(provide_selected_tissue_TS()));
	QObject::connect(OutlineCorrect_widget, SIGNAL(signal_request_selected_tissue_BG()), this, SLOT(provide_selected_tissue_BG()));
	QObject::connect(smoothing_widget, SIGNAL(begin_datachange(common::DataSelection &, QWidget *, bool)), this, SLOT(handle_begin_datachange(common::DataSelection &, QWidget *, bool)));
	QObject::connect(smoothing_widget, SIGNAL(end_datachange(QWidget *, common::EndUndoAction)), this, SLOT(handle_end_datachange(QWidget *, common::EndUndoAction)));
	QObject::connect(edge_widg, SIGNAL(begin_datachange(common::DataSelection &, QWidget *, bool)), this, SLOT(handle_begin_datachange(common::DataSelection &, QWidget *, bool)));
	QObject::connect(edge_widg, SIGNAL(end_datachange(QWidget *, common::EndUndoAction)), this, SLOT(handle_end_datachange(QWidget *, common::EndUndoAction)));
	QObject::connect(morph_widget, SIGNAL(begin_datachange(common::DataSelection &, QWidget *, bool)), this, SLOT(handle_begin_datachange(common::DataSelection &, QWidget *, bool)));
	QObject::connect(morph_widget, SIGNAL(end_datachange(QWidget *, common::EndUndoAction)), this, SLOT(handle_end_datachange(QWidget *, common::EndUndoAction)));
	QObject::connect(wshed_widget, SIGNAL(begin_datachange(common::DataSelection &, QWidget *, bool)), this, SLOT(handle_begin_datachange(common::DataSelection &, QWidget *, bool)));
	QObject::connect(wshed_widget, SIGNAL(end_datachange(QWidget *, common::EndUndoAction)), this, SLOT(handle_end_datachange(QWidget *, common::EndUndoAction)));
	QObject::connect(lw_widget, SIGNAL(begin_datachange(common::DataSelection &, QWidget *, bool)), this, SLOT(handle_begin_datachange(common::DataSelection &, QWidget *, bool)));
	QObject::connect(lw_widget, SIGNAL(end_datachange(QWidget *, common::EndUndoAction)), this, SLOT(handle_end_datachange(QWidget *, common::EndUndoAction)));
	QObject::connect(iftrg_widget, SIGNAL(begin_datachange(common::DataSelection &, QWidget *, bool)), this, SLOT(handle_begin_datachange(common::DataSelection &, QWidget *, bool)));
	QObject::connect(iftrg_widget, SIGNAL(end_datachange(QWidget *, common::EndUndoAction)), this, SLOT(handle_end_datachange(QWidget *, common::EndUndoAction)));
	QObject::connect(FMF_widget, SIGNAL(begin_datachange(common::DataSelection &, QWidget *, bool)), this, SLOT(handle_begin_datachange(common::DataSelection &, QWidget *, bool)));
	QObject::connect(FMF_widget, SIGNAL(end_datachange(QWidget *, common::EndUndoAction)), this, SLOT(handle_end_datachange(QWidget *, common::EndUndoAction)));
	QObject::connect(interpolwidget, SIGNAL(begin_datachange(common::DataSelection &, QWidget *, bool)), this, SLOT(handle_begin_datachange(common::DataSelection &, QWidget *, bool)));
	QObject::connect(interpolwidget, SIGNAL(end_datachange(QWidget *, common::EndUndoAction)), this, SLOT(handle_end_datachange(QWidget *, common::EndUndoAction)));
	QObject::connect(scalewidget, SIGNAL(begin_datachange(common::DataSelection &, QWidget *, bool)), this, SLOT(handle_begin_datachange(common::DataSelection &, QWidget *, bool)));
	QObject::connect(scalewidget, SIGNAL(end_datachange(QWidget *, common::EndUndoAction)), this, SLOT(handle_end_datachange(QWidget *, common::EndUndoAction)));
	QObject::connect(imagemathwidget, SIGNAL(begin_datachange(common::DataSelection &, QWidget *, bool)), this, SLOT(handle_begin_datachange(common::DataSelection &, QWidget *, bool)));
	QObject::connect(imagemathwidget, SIGNAL(end_datachange(QWidget *, common::EndUndoAction)), this, SLOT(handle_end_datachange(QWidget *, common::EndUndoAction)));
	QObject::connect(imageoverlaywidget, SIGNAL(begin_datachange(common::DataSelection &, QWidget *, bool)), this, SLOT(handle_begin_datachange(common::DataSelection &, QWidget *, bool)));
	QObject::connect(imageoverlaywidget, SIGNAL(end_datachange(QWidget *, common::EndUndoAction)), this, SLOT(handle_end_datachange(QWidget *, common::EndUndoAction)));
	QObject::connect(bitstack_widget, SIGNAL(begin_datachange(common::DataSelection &, QWidget *, bool)), this, SLOT(handle_begin_datachange(common::DataSelection &, QWidget *, bool)));
	QObject::connect(bitstack_widget, SIGNAL(end_datachange(QWidget *, common::EndUndoAction)), this, SLOT(handle_end_datachange(QWidget *, common::EndUndoAction)));
	QObject::connect(bitstack_widget, SIGNAL(begin_dataexport(common::DataSelection &, QWidget *)), this, SLOT(handle_begin_dataexport(common::DataSelection &, QWidget *)));
	QObject::connect(bitstack_widget, SIGNAL(end_dataexport(QWidget *)), this, SLOT(handle_end_dataexport(QWidget *)));
	QObject::connect(hyst_widget, SIGNAL(begin_datachange(common::DataSelection &, QWidget *, bool)), this, SLOT(handle_begin_datachange(common::DataSelection &, QWidget *, bool)));
	QObject::connect(hyst_widget, SIGNAL(end_datachange(QWidget *, common::EndUndoAction)), this, SLOT(handle_end_datachange(QWidget *, common::EndUndoAction)));
	QObject::connect(pickerwidget, SIGNAL(begin_datachange(common::DataSelection &, QWidget *, bool)), this, SLOT(handle_begin_datachange(common::DataSelection &, QWidget *, bool)));
	QObject::connect(pickerwidget, SIGNAL(end_datachange(QWidget *, common::EndUndoAction)), this, SLOT(handle_end_datachange(QWidget *, common::EndUndoAction)));
	QObject::connect(transfrmWidget, SIGNAL(begin_datachange(common::DataSelection &, QWidget *, bool)), this, SLOT(handle_begin_datachange(common::DataSelection &, QWidget *, bool)));
	QObject::connect(transfrmWidget, SIGNAL(end_datachange(QWidget *, common::EndUndoAction)), this, SLOT(handle_end_datachange(QWidget *, common::EndUndoAction)));
	QObject::connect(overlay_widget, SIGNAL(overlay_changed()), bmp_show, SLOT(overlay_changed()));
	QObject::connect(overlay_widget, SIGNAL(overlay_changed()), work_show, SLOT(overlay_changed()));
	QObject::connect(overlay_widget, SIGNAL(overlayalpha_changed(float)), bmp_show, SLOT(set_overlayalpha(float)));
	QObject::connect(overlay_widget, SIGNAL(overlayalpha_changed(float)), work_show, SLOT(set_overlayalpha(float)));
	QObject::connect(overlay_widget, SIGNAL(bmpoverlayvisible_changed(bool)), bmp_show, SLOT(set_overlayvisible(bool)));
	QObject::connect(overlay_widget, SIGNAL(workoverlayvisible_changed(bool)), work_show, SLOT(set_overlayvisible(bool)));

	QObject::connect(m_MultiDataset_widget, SIGNAL(dataset_changed()), bmp_show, SLOT(overlay_changed()));
	QObject::connect(m_MultiDataset_widget, SIGNAL(dataset_changed()), work_show, SLOT(overlay_changed()));
	QObject::connect(m_MultiDataset_widget, SIGNAL(dataset_changed()), this, SLOT(DatasetChanged()));
	QObject::connect(m_MultiDataset_widget, SIGNAL(begin_datachange(common::DataSelection &, QWidget *, bool)), this, SLOT(handle_begin_datachange(common::DataSelection &, QWidget *, bool)));
	QObject::connect(m_MultiDataset_widget, SIGNAL(end_datachange(QWidget *, common::EndUndoAction)), this, SLOT(handle_end_datachange(QWidget *, common::EndUndoAction)));

	QObject::connect(cb_tissuelock, SIGNAL(clicked()), this, SLOT(tissuelock_toggled()));

	QObject::connect(sl_contrastbmp, SIGNAL(valueChanged(int)), this, SLOT(sl_contrastbmp_moved(int)));
	QObject::connect(sl_contrastwork, SIGNAL(valueChanged(int)), this, SLOT(sl_contrastwork_moved(int)));
	QObject::connect(sl_brightnessbmp, SIGNAL(valueChanged(int)), this, SLOT(sl_brightnessbmp_moved(int)));
	QObject::connect(sl_brightnesswork, SIGNAL(valueChanged(int)), this, SLOT(sl_brightnesswork_moved(int)));

	QObject::connect(le_contrastbmp_val, SIGNAL(editingFinished()), this, SLOT(le_contrastbmp_val_edited()));
	QObject::connect(le_contrastwork_val, SIGNAL(editingFinished()), this, SLOT(le_contrastwork_val_edited()));
	QObject::connect(le_brightnessbmp_val, SIGNAL(editingFinished()), this, SLOT(le_brightnessbmp_val_edited()));
	QObject::connect(le_brightnesswork_val, SIGNAL(editingFinished()), this, SLOT(le_brightnesswork_val_edited()));


	m_widget_signal_mapper = new QSignalMapper(this);
	for (int i = 0; i < nrtabbuttons; ++i)
	{
		QObject::connect(pb_tab[i], SIGNAL(clicked()), m_widget_signal_mapper, SLOT(map()));
		m_widget_signal_mapper->setMapping(pb_tab[i], i);
		QObject::connect(m_widget_signal_mapper, SIGNAL(mapped(int)), this, SLOT(pb_tab_pressed(int)));
	}

	QObject::connect(bmp_show, SIGNAL(wheelrotatedctrl_sign(int)), this, SLOT(wheelrotated(int)));
	QObject::connect(work_show, SIGNAL(wheelrotatedctrl_sign(int)), this, SLOT(wheelrotated(int)));

	QObject::connect(bmp_show, SIGNAL(mousePosZoom_sign(QPoint)), this, SLOT(mousePosZoom_changed(QPoint)));
	QObject::connect(work_show, SIGNAL(mousePosZoom_sign(QPoint)), this, SLOT(mousePosZoom_changed(QPoint)));

	//	QObject::connect(pb_work2tissue,SIGNAL(clicked()),this,SLOT(do_work2tissue()));

	m_acc_sliceup = new Q3Accel(this);
	m_acc_sliceup->connectItem(m_acc_sliceup->insertItem(QKeySequence(Qt::CTRL + Qt::Key_Right)), this, SLOT(slicenr_up()));
	m_acc_slicedown = new Q3Accel(this);
	m_acc_slicedown->connectItem(m_acc_slicedown->insertItem(QKeySequence(Qt::CTRL + Qt::Key_Left)), this, SLOT(slicenr_down()));
	m_acc_sliceup1 = new Q3Accel(this);
	m_acc_sliceup->connectItem(m_acc_sliceup->insertItem(QKeySequence(Qt::Key_Next)), this, SLOT(slicenr_up()));
	m_acc_slicedown1 = new Q3Accel(this);
	m_acc_slicedown->connectItem(m_acc_slicedown->insertItem(QKeySequence(Qt::Key_Prior)), this, SLOT(slicenr_down()));
	m_acc_zoomin = new Q3Accel(this);
	m_acc_zoomin->connectItem(m_acc_zoomin->insertItem(QKeySequence(Qt::CTRL + Qt::Key_Up)), this, SLOT(zoom_in()));
	m_acc_zoomout = new Q3Accel(this);
	m_acc_zoomout->connectItem(m_acc_zoomout->insertItem(QKeySequence(Qt::CTRL + Qt::Key_Down)), this, SLOT(zoom_out()));
	m_acc_add = new Q3Accel(this);
	m_acc_add->connectItem(m_acc_add->insertItem(QKeySequence(Qt::CTRL + Qt::Key_Plus)), this, SLOT(add_tissue_shortkey()));
	m_acc_sub = new Q3Accel(this);
	m_acc_sub->connectItem(m_acc_sub->insertItem(QKeySequence(Qt::CTRL + Qt::Key_Minus)), this, SLOT(subtract_tissue_shortkey()));
	m_acc_undo = new Q3Accel(this);
	m_acc_undo->connectItem(m_acc_undo->insertItem(QKeySequence(Qt::Key_Escape)), this, SLOT(execute_undo()));
	m_acc_undo2 = new Q3Accel(this);
	m_acc_undo2->connectItem(m_acc_undo2->insertItem(QKeySequence("Ctrl+Z")), this, SLOT(execute_undo()));
	m_acc_redo = new Q3Accel(this);
	m_acc_redo->connectItem(m_acc_redo->insertItem(QKeySequence("Ctrl+Y")), this, SLOT(execute_redo()));
...
void MainWindow::closeEvent(QCloseEvent *qce)
{
	if (maybeSafe())
	{
		...
		{
			xsliceshower->close();
			QObject::disconnect(bmp_show, SIGNAL(scaleoffsetfactor_changed(float, float, bool)), xsliceshower, SLOT(set_scale(float, float, bool)));
			QObject::disconnect(work_show, SIGNAL(scaleoffsetfactor_changed(float, float, bool)), xsliceshower, SLOT(set_scale(float, float, bool)));
			QObject::disconnect(xsliceshower, SIGNAL(slice_changed(int)), this, SLOT(xshower_slicechanged()));
			QObject::disconnect(zoom_widget, SIGNAL(set_zoom(double)), xsliceshower, SLOT(set_zoom(double)));
			QObject::disconnect(xsliceshower, SIGNAL(hasbeenclosed()), this, SLOT(xslice_closed()));
			delete xsliceshower;
		}
		if (ysliceshower != NULL)
		{
			ysliceshower->close();
			QObject::disconnect(bmp_show, SIGNAL(scaleoffsetfactor_changed(float, float, bool)), ysliceshower, SLOT(set_scale(float, float, bool)));
			QObject::disconnect(work_show, SIGNAL(scaleoffsetfactor_changed(float, float, bool)), ysliceshower, SLOT(set_scale(float, float, bool)));
			QObject::disconnect(ysliceshower, SIGNAL(slice_changed(int)), this, SLOT(yshower_slicechanged()));
			QObject::disconnect(zoom_widget, SIGNAL(set_zoom(double)), ysliceshower, SLOT(set_zoom(double)));
			QObject::disconnect(ysliceshower, SIGNAL(hasbeenclosed()), this, SLOT(yslice_closed()));
			delete ysliceshower;
		}
		if (SV3D != NULL)
		{
			SV3D->close();
			QObject::disconnect(SV3D, SIGNAL(hasbeenclosed()), this, SLOT(SV3D_closed()));
			delete SV3D;
		}
		if (SV3Dbmp != NULL)
		{
			SV3Dbmp->close();
			QObject::disconnect(SV3Dbmp, SIGNAL(hasbeenclosed()), this, SLOT(SV3Dbmp_closed()));
			delete SV3Dbmp;
		}
		if (VV3D != NULL)
		{
			VV3D->close();
			QObject::disconnect(VV3D, SIGNAL(hasbeenclosed()), this, SLOT(VV3D_closed()));
			delete VV3D;
		}
		if (VV3Dbmp != NULL)
		{
			VV3Dbmp->close();
			QObject::disconnect(VV3Dbmp, SIGNAL(hasbeenclosed()), this, SLOT(VV3Dbmp_closed()));
			delete VV3Dbmp;
		}
...
bool MainWindow::maybeSafe()
{
	if (modified())
	{
		int ret = QMessageBox::warning(this, "iSeg", "Do you want to save your changes?",
		...
		if (ret == QMessageBox::Yes)
		{
			...
			if ((!m_editingmode && !m_saveprojfilename.isEmpty()) || (m_editingmode && !m_S4Lcommunicationfilename.isEmpty()))
			{
				execute_saveproj();
			}
			else
...
void MainWindow::provide_selected_tissue_BG()
{
	...
	{
		QMessageBox::warning(this, "iSeg", "Error: Unable to get " + tissueTreeWidget->get_current_name() + " because it is locked.\nPlease, unlock it.", QMessageBox::Ok | QMessageBox::Default);
		return;
	}

	OutlineCorrect_widget->Select_selected_tissue_BG(tissueTreeWidget->get_current_name(), tissueTreeWidget->get_current_type());
}
void MainWindow::provide_selected_tissue_TS()
{
	...
	OutlineCorrect_widget->Select_selected_tissue_TS(tissueTreeWidget->get_current_name(), tissueTreeWidget->get_current_type());
}
	...
	if (m_MultiDataset_widget->isVisible() && swapExtraDatasets)
	{
		...
		for (int i = 0; i < m_MultiDataset_widget->GetNumberOfDatasets(); i++)
		{
			...
			if (!m_MultiDataset_widget->IsActive(i))
			{
				...
				if (SlicesHandler::SaveRaw_xy_swapped(str1.ascii(), m_MultiDataset_widget->GetBmpData(i), w, h, nrslices) != 0)
					ok = false;

				tempFileName = "work_float_eds_" + std::to_string(i) + ".raw";
				str1 = QDir::temp().absFilePath(QString(tempFileName.c_str()));
				if (SlicesHandler::SaveRaw_xy_swapped(str1.ascii(), m_MultiDataset_widget->GetWorkingData(i), w, h, nrslices) != 0)
					ok = false;
				...
				if (ok)
				{
					...
					m_MultiDataset_widget->SetWorkingData(i, SlicesHandler::LoadRawFloat(str1.ascii(), handler3D->return_startslice(), handler3D->return_endslice(), 0, w * h));
				}
				...
				if (ok)
				{
					...
					m_MultiDataset_widget->SetBmpData(i, SlicesHandler::LoadRawFloat(str1.ascii(), handler3D->return_startslice(), handler3D->return_endslice(), 0, w * h));
				}
			}
		}
	...
	if (m_MultiDataset_widget->isVisible() && swapExtraDatasets)
	{
		...
		for (int i = 0; i < m_MultiDataset_widget->GetNumberOfDatasets(); i++)
		{
			...
			if (!m_MultiDataset_widget->IsActive(i))
			{
				...
				str1 = QDir::temp().absFilePath(QString(tempFileName.c_str()));
				if (SlicesHandler::SaveRaw_xz_swapped(str1.ascii(), m_MultiDataset_widget->GetBmpData(i), w, h, nrslices) != 0)
					ok = false;

				tempFileName = "work_float_" + std::to_string(i) + ".raw";
				str1 = QDir::temp().absFilePath(QString(tempFileName.c_str()));
				if (SlicesHandler::SaveRaw_xz_swapped(str1.ascii(), m_MultiDataset_widget->GetWorkingData(i), w, h, nrslices) != 0)
					ok = false;
	...
	if (m_MultiDataset_widget->isVisible() && swapExtraDatasets)
	{
		...
		for (int i = 0; i < m_MultiDataset_widget->GetNumberOfDatasets(); i++)
		{
			...
			if (!m_MultiDataset_widget->IsActive(i))
			{
				std::string tempFileName = "bmp_float_" + std::to_string(i) + ".raw";
				str1 = QDir::temp().absFilePath(QString(tempFileName.c_str()));
				if (SlicesHandler::SaveRaw_yz_swapped(str1.ascii(), m_MultiDataset_widget->GetBmpData(i), w, h, nrslices) != 0)
					ok = false;

				tempFileName = "work_float_" + std::to_string(i) + ".raw";
				str1 = QDir::temp().absFilePath(QString(tempFileName.c_str()));
				if (SlicesHandler::SaveRaw_yz_swapped(str1.ascii(), m_MultiDataset_widget->GetWorkingData(i), w, h, nrslices) != 0)
					ok = false;
...
void MainWindow::execute_resize(int resizetype)
{
	...
	ResizeDialog RD(handler3D, static_cast<ResizeDialog::eResizeType>(resizetype), this);
	RD.move(QCursor::pos());
	if (!RD.exec())
	{
		return;
	...
	bool ok = true;
	str1 = QDir::temp().absFilePath(QString("bmp_float.raw"));
	if (handler3D->SaveRaw_resized(str1.ascii(), dxm, dxp, dym, dyp, dzm, dzp, false) != 0) ok = false;
	str1 = QDir::temp().absFilePath(QString("work_float.raw"));
	if (handler3D->SaveRaw_resized(str1.ascii(), dxm, dxp, dym, dyp, dzm, dzp, true) != 0) ok = false;
	str1 = QDir::temp().absFilePath(QString("tissues.raw"));
	if (handler3D->SaveTissuesRaw_resized(str1.ascii(), dxm, dxp, dym, dyp, dzm, dzp) != 0) ok = false;

	if (ok)
	{
		str1 = QDir::temp().absFilePath(QString("work_float.raw"));
		if (handler3D->ReadRawFloat(str1.ascii(), w + dxm + dxp, h + dym + dyp, 0, nrslices + dzm + dzp) != 1) ok = false;
	}
	if (ok)
	{
		str1 = QDir::temp().absFilePath(QString("bmp_float.raw"));
		if (handler3D->ReloadRawFloat(str1.ascii(), 0) != 1) ok = false;
	}
	if (ok)
	{
		str1 = QDir::temp().absFilePath(QString("tissues.raw"));
		if (handler3D->ReloadRawTissues(str1.ascii(), sizeof(tissues_size_t) * 8, 0) != 1) ok = false;
	}
...
void MainWindow::execute_pad()
{
	execute_resize(1);
}

void MainWindow::execute_crop()
{
	execute_resize(2);
}
...
void MainWindow::le_contrastbmp_val_edited()
{
	...
	float contrast = max(0.01f, min(le_contrastbmp_val->text().toFloat(), 100.0f));
	le_contrastbmp_val->setText(QString("%1").arg(contrast, 6, 'f', 2));
...
{
	// Clamp to range and round to precision
	float contrast = max(0.01f, min(le_contrastwork_val->text().toFloat(), 100.0f));
	le_contrastwork_val->setText(QString("%1").arg(contrast, 6, 'f', 2));
...
{
	// Clamp to range and round to precision
	int brightness = (int)max(-50.0f, min(std::floor(le_brightnessbmp_val->text().toFloat() + 0.5f), 50.0f));
...
{
	// Clamp to range and round to precision
	int brightness = (int)max(-50.0f, min(std::floor(le_brightnesswork_val->text().toFloat() + 0.5f), 50.0f));
...
void MainWindow::update_brightnesscontrast(bool bmporwork, bool paint)
{
	...
	else
	{
		...
		float brightness = (le_brightnesswork_val->text().toFloat() + 50.0f) * 0.01f;
...
void MainWindow::execute_loadpng()
{
	...
	QStringList files = Q3FileDialog::getOpenFileNames("Images (*.png)\n" "All(*.*)",
	                    QString::null, this, "open files dialog", "Select one or more files to open");
...
void MainWindow::execute_loadjpg()
{
	...
	QStringList files = Q3FileDialog::getOpenFileNames("Images (*.jpg)\n" "All(*.*)",
	                    QString::null, this, "open files dialog", "Select one or more files to open");
...
void MainWindow::execute_loaddicom()
{
	...
	QStringList files = Q3FileDialog::getOpenFileNames("Images (*.dcm *.dicom)\n" "All(*.*)",
	                    QString::null, this, "open files dialog", "Select one or more files to open");
...
void MainWindow::execute_loadmhd()
{
	...
	QString loadfilename = Q3FileDialog::getOpenFileName(QString::null, "Metaheader (*.mhd *.mha)\n" "All(*.*)", this);
...
void MainWindow::execute_loadvtk()
{
	...
	bool res = true;
	QString loadfilename = Q3FileDialog::getOpenFileName(QString::null, "VTK (*.vti *.vtk)\n" "All(*.*)", this);
	if (!loadfilename.isEmpty())
	{
...
void MainWindow::execute_loadnifti()
{
	...
	QString loadfilename = Q3FileDialog::getOpenFileName(QString::null, "NIFTI (*.nii *.hdr *.img *.nii.gz)\n" "All(*.*)", this);
	if (!loadfilename.isEmpty())
	{
		handler3D->ReadImage(loadfilename.ascii());
	}
...
void MainWindow::execute_loadavw()
{
	...
	QString loadfilename = Q3FileDialog::getOpenFileName(QString::null, "AnalzyeAVW (*.avw)\n" "All(*.*)", this);
	if (!loadfilename.isEmpty())
	{
...
void MainWindow::execute_reloadbmp()
{
	...
	if ((unsigned short)files.size() == handler3D->return_nrslices() || (unsigned short)files.size() == (handler3D->return_endslice() - handler3D->return_startslice()))
	{
	...
	else
	{
		if (!files.empty())
		{
			QMessageBox::information(this, "Segm. Tool",
...
void MainWindow::execute_reloadmhd()
{
	...
	if (!loadfilename.isEmpty())
	{
		handler3D->ReloadImage(loadfilename.ascii(), handler3D->return_startslice());
		reset_brightnesscontrast();
	}
	emit end_datachange(this, common::ClearUndo);
...
void MainWindow::execute_reloadnifti()
{
	...
	if (!loadfilename.isEmpty())
	{
		handler3D->ReloadImage(loadfilename.ascii(), handler3D->return_startslice()); // TODO: handle failure
		reset_brightnesscontrast();
	}
	emit end_datachange(this, common::ClearUndo);
...
void MainWindow::execute_loadsurface()
{
	...
	bool ok = true;
	QString loadfilename = Q3FileDialog::getOpenFileName(QString::null, "STL (*.stl)\n" "All(*.*)", this);
	if (!loadfilename.isEmpty())
	{
		QMessageBox msgBox;
...
void MainWindow::execute_loadrtstruct()
{
	...
	QObject::connect(&RI, SIGNAL(begin_datachange(common::DataSelection &, QWidget *, bool)), this, SLOT(handle_begin_datachange(common::DataSelection &, QWidget *, bool)));
	QObject::connect(&RI, SIGNAL(end_datachange(QWidget *, common::EndUndoAction)), this, SLOT(handle_end_datachange(QWidget *, common::EndUndoAction)));
	...
	QObject::disconnect(&RI, SIGNAL(begin_datachange(common::DataSelection &, QWidget *, bool)), this, SLOT(handle_begin_datachange(common::DataSelection &, QWidget *, bool)));
	QObject::disconnect(&RI, SIGNAL(end_datachange(QWidget *, common::EndUndoAction)), this, SLOT(handle_end_datachange(QWidget *, common::EndUndoAction)));
...
void MainWindow::execute_loadrtdose()
{
	...
	QString loadfilename = Q3FileDialog::getOpenFileName(QString::null, "RTdose (*.dcm)\n" "All(*.*)", this);
	if (!loadfilename.isEmpty())
	{
...
{
	QString loadfilename = Q3FileDialog::getOpenFileName(QString::null, "S4L Link (*.h5)\n" "All(*.*)", this);
	if (!loadfilename.isEmpty())
	{
...
void MainWindow::execute_saveprojas()
{
	...
	QString savefilename = Q3FileDialog::getSaveFileName(QString::null, "Projects (*.prj)\n", this);//, filename);

	if (!savefilename.isEmpty())
	{
		if (savefilename.length() <= 4 || !savefilename.endsWith(QString(".prj"))) savefilename.append(".prj");
		...
		if (afterDot != 0)
			tempFileName = tempFileName.remove(afterDot, tempFileName.length() - afterDot) + "Temp.prj";
		else
		...
		int numTasks = 3;
		QProgressDialog progress("Save in progress...", "Cancel", 0, numTasks, this);
		progress.show();
		...
		setCaption(QString(" iSeg ") + QString::number(iSegVersion) + QString(".") + QString::number(iSegSubversion) + QString(" B") + QString::number(build)
		...
		unsigned short combinedVersion = (unsigned short)common::CombineTissuesVersion(saveProjVersion, 1);
		...
		QFile::rename(tempFileNameWithoutExtension + ".xmf", sourceFileNameWithoutExtension + ".xmf");

		if (QFile::exists(sourceFileNameWithoutExtension + ".prj"))
			QFile::remove(sourceFileNameWithoutExtension + ".prj");
		QFile::rename(tempFileNameWithoutExtension + ".prj", sourceFileNameWithoutExtension + ".prj");

		if (QFile::exists(sourceFileNameWithoutExtension + ".h5"))
			QFile::remove(sourceFileNameWithoutExtension + ".h5");
		QFile::rename(tempFileNameWithoutExtension + ".h5", sourceFileNameWithoutExtension + ".h5");
...
void MainWindow::LoadSettings(const char *loadfilename)
{
	...
	common::ExtractTissuesVersion((int)combinedVersion, loadProjVersion, tissuesVersion);
	...
	if (loadProjVersion > 7)
	{
		...
		cerr << "NumberOfUndoSteps = " << this->handler3D->GetNumberOfUndoSteps() << endl;
		this->handler3D->SetNumberOfUndoArrays(settings.value("NumberOfUndoArrays", 20).toUInt());
		cerr << "NumberOfUndoArrays = " << this->handler3D->GetNumberOfUndoArrays() << endl;
		...
		cerr << "ContiguousMemory = " << this->handler3D->GetContiguousMemory() << endl;
		settings.endGroup();

		if (this->handler3D->return_nrundo() == 0) this->editmenu->setItemEnabled(undonr, false);
		else editmenu->setItemEnabled(undonr, true);
	}
...
void MainWindow::execute_saveproj()
{
	...
	else
	{
		if (!m_saveprojfilename.isEmpty())
		{
			...
			mBox.setText("The project you are trying to save is open somewhere else. Please, close it before continuing and press OK or press Cancel to stop saving process.");
			mBox.addButton(QMessageBox::Ok);
			mBox.addButton(QMessageBox::Cancel);

			if (QFile::exists(sourceFileNameWithoutExtension + ".xmf"))
			{
				bool removeSuccess = QFile::remove(sourceFileNameWithoutExtension + ".xmf");
				while (!removeSuccess)
				{
					int ret = mBox.exec();
					...
					removeSuccess = QFile::remove(sourceFileNameWithoutExtension + ".xmf");
				}
			}
			QFile::rename(tempFileNameWithoutExtension + ".xmf", sourceFileNameWithoutExtension + ".xmf");
			...
			if (QFile::exists(sourceFileNameWithoutExtension + ".prj"))
			{
				bool removeSuccess = QFile::remove(sourceFileNameWithoutExtension + ".prj");
				while (!removeSuccess)
				{
					int ret = mBox.exec();
					if (ret == QMessageBox::Cancel)
					...
					removeSuccess = QFile::remove(sourceFileNameWithoutExtension + ".prj");
				}
			}
			QFile::rename(tempFileNameWithoutExtension + ".prj", sourceFileNameWithoutExtension + ".prj");
			...
			if (QFile::exists(sourceFileNameWithoutExtension + ".h5"))
			{
				bool removeSuccess = QFile::remove(sourceFileNameWithoutExtension + ".h5");
				while (!removeSuccess)
				{
					int ret = mBox.exec();
					if (ret == QMessageBox::Cancel)
	...
	if (stillopen)
	{
		...
		if (fread(&combinedVersion, sizeof(unsigned short), 1, fp) > 0)
		{
			...
			if (loadProjVersion >= 3)
			{
				...
				tissues_size_t currTissueType = tissueTreeWidget->get_current_type();
				cb_tissuelock->setChecked(TissueInfos::GetTissueLocked(currTissueType + 1));
...
void MainWindow::execute_mergeprojects()
{
	...
	// Get save file name
	QString savefilename = Q3FileDialog::getSaveFileName(QString::null, "Projects (*.prj)", this, "iSeg", "Save merged project as");
	if (savefilename.length() <= 4 || !savefilename.endsWith(QString(".prj"))) savefilename.append(".prj");
	...
	if (!savefilename.isEmpty() && mergefilenames.size() > 0)
	{
		...
		// Load merged project
		if (QMessageBox::question(this, "iSeg", "Would you like to load the merged project?",
...
{
	boneConnectivityDialog = new CheckBoneConnectivityDialog(handler3D, "Bone Connectivity", this, Qt::Window);
	QObject::connect(boneConnectivityDialog, SIGNAL(slice_changed()), this, SLOT(update_slice()));
...
void MainWindow::execute_loadproj()
{
	...
	QString loadfilename = Q3FileDialog::getOpenFileName(QString::null, "Projects (*.prj)\n" "All(*.*)", this);//, filename);
...
{
	AtlasWidget *AW = new AtlasWidget(m_atlasfilename.m_atlasdir.absFilePath(m_atlasfilename.m_atlasfilename[i]).ascii(), m_picpath);
	if (AW->isOK)
	{
		AW->show();
		AW->raise();
...
void MainWindow::execute_createatlas()
{
	...
	QString savefilename = Q3FileDialog::getSaveFileName(QString::null, "Atlas file (*.atl)", this);//, filename);
...
{
	QString savefilename = Q3FileDialog::getSaveFileName(QString::null, QString::null, this);//, filename);
...
void MainWindow::execute_exportlabelfield()
{
	...
	QString savefilename = Q3FileDialog::getSaveFileName(QString::null, "AmiraMesh Ascii (*.am)", this);//, filename);
...
void MainWindow::execute_exportmat()
{
	...
	QString savefilename = Q3FileDialog::getSaveFileName(QString::null, "Matlab (*.mat)", this);//, filename);
...
void MainWindow::execute_exporthdf()
{
	...
	QString savefilename = Q3FileDialog::getSaveFileName(QString::null, "HDF (*.h5)", this);//, filename);
...
void MainWindow::execute_exportvtkascii()
{
	...
	QString savefilename = Q3FileDialog::getSaveFileName(QString::null, "VTK Ascii (*.vti *.vtk)", this);//, filename);
...
void MainWindow::execute_exportvtkbinary()
{
	...
	QString savefilename = Q3FileDialog::getSaveFileName(QString::null, "VTK bin (*.vti *.vtk)", this);//, filename);
...
void MainWindow::execute_exportvtkcompressedascii()
{
	...
	QString savefilename = Q3FileDialog::getSaveFileName(QString::null, "VTK comp (*.vti)", this);//, filename);
...
void MainWindow::execute_exportxmlregionextent()
{
	...
	QString savefilename = Q3FileDialog::getSaveFileName(QString::null, "XML extent (*.xml)", this);
	...
	if (!savefilename.isEmpty())
	{
		if (relfilename.isEmpty())
			...
			handler3D->print_xmlregionextent(savefilename.ascii(), true, relfilename.ascii());
	}
...
void MainWindow::execute_exporttissueindex()
{
	...
	QString savefilename = Q3FileDialog::getSaveFileName(QString::null, "tissue index (*.txt)", this);
	...
	if (!savefilename.isEmpty())
	{
		if (relfilename.isEmpty())
			...
			handler3D->print_tissueindex(savefilename.ascii(), true, relfilename.ascii());
	}
...
{
	QString loadfilename = Q3FileDialog::getOpenFileName(QString::null, QString::null, this);//, filename);
	...
	if (!loadfilename.isEmpty())
	{
		...
		{
			TissueInfos::LoadTissuesReadable(loadfilename.ascii(), handler3D, removeTissuesRange);
		...
		{
			if (TissueInfos::LoadTissuesReadable(loadfilename.ascii(), handler3D, removeTissuesRange))
			{
...
{
	TissueInfos::SaveDefaultTissueList(m_tmppath.absFilePath(QString("def_tissues.txt")));
}
...
void MainWindow::update_tissue()
{
	...
	if (xsliceshower != NULL) xsliceshower->tissue_changed();
	if (ysliceshower != NULL) ysliceshower->tissue_changed();
	if (VV3D != NULL) VV3D->tissue_changed();
	if (SV3D != NULL) SV3D->tissue_changed();
}
...
void MainWindow::SV3Dbmp_closed()
{
	if (SV3Dbmp != NULL)
	{
		QObject::disconnect(SV3D, SIGNAL(hasbeenclosed()), this, SLOT(SV3Dbmp_closed()));
		delete SV3Dbmp;
		SV3Dbmp = NULL;
	}
}
...
void MainWindow::execute_scale()
{
	...
	QObject::connect(&SW, SIGNAL(begin_datachange(common::DataSelection &, QWidget *, bool)), this, SLOT(handle_begin_datachange(common::DataSelection &, QWidget *, bool)));
	QObject::connect(&SW, SIGNAL(end_datachange(QWidget *, common::EndUndoAction)), this, SLOT(handle_end_datachange(QWidget *, common::EndUndoAction)));
	SW.move(QCursor::pos());
	SW.exec();
	QObject::disconnect(&SW, SIGNAL(begin_datachange(common::DataSelection &, QWidget *, bool)), this, SLOT(handle_begin_datachange(common::DataSelection &, QWidget *, bool)));
	QObject::disconnect(&SW, SIGNAL(end_datachange(QWidget *, common::EndUndoAction)), this, SLOT(handle_end_datachange(QWidget *, common::EndUndoAction)));
...
void MainWindow::execute_imagemath()
{
	...
	QObject::connect(&IM, SIGNAL(begin_datachange(common::DataSelection &, QWidget *, bool)), this, SLOT(handle_begin_datachange(common::DataSelection &, QWidget *, bool)));
	QObject::connect(&IM, SIGNAL(end_datachange(QWidget *, common::EndUndoAction)), this, SLOT(handle_end_datachange(QWidget *, common::EndUndoAction)));
	IM.move(QCursor::pos());
	IM.exec();
	QObject::disconnect(&IM, SIGNAL(begin_datachange(common::DataSelection &, QWidget *, bool)), this, SLOT(handle_begin_datachange(common::DataSelection &, QWidget *, bool)));
	QObject::disconnect(&IM, SIGNAL(end_datachange(QWidget *, common::EndUndoAction)), this, SLOT(handle_end_datachange(QWidget *, common::EndUndoAction)));
...
void MainWindow::execute_overlay()
{
	...
	QObject::connect(&IO, SIGNAL(begin_datachange(common::DataSelection &, QWidget *, bool)), this, SLOT(handle_begin_datachange(common::DataSelection &, QWidget *, bool)));
	QObject::connect(&IO, SIGNAL(end_datachange(QWidget *, common::EndUndoAction)), this, SLOT(handle_end_datachange(QWidget *, common::EndUndoAction)));
	IO.move(QCursor::pos());
	IO.exec();
	QObject::disconnect(&IO, SIGNAL(begin_datachange(common::DataSelection &, QWidget *, bool)), this, SLOT(handle_begin_datachange(common::DataSelection &, QWidget *, bool)));
	QObject::disconnect(&IO, SIGNAL(end_datachange(QWidget *, common::EndUndoAction)), this, SLOT(handle_end_datachange(QWidget *, common::EndUndoAction)));
...
void MainWindow::execute_undoconf()
{
	...
	if (handler3D->return_nrundo() == 0) editmenu->setItemEnabled(undonr, false);
	else editmenu->setItemEnabled(undonr, true);

	if (handler3D->return_nrredo() == 0) editmenu->setItemEnabled(redonr, false);
	else editmenu->setItemEnabled(redonr, true);
...
void MainWindow::execute_hideparameters(bool checked)
{
	...
	if (tab_old != NULL) tab_old->hideparams_changed();
}
...
void MainWindow::execute_showtabtoggled(bool)
{
	...
	unsigned short i = 0;
	while ((i < nrtabbuttons) && (currentwidget != tabwidgets[i])) i++;
	if (i == nrtabbuttons)
	{
		updateTabvisibility();
		return;
	...
	{
		i = 0;
		while ((i < nrtabbuttons) && !showpb_tab[i]) i++;
		if (i != nrtabbuttons)
		{
...
void MainWindow::execute_xslice()
{
	...
	if (xsliceshower == NULL)
	{
		xsliceshower = new sliceshower_widget(handler3D, true, handler3D->get_slicethickness(), zoom_widget->get_zoom(), 0, 0, Qt::WStyle_StaysOnTop);
		...
		QObject::connect(bmp_show, SIGNAL(scaleoffsetfactor_changed(float, float, bool)), xsliceshower, SLOT(set_scale(float, float, bool)));
		QObject::connect(work_show, SIGNAL(scaleoffsetfactor_changed(float, float, bool)), xsliceshower, SLOT(set_scale(float, float, bool)));
		QObject::connect(xsliceshower, SIGNAL(slice_changed(int)), this, SLOT(xshower_slicechanged()));
		QObject::connect(xsliceshower, SIGNAL(hasbeenclosed()), this, SLOT(xslice_closed()));
		QObject::connect(zoom_widget, SIGNAL(set_zoom(double)), xsliceshower, SLOT(set_zoom(double)));
...
void MainWindow::execute_yslice()
{
	...
	if (ysliceshower == NULL)
	{
		ysliceshower = new sliceshower_widget(handler3D, false, handler3D->get_slicethickness(), zoom_widget->get_zoom(), 0, 0, Qt::WStyle_StaysOnTop);
		...
		QObject::connect(bmp_show, SIGNAL(scaleoffsetfactor_changed(float, float, bool)), ysliceshower, SLOT(set_scale(float, float, bool)));
		QObject::connect(work_show, SIGNAL(scaleoffsetfactor_changed(float, float, bool)), ysliceshower, SLOT(set_scale(float, float, bool)));
		QObject::connect(ysliceshower, SIGNAL(slice_changed(int)), this, SLOT(yshower_slicechanged()));
		QObject::connect(ysliceshower, SIGNAL(hasbeenclosed()), this, SLOT(yslice_closed()));
		QObject::connect(zoom_widget, SIGNAL(set_zoom(double)), ysliceshower, SLOT(set_zoom(double)));
	}
...
{
	QString filename = Q3FileDialog::getOpenFileName(QString::null, "Text (*.txt)\n" "All(*.*)", this);
	if (!filename.isEmpty())
	{
		...
		if (read_tissues(filename.ascii(), types))
		{
			...
			handler3D->remove_tissues(std::set<tissues_size_t>(types.begin(), types.end()));
...
void MainWindow::execute_about()
{
	...
	stream1 << "\n\niSeg            \nVersion: " << iSegVersion << "." << iSegSubversion << "\nBuild: " << build;
...
void MainWindow::select_tissue(Point p)
{
	...
	tissueTreeWidget->set_current_tissue(handler3D->get_tissue_pt(p, handler3D->get_activeslice()));
}
...
void MainWindow::add_tissue_connected(Point p)
{
	...
	tissues_size_t currTissueType = tissueTreeWidget->get_current_type();
	handler3D->add2tissue_connected(currTissueType, p, cb_addsuboverride->isChecked());
...
void MainWindow::add_tissue_clicked(Point p)
{
	...
	pb_add->setOn(false);
	QObject::connect(work_show, SIGNAL(mousereleased_sign(Point)), this, SLOT(reconnectmouse_afterrelease(Point)));
...
void MainWindow::addhold_tissue_clicked(Point p)
{
	...
	if (cb_addsub3d->isChecked())
	{
		...
		if (cb_addsubconn->isChecked())
			handler3D->add2tissueall_connected(currTissueType, p, cb_addsuboverride->isChecked());
		else
			handler3D->add2tissueall(currTissueType, p, cb_addsuboverride->isChecked());
...
void MainWindow::add_tissue_pushed()
{
	...
	if (pb_subhold->isOn())
	{
		QObject::disconnect(work_show, SIGNAL(mousepressed_sign(Point)), this, SLOT(subtracthold_tissue_clicked(Point)));
		pb_subhold->setOn(false);
	}
	if (pb_addhold->isOn())
	{
		QObject::disconnect(work_show, SIGNAL(mousepressed_sign(Point)), this, SLOT(addhold_tissue_clicked(Point)));
	...
	if (pb_add->isOn())
	{
		...
		QObject::connect(work_show, SIGNAL(mousepressed_sign(Point)), this, SLOT(add_tissue_clicked(Point)));
...
void MainWindow::addhold_tissue_pushed()
{
	...
	if (pb_addhold->isOn())
	{
		QObject::connect(work_show, SIGNAL(mousepressed_sign(Point)), this, SLOT(addhold_tissue_clicked(Point)));
		disconnect_mouseclick();
...
void MainWindow::subtract_tissue_pushed()
{
	...
	if (pb_sub->isOn())
	{
		QObject::connect(work_show, SIGNAL(mousepressed_sign(Point)), this, SLOT(subtract_tissue_clicked(Point)));
		disconnect_mouseclick();
	}
	else
	{
...
void MainWindow::subtracthold_tissue_pushed()
{
	...
	if (pb_subhold->isOn())
	{
		QObject::connect(work_show, SIGNAL(mousepressed_sign(Point)), this, SLOT(subtracthold_tissue_clicked(Point)));
		disconnect_mouseclick();
	}
	else
	{
...
void MainWindow::merge()
{
	...
	int ret = QMessageBox::warning(this, "iSeg", "Do you really want to merge the selected tissues?",
	                               QMessageBox::Yes | QMessageBox::Default, QMessageBox::No, QMessageBox::Cancel | QMessageBox::Escape);
	if (ret == QMessageBox::Yes)
	{
		bool tmp;
...
{
	bool ok = false;
	QString newFolderName = QInputDialog::getText("Folder name", "Enter a name for the folder:", QLineEdit::Normal, tissueTreeWidget->get_current_name(), &ok, this);
	if (ok)
	{
...
void MainWindow::removeselected()
{
	...
	for (auto a = list.begin(); a != list.end(); ++a)
	{
		...
		if (removeCompletely)
		{
			...
			if (tissueCount < 2)
			{
				QMessageBox::information(this, "iSeg", "It is not possible to erase this tissue.\nAt least one tissue must be defined.");
				return;
			}
...
void MainWindow::removeFolder()
{
	if (tissueTreeWidget->get_current_has_children())
	{
		...
		else if (ret == QMessageBox::No)
		{
			...
			bool anyLocked = false;
			std::set<tissues_size_t> removeTissues;
			for (std::map<tissues_size_t, unsigned short>::iterator iter = childTissues.begin();
			...
			{
				QMessageBox::information(this, "iSeg", "It is not possible to erase this folder.\nAt least one tissue must be defined.");
				return;
			}
...
void MainWindow::removeTissueFolderAllPressed()
{
	...
	int ret = QMessageBox::warning(this, "iSeg", "Do you really want to remove all tissues and folders?",
	                               QMessageBox::Yes | QMessageBox::Default, QMessageBox::No, QMessageBox::Cancel | QMessageBox::Escape);
	if (ret == QMessageBox::Yes)
	{
...
void MainWindow::cleartissues()
{
	...
	int ret = QMessageBox::warning(this, "iSeg", "Do you really want to clear all tissues?",
	                               QMessageBox::Yes | QMessageBox::Default, QMessageBox::No, QMessageBox::Cancel | QMessageBox::Escape);
	if (ret == QMessageBox::Yes)
	{
		common::DataSelection dataSelection;
...
{
	bool isLocked = TissueInfos::GetTissueLocked(tissueTreeWidget->get_current_type());
	if (isLocked)
	{
	...
	int ret = QMessageBox::warning(this, "iSeg", "Do you really want to clear the tissue?",
	                               QMessageBox::Yes | QMessageBox::Default, QMessageBox::No, QMessageBox::Cancel | QMessageBox::Escape);
	if (ret == QMessageBox::Yes)
	{
...
void MainWindow::clearselected()
{
	...
	int ret = QMessageBox::warning(this, "iSeg", "Do you really want to clear tissues?",
	                               QMessageBox::Yes | QMessageBox::Default, QMessageBox::No, QMessageBox::Cancel | QMessageBox::Escape);
	if (ret == QMessageBox::Yes)
	{
...
void MainWindow::slices3d_changed(bool new_bitstack)
{
	...
	if (handler3D->return_nrslices() != nrslices)
	{
		...
		QObject::connect(sb_slicenr, SIGNAL(valueChanged(int)), this, SLOT(sb_slicenr_changed()));
		lb_slicenr->setText(QString(" of ") + QString::number((int)handler3D->return_nrslices()));
...
void MainWindow::slicethickness_changed()
{
	...
	if (b1)
	{
		...
		if (xsliceshower != NULL) xsliceshower->thickness_changed(thickness);
		if (ysliceshower != NULL) ysliceshower->thickness_changed(thickness);
		if (VV3D != NULL) VV3D->thickness_changed(thickness);
		if (VV3Dbmp != NULL) VV3Dbmp->thickness_changed(thickness);
		if (SV3D != NULL) SV3D->thickness_changed(thickness);
		if (SV3Dbmp != NULL) SV3Dbmp->thickness_changed(thickness);
	}
	else
	{
...
void MainWindow::slicethickness_changed1()
{
	...
	if (xsliceshower != NULL) xsliceshower->thickness_changed(handler3D->get_slicethickness());
	if (ysliceshower != NULL) ysliceshower->thickness_changed(handler3D->get_slicethickness());
	if (VV3D != NULL) VV3D->thickness_changed(handler3D->get_slicethickness());
	if (VV3Dbmp != NULL) VV3Dbmp->thickness_changed(handler3D->get_slicethickness());
	if (SV3D != NULL) SV3D->thickness_changed(handler3D->get_slicethickness());
	if (SV3Dbmp != NULL) SV3Dbmp->thickness_changed(handler3D->get_slicethickness());
}
...
void MainWindow::tissuenr_changed(int i)
{
	...
	if (qw == iftrg_widget)  iftrg_widget->tissuenr_changed(i);
	else if (qw == interpolwidget) interpolwidget->tissuenr_changed(i);
...
void MainWindow::clear_stack()
{
	bitstack_widget->clear_stack();
}
...
void MainWindow::tab_changed(QWidget *qw)
{
	...
	if (qw != tab_old)
	{
		...
		{
			QObject::disconnect(this, SIGNAL(bmp_changed()), threshold_widget, SLOT(bmp_changed()));
		}
		else if (tab_old == measurement_widget)
		{
			measurement_widget->cleanup();
			//		QObject::connect(this,SIGNAL(marks_changed()),work_show,SLOT(mark_changed()));
			QObject::disconnect(this, SIGNAL(marks_changed()), measurement_widget, SLOT(marks_changed()));
			QObject::disconnect(bmp_show, SIGNAL(mousepressed_sign(Point)), measurement_widget, SLOT(pt_clicked(Point)));
			QObject::disconnect(work_show, SIGNAL(mousepressed_sign(Point)), measurement_widget, SLOT(pt_clicked(Point)));
			QObject::disconnect(measurement_widget, SIGNAL(vp1_changed(std::vector<Point> *)), bmp_show, SLOT(set_vp1(std::vector<Point> *)));
			QObject::disconnect(measurement_widget, SIGNAL(vp1_changed(std::vector<Point> *)), work_show, SLOT(set_vp1(std::vector<Point> *)));
			QObject::disconnect(measurement_widget, SIGNAL(vpdyn_changed(std::vector<Point> *)), bmp_show, SLOT(set_vpdyn(std::vector<Point> *)));
			QObject::disconnect(measurement_widget, SIGNAL(vpdyn_changed(std::vector<Point> *)), work_show, SLOT(set_vpdyn(std::vector<Point> *)));
			QObject::disconnect(measurement_widget, SIGNAL(vp1dyn_changed(std::vector<Point> *, std::vector<Point> *, bool)), bmp_show, SLOT(set_vp1_dyn(std::vector<Point> *, std::vector<Point> *, bool)));
			QObject::disconnect(measurement_widget, SIGNAL(vp1dyn_changed(std::vector<Point> *, std::vector<Point> *, bool)), work_show, SLOT(set_vp1_dyn(std::vector<Point> *, std::vector<Point> *, bool)));
			QObject::disconnect(bmp_show, SIGNAL(mousemoved_sign(Point)), measurement_widget, SLOT(pt_moved(Point)));
			QObject::disconnect(work_show, SIGNAL(mousemoved_sign(Point)), measurement_widget, SLOT(pt_moved(Point)));
		...
		else if (tab_old == vesselextr_widget)
		{
			...
			QObject::disconnect(this, SIGNAL(marks_changed()), measurement_widget, SLOT(marks_changed()));
			QObject::disconnect(vesselextr_widget, SIGNAL(vp1_changed(std::vector<Point> *)), bmp_show, SLOT(set_vp1(std::vector<Point> *)));
		}
		else if (tab_old == wshed_widget)
		{
			//		QObject::connect(this,SIGNAL(marks_changed()),work_show,SLOT(mark_changed()));
			QObject::disconnect(this, SIGNAL(marks_changed()), wshed_widget, SLOT(marks_changed()));
		}
		else if (tab_old == hyst_widget)
		{
			hyst_widget->clean_up();

			QObject::disconnect(this, SIGNAL(bmp_changed()), hyst_widget, SLOT(bmp_changed()));
			QObject::disconnect(bmp_show, SIGNAL(mousepressed_sign(Point)), hyst_widget, SLOT(pt_clicked(Point)));
			QObject::disconnect(bmp_show, SIGNAL(mousemoved_sign(Point)), hyst_widget, SLOT(pt_moved(Point)));
			QObject::disconnect(bmp_show, SIGNAL(mousereleased_sign(Point)), hyst_widget, SLOT(pt_released(Point)));

			QObject::disconnect(hyst_widget, SIGNAL(vp1_changed(std::vector<Point> *)), bmp_show, SLOT(set_vp1(std::vector<Point> *)));
			QObject::disconnect(hyst_widget, SIGNAL(vpdyn_changed(std::vector<Point> *)), bmp_show, SLOT(set_vpdyn(std::vector<Point> *)));
			QObject::disconnect(hyst_widget, SIGNAL(vp1dyn_changed(std::vector<Point> *, std::vector<Point> *)), bmp_show, SLOT(set_vp1_dyn(std::vector<Point> *, std::vector<Point> *)));
		}
		else if (tab_old == lw_widget)
		{
			lw_widget->cleanup();

			QObject::disconnect(this, SIGNAL(bmp_changed()), lw_widget, SLOT(bmp_changed()));
			QObject::disconnect(bmp_show, SIGNAL(mousepressed_sign(Point)), lw_widget, SLOT(pt_clicked(Point)));
			QObject::disconnect(bmp_show, SIGNAL(mousedoubleclick_sign(Point)), lw_widget, SLOT(pt_doubleclicked(Point)));
			QObject::disconnect(bmp_show, SIGNAL(mousepressedmid_sign(Point)), lw_widget, SLOT(pt_midclicked(Point)));
			QObject::disconnect(bmp_show, SIGNAL(mousedoubleclickmid_sign(Point)), lw_widget, SLOT(pt_doubleclickedmid(Point)));
			QObject::disconnect(bmp_show, SIGNAL(mousemoved_sign(Point)), lw_widget, SLOT(pt_moved(Point)));
			QObject::disconnect(bmp_show, SIGNAL(mousereleased_sign(Point)), lw_widget, SLOT(pt_released(Point)));
			QObject::disconnect(lw_widget, SIGNAL(vp1_changed(std::vector<Point> *)), bmp_show, SLOT(set_vp1(std::vector<Point> *)));
			QObject::disconnect(lw_widget, SIGNAL(vpdyn_changed(std::vector<Point> *)), bmp_show, SLOT(set_vpdyn(std::vector<Point> *)));
			QObject::disconnect(lw_widget, SIGNAL(vp1dyn_changed(std::vector<Point> *, std::vector<Point> *)), bmp_show, SLOT(set_vp1_dyn(std::vector<Point> *, std::vector<Point> *)));
		...
		else if (tab_old == iftrg_widget)
		{
			...
			QObject::disconnect(this, SIGNAL(bmp_changed()), iftrg_widget, SLOT(bmp_changed()));

			QObject::disconnect(bmp_show, SIGNAL(mousepressed_sign(Point)), iftrg_widget, SLOT(mouse_clicked(Point)));
			QObject::disconnect(bmp_show, SIGNAL(mousereleased_sign(Point)), iftrg_widget, SLOT(mouse_released(Point)));
			QObject::disconnect(bmp_show, SIGNAL(mousemoved_sign(Point)), iftrg_widget, SLOT(mouse_moved(Point)));

			QObject::disconnect(iftrg_widget, SIGNAL(vm_changed(std::vector<mark> *)), bmp_show, SLOT(set_vm(std::vector<mark> *)));
			QObject::disconnect(iftrg_widget, SIGNAL(vmdyn_changed(std::vector<Point> *)), bmp_show, SLOT(set_vpdyn(std::vector<Point> *)));
		...
		else if (tab_old == FMF_widget)
		{
			...
			QObject::disconnect(this, SIGNAL(bmp_changed()), FMF_widget, SLOT(bmp_changed()));

			QObject::disconnect(bmp_show, SIGNAL(mousepressed_sign(Point)), FMF_widget, SLOT(mouse_clicked(Point)));
			QObject::disconnect(bmp_show, SIGNAL(mousereleased_sign(Point)), FMF_widget, SLOT(mouse_released(Point)));
			QObject::disconnect(bmp_show, SIGNAL(mousemoved_sign(Point)), FMF_widget, SLOT(mouse_moved(Point)));

			QObject::disconnect(FMF_widget, SIGNAL(vpdyn_changed(std::vector<Point> *)), bmp_show, SLOT(set_vpdyn(std::vector<Point> *)));
		}
		else if (tab_old == OutlineCorrect_widget)
		{
			OutlineCorrect_widget->cleanup();
			QObject::disconnect(bmp_show, SIGNAL(mousepressed_sign(Point)), OutlineCorrect_widget, SLOT(mouse_clicked(Point)));
			QObject::disconnect(bmp_show, SIGNAL(mousereleased_sign(Point)), OutlineCorrect_widget, SLOT(mouse_released(Point)));
			QObject::disconnect(bmp_show, SIGNAL(mousemoved_sign(Point)), OutlineCorrect_widget, SLOT(mouse_moved(Point)));

			QObject::disconnect(work_show, SIGNAL(mousepressed_sign(Point)), OutlineCorrect_widget, SLOT(mouse_clicked(Point)));
			QObject::disconnect(work_show, SIGNAL(mousereleased_sign(Point)), OutlineCorrect_widget, SLOT(mouse_released(Point)));
			QObject::disconnect(work_show, SIGNAL(mousemoved_sign(Point)), OutlineCorrect_widget, SLOT(mouse_moved(Point)));

			QObject::disconnect(this, SIGNAL(work_changed()), OutlineCorrect_widget, SLOT(workbits_changed()));

			QObject::disconnect(OutlineCorrect_widget, SIGNAL(vpdyn_changed(std::vector<Point> *)), bmp_show, SLOT(set_vpdyn(std::vector<Point> *)));
			QObject::disconnect(OutlineCorrect_widget, SIGNAL(vpdyn_changed(std::vector<Point> *)), work_show, SLOT(set_vpdyn(std::vector<Point> *)));
		}
		else if (tab_old == feature_widget)
		{
			QObject::disconnect(bmp_show, SIGNAL(mousepressed_sign(Point)), feature_widget, SLOT(pt_clicked(Point)));
			QObject::disconnect(bmp_show, SIGNAL(mousemoved_sign(Point)), feature_widget, SLOT(pt_moved(Point)));
			QObject::disconnect(bmp_show, SIGNAL(mousereleased_sign(Point)), feature_widget, SLOT(pt_released(Point)));
			QObject::disconnect(work_show, SIGNAL(mousepressed_sign(Point)), feature_widget, SLOT(pt_clicked(Point)));
			QObject::disconnect(work_show, SIGNAL(mousemoved_sign(Point)), feature_widget, SLOT(kpt_moved(Point)));
			QObject::disconnect(work_show, SIGNAL(mousereleased_sign(Point)), feature_widget, SLOT(pt_released(Point)));
			QObject::disconnect(feature_widget, SIGNAL(vpdyn_changed(std::vector<Point> *)), bmp_show, SLOT(set_vpdyn(std::vector<Point> *)));
			QObject::disconnect(feature_widget, SIGNAL(vpdyn_changed(std::vector<Point> *)), work_show, SLOT(set_vpdyn(std::vector<Point> *)));
		...
		else if (tab_old == pickerwidget)
		{
			...
			QObject::disconnect(bmp_show, SIGNAL(mousepressed_sign(Point)), pickerwidget, SLOT(pt_clicked(Point)));
			QObject::disconnect(work_show, SIGNAL(mousepressed_sign(Point)), pickerwidget, SLOT(pt_clicked(Point)));

			QObject::disconnect(pickerwidget, SIGNAL(vp1_changed(std::vector<Point> *)), bmp_show, SLOT(set_vp1(std::vector<Point> *)));
			QObject::disconnect(pickerwidget, SIGNAL(vp1_changed(std::vector<Point> *)), work_show, SLOT(set_vp1(std::vector<Point> *)));
		}
		else if (tab_old == transfrmWidget)
		{
			transfrmWidget->cleanup();
			QObject::disconnect(bmp_show, SIGNAL(mousepressed_sign(Point)), transfrmWidget, SLOT(pt_clicked(Point)));
			QObject::disconnect(work_show, SIGNAL(mousepressed_sign(Point)), transfrmWidget, SLOT(pt_clicked(Point)));
			QObject::disconnect(this, SIGNAL(bmp_changed()), transfrmWidget, SLOT(bmp_changed()));
			QObject::disconnect(this, SIGNAL(work_changed()), transfrmWidget, SLOT(work_changed()));
			QObject::disconnect(this, SIGNAL(tissues_changed()), transfrmWidget, SLOT(tissues_changed()));
		}

		if (qw == threshold_widget)
		{
			QObject::connect(this, SIGNAL(bmp_changed()), threshold_widget, SLOT(bmp_changed()));
			//			threshold_widget->init();
		}
		else if (qw == measurement_widget)
		{
			QObject::connect(this, SIGNAL(marks_changed()), measurement_widget, SLOT(marks_changed()));
			QObject::connect(bmp_show, SIGNAL(mousepressed_sign(Point)), measurement_widget, SLOT(pt_clicked(Point)));
			QObject::connect(work_show, SIGNAL(mousepressed_sign(Point)), measurement_widget, SLOT(pt_clicked(Point)));
			QObject::connect(measurement_widget, SIGNAL(vp1_changed(std::vector<Point> *)), bmp_show, SLOT(set_vp1(std::vector<Point> *)));
			QObject::connect(measurement_widget, SIGNAL(vp1_changed(std::vector<Point> *)), work_show, SLOT(set_vp1(std::vector<Point> *)));
			QObject::connect(measurement_widget, SIGNAL(vpdyn_changed(std::vector<Point> *)), bmp_show, SLOT(set_vpdyn(std::vector<Point> *)));
			QObject::connect(measurement_widget, SIGNAL(vpdyn_changed(std::vector<Point> *)), work_show, SLOT(set_vpdyn(std::vector<Point> *)));
			QObject::connect(measurement_widget, SIGNAL(vp1dyn_changed(std::vector<Point> *, std::vector<Point> *, bool)), bmp_show, SLOT(set_vp1_dyn(std::vector<Point> *, std::vector<Point> *, bool)));
			QObject::connect(measurement_widget, SIGNAL(vp1dyn_changed(std::vector<Point> *, std::vector<Point> *, bool)), work_show, SLOT(set_vp1_dyn(std::vector<Point> *, std::vector<Point> *, bool)));
			QObject::connect(bmp_show, SIGNAL(mousemoved_sign(Point)), measurement_widget, SLOT(pt_moved(Point)));
			QObject::connect(work_show, SIGNAL(mousemoved_sign(Point)), measurement_widget, SLOT(pt_moved(Point)));
			work_show->setMouseTracking(true);
			bmp_show->setMouseTracking(true);
		}
		else if (qw == vesselextr_widget)
		{
			QObject::connect(this, SIGNAL(marks_changed()), vesselextr_widget, SLOT(marks_changed()));
			QObject::connect(vesselextr_widget, SIGNAL(vp1_changed(std::vector<Point> *)), bmp_show, SLOT(set_vp1(std::vector<Point> *)));
		}
		else if (qw == wshed_widget)
		{
			//		QObject::disconnect(this,SIGNAL(marks_changed()),work_show,SLOT(mark_changed()));
			QObject::connect(this, SIGNAL(marks_changed()), wshed_widget, SLOT(marks_changed()));
			//			wshed_widget->init();
		}
		else if (qw == hyst_widget)
		{
			QObject::connect(this, SIGNAL(bmp_changed()), hyst_widget, SLOT(bmp_changed()));
			QObject::connect(bmp_show, SIGNAL(mousepressed_sign(Point)), hyst_widget, SLOT(pt_clicked(Point)));
			QObject::connect(bmp_show, SIGNAL(mousemoved_sign(Point)), hyst_widget, SLOT(pt_moved(Point)));
			QObject::connect(bmp_show, SIGNAL(mousereleased_sign(Point)), hyst_widget, SLOT(pt_released(Point)));

			QObject::connect(hyst_widget, SIGNAL(vp1_changed(std::vector<Point> *)), bmp_show, SLOT(set_vp1(std::vector<Point> *)));
			QObject::connect(hyst_widget, SIGNAL(vpdyn_changed(std::vector<Point> *)), bmp_show, SLOT(set_vpdyn(std::vector<Point> *)));
			QObject::connect(hyst_widget, SIGNAL(vp1dyn_changed(std::vector<Point> *, std::vector<Point> *)), bmp_show, SLOT(set_vp1_dyn(std::vector<Point> *, std::vector<Point> *)));

			//			hyst_widget->init();
		}
		else if (qw == lw_widget)
		{
			QObject::connect(this, SIGNAL(bmp_changed()), lw_widget, SLOT(bmp_changed()));
			QObject::connect(bmp_show, SIGNAL(mousepressed_sign(Point)), lw_widget, SLOT(pt_clicked(Point)));
			QObject::connect(bmp_show, SIGNAL(mousedoubleclick_sign(Point)), lw_widget, SLOT(pt_doubleclicked(Point)));
			QObject::connect(bmp_show, SIGNAL(mousepressedmid_sign(Point)), lw_widget, SLOT(pt_midclicked(Point)));
			QObject::connect(bmp_show, SIGNAL(mousedoubleclickmid_sign(Point)), lw_widget, SLOT(pt_doubleclickedmid(Point)));
			QObject::connect(bmp_show, SIGNAL(mousemoved_sign(Point)), lw_widget, SLOT(pt_moved(Point)));
			QObject::connect(bmp_show, SIGNAL(mousereleased_sign(Point)), lw_widget, SLOT(pt_released(Point)));
			QObject::connect(lw_widget, SIGNAL(vp1_changed(std::vector<Point> *)), bmp_show, SLOT(set_vp1(std::vector<Point> *)));
			QObject::connect(lw_widget, SIGNAL(vpdyn_changed(std::vector<Point> *)), bmp_show, SLOT(set_vpdyn(std::vector<Point> *)));
			QObject::connect(lw_widget, SIGNAL(vp1dyn_changed(std::vector<Point> *, std::vector<Point> *)), bmp_show, SLOT(set_vp1_dyn(std::vector<Point> *, std::vector<Point> *)));
		...
		else if (qw == iftrg_widget)
		{
			...
			QObject::connect(this, SIGNAL(bmp_changed()), iftrg_widget, SLOT(bmp_changed()));

			QObject::connect(bmp_show, SIGNAL(mousepressed_sign(Point)), iftrg_widget, SLOT(mouse_clicked(Point)));
			QObject::connect(bmp_show, SIGNAL(mousereleased_sign(Point)), iftrg_widget, SLOT(mouse_released(Point)));
			QObject::connect(bmp_show, SIGNAL(mousemoved_sign(Point)), iftrg_widget, SLOT(mouse_moved(Point)));

			QObject::connect(iftrg_widget, SIGNAL(vm_changed(std::vector<mark> *)), bmp_show, SLOT(set_vm(std::vector<mark> *)));
			QObject::connect(iftrg_widget, SIGNAL(vmdyn_changed(std::vector<Point> *)), bmp_show, SLOT(set_vpdyn(std::vector<Point> *)));
		}
		else if (qw == FMF_widget)
		{
			QObject::connect(this, SIGNAL(bmp_changed()), FMF_widget, SLOT(bmp_changed()));

			QObject::connect(bmp_show, SIGNAL(mousepressed_sign(Point)), FMF_widget, SLOT(mouse_clicked(Point)));
			QObject::connect(bmp_show, SIGNAL(mousereleased_sign(Point)), FMF_widget, SLOT(mouse_released(Point)));
			QObject::connect(bmp_show, SIGNAL(mousemoved_sign(Point)), FMF_widget, SLOT(mouse_moved(Point)));

			QObject::connect(FMF_widget, SIGNAL(vpdyn_changed(std::vector<Point> *)), bmp_show, SLOT(set_vpdyn(std::vector<Point> *)));
		}
		else if (qw == OutlineCorrect_widget)
		{
			QObject::connect(bmp_show, SIGNAL(mousepressed_sign(Point)), OutlineCorrect_widget, SLOT(mouse_clicked(Point)));
			QObject::connect(bmp_show, SIGNAL(mousereleased_sign(Point)), OutlineCorrect_widget, SLOT(mouse_released(Point)));
			QObject::connect(bmp_show, SIGNAL(mousemoved_sign(Point)), OutlineCorrect_widget, SLOT(mouse_moved(Point)));

			QObject::connect(work_show, SIGNAL(mousepressed_sign(Point)), OutlineCorrect_widget, SLOT(mouse_clicked(Point)));
			QObject::connect(work_show, SIGNAL(mousereleased_sign(Point)), OutlineCorrect_widget, SLOT(mouse_released(Point)));
			QObject::connect(work_show, SIGNAL(mousemoved_sign(Point)), OutlineCorrect_widget, SLOT(mouse_moved(Point)));

			QObject::connect(this, SIGNAL(work_changed()), OutlineCorrect_widget, SLOT(workbits_changed()));

			QObject::connect(OutlineCorrect_widget, SIGNAL(vpdyn_changed(std::vector<Point> *)), bmp_show, SLOT(set_vpdyn(std::vector<Point> *)));
			QObject::connect(OutlineCorrect_widget, SIGNAL(vpdyn_changed(std::vector<Point> *)), work_show, SLOT(set_vpdyn(std::vector<Point> *)));
		...
		{
			QObject::connect(bmp_show, SIGNAL(mousepressed_sign(Point)), feature_widget, SLOT(pt_clicked(Point)));
			QObject::connect(bmp_show, SIGNAL(mousemoved_sign(Point)), feature_widget, SLOT(pt_moved(Point)));
			QObject::connect(bmp_show, SIGNAL(mousereleased_sign(Point)), feature_widget, SLOT(pt_released(Point)));
			QObject::connect(work_show, SIGNAL(mousepressed_sign(Point)), feature_widget, SLOT(pt_clicked(Point)));
			QObject::connect(work_show, SIGNAL(mousemoved_sign(Point)), feature_widget, SLOT(pt_moved(Point)));
			QObject::connect(work_show, SIGNAL(mousereleased_sign(Point)), feature_widget, SLOT(pt_released(Point)));
			QObject::connect(feature_widget, SIGNAL(vpdyn_changed(std::vector<Point> *)), bmp_show, SLOT(set_vpdyn(std::vector<Point> *)));
			QObject::connect(feature_widget, SIGNAL(vpdyn_changed(std::vector<Point> *)), work_show, SLOT(set_vpdyn(std::vector<Point> *)));
		...
		{
			QObject::connect(bmp_show, SIGNAL(mousepressed_sign(Point)), pickerwidget, SLOT(pt_clicked(Point)));
			QObject::connect(work_show, SIGNAL(mousepressed_sign(Point)), pickerwidget, SLOT(pt_clicked(Point)));

			QObject::connect(pickerwidget, SIGNAL(vp1_changed(vector<Point> *)), bmp_show, SLOT(set_vp1(vector<Point> *)));
			QObject::connect(pickerwidget, SIGNAL(vp1_changed(vector<Point> *)), work_show, SLOT(set_vp1(vector<Point> *)));
		}
		else if (qw == transfrmWidget)
		{
			QObject::connect(bmp_show, SIGNAL(mousepressed_sign(Point)), transfrmWidget, SLOT(pt_clicked(Point)));
			QObject::connect(work_show, SIGNAL(mousepressed_sign(Point)), transfrmWidget, SLOT(pt_clicked(Point)));
			QObject::connect(this, SIGNAL(bmp_changed()), transfrmWidget, SLOT(bmp_changed()));
			QObject::connect(this, SIGNAL(work_changed()), transfrmWidget, SLOT(work_changed()));
			QObject::connect(this, SIGNAL(tissues_changed()), transfrmWidget, SLOT(tissues_changed()));
		}
...
void MainWindow::updateMethodButtonsPressed(QWidget1 *qw)
{
	...
	for (unsigned short i = 0; i < nrtabbuttons; i++)
	{
		if (showpb_tab[i])
		{
			if (tabwidgets[i] == qw) pos = counter;
			counter++;
		}
	}
	...
	{
		if (counter1 == pos) pb_tab[i]->setOn(true);
		else pb_tab[i]->setOn(false);
	}

	for (unsigned short i = (nrtabbuttons + 1) / 2; counter1 < counter; i++, counter1++)
	{
		if (counter1 == pos) pb_tab[i]->setOn(true);
		else pb_tab[i]->setOn(false);
	}
...
void MainWindow::LoadAtlas(QDir path1)
{
	...
	if (m_atlasfilename.nratlases > m_atlasfilename.maxnr) m_atlasfilename.nratlases = m_atlasfilename.maxnr;
	for (size_t i = 0; i < m_atlasfilename.nratlases; i++)
	{
		m_atlasfilename.m_atlasfilename[i] = names1[i];
		QFileInfo names1fi(names1[i]);
		atlasmenu->changeItem(m_atlasfilename.atlasnr[i], names1fi.completeBaseName());
...
void MainWindow::LoadLoadProj(QString path1)
{
	...
	while (fplatestproj != NULL && projcounter < 4)
	{
		...
		if (!qs_filename1.isEmpty()) AddLoadProj(qs_filename1);
	}
	if (fplatestproj != 0) fclose(fplatestproj);
}

void MainWindow::AddLoadProj(QString path1)
{
	if (m_loadprojfilename.m_loadprojfilename1 != path1 && m_loadprojfilename.m_loadprojfilename2 != path1 &&
	        m_loadprojfilename.m_loadprojfilename3 != path1 && m_loadprojfilename.m_loadprojfilename4 != path1)
	{
		m_loadprojfilename.m_loadprojfilename4 = m_loadprojfilename.m_loadprojfilename3;
		m_loadprojfilename.m_loadprojfilename3 = m_loadprojfilename.m_loadprojfilename2;
		m_loadprojfilename.m_loadprojfilename2 = m_loadprojfilename.m_loadprojfilename1;
	...
	else
	{
		...
		if (m_loadprojfilename.m_loadprojfilename3 == path1)
		{
			...
			m_loadprojfilename.m_loadprojfilename3 = m_loadprojfilename.m_loadprojfilename1;
			m_loadprojfilename.m_loadprojfilename1 = dummy;
		}
		if (m_loadprojfilename.m_loadprojfilename4 == path1)
		{
			QString dummy = m_loadprojfilename.m_loadprojfilename4;
			m_loadprojfilename.m_loadprojfilename4 = m_loadprojfilename.m_loadprojfilename1;
			m_loadprojfilename.m_loadprojfilename1 = dummy;
		}
	}
	...
	if (m_loadprojfilename.m_loadprojfilename1 != QString(""))
	{
		...
		if (pos != -1 && (int)m_loadprojfilename.m_loadprojfilename1.length() > pos + 1)
		{
			QString name1 = m_loadprojfilename.m_loadprojfilename1.right(m_loadprojfilename.m_loadprojfilename1.length() - pos - 1);
	...
	if (m_loadprojfilename.m_loadprojfilename2 != QString(""))
	{
		...
		if (pos != -1 && (int)m_loadprojfilename.m_loadprojfilename2.length() > pos + 1)
		{
			QString name1 = m_loadprojfilename.m_loadprojfilename2.right(m_loadprojfilename.m_loadprojfilename2.length() - pos - 1);
	...
	if (m_loadprojfilename.m_loadprojfilename3 != QString(""))
	{
		...
		if (pos != -1 && (int)m_loadprojfilename.m_loadprojfilename3.length() > pos + 1)
		{
			QString name1 = m_loadprojfilename.m_loadprojfilename3.right(m_loadprojfilename.m_loadprojfilename3.length() - pos - 1);
	...
	if (m_loadprojfilename.m_loadprojfilename4 != QString(""))
	{
		...
		if (pos != -1 && (int)m_loadprojfilename.m_loadprojfilename4.length() > pos + 1)
		{
			QString name1 = m_loadprojfilename.m_loadprojfilename4.right(m_loadprojfilename.m_loadprojfilename4.length() - pos - 1);
...
{
	if (latestprojpath == QString("")) return;
...
void MainWindow::pb_tab_pressed(int nr)
{
	...
	for (unsigned short i = 0; i < nrtabbuttons; i++) if (showpb_tab[i]) pos1++;
	if ((tabnr > (nrtabbuttons + 1) / 2)) tabnr = tabnr + (pos1 + 1) / 2 - (nrtabbuttons + 1) / 2;
	if (tabnr > pos1) return;
	pos1 = 0;
	unsigned short pos2 = 0;
	while (pos1 < tabnr)
	{
		if (showpb_tab[pos2]) pos1++;
		pos2++;
	}
	pos2--;
...
void MainWindow::execute_cleanup()
{
	...
	for (unsigned short i = handler3D->return_startslice(); i < handler3D->return_endslice(); i++)
	{
		slices[i - handler3D->return_startslice()] = handler3D->return_tissues(activelayer, i);
	}
	TissueCleaner TC(slices, handler3D->return_endslice() - handler3D->return_startslice(), handler3D->return_width(), handler3D->return_height());
	if (!TC.Allocate())
	{
		QMessageBox::information(0, "iSeg", "Not enough memory.\nThis operation cannot be performed.\n");
	}
	else
	{
		int rate, minsize;
...
{
	QObject::disconnect(work_show, SIGNAL(mousereleased_sign(Point)), this, SLOT(reconnectmouse_afterrelease(Point)));
	connect_mouseclick();
}

void MainWindow::handle_begin_datachange(common::DataSelection &dataSelection, QWidget *sender, bool beginUndo)
{
...
void MainWindow::handle_end_datachange(QWidget *sender, common::EndUndoAction undoAction)
{
	// End undo
...
void MainWindow::handle_end_datachange(QRect rect, QWidget *sender, common::EndUndoAction undoAction)
{
	// End undo
	end_undo_helper(undoAction);
...
{
	QObject::disconnect(transfrmWidget, SIGNAL(begin_datachange(common::DataSelection &, QWidget *, bool)), this, SLOT(handle_begin_datachange(common::DataSelection &, QWidget *, bool)));
	QObject::disconnect(transfrmWidget, SIGNAL(end_datachange(QWidget *, common::EndUndoAction)), this, SLOT(handle_end_datachange(QWidget *, common::EndUndoAction)));
...
void MainWindow::handle_begin_dataexport(common::DataSelection &dataSelection, QWidget *sender)
{
	// Handle pending transforms
	if (methodTab->visibleWidget() == transfrmWidget
...
void MainWindow::startwidget()
{
	...
	dimension[2] = (int)(handler3D->return_endslice() - handler3D->return_startslice());
	...
	{
		voxel.push_back(new unsigned char[dimension[0] * dimension[1] * dimension[2]]);
