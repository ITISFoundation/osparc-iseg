		...
		: QWidget1(parent, name, wFlags), handler3D(hand3D)
{
	...
	rb_batchinter->setToolTip(
			Format("Select the stride of (distance between) segmented slices and the "
	...
	rb_tissueall->setToolTip(
			Format("Works on all tissue at once (not available for extrapolation)."));
	...
	cb_medianset->setToolTip(Format(
			...
			"is employed. Otherwise shape-based interpolation [2] is used. Multiple "
			"objects (with "
	...
	pushstart->setToolTip(Format(
			"Interpolation/extrapolation is based on 2 slices. Click start to select "
			...
			"The user can add it with Adder function. The 'All Tissues' option adds "
...
void interpol_widget::execute()
{
	...
	if (current != startnr)
	{
		...
		if (rb_extra->isOn())
		{
			...
			if (rb_work->isOn())
			{
				handler3D->extrapolatework(startnr, current,
																	 (unsigned short)sb_slicenr->value() - 1);
			}
			else
			{
				handler3D->extrapolatetissue(startnr, current,
																		 (unsigned short)sb_slicenr->value() - 1,
																		 tissuenr);
			}
		...
		else if (rb_inter->isOn())
		{
			...
			if (rb_work->isOn())
			{
				...
				{
					handler3D->interpolateworkgrey_medianset(startnr, current,
			...
			else if (rb_tissue->isOn())
			{
				...
				if (cb_medianset->isChecked())
				{
					handler3D->interpolatetissue_medianset(startnr, current, tissuenr,
																								 rb_8connectivity->isOn());
				}
				else
				{
			...
			else if (rb_tissueall->isOn())
			{
				...
				if (cb_medianset->isChecked())
				{
					handler3D->interpolatetissuegrey_medianset(startnr, current,
																										 rb_8connectivity->isOn());
				}
				else
				{
		...
		else if (rb_batchinter->isOn())
		{
			...
			if (rb_work->isOn())
			{
				...
				if (cb_medianset->isChecked())
				{
					...
					for (batchstart = startnr; batchstart <= current - batchstride;
							 batchstart += batchstride)
					{
						handler3D->interpolateworkgrey_medianset(
								batchstart, batchstart + batchstride, rb_8connectivity->isOn());
					}
					// Last batch with smaller stride
					if (batchstart > current && current - (batchstart - batchstride) >= 2)
					{
						handler3D->interpolateworkgrey_medianset(
								batchstart - batchstride, current, rb_8connectivity->isOn());
					}
				}
				else
				{
					...
					for (batchstart = startnr; batchstart <= current - batchstride;
							 batchstart += batchstride)
					{
						handler3D->interpolateworkgrey(batchstart,
																					 batchstart + batchstride);
					}
					// Last batch with smaller stride
					if (batchstart > current && current - (batchstart - batchstride) >= 2)
			...
			else if (rb_tissue->isOn())
			{
				...
				if (cb_medianset->isChecked())
				{
					...
					if (batchstart > current && current - (batchstart - batchstride) >= 2)
					{
						handler3D->interpolatetissue_medianset(batchstart - batchstride,
																									 current, tissuenr,
																									 rb_8connectivity->isOn());
					}
				}
				else
				{
					...
					for (batchstart = startnr; batchstart <= current - batchstride;
							 batchstart += batchstride)
					{
						handler3D->interpolatetissue(batchstart, batchstart + batchstride,
																				 tissuenr);
					}
					// Last batch with smaller stride
					if (batchstart > current && current - (batchstart - batchstride) >= 2)
					{
						handler3D->interpolatetissue(batchstart - batchstride, current,
																				 tissuenr);
					}
				}
			}
			else if (rb_tissueall->isOn())
			{
				...
				else
				{
					...
					for (batchstart = startnr; batchstart <= current - batchstride;
							 batchstart += batchstride)
					{
						handler3D->interpolatetissuegrey(batchstart,
																						 batchstart + batchstride);
					}
					// Last batch with smaller stride
					if (batchstart > current && current - (batchstart - batchstride) >= 2)
