...
interpol_widget::interpol_widget(SlicesHandler *hand3D, QWidget *parent, const char *name, Qt::WindowFlags wFlags)
  : QWidget1( parent, name, wFlags ),handler3D(hand3D)
{
	...
	rb_batchinter->setToolTip(Format("Select the stride of (distance between) segmented slices and the start slice to batch segment multiple slices."));
	...
	rb_tissue->setToolTip(Format("Works on the tissue distribution of the currently selected tissue."));
	rb_tissueall=new QRadioButton(QString("All Tissues"),vboxdataselect);
	rb_tissueall->setToolTip(Format("Works on all tissue at once (not available for extrapolation)."));
	...
	sb_slicenr->setToolTip(Format("Set the slice index where the Tissue/Target distribution will be extrapolated."));
	...
	sb_batchstride->setToolTip(Format("The stride is the distance between segmented slices. For example, "
		"you may segment every fifth slice (stride=5), then interpolate in between."));

	cb_medianset=new QCheckBox(QString("Median Set"),vboxparams);
	cb_medianset->setChecked(true);
	cb_medianset->setToolTip(Format("If Median Set is ON, the algorithm described in [1] "
		"is employed. Otherwise shape-based interpolation [2] is used. Multiple objects (with "
		"different gray values or tissue assignments, respectively) can be interpolated jointly "
	...
	pushstart->setToolTip(Format("Interpolation/extrapolation is based on 2 slices. Click start to select "
		"the first slice and Execute to select the second slice. Interpolation automatically interpolates the intermediate slices."
		"<br>"
		"Note:<br>The result is displayed in the Target but is not directly added to the tissue distribution. "
		"The user can add it with Adder function. The 'All Tissues' option adds the result directly to the tissue."));
...
QSize interpol_widget::sizeHint() const
{
...
/*				if(startnr==current+2||current==startnr+2){
			...
			if(rb_work->isOn()){
				...
				if (cb_medianset->isChecked()) {
					...
					for (batchstart = startnr; batchstart <= current-batchstride; batchstart += batchstride) {
					...
					unsigned short batchstart;
					for (batchstart = startnr; batchstart <= current-batchstride; batchstart += batchstride) {
						handler3D->interpolateworkgrey(batchstart, batchstart + batchstride);
					}
					// Last batch with smaller stride
					if (batchstart > current && current-(batchstart-batchstride) >= 2) {
						handler3D->interpolateworkgrey(batchstart-batchstride, current);
					}
				}
			} else if (rb_tissue->isOn()) {
				dataSelection.work = true;
				...
				if (cb_medianset->isChecked()) {
					...
					unsigned short batchstart;
					for (batchstart = startnr; batchstart <= current-batchstride; batchstart += batchstride) {
						handler3D->interpolatetissue(batchstart, batchstart + batchstride, tissuenr);
					}
					// Last batch with smaller stride
					if (batchstart > current && current-(batchstart-batchstride) >= 2) {
						handler3D->interpolatetissue(batchstart-batchstride, current, tissuenr);
					}
				}
			} else if (rb_tissueall->isOn()) {
				dataSelection.tissues = true;
				...
				if (cb_medianset->isChecked()) {
					unsigned short batchstart;
					for (batchstart = startnr; batchstart <= current-batchstride; batchstart += batchstride) {
						handler3D->interpolatetissuegrey_medianset(batchstart, batchstart + batchstride, rb_8connectivity->isOn());
					}
					// Last batch with smaller stride
					if (batchstart > current && current-(batchstart-batchstride) >= 2) {
...
{
	if(version>=2) {
		QObject::disconnect(modegroup,SIGNAL(buttonClicked(int)),this,SLOT(method_changed()));
		QObject::disconnect(sourcegroup,SIGNAL(buttonClicked(int)),this,SLOT(source_changed()));
