...
bool dicomread::load_picture(float *bits)
{
	...
	if(go_label(32736, 16)){
		if(fread(buffer1, 1, 4, fp)==4){
			...
			i=(unsigned)buffer1[0]+256*((unsigned)buffer1[1]+
...
bool dicomread::load_picture(float *bits, Point p, unsigned short dx, unsigned short dy)
{
	get_height();
	get_width();
	get_bitdepth();
...
bool dicomread::load_picture(float *bits, float center, float contrast, Point p, unsigned short dx, unsigned short dy)
{
...
float dicomread::slicepos()
{
	...
	if(read_field(32, 50)){
		...
		if(read_field(32, 55)) {
			...
			if(l>0)	f=(vec1[0]*loc[0]+vec1[1]*loc[1]+vec1[2]*loc[2])/sqrt(l);
		}
	}
...
Pair dicomread::pixelsize()
{
	...
	if (read_field(40, 48)) {
		...
		if(pos<(int)l&&(buffer[pos]=='e'||buffer[pos]=='E')) {
			...
			while((pos<length&&buffer[pos]>='0'&&buffer[pos]<='9')||buffer[pos]==' ') {
				if(buffer[pos]!=' ')
		...
		if(pos<(int)l&&(buffer[pos]=='\\'||buffer[pos]==':'||buffer[pos]=='/')) {
			p.low=f/d;
			...
			else p.low=p.high=1.0f;
		} else p.high=p.low=1.0f;
...
//			fclose(fp3);
			...
			if(!read_fieldnr(a1,b1)) return false;
		}
//		FILE *fp3=fopen("D:\\Development\\segmentation\\sample images\\test100.txt","a");
//		fprintf(fp3,"b%i %i \n",(int)a1,(int)b1);
//		fclose(fp3);
		if(a1==a&&b1==b) {
			if(read_fieldcontent()) return true;
			return false;
		} else {
			go_start();
			return false;
...
bool dicomread::read_fieldnr(unsigned &a, unsigned &b)
{
	if(fread(buffer1, 1, 4, fp)==4){
		...
		if(a==65534&&b==57344) {
			if(fread(buffer1, 1, 4, fp)==4) {
				if(buffer1[3]==255&&buffer1[2]==255&&buffer1[1]==255&&buffer1[0]==255) {
					depth++;
					//FILE *fp3=fopen("D:\\Development\\segmentation\\sample images\\test100.txt","a");
					//fprintf(fp3,"+1 %i\n",(int)depth);
					//fclose(fp3);
...
unsigned dicomread::read_length()
{
	unsigned i=0;
	if(fread(buffer1, 1, 4, fp)==4){
		if(buffer1[0]>='A'&&buffer1[0]<='z'&&buffer1[1]>='A'&&buffer1[1]<='z'){
			if(((buffer1[0]!='O'||buffer1[1]!='B')&&(buffer1[0]!='O'||buffer1[1]!='W')&&(buffer1[0]!='S'||buffer1[1]!='Q')&&(buffer1[0]!='U'||buffer1[1]!='N'))) {
...
void dicomread::go_start()
{
	...
	if(result==0){
		if(fread(buffer1, 1, 4, fp)==4){
			if(buffer1[0]!='D'||buffer1[1]!='I'||buffer1[2]!='C'||buffer1[3]!='M')
#ifdef _MSC_VER
...
void dicomread::dsarray2float(float *vals, unsigned short nrvals)
{
	...
	for(unsigned short i=0;i<nrvals;i++) vals[i]=0;
	while(counter<nrvals) {
		vals[counter]=ds2float(pos);
		if(buffer[pos]!='\\'&&counter+1!=nrvals) return;
		pos++;
		counter++;
	}
}
