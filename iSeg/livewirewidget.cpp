...
livewire_widget::livewire_widget(SlicesHandler *hand3D, QWidget *parent, const char *name, Qt::WindowFlags wFlags)
  : QWidget1( parent, name, wFlags ),handler3D(hand3D)
{
	setToolTip(Format("Use the Auto Trace to follow ideal contour path or draw contours around a tissue to segment it."));
	...
	autotrace->setToolTip(Format("The Livewire (intelligent scissors) algorithm "
		"to automatically identify the ideal contour path.This algorithm uses information "
		"about the strength and orientation of the(smoothed) image gradient, "
		"the zero - crossing of the Laplacian (for fine tuning) together with some weighting "
		"to favor straighter lines to determine the most likely contour path. The "
		"contouring is started by clicking the left mouse button. Each subsequent left "
		"button click fixes another point and the suggested contour line in between. "
		"<br>"
		"Successive removing of unwanted points is achieved by clicking the middle "
		"mouse button. A double left click closes the contour while a double middle "
	...
	cb_freezing->setToolTip(Format("Specify the number of seconds after which a line segment is frozen even without mouse click if it has not changed."));
	...
	QObject::connect(drawmode,SIGNAL(buttonClicked(int)),this,SLOT(mode_changed()));
	QObject::connect(cb_freezing,SIGNAL(clicked()),this,SLOT(freezing_changed()));
	QObject::connect(sb_freezing,SIGNAL(valueChanged(int)),this,SLOT(sbfreezing_changed(int)));
...
livewire_widget::~livewire_widget()
{
	...
	if(lwfirst!=NULL) delete lwfirst;
}

QSize livewire_widget::sizeHint() const
{
...
void livewire_widget::pt_clicked(Point p)
{
	...
	//		lwfirst=bmphand->livewireinit(p);
		...
		if(straight->isOn()) {
			...
			dynamic.clear();
			if(cb_closing->isChecked()) addLine(&dynamic,p2,p);
			p1=p;
		} else {
			lw->return_path(p,&dynamic);
			established.insert(established.end(),dynamic.begin(),dynamic.end());		
			if(cb_closing->isChecked()) lwfirst->return_path(p,&dynamic);
			else dynamic.clear();
			lw->change_pt(p);
			if(cooling){
				times.clear();
	...
	if(drawing){
		if(clicks.size()==1) {
			...
			if(straight->isOn()) {
				...
				for(int i=0;i<establishedlengths.back();i++) it1++;
...
void livewire_widget::pt_moved(Point p)
{
	if(drawing){
		if(freedraw->isOn()) {
			...
			lw->return_path(p,&dynamic);
			if(cb_closing->isChecked()) lwfirst->return_path(p,&dynamic1);
			...
			if(cooling){
				...
				dynamicold.clear();
				dynamicold.insert(dynamicold.begin(),dynamic.begin(),dynamic.end());
...
//					dynamic.erase(dynamic.rbegin(),rit);
					...
					if(cb_closing->isChecked()) dynamic.insert(dynamic.end(),dynamic1.begin(),dynamic1.end());
					emit vp1dyn_changed(&established,&dynamic);
				} else {
					if(cb_closing->isChecked()) dynamic.insert(dynamic.end(),dynamic1.begin(),dynamic1.end());
					emit vpdyn_changed(&dynamic);
...
{
	if(version>=2) {
		QObject::disconnect(drawmode,SIGNAL(buttonClicked(int)),this,SLOT(mode_changed()));
		QObject::disconnect(cb_freezing,SIGNAL(clicked()),this,SLOT(freezing_changed()));
		QObject::disconnect(sb_freezing,SIGNAL(valueChanged(int)),this,SLOT(sbfreezing_changed(int)));
...
{
	mode_changed();
	if(hideparams&&!cb_freezing->isChecked()) hbox2->hide();
}