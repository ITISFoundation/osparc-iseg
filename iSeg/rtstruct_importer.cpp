...
RtstructImport::RtstructImport(QString loadfilename, SlicesHandler *hand3D, QWidget *parent, const char *name, Qt::WindowFlags wFlags)
  : QDialog( parent, name,TRUE, wFlags ),handler3D(hand3D)
{
	...
	for(size_t i=0;i<tissues.size();i++) {
		...
		for (tissuenr=0; tissuenr < TissueInfos::GetTissueCount() && tissues[i]->name != TissueInfos::GetTissueName(tissuenr+1).toAscii().data(); tissuenr++) {}
	...
	le_name=new QLineEdit(vectissuenames[cb_solids->currentItem()].c_str(),hbox2);
	...
	QObject::connect(cb_solids,SIGNAL(activated(int)),this,SLOT(solid_changed(int)));
	QObject::connect(pb_cancel,SIGNAL(clicked()),this,SLOT(close()));
	QObject::connect(cb_new,SIGNAL(clicked()),this,SLOT(new_changed()));
	QObject::connect(cb_ignore,SIGNAL(clicked()),this,SLOT(ignore_changed()));
	QObject::connect(pb_ok,SIGNAL(clicked()),this,SLOT(ok_pressed()));
	QObject::connect(infoButton,SIGNAL(clicked()),this,SLOT(show_priorityInfo()));
}

RtstructImport::~RtstructImport()
{
	delete vbox1;
}
...
void RtstructImport::solid_changed(int i)
{
	...
	QObject::disconnect(cb_ignore,SIGNAL(clicked()),this,SLOT(ignore_changed()));
	currentitem=i;
...
void RtstructImport::ok_pressed()
{
	...
	for(size_t i=0;i<tissues.size();i++) {
		if((vecignore[i]==false)&&(!vectissuenames[i].empty())&&(vecnew[i]==true)) nrnew++;
	}
	...
	unsigned short pixel_extents[2] = {handler3D->return_width(), handler3D->return_height()};
	float pixel_size[2] = {p.high, p.low};

	if(abs(dc[0]) != 1.0f || abs(dc[4]) != 1.0f) {
		QMessageBox::about(this,"Warning","Arbitrary rotations of image orientation (patient) not supported");
		return;
	}
...
{
	QMessageBox::information(this,"Priority Information",
