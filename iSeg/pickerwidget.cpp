...
picker_widget::picker_widget(SlicesHandler *hand3D, QWidget *parent, const char *name, Qt::WindowFlags wFlags)
  : QWidget1( parent, name, wFlags ),handler3D(hand3D)
{
	setToolTip(Format("Copy and erase regions. Copying can be used to transfer segmented regions from one slice to another. All the	functions are based on the current region selection."));
	...
	pb_cut->setToolTip(Format("Tissues or target image pixels inside the region selection are erased but a copy "
		"is placed on the clipboard."));
	pb_delete=new QPushButton("Delete",hbox3);
	pb_delete->setToolTip(Format("Tissues (resp. target image pixels) inside the region selection are erased."));
	...
	QObject::connect(worktissuegroup,SIGNAL(buttonClicked(int)),this,SLOT(worktissue_changed(int)));
...
QSize picker_widget::sizeHint() const
{
...
{
	if(version>=6) {
		QObject::disconnect(worktissuegroup,SIGNAL(buttonClicked(int)),this,SLOT(worktissue_changed(int)));
...
void picker_widget::pt_clicked(Point p)
{
	...
	if(rb_work->isOn())
		...
		bmphand->change2mask_connectedtissue(handler3D->get_active_tissuelayer(),currentselection, p,addorsub);
...
void picker_widget::worktissue_changed(int)
{
	update_active();
}
...
void picker_widget::copy_pressed()
{
	...
	if(clipboardworkortissue) {
		...
		tissues_size_t *tissues=bmphand->return_tissues(handler3D->get_active_tissuelayer());
		for(unsigned int i=0;i<area;i++){
...
{
	if(clipboardworkortissue!=rb_work->isOn()) return;
	unsigned int area=bmphand->return_area();
	...
	if(clipboardworkortissue) {
		...
		bmphand->copy2tissue(handler3D->get_active_tissuelayer(),valuedistrib2,mask);
...
void picker_widget::delete_pressed()
{
	...
	if(rb_work->isOn()) {
		...
		if(rb_erase->isOn())
			bmphand->erasetissue(handler3D->get_active_tissuelayer(), currentselection);
		else
			bmphand->floodtissue(handler3D->get_active_tissuelayer(), currentselection);
	}
