	...
	vtkInformationVector *vtkNotUsed(outputVector))
{
	...
	{
		cerr << "vtkGenericDataSetWriter::RequestData(): Cannot write this file format." << endl;
		return 0;
	}
...
bool vtkGenericDataSetWriter::WriteEsraTriangles( vtkPolyData* surface, const char* fname )
{
	...
	vtkDataArray *tissueArray = surface->GetCellData()->GetArray(this->MaterialArrayName);
	...
	vtkStringArray* tissueNames = vtkStringArray::SafeDownCast(surface->GetFieldData()->GetAbstractArray("TissueNames"));
	...
	if(tissueArray != NULL)
	{
		for (vtkIdType k=0; k<surface->GetNumberOfCells(); k++)
		{
			...
			if(surface->GetCellType(k) != VTK_TRIANGLE)
			{
				...
				vtkWarningMacro(<<"surface->GetCellType(k) != VTK_TRIANGLE " << surface->GetCellType(k) << " " << npts);
				return false;
			}
			int tissueId;
			if(tissueArrayInt) tissueId = tissueArrayInt->GetValue(k);
			else if(tissueArrayShort) tissueId = tissueArrayShort->GetValue(k);
			else tissueId = tissueArray->GetTuple1(k);
	...
	for (it = tissueCells.begin(); it != tissueCells.end(); ++it)
	{
		...
		if(Loud) std::cerr << "Color " << rgb[0] << " " << rgb[1] << " " << rgb[2] << std::endl;
		if(Loud) std::cerr << "Tissue " << name << std::endl;
		...
		if(Loud) std::cerr << "Ntris " << Ntriids/3 << std::endl;
	...
	if(Loud) std::cerr << "Npoints " << Npoints << std::endl;
