	...
	: QMainWindow(parent, name, wFlags)
{
	...
	tobmpBtn = new QPushButton(
		QIcon(m_picpath.absFilePath(QString("previous.png")).ascii()), "",
		this);
	...
	getTissueAll->setToolTip(
		Format("Get all tissue creates a mask in the Target "
	...
	bitstack_widget =
		new bits_stack(handler3D, this, "new window",
					   Qt::WDestructiveClose | Qt::WResizeNoErase);
	overlay_widget =
	...
	tissueadddock->setToolTip(
		Format("The tissue adder provides functionality "
	...
	{
		file->insertItem(
			QIcon(m_picpath.absFilePath(QString("filenew.png")).ascii()),
			"&New...", this, SLOT(execute_new()));
	...
	reloadmenu->insertItem("Reopen NIfTI...", this,
						   SLOT(execute_reloadnifti()));
#ifndef NORTDOSESUPPORT
	...
	if (!m_editingmode)
	{
		...
		importS4LLink->setToolTip(
			"Loads a Sim4Life live link file (the h5 file, "
	...
	exportmenu->insertItem("Export hdf...(h5)", this,
						   SLOT(execute_exporthdf()));
	...
	undonr = editmenu->insertItem(
		QIcon(m_picpath.absFilePath(QString("undo.png")).ascii()), "&Undo",
	...
	for (unsigned short i = 0; i < nrtabbuttons; i++)
	{
		...
		showtab_action[i] =
			new Q3Action(tabwidgets[i]->GetName().c_str(), 0, this);
	...
	toolmenu->insertItem("&Group Tissues...", this,
						 SLOT(execute_grouptissues()));
	...
	helpmenu->insertItem(
		QIcon(m_picpath.absFilePath(QString("help.png")).ascii()), "About",
		this, SLOT(execute_about()));

	QObject::connect(toworkBtn, SIGNAL(clicked()), this,
					 SLOT(execute_bmp2work()));
	QObject::connect(tobmpBtn, SIGNAL(clicked()), this,
					 SLOT(execute_work2bmp()));
	...
	QObject::connect(getTissueAll, SIGNAL(clicked()), this,
					 SLOT(tissue2workall()));
	QObject::connect(clearTissue, SIGNAL(clicked()), this,
					 SLOT(clearselected()));
	QObject::connect(clearTissues, SIGNAL(clicked()), this,
					 SLOT(cleartissues()));
	...
	QObject::connect(pb_first, SIGNAL(clicked()), this,
					 SLOT(pb_first_pressed()));
	...
	QObject::connect(pb_add, SIGNAL(clicked()), this,
					 SLOT(add_tissue_pushed()));
	...
	QObject::connect(
		...
		SLOT(handle_begin_datachange(iseg::DataSelection&, QWidget*, bool)));
	QObject::connect(
		threshold_widget, SIGNAL(end_datachange(QWidget*, iseg::EndUndoAction)),
	...
	QObject::connect(
		OutlineCorrect_widget,
		SIGNAL(end_datachange(QRect, QWidget*, iseg::EndUndoAction)), this,
	...
	QObject::connect(
		...
		SLOT(handle_begin_datachange(iseg::DataSelection&, QWidget*, bool)));
	QObject::connect(
		smoothing_widget, SIGNAL(end_datachange(QWidget*, iseg::EndUndoAction)),
		this, SLOT(handle_end_datachange(QWidget*, iseg::EndUndoAction)));
	QObject::connect(
		edge_widg,
		SIGNAL(begin_datachange(iseg::DataSelection&, QWidget*, bool)), this,
		SLOT(handle_begin_datachange(iseg::DataSelection&, QWidget*, bool)));
	QObject::connect(
		edge_widg, SIGNAL(end_datachange(QWidget*, iseg::EndUndoAction)), this,
		SLOT(handle_end_datachange(QWidget*, iseg::EndUndoAction)));
	QObject::connect(
		morph_widget,
		SIGNAL(begin_datachange(iseg::DataSelection&, QWidget*, bool)), this,
		SLOT(handle_begin_datachange(iseg::DataSelection&, QWidget*, bool)));
	QObject::connect(
		morph_widget, SIGNAL(end_datachange(QWidget*, iseg::EndUndoAction)),
		this, SLOT(handle_end_datachange(QWidget*, iseg::EndUndoAction)));
	QObject::connect(
		wshed_widget,
		SIGNAL(begin_datachange(iseg::DataSelection&, QWidget*, bool)), this,
		SLOT(handle_begin_datachange(iseg::DataSelection&, QWidget*, bool)));
	QObject::connect(
		wshed_widget, SIGNAL(end_datachange(QWidget*, iseg::EndUndoAction)),
		this, SLOT(handle_end_datachange(QWidget*, iseg::EndUndoAction)));
	QObject::connect(
		lw_widget,
		SIGNAL(begin_datachange(iseg::DataSelection&, QWidget*, bool)), this,
		SLOT(handle_begin_datachange(iseg::DataSelection&, QWidget*, bool)));
	QObject::connect(
		lw_widget, SIGNAL(end_datachange(QWidget*, iseg::EndUndoAction)), this,
		SLOT(handle_end_datachange(QWidget*, iseg::EndUndoAction)));
	QObject::connect(
		iftrg_widget,
		SIGNAL(begin_datachange(iseg::DataSelection&, QWidget*, bool)), this,
		SLOT(handle_begin_datachange(iseg::DataSelection&, QWidget*, bool)));
	QObject::connect(
		iftrg_widget, SIGNAL(end_datachange(QWidget*, iseg::EndUndoAction)),
		this, SLOT(handle_end_datachange(QWidget*, iseg::EndUndoAction)));
	QObject::connect(
		FMF_widget,
		SIGNAL(begin_datachange(iseg::DataSelection&, QWidget*, bool)), this,
		SLOT(handle_begin_datachange(iseg::DataSelection&, QWidget*, bool)));
	QObject::connect(
		FMF_widget, SIGNAL(end_datachange(QWidget*, iseg::EndUndoAction)), this,
		SLOT(handle_end_datachange(QWidget*, iseg::EndUndoAction)));
	QObject::connect(
		interpolwidget,
		SIGNAL(begin_datachange(iseg::DataSelection&, QWidget*, bool)), this,
		SLOT(handle_begin_datachange(iseg::DataSelection&, QWidget*, bool)));
	QObject::connect(
		interpolwidget, SIGNAL(end_datachange(QWidget*, iseg::EndUndoAction)),
		this, SLOT(handle_end_datachange(QWidget*, iseg::EndUndoAction)));
	QObject::connect(
		scalewidget,
		SIGNAL(begin_datachange(iseg::DataSelection&, QWidget*, bool)), this,
		SLOT(handle_begin_datachange(iseg::DataSelection&, QWidget*, bool)));
	QObject::connect(
		scalewidget, SIGNAL(end_datachange(QWidget*, iseg::EndUndoAction)),
		this, SLOT(handle_end_datachange(QWidget*, iseg::EndUndoAction)));
	QObject::connect(
		imagemathwidget,
		SIGNAL(begin_datachange(iseg::DataSelection&, QWidget*, bool)), this,
		SLOT(handle_begin_datachange(iseg::DataSelection&, QWidget*, bool)));
	QObject::connect(
		imagemathwidget, SIGNAL(end_datachange(QWidget*, iseg::EndUndoAction)),
		this, SLOT(handle_end_datachange(QWidget*, iseg::EndUndoAction)));
	QObject::connect(
		imageoverlaywidget,
		SIGNAL(begin_datachange(iseg::DataSelection&, QWidget*, bool)), this,
	...
	QObject::connect(
		...
		SLOT(handle_begin_datachange(iseg::DataSelection&, QWidget*, bool)));
	QObject::connect(
		bitstack_widget, SIGNAL(end_datachange(QWidget*, iseg::EndUndoAction)),
		this, SLOT(handle_end_datachange(QWidget*, iseg::EndUndoAction)));
	QObject::connect(
		bitstack_widget,
	...
	QObject::connect(
		...
		SLOT(handle_begin_datachange(iseg::DataSelection&, QWidget*, bool)));
	QObject::connect(
		hyst_widget, SIGNAL(end_datachange(QWidget*, iseg::EndUndoAction)),
		this, SLOT(handle_end_datachange(QWidget*, iseg::EndUndoAction)));
	QObject::connect(
		pickerwidget,
		SIGNAL(begin_datachange(iseg::DataSelection&, QWidget*, bool)), this,
		SLOT(handle_begin_datachange(iseg::DataSelection&, QWidget*, bool)));
	QObject::connect(
		pickerwidget, SIGNAL(end_datachange(QWidget*, iseg::EndUndoAction)),
		this, SLOT(handle_end_datachange(QWidget*, iseg::EndUndoAction)));
	QObject::connect(
		transfrmWidget,
		SIGNAL(begin_datachange(iseg::DataSelection&, QWidget*, bool)), this,
	...
	QObject::connect(m_MultiDataset_widget, SIGNAL(dataset_changed()), bmp_show,
					 SLOT(overlay_changed()));
	QObject::connect(m_MultiDataset_widget, SIGNAL(dataset_changed()),
					 work_show, SLOT(overlay_changed()));
	...
	m_acc_slicedown->connectItem(
		m_acc_slicedown->insertItem(QKeySequence(Qt::CTRL + Qt::Key_Left)),
		this, SLOT(slicenr_down()));
	...
	m_acc_undo->connectItem(
		m_acc_undo->insertItem(QKeySequence(Qt::Key_Escape)), this,
		SLOT(execute_undo()));
	...
	m_acc_redo->connectItem(m_acc_redo->insertItem(QKeySequence("Ctrl+Y")),
							this, SLOT(execute_redo()));
...
void MainWindow::closeEvent(QCloseEvent* qce)
{
	if (maybeSafe())
	{
		if (xsliceshower != NULL)
		{
			xsliceshower->close();
			QObject::disconnect(
				bmp_show, SIGNAL(scaleoffsetfactor_changed(float, float, bool)),
			...
			QObject::disconnect(zoom_widget, SIGNAL(set_zoom(double)),
								xsliceshower, SLOT(set_zoom(double)));
		...
		if (ysliceshower != NULL)
		{
			...
			QObject::disconnect(zoom_widget, SIGNAL(set_zoom(double)),
								ysliceshower, SLOT(set_zoom(double)));
		...
		for (int i = 0; i < m_MultiDataset_widget->GetNumberOfDatasets(); i++)
		{
			...
			if (!m_MultiDataset_widget->IsActive(i))
			{
				...
				if (SlicesHandler::SaveRaw_xy_swapped(
						str1.ascii(), m_MultiDataset_widget->GetBmpData(i), w,
						h, nrslices) != 0)
					ok = false;
				...
				str1 = QDir::temp().absFilePath(QString(tempFileName.c_str()));
				if (SlicesHandler::SaveRaw_xy_swapped(
						str1.ascii(), m_MultiDataset_widget->GetWorkingData(i),
						w, h, nrslices) != 0)
					ok = false;
				...
				{
					tempFileName =
						"work_float_eds_" + std::to_string(i) + ".raw";
					str1 =
						QDir::temp().absFilePath(QString(tempFileName.c_str()));
...
void MainWindow::execute_swapxz()
{
	...
	if (m_MultiDataset_widget->isVisible() && swapExtraDatasets)
	{
		...
		for (int i = 0; i < m_MultiDataset_widget->GetNumberOfDatasets(); i++)
		{
			...
			{
				std::string tempFileName =
					"bmp_float_" + std::to_string(i) + ".raw";
				str1 = QDir::temp().absFilePath(QString(tempFileName.c_str()));
...
void MainWindow::execute_resize(int resizetype)
{
	...
	ResizeDialog RD(handler3D,
					static_cast<ResizeDialog::eResizeType>(resizetype), this);
	RD.move(QCursor::pos());
	if (!RD.exec())
	{
		return;
	...
	{
		str1 = QDir::temp().absFilePath(QString("work_float.raw"));
		if (handler3D->ReadRawFloat(str1.ascii(), w + dxm + dxp, h + dym + dyp,
									0, nrslices + dzm + dzp) != 1)
			ok = false;
	}
	if (ok)
	{
	...
	{
		str1 = QDir::temp().absFilePath(QString("tissues.raw"));
		if (handler3D->ReloadRawTissues(str1.ascii(),
										sizeof(tissues_size_t) * 8, 0) != 1)
			ok = false;
	}
...
void MainWindow::update_brightnesscontrast(bool bmporwork, bool paint)
{
	if (bmporwork)
	{
		...
		float brightness =
			(le_brightnessbmp_val->text().toFloat() + 50.0f) * 0.01f;
...
void MainWindow::execute_loadvtk()
{
	...
	{
		QMessageBox::warning(
			this, "iSeg", "Error: Could not load file\n" + loadfilename + "\n",
			QMessageBox::Ok | QMessageBox::Default);
	}
}
...
void MainWindow::execute_reloadavw()
{
	...
	if (!loadfilename.isEmpty())
	{
		handler3D->ReloadAVW(loadfilename.ascii(),
...
void MainWindow::execute_loadsurface()
{
	...
	if (!loadfilename.isEmpty())
	{
		...
		msgBox.setText(
			"What would you like to do what with the imported surface?");
...
void MainWindow::execute_saveprojas()
{
	...
	if (!savefilename.isEmpty())
	{
		if (savefilename.length() <= 4 ||
			!savefilename.endsWith(QString(".prj")))
		...
		if (afterDot != 0)
			tempFileName = tempFileName.remove(afterDot, tempFileName.length() -
															 afterDot) +
						   "Temp.prj";
		else
...
void MainWindow::SaveSettings()
{
	...
	settings.setValue("ContiguousMemory",
					  this->handler3D->GetContiguousMemory());
	settings.endGroup();
	settings.sync();
}
...
void MainWindow::LoadSettings(const char* loadfilename)
{
	...
	if (loadProjVersion > 7)
	{
		...
		cerr << "NumberOfUndoSteps = "
			 << this->handler3D->GetNumberOfUndoSteps() << endl;
		this->handler3D->SetNumberOfUndoArrays(
			settings.value("NumberOfUndoArrays", 20).toUInt());
		cerr << "NumberOfUndoArrays = "
			 << this->handler3D->GetNumberOfUndoArrays() << endl;
		this->handler3D->SetCompression(
			settings.value("Compression", 0).toInt());
...
void MainWindow::execute_saveproj()
{
	...
	if (m_editingmode)
	{
		...
		{
			handler3D->SaveCommunicationFile(
				m_S4Lcommunicationfilename.ascii());
		}
	}
	else
	{
		if (!m_saveprojfilename.isEmpty())
		{
			...
			int afterDot = tempFileName.lastIndexOf('.');
			if (afterDot != 0)
				tempFileName = tempFileName.remove(
								   afterDot, tempFileName.length() - afterDot) +
							   "Temp.prj";
			else
				tempFileName = tempFileName + "Temp.prj";
			...
			if (afterDot != -1)
				sourceFileNameWithoutExtension =
					m_saveprojfilename.mid(0, afterDot);
			...
			int numTasks = 3;
			QProgressDialog progress("Save in progress...", "Cancel", 0,
									 numTasks, this);
			progress.show();
			QApplication::processEvents(QEventLoop::ExcludeUserInputEvents);
			progress.setWindowModality(Qt::WindowModal);
			...
			mBox.setText(
				"The project you are trying to save is open somewhere else. "
...
void MainWindow::execute_mergeprojects()
{
	...
	{
		FILE* fp =
			handler3D->MergeProjects(savefilename.ascii(), mergefilenames);
		if (!fp)
		{
			QMessageBox::warning(
				this, "iSeg",
...
{
	AtlasWidget* AW =
		new AtlasWidget(m_atlasfilename.m_atlasdir
							.absFilePath(m_atlasfilename.m_atlasfilename[i])
							.ascii(),
						m_picpath);
	if (AW->isOK)
	{
...
void MainWindow::execute_exportxmlregionextent()
{
	...
	QString savefilename = Q3FileDialog::getSaveFileName(
		QString::null, "XML extent (*.xml)", this);
	...
	if (!m_saveprojfilename.isEmpty())
		relfilename = m_saveprojfilename.right(
			m_saveprojfilename.length() - m_saveprojfilename.findRev("/") - 1);
...
void MainWindow::execute_loadtissues()
{
	...
	if (!loadfilename.isEmpty())
	{
		...
		{
			if (TissueInfos::LoadTissuesReadable(loadfilename.ascii(),
												 handler3D, removeTissuesRange))
			{
				if (removeTissuesRange > 0)
				{
					...
					if (ret == QMessageBox::Yes)
					{
						...
						for (tissues_size_t type = 1;
							 type <= removeTissuesRange; ++type)
						{
...
#else
					std::set<tissues_size_t> removeTissues;
					for (tissues_size_t type = 1; type <= removeTissuesRange;
						 ++type)
					{
						removeTissues.insert(type);
					}
...
void MainWindow::execute_3Dsurfaceviewer()
{
	...
	if (SV3D == NULL)
	{
		...
		QObject::connect(SV3D, SIGNAL(hasbeenclosed()), this,
						 SLOT(SV3D_closed()));
	}
...
void MainWindow::execute_3Dvolumeviewertissue()
{
	...
	if (VV3D == NULL)
	{
		...
		QObject::connect(VV3D, SIGNAL(hasbeenclosed()), this,
						 SLOT(VV3D_closed()));
	}
...
void MainWindow::xslice_closed()
{
	if (xsliceshower != NULL)
	{
		QObject::disconnect(
			...
			xsliceshower, SLOT(set_scale(float, float, bool)));
		QObject::disconnect(
			work_show, SIGNAL(scaleoffsetfactor_changed(float, float, bool)),
			xsliceshower, SLOT(set_scale(float, float, bool)));
		QObject::disconnect(xsliceshower, SIGNAL(slice_changed(int)), this,
							SLOT(xshower_slicechanged()));
		QObject::disconnect(xsliceshower, SIGNAL(hasbeenclosed()), this,
...
void MainWindow::addhold_tissue_clicked(Point p)
{
	...
	else
	{
		if (cb_addsubconn->isChecked())
			...
			handler3D->add2tissue(currTissueType, p,
								  cb_addsuboverride->isChecked());
	}
...
void MainWindow::merge()
{
	...
	{
		anyLocked |=
			TissueInfos::GetTissueLocked(tissueTreeWidget->get_type(*a));
	}
	if (anyLocked)
	{
...
void MainWindow::removeFolder()
{
	if (tissueTreeWidget->get_current_has_children())
	{
		...
		msgBox.setInformativeText(
			"Do you want to keep all tissues\nand folders "
		...
		else if (ret == QMessageBox::No)
		{
			...
			{
				QMessageBox::warning(this, "iSeg",
									 "Locked tissues can not be removed.",
									 QMessageBox::Ok | QMessageBox::Default);
				return;
			}
...
void MainWindow::tissuelock_toggled()
{
	if (tissueTreeWidget->get_current_is_folder())
	{
		...
		for (std::map<tissues_size_t, unsigned short>::iterator iter =
			 ...
			 iter != childTissues.end(); ++iter)
		{
			TissueInfos::SetTissueLocked(iter->first,
										 cb_tissuelock->isChecked());
		}
	}
	else
	{
		...
		for (auto a = list.begin(); a != list.end(); ++a)
		{
			...
			tissues_size_t currTissueType = tissueTreeWidget->get_type(item);
			//tissues_size_t currTissueType = tissueTreeWidget->get_current_type();
			TissueInfos::SetTissueLocked(currTissueType,
										 cb_tissuelock->isChecked());
		}
	}
	tissueTreeWidget->update_tissue_icons();
...
void MainWindow::tab_changed(QWidget* qw)
{
	...
	if (qw != tab_old)
	{
		...
		else if (tab_old == measurement_widget)
		{
			...
			QObject::disconnect(this, SIGNAL(marks_changed()),
								measurement_widget, SLOT(marks_changed()));
			...
			QObject::disconnect(measurement_widget,
								SIGNAL(vp1_changed(std::vector<Point>*)),
								bmp_show, SLOT(set_vp1(std::vector<Point>*)));
			QObject::disconnect(measurement_widget,
								SIGNAL(vp1_changed(std::vector<Point>*)),
								work_show, SLOT(set_vp1(std::vector<Point>*)));
			QObject::disconnect(measurement_widget,
								SIGNAL(vpdyn_changed(std::vector<Point>*)),
								bmp_show, SLOT(set_vpdyn(std::vector<Point>*)));
			QObject::disconnect(
				measurement_widget, SIGNAL(vpdyn_changed(std::vector<Point>*)),
		...
		else if (tab_old == hyst_widget)
		{
			...
			QObject::disconnect(bmp_show, SIGNAL(mousemoved_sign(Point)),
								hyst_widget, SLOT(pt_moved(Point)));
			...
			QObject::disconnect(
				hyst_widget,
				SIGNAL(
					vp1dyn_changed(std::vector<Point>*, std::vector<Point>*)),
				bmp_show,
				SLOT(set_vp1_dyn(std::vector<Point>*, std::vector<Point>*)));
		}
		else if (tab_old == lw_widget)
		{
			...
			QObject::disconnect(this, SIGNAL(bmp_changed()), lw_widget,
								SLOT(bmp_changed()));
			QObject::disconnect(bmp_show, SIGNAL(mousepressed_sign(Point)),
								lw_widget, SLOT(pt_clicked(Point)));
			...
			QObject::disconnect(bmp_show,
								SIGNAL(mousedoubleclickmid_sign(Point)),
								lw_widget, SLOT(pt_doubleclickedmid(Point)));
			QObject::disconnect(bmp_show, SIGNAL(mousemoved_sign(Point)),
								lw_widget, SLOT(pt_moved(Point)));
			QObject::disconnect(bmp_show, SIGNAL(mousereleased_sign(Point)),
		...
		else if (tab_old == iftrg_widget)
		{
			...
			QObject::disconnect(iftrg_widget,
								SIGNAL(vm_changed(std::vector<Mark>*)),
								bmp_show, SLOT(set_vm(std::vector<Mark>*)));
			QObject::disconnect(iftrg_widget,
								SIGNAL(vmdyn_changed(std::vector<Point>*)),
								bmp_show, SLOT(set_vpdyn(std::vector<Point>*)));
		...
		else if (tab_old == FMF_widget)
		{
			...
			QObject::disconnect(bmp_show, SIGNAL(mousemoved_sign(Point)),
								FMF_widget, SLOT(mouse_moved(Point)));
		...
		else if (tab_old == OutlineCorrect_widget)
		{
			...
			QObject::disconnect(this, SIGNAL(work_changed()),
								OutlineCorrect_widget,
		...
		else if (tab_old == feature_widget)
		{
			...
			QObject::disconnect(feature_widget,
								...
								bmp_show, SLOT(set_vpdyn(std::vector<Point>*)));
			QObject::disconnect(
				feature_widget, SIGNAL(vpdyn_changed(std::vector<Point>*)),
				work_show, SLOT(set_vpdyn(std::vector<Point>*)));
		...
		else if (qw == measurement_widget)
		{
			...
			QObject::connect(measurement_widget,
							 ...
							 work_show, SLOT(set_vpdyn(std::vector<Point>*)));
			QObject::connect(measurement_widget,
							 SIGNAL(vp1dyn_changed(std::vector<Point>*,
												   std::vector<Point>*, bool)),
							 bmp_show,
							 SLOT(set_vp1_dyn(std::vector<Point>*,
											  std::vector<Point>*, bool)));
			QObject::connect(measurement_widget,
							 SIGNAL(vp1dyn_changed(std::vector<Point>*,
												   std::vector<Point>*, bool)),
		...
		else if (qw == lw_widget)
		{
			...
			QObject::connect(bmp_show, SIGNAL(mousedoubleclick_sign(Point)),
							 lw_widget, SLOT(pt_doubleclicked(Point)));
			QObject::connect(bmp_show, SIGNAL(mousepressedmid_sign(Point)),
							 lw_widget, SLOT(pt_midclicked(Point)));
		...
		else if (qw == iftrg_widget)
		{
			...
			QObject::connect(iftrg_widget,
							 SIGNAL(vm_changed(std::vector<Mark>*)), bmp_show,
		...
		else if (qw == OutlineCorrect_widget)
		{
			...
			QObject::connect(this, SIGNAL(work_changed()),
							 OutlineCorrect_widget, SLOT(workbits_changed()));
...
void MainWindow::execute_cleanup()
{
	tissues_size_t** slices =
		new tissues_size_t*[handler3D->return_endslice() -
							handler3D->return_startslice()];
...
{
	zoom_widget->zoom_changed(work_show->return_zoom() *
							  pow(1.2, delta / 120.0));
}
										 ...
										 QWidget* sender, bool beginUndo)
{
	...
	if (beginUndo)
	{
		if (changeData.allSlices)
		{
			...
			canUndo3D = handler3D->return_undo3D() &&
						handler3D->start_undoall(changeData);
		}
		else
		{
									   ...
									   iseg::EndUndoAction undoAction)
{
	...
	if (sender == methodTab->visibleWidget())
	{
		QObject::connect(this, SIGNAL(bmp_changed()), sender,
						 SLOT(bmp_changed()));
