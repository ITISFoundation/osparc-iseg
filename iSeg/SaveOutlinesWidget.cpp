...
void SaveOutlinesWidget::file_pushed()
{
	...
	if(rb_triang->isOn())
		loadfilename = Q3FileDialog::getSaveFileName(QString::null, "Surface grids (*.vtp *.dat *.stl)",this);
	else
		loadfilename = Q3FileDialog::getSaveFileName(QString::null, "Files (*.txt)",this);
...
void SaveOutlinesWidget::save_pushed()
{
	...
	if(!(le_file->text()).isEmpty())
	{
		...
		if(rb_triang->isOn())
		{
			...
			if(rb_none->isChecked()) ratio = 1.0;

			unsigned int smoothingiter = 0;
			if(cb_smooth->isChecked()) smoothingiter = 9;

			bool usemc = false;
			if(cb_marchingcubes->isChecked()) usemc = true;


			// Call method to extract surface, smooth, simplify and finally save it to file
			int numberOfErrors = handler3D->extract_tissue_surfaces(loadfilename.ascii(), vtissues, 
				usemc, ratio, smoothingiter);

			if(numberOfErrors < 0)
			{
				QMessageBox::warning(this,"iSeg",
					"Surface extraction failed.\n\nMaybe something is wrong the pixel type or label field.",
					QMessageBox::Ok|QMessageBox::Default);
			}
			else if(numberOfErrors > 0)
			{
				QMessageBox::warning(this,"iSeg",
					"Surface extraction might have failed.\n\nPlease verify the tissue names and colors carefully.",
					QMessageBox::Ok|QMessageBox::Default);
			}
		}
		else
		{
			...
			if(sb_between->value()==0)
			{
				...
				{
					handler3D->extract_contours2_xmirrored(sb_minsize->value(), vtissues, sl_f->value()*0.05f);
				...
				handler3D->shift_contours(-(int)handler3D->return_width(),-(int)handler3D->return_height());
				if(cb_extrusion->isChecked())
				{
					handler3D->setextrusion_contours(sb_topextrusion->value()-1,sb_bottomextrusion->value()-1);
				}
...
#define ROTTERDAM
				...
				fprintf(fp,"V1\nX%i %i %i %i O%f %f %f\n",-(int)handler3D->return_width(),(int)handler3D->return_width(),
					-(int)handler3D->return_height(),(int)handler3D->return_height(),disp[0],disp[1],disp[2]); // TODO: rotation
				vector<augmentedmark> labels;
				handler3D->get_labels(&labels);
				if(!labels.empty()) fprintf(fp,"V1\n");
				for(size_t i=0;i<labels.size();i++) {
					fprintf(fp,"S%i 1\nL%i %i %s\n",(int)labels[i].slicenr,
						(int)handler3D->return_width()-1-(int)labels[i].p.px*2,
						(int)labels[i].p.py*2-(int)handler3D->return_height()+1,labels[i].name.c_str());
				}
				fclose(fp);
#endif
			...
			{
				handler3D->extractinterpolatesave_contours2_xmirrored(sb_minsize->value(),vtissues,
					sb_between->value(),rb_dougpeuck->isOn(),sl_f->value()*0.05f,loadfilename.ascii());
			}
		}
		close();	
	} else {
