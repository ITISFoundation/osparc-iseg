...
vessel_widget::vessel_widget(SlicesHandler *hand3D, QWidget *parent, const char *name, Qt::WindowFlags wFlags)
...
//	setFixedSize(vbox1->size());
	...
	QObject::connect(sb_nrend,SIGNAL(valueChanged(int)),this,SLOT(nrend_changed(int)));
	QObject::connect(sb_endnr,SIGNAL(valueChanged(int)),this,SLOT(endnr_changed(int)));
	QObject::connect(pb_exec,SIGNAL(clicked()),this,SLOT(execute()));
	QObject::connect(pb_store,SIGNAL(clicked()),this,SLOT(savevessel()));
	QObject::connect(cbb_lb1,SIGNAL(activated(int)),this,SLOT(cbb1_changed(int)));
	QObject::connect(cbb_lb2,SIGNAL(activated(int)),this,SLOT(cbb2_changed(int)));
}

vessel_widget::~vessel_widget()
{
	delete vbox1;
}

QSize vessel_widget::sizeHint() const
{
...
void vessel_widget::init()
{
	...
	if(branchTree.getSize()>0){
		branchTree.getItem()->getCenterListSlice_inclchildren(handler3D->get_activeslice(),vp);
	}
	emit vp1_changed(&vp);
}
...
void vessel_widget::getlabels()
{
	...
	QObject::disconnect(cbb_lb1,SIGNAL(activated(int)),this,SLOT(cbb1_changed(int)));
	QObject::disconnect(cbb_lb2,SIGNAL(activated(int)),this,SLOT(cbb2_changed(int)));
	cbb_lb1->clear();
	cbb_lb2->clear();
	...
	for(size_t i=0;i<selectedlabels.size();i++) {
		size_t j=0;
		while((j<labels.size())&&(labels[j]!=selectedlabels[i])) j++;
		if(j==labels.size()) {
			if(labels.empty()) {
...
void vessel_widget::marks_changed()
{
	getlabels();
}
...
void vessel_widget::execute()
{
	...
	for(int i=1;i<=sb_nrend->value();i++) {	
		int j=0;
		while((j<i)&&(selectedlabels[j]!=selectedlabels[i])) j++;
		if(j==i) {
			vec3 start;
	...
	if(!seeds.empty()&&(handler3D->return_nrslices()>0)&&(handler3D->return_width()>0)&&(handler3D->return_height()>0)) {
...
void vessel_widget::endnr_changed(int newval)
{
	...
	size_t i=0;
	while((i<labels.size())&&(labels[i]!=selectedlabels[newval])) i++;
...
{
	QString savefilename = Q3FileDialog::getSaveFileName(QString::null, "Vessel-Tracks (*.txt)\n", this);//, filename);
