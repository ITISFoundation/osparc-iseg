...
TissueLayerInfoStruct *TissueLayerInfos::GetTissueLayerInfo(tissuelayers_size_t layerIdx)
{
...
TissueLayerOverlayMode TissueLayerInfos::GetTissueLayerOverlayMode(tissuelayers_size_t layerIdx)
{
...
void TissueLayerInfos::SetTissueLayersVisible(bool val)
{
	...
	for (vecIt = tissueLayerInfosVector.begin() + 1; vecIt != tissueLayerInfosVector.end(); ++vecIt) {
		vecIt->visible = val;
	}
}
...
void TissueLayerInfos::SetTissueLayerOverlayMode(tissuelayers_size_t layerIdx, TissueLayerOverlayMode val)
{
...
void TissueLayerInfos::AddTissueLayer(TissueLayerInfoStruct &layer)
{
	...
	tissueLayerIndexMap.insert(TissueLayerIndexMapEntryType(layer.name, GetTissueLayerCount()-1));
}
...
void TissueLayerInfos::CreateTissueLayerIndexMap()
{
	...
	for (tissuelayers_size_t idx = 0; idx < GetTissueLayerCount(); ++idx) {
		tissueLayerIndexMap.insert(TissueLayerIndexMapEntryType(tissueLayerInfosVector[idx].name, idx));
	}
}
