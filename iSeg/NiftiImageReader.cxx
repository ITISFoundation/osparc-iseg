...
int NiftiImageReader::ReadHeader()
{
	...
	cerr << "dim[0..7] = (" << im->dim[0] << ", " << im->dim[1] << ", "
			 << im->dim[2] << ", " << im->dim[3] << ", " << im->dim[4] << ", "
			 << im->dim[5] << ", " << im->dim[6] << ", " << im->dim[7] << ")" << endl;
	cerr << "(dx, dy, dz, dt, du, dv, dw) = (" << im->dx << ", " << im->dy << ", "
			 << im->dz << ", " << im->dt << ", " << im->du << ", " << im->dv << ", "
			 << im->dw << ")" << endl;
	...
	cerr << "(nx, ny, nz, nt, nu, nv, nw) = (" << im->nx << ", " << im->ny << ", "
			 << im->nz << ", " << im->nt << ", " << im->nu << ", " << im->nv << ", "
			 << im->nw << ")" << endl;
	...
	switch (unit) {
	case NIFTI_UNITS_METER: scaling = 1000.0f; break;
	case NIFTI_UNITS_MM: scaling = 1.0f; break;
	case NIFTI_UNITS_MICRON: scaling = 0.001f; break;
	}
	cerr << "Unit scaling: " << scaling << endl;

	// Dataset transform
	if (im->qform_code == NIFTI_XFORM_UNKNOWN &&
		...
		float tmp = im->quatern_b * im->quatern_b + im->quatern_c * im->quatern_c +
								im->quatern_d * im->quatern_d;
		if (tmp < 1.0f) {
			this->Quaternion[0] = sqrt(1.0 - tmp);
		}
		else {
			this->Quaternion[0] =
					0.0f; // tmp can be slightly larger than 1.0 due to precision error
		}
...
int NiftiImageReader::Read(unsigned short startslice, unsigned short nrslices)
{
	...
	if (im->intent_code == NIFTI_INTENT_LABEL) {
		// Tissue
		cerr << "reading tissue data of grid type: "
				 << nifti_datatype_string(im->datatype) << "..." << endl;
		switch (im->datatype) {
		...
		// Source
		cerr << "reading image data of grid type: "
				 << nifti_datatype_string(im->datatype) << "..." << endl;
		switch (im->datatype) {
		case DT_UINT8: /* unsigned char (8 bits/voxel) */
