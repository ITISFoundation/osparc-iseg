...
morpho_widget::morpho_widget(SlicesHandler *hand3D, QWidget *parent, const char *name, Qt::WindowFlags wFlags)
 : QWidget1( parent, name, wFlags ),handler3D(hand3D)
{
	setToolTip(Format("Apply morphological operations to the Target image. Morphological operations are "
		"based on expanding or shrinking (Dilate/Erode) regions by a given number of pixel layers (n)."
		"<br>"
		"The functions act on the Target image (to modify a tissue use Get Tissue and Adder)."));
	...
	connectgroup->insert(rb_4connect = new QRadioButton(QString("4-connectivity"), hbox2));
	connectgroup->insert(rb_8connect = new QRadioButton(QString("8-connectivity"), hbox2));
	...
	modegroup->insert(rb_dilate = new QRadioButton(QString("Dilate"), vboxmethods));
	rb_open->setChecked(TRUE);

	rb_open->setToolTip(Format("First shrinking before growing is called Open and results in the deletion of small islands and thin links between structures."));
	rb_close->setToolTip(Format("Growing followed by shrinking results in the closing of small (< 2n) gaps and holes."));
...
QSize morpho_widget::sizeHint() const
{
