...
smooth_widget::smooth_widget(SlicesHandler *hand3D, QWidget *parent, const char *name, Qt::WindowFlags wFlags)
 : QWidget1( parent, name, wFlags ),handler3D(hand3D)
{
	...
	sl_sigma->setToolTip("Sigma gives the radius of the smoothing filter. Larger values remove more details.");
	...
	sl_k->setRange(0,100);
	sl_k->setValue(50);
	sl_k->setToolTip("Together with value on the right, defines the Sigma of the smoothing filter.");
	sb_kmax=new QSpinBox(1,100,1,hbox4);
	sb_kmax->setValue(10);
	sb_kmax->setToolTip("Gives the max. radius of the smoothing filter. This value defines the scale used in the slider bar.");
	...
	rb_gaussian->setToolTip("Gaussian smoothing blurs the image and can remove noise and details (high frequency).");
	rb_average=new QRadioButton(QString("Average"),vboxmethods);
	rb_average->setToolTip("Mean smoothing blurs the image and can remove noise and details.");
	rb_median=new QRadioButton(QString("Median"),vboxmethods);
	rb_median->setToolTip("Median filtering can remove (speckle or salt-and-pepper) noise.");
	rb_sigmafilter=new QRadioButton(QString("Sigma Filter"),vboxmethods);
	rb_sigmafilter->setToolTip("Sigma filtering is a mixture between Gaussian and Average filtering. It preserves edges better than Average filtering.");
	rb_anisodiff=new QRadioButton(QString("Anisotropic Diffusion"),vboxmethods);
	rb_anisodiff->setToolTip("Anisotropic diffusion can remove noise, while preserving significant details, such as edges.");
...
//	setFixedSize(vbox1->size());
	...
	QObject::connect(sl_sigma,SIGNAL(valueChanged(int)),this,SLOT(sigmaslider_changed(int)));
	QObject::connect(sl_sigma,SIGNAL(sliderPressed()),this,SLOT(slider_pressed()));
	QObject::connect(sl_sigma,SIGNAL(sliderReleased()),this,SLOT(slider_released()));
	QObject::connect(sl_k,SIGNAL(valueChanged(int)),this,SLOT(kslider_changed(int)));
	QObject::connect(sl_k,SIGNAL(sliderPressed()),this,SLOT(slider_pressed()));
	QObject::connect(sl_k,SIGNAL(sliderReleased()),this,SLOT(slider_released()));
	QObject::connect(sb_n,SIGNAL(valueChanged(int)),this,SLOT(n_changed(int)));
	QObject::connect(sb_kmax,SIGNAL(valueChanged(int)),this,SLOT(kmax_changed(int)));
...
void smooth_widget::execute()
{
	...
	if(allslices->isChecked()){
		if(rb_gaussian->isOn()){
			...
			handler3D->sigmafilter((sl_k->value()+1)*0.01f*sb_kmax->value(),(short unsigned)sb_n->value(),(short unsigned)sb_n->value());
		} else {
			handler3D->aniso_diff(1.0f, sb_iter->value(), f2, sl_k->value()*0.01f*sb_kmax->value(), sl_restrain->value()*0.01f);
		}
	} else {
		if(rb_gaussian->isOn()){
			...
			bmphand->sigmafilter((sl_k->value()+1)*0.01f*sb_kmax->value(),(short unsigned)sb_n->value(),(short unsigned)sb_n->value());
		} else {
			bmphand->aniso_diff(1.0f, sb_iter->value(), f2, sl_k->value()*0.01f*sb_kmax->value(), sl_restrain->value()*0.01f);
		}
	}
...
void smooth_widget::continue_diff()
{
	...
	if(allslices->isChecked()) {
		handler3D->cont_anisodiff(1.0f, sb_iter->value(), f2, sl_k->value()*0.01f*sb_kmax->value(), sl_restrain->value()*0.01f);
	} else {
		bmphand->cont_anisodiff(1.0f, sb_iter->value(), f2, sl_k->value()*0.01f*sb_kmax->value(), sl_restrain->value()*0.01f);
	}
...
QSize smooth_widget::sizeHint() const
{
...
FILE *smooth_widget::LoadParams(FILE *fp, int version)
{
	if(version>=2) {
		...
		QObject::disconnect(sl_sigma,SIGNAL(valueChanged(int)),this,SLOT(sigmaslider_changed(int)));
		QObject::disconnect(sl_k,SIGNAL(valueChanged(int)),this,SLOT(kslider_changed(int)));
		QObject::disconnect(sb_n,SIGNAL(valueChanged(int)),this,SLOT(n_changed(int)));
		QObject::disconnect(sb_kmax,SIGNAL(valueChanged(int)),this,SLOT(kmax_changed(int)));
...
void smooth_widget::hideparams_changed()
{
	method_changed(0);
}