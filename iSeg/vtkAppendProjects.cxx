...
//		error("no scalar");
	...
	if(fnames.size()<2) error("need at least 2 inputs");
	if(ofname.empty()) error("need output filename");
...
//		append->AddInput(reader->GetOutput());
      ...
      if(shift)
      {
         ...
         else
         {
            ...
            cerr << "new origin: " << org0[0] << " " << org0[1] << " " << org0[2] << endl;
            for(int j=0; j<3; ++j)
            {
               dorg[j] = org[j]-org0[j];
            }
            ...
            cerr << "new extent: " << ext1[0] << " " << ext1[1] << " " << ext1[2] << " " << ext1[3] << " " << ext1[4] << " " << ext1[5] << endl;
