...
public:
	...
	// > SOP Instance Reference Macro (Table 10-11)
	std::string GetReferencedStudyClassUID()
	{
		return std::string("1.2.840.10008.3.1.2.3.2");
	}; // Study Componenet Management SOP Class
	void SetReferencedStudyInstanceUID(unsigned short idx, std::string value)
	{
		ReferencedStudySequence[idx] = value;
	};
	void AddReferencedStudyInstanceUID(std::string value)
	{
		ReferencedStudySequence.push_back(value);
	};
	std::string GetReferencedStudyInstanceUID(unsigned short idx)
	{
		return ReferencedStudySequence[idx];
	};
	unsigned short GetReferencedStudyInstanceUIDCount()
	{
		return ReferencedStudySequence.size();
	};
	...
	void SetSoftwareVersions(unsigned short idx, std::string value)
	{
		SoftwareVersions[idx] = value;
	};
	void AddSoftwareVersions(std::string value)
	{
		SoftwareVersions.push_back(value);
	};
	std::string GetSoftwareVersions(unsigned short idx)
	{
		return SoftwareVersions[idx];
	};
...
protected:
	...
	void FormatDate(unsigned short &year, unsigned short &month,
									unsigned short &day, std::string &result);
	void FormatTime(unsigned short &hours, unsigned short &minutes,
	...
	template<typename T>
	void StringEncodeVector(const T *offsets, unsigned int count,
													std::string &value);
	template<typename T>
	void ByteEncodeVector(const T *offsets, unsigned int count,
												std::string &value);
	void ReplaceStringSection(std::string &text, const std::string oldSection,
														const std::string newSection);
	void InsertDataElement(DataSet &dataset, const std::string keyword,
												 std::string value, unsigned int vm = 1);
	void ReplaceDataElement(DataSet &dataset, const std::string keyword,
													std::string value, unsigned int vm = 1);
	void InsertSequence(DataSet &dataset, const std::string keyword,
...
protected:
	...
	std::map<VR::VRType, unsigned short>
			_vrTypeMaxLength; // Maps a VR type to its maximal byte length
	std::map<std::string, TagVRTypePair>
			_attributeTagVRType; // Maps an attribute keyword to its tag and VR type
