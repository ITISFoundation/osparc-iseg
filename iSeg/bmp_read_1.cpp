...
{
	for (list<float *>::iterator it = bits_stack.begin(); it != bits_stack.end(); it++)
...
float *bmphandler::return_bmp()
{
	return bmp_bits;
}

const float * bmphandler::return_bmp() const
{
	return bmp_bits;
}

float *bmphandler::return_work()
{
	return work_bits;
}

const float *bmphandler::return_work() const
{
	return work_bits;
}
...
float *bmphandler::return_help()
{
	return help_bits;
}

float **bmphandler::return_bmpfield()
{
	return &bmp_bits;
}

float **bmphandler::return_workfield()
{
	return &work_bits;
}
...
vector<mark> *bmphandler::return_marks()
{
	return &marks;
}

void bmphandler::copy2marks(vector<mark> *marks1)
{
	marks=*marks1;
}
...
{
	if(loaded){
		for(unsigned i=0;i<area;i++) bmp_bits[i]=bits[i];
		mode1=mode;
	}
	return;
}
...
{
	if(loaded){
		for(unsigned i=0;i<area;i++) work_bits[i]=bits[i];
		mode2=mode;
	}
	return;
}
...
void bmphandler::copy2tissue(tissuelayers_size_t idx, tissues_size_t *bits)
{
	if(loaded){
		tissues_size_t *tissues = tissuelayers[idx];
		for(unsigned i=0;i<area;i++) tissues[i]=bits[i];
	}
	return;
}
...
{
	if(loaded){
		for(unsigned i=0;i<area;i++) bits[i]=bmp_bits[i];
	}
	return;
}
...
{
	if(loaded){
		for(unsigned i=0;i<area;i++) bits[i]=work_bits[i];
	}
	return;
}
...
void bmphandler::copyfromtissue(tissuelayers_size_t idx, tissues_size_t *bits)
{
	if(loaded){
		tissues_size_t *tissues = tissuelayers[idx];
		for(unsigned i=0;i<area;i++) bits[i]=tissues[i];
	}
	return;
}
...
void bmphandler::copyfromtissue(tissuelayers_size_t idx, unsigned char *bits)
{
	if(loaded){
		tissues_size_t *tissues = tissuelayers[idx];
		for(unsigned i=0;i<area;i++) bits[i]=(unsigned char)tissues[i];
	}
	return;
}
...
void bmphandler::copyfromtissuepadded(tissuelayers_size_t idx, tissues_size_t *bits, unsigned short padding)
{
	if(loaded){
		unsigned int pos1=0;
		unsigned int pos2=0;
		for(;pos1<(unsigned int)(width+2*padding)*padding+padding;pos1++) bits[pos1]=0;
		tissues_size_t *tissues = tissuelayers[idx];
		for(unsigned short j=0;j<height;j++) {
			for(unsigned short i=0;i<width;i++,pos1++,pos2++) {
				bits[pos1]=tissues[pos2];
			}
			for(unsigned short i=0;i<2*padding;i++,pos1++) bits[pos1]=0;
		}
		unsigned int maxval=area+2*padding*width+2*padding*height+padding*padding;
		for(;pos1<maxval;pos1++) bits[pos1]=0;
	}
	return;
}
...
{
	for(unsigned int i=0;i<area;i++) bmp_bits[i]=0;
...
{
	for(unsigned int i=0;i<area;i++) work_bits[i]=0;

	return;
}

inline unsigned bmphandler::pt2coord(Point p){ return p.px+(p.py*(unsigned)width);}

void bmphandler::bmp_abs()
{
	for(unsigned int i=0;i<area;i++) work_bits[i]=abs(work_bits[i]);
...
{
	for(unsigned int i=0;i<area;i++) work_bits[i]=255-work_bits[i];
...
{
	for(unsigned int i=0;i<area;i++) work_bits[i]=work_bits[i]+bmp_bits[i];
...
{
	for(unsigned int i=0;i<area;i++) work_bits[i]=work_bits[i]+f;
...
{
	for(unsigned int i=0;i<area;i++) work_bits[i]*=bmp_bits[i];
...
{
	for(unsigned int i=0;i<area;i++) work_bits[i]=f*work_bits[i];
...
void bmphandler::bmp_overlay(float alpha)
{
	...
	for(unsigned int i=0;i<area;i++) work_bits[i] = alpha*bmp_bits[i] + tmp*work_bits[i];
	mode2=2;
...
{
	for(unsigned int i=0;i<area;i++) if(work_bits[i]==0) work_bits[i]=pict2[i];
...
float *bmphandler::copy_work()
{
	...
	for(unsigned i=0;i<area;i++) results[i]=work_bits[i];
...
{
	float *results=sliceprovide->give_me();
	for(unsigned i=0;i<area;i++) results[i]=bmp_bits[i];
...
{
	tissues_size_t *results=(tissues_size_t *)malloc(sizeof(tissues_size_t)*area);
	tissues_size_t *tissues = tissuelayers[idx];
	for(unsigned i=0;i<area;i++) results[i]=tissues[i];
...
{
	for(unsigned i=0;i<area;i++) output[i]=work_bits[i];
	return;
}

void bmphandler::copy_bmp(float *output)
{
	for(unsigned i=0;i<area;i++) output[i]=bmp_bits[i];
	return;
}
...
void bmphandler::copy_tissue(tissuelayers_size_t idx, tissues_size_t *output)
{
	...
	tissues_size_t *tissues = tissuelayers[idx];
	for(unsigned i=0;i<area;i++) output[i]=tissues[i];
	return;
}
...
int bmphandler::ConvertImageTo8BitBMP(const char *filename, unsigned char *&bits_tmp)
{
	...
	for(int j=height-1;j>0;j--){
		for(int i=0;i<width;i++){
			...
			bits_tmp[counter] = (unsigned char)(redFactor*((double)r) + greenFactor*((double)g) + blueFactor*((double)b));
			counter++;
		}
	}
...
int bmphandler::ConvertPNGImageTo8BitBMP(const char *filename, unsigned char *&bits_tmp)
{
	...
	for(int y = sourceImage.height()-1; y>=0; y--)
	{
		for(int x = 0; x<sourceImage.width(); x++)
		{
			...
			bits_tmp[counter] = (unsigned char)(redFactor*oldColor.red()+greenFactor*oldColor.green()+blueFactor*oldColor.blue());
			counter++;
		}
	}
...
//		free(*info);
    ...
    if ((bitsize = bmpinfo->bmiHeader.biSizeImage) == 0){
		int padding=width%4;
		if(padding!=4) padding=4-padding;
		bitsize=(unsigned int)(width+padding)*height;
	}
...
bool bmphandler::ReloadDICOM(const char *filename, Point p)
{
	...
	if(width+p.px>dcmread.get_width()||height+p.py>dcmread.get_height()){
		dcmread.closedicom();
		return (NULL);
	}
...
FILE *bmphandler::save_proj(FILE *fp, bool inclpics)
{
	if(loaded){
		...
		for(vector<mark>::iterator it=marks.begin();it!=marks.end();it++){
			...
			fwrite(&(it->p.py), 1,sizeof(unsigned short), fp);
			if(marksVersion>=2) {
				int size1=it->name.length();
				fwrite(&size1, 1,sizeof(int), fp);
				fwrite(it->name.c_str(), 1,sizeof(char)*size1, fp);
			}
		}
		size=int(vvm.size());
		fwrite(&size, 1,sizeof(int), fp);

		for(vector<vector<mark> >::iterator it1=vvm.begin();it1!=vvm.end();it1++){
			size=int(it1->size());
			fwrite(&size, 1,sizeof(int), fp);
		...
		fwrite(&size, 1,sizeof(int), fp);
		for(vector<vector<Point> >::iterator it1=limits.begin();it1!=limits.end();it1++){
			size=int(it1->size());
			fwrite(&size, 1,sizeof(int), fp);
...
{		
	if(loaded){
		...
		for(list<unsigned>::iterator it=stackindex.begin();it!=stackindex.end();it++){
		...
		fwrite(&size, 1,sizeof(int), fp);
		for(list<unsigned char>::iterator it=mode_stack.begin();it!=mode_stack.end();it++){
...
FILE *bmphandler::load_proj(FILE *fp, int tissuesVersion, bool inclpics)
{
	...
	/*FILE *fp3=fopen("C:\\tissue.raw","ab");
	fwrite(tissues, 1,sizeof(unsigned char)*area, fp3);
	fclose(fp3);*/
...
//		if(marksVersion<1) fseek(fp,-1, SEEK_CUR);
	...
	for(int j1=0;j1<size1;j1++){
		...
		if(size>0) {
			maxim_store=max(maxim_store,vvm[j1].begin()->mark);
		}
	}
...
int bmphandler::SaveTissueBitmap(tissuelayers_size_t idx, const char *filename)
{
	...
	unsigned char *field = (unsigned char *)imageSource->GetScalarPointer(0,0,0);
...
#ifdef _MSC_VER
	int result = _fseeki64(fp, (__int64)(bitsize)*sizeof(float)*slicenr, SEEK_SET);
#else
...
int bmphandler::ReadRawFloat(const char *filename, short unsigned w, short unsigned h, unsigned short slicenr, Point p, unsigned short dx, unsigned short dy)
{
	...
	for(unsigned short n=0;n<dy;n++){
		if((unsigned short)fread(bmp_bits+n*dx,1,dx*sizeof(float),fp)<sizeof(float)*dx)
		{
			/*				free(bmp_bits);
			free(work_bits);
			free(help_bits);*/
...
int bmphandler::ReloadRaw(const char *filename, short unsigned w, short unsigned h, unsigned bitdepth, unsigned slicenr, Point p)
{
	...
	if(bytedepth==1){
		...
		for(unsigned short n=0;n<height;n++){
			if((unsigned short)fread(bits_tmp+n*width,1,width*2,fp)<2*width)
			{
				free(bits_tmp);
				fclose(fp);
				return (NULL);
...
#ifdef _MSC_VER
	int result = _fseeki64(fp, (__int64)(area)*sizeof(float)*slicenr, SEEK_SET);
#else
...
int bmphandler::ReloadRawFloat(const char *filename, short unsigned w, short unsigned h, unsigned slicenr, Point p)
{
	...
	for(unsigned short n=0;n<height;n++){
		if((unsigned short)fread(bmp_bits+n*width,1,width*sizeof(float),fp)<sizeof(float)*width)
		{
			fclose(fp);
			return (NULL);
		}
...
int bmphandler::ReloadRawTissues(const char *filename, short unsigned w, short unsigned h, unsigned bitdepth, unsigned slicenr, Point p)
{
	...
	if(bytedepth==1){
...
#ifdef _MSC_VER
		int result = _fseeki64(fp,(__int64)(area2)*slicenr*tissuelayers.size()+(__int64)(w)*p.py+p.px, SEEK_SET);
#else
		int result = fseek(fp,(size_t)(area2)*slicenr*tissuelayers.size()+(size_t)(w)*p.py+p.px, SEEK_SET);
#endif
		if( result ){
			free(bits_tmp);
			fclose(fp);
		...
		for (tissuelayers_size_t idx = 0; idx < tissuelayers.size(); ++idx) {
			for(unsigned short n=0;n<height;n++){
				if((unsigned short)fread(bits_tmp+area*idx+n*width,1,width,fp)<width)
				{
					free(bits_tmp);
					fclose(fp);
					return (NULL);
...
#ifdef _MSC_VER
		int result = _fseeki64(fp, ((__int64)(area2)*slicenr*tissuelayers.size()+(__int64)(w)*p.py+p.px)*2, SEEK_SET);
#else
		int result = fseek(fp, ((size_t)(area2)*slicenr*tissuelayers.size()+(size_t)(w)*p.py+p.px)*2, SEEK_SET);
#endif
		if( result ){
			free(bits_tmp);
			fclose(fp);
		...
		for (tissuelayers_size_t idx = 0; idx < tissuelayers.size(); ++idx) {
			for(unsigned short n=0;n<height;n++){
				if((unsigned short)fread(bits_tmp+area*idx+n*width,1,width*2,fp)<2*width)
				{
					free(bits_tmp);
					fclose(fp);
					return (NULL);
...
int bmphandler::SaveRaw(const char *filename, float *p_bits)
{
	...
	for(unsigned int i=0;i<area;i++){
		bits_tmp[i]=(unsigned char)(min(255.0,max(0.0,p_bits[i]+0.5)));
	}
...
int bmphandler::SaveTissueRaw(tissuelayers_size_t idx, const char *filename)
{
	...
	if ((TissueInfos::GetTissueCount() <= 255) && (sizeof(tissues_size_t) > sizeof(unsigned char))) {
		...
		if (fwrite(tissuelayers[idx], sizeof(tissues_size_t), bitsize, fp) < bitsize) {
			fclose(fp);
			return (-1);
		}
	}
...
/*float bmphandler::get_work_pt(Point p)
{
	return work_bits[width*p.py+p.px];
}*/
...
float bmphandler::bmp_pt(Point p)
{
	return bmp_bits[width*p.py+p.px];
}

float bmphandler::work_pt(Point p)
{
	return work_bits[width*p.py+p.px];
}
...
unsigned int bmphandler::return_area()
{
	return area;
}

unsigned int bmphandler::return_width()
{
	return width;
}

unsigned int bmphandler::return_height()
{
	return height;
}
...
unsigned int bmphandler::make_histogram(bool includeoutofrange)
{
	...
	for(unsigned int i=0;i<area;i++) 
	{
		k=work_bits[i];
		if(k<0||k>=256){
			if(includeoutofrange&&k<0) histogram[0]++;
			if(includeoutofrange&&k>=256) histogram[255]++;
			j++;
		}
		else
			histogram[(int)k]++;
...
unsigned int bmphandler::make_histogram(float *mask, float f, bool includeoutofrange)
{
	unsigned int j=0;
	float k;
	for(int i=0;i<256;i++) histogram[i]=0;
...
unsigned int bmphandler::make_histogram(Point p, unsigned short dx, unsigned short dy, bool includeoutofrange)
{
	unsigned int i,l;
	l=0;
	float m;
...
unsigned int *bmphandler::return_histogram()
{
	return histogram;
}
...
void bmphandler::threshold(float *thresholds)
{
	...
	if(n>0){
		...
		for(unsigned int i=0;i<area;i++){
			j=0;
			while(j<n&&bmp_bits[i]>thresholds[j+1]) j++;
...
void bmphandler::threshold(float *thresholds,Point p, unsigned short dx, unsigned short dy)
{
	...
	if(n>0){
		...
		for(int j=0;j<dy;j++){
			for(int k=0;k<dx;k++){
				l=0;
				while(bmp_bits[i]>thresholds[l+1]&&l<n) l++;
...
void bmphandler::work2tissue(tissuelayers_size_t idx)
{
	tissues_size_t *tissues = tissuelayers[idx];
	for(unsigned int i=0;i<area;i++) {
		if(work_bits[i]<0.0f) tissues[i]=0;
		else if(work_bits[i]>(float)TISSUES_SIZE_MAX) tissues[i]=TISSUES_SIZE_MAX;
		else tissues[i]=(tissues_size_t)floor(work_bits[i]+0.5f);
	}
...
void bmphandler::mergetissue(tissues_size_t tissuetype, tissuelayers_size_t idx)
{
	tissues_size_t *tissues = tissuelayers[idx];
	for(unsigned int i=0;i<area;i++) {
			if(work_bits[i]>0.0f) tissues[i]=tissuetype;	
	}
...
float *bmphandler::make_gaussfilter(float sigma,int n)
{
	...
	if(n%2==0) return NULL;
		...
		for(int i=-n/2;i<=n/2;i++) filter[i+n/2+1]=exp(-float(i*i)/(2*sigma*sigma));		
		filter[n/2+1]=exp(-1/(16*sigma*sigma));
	
		float summe=0;
		for(int i=1;i<=n;i++) summe+=filter[i];
		for(int i=1;i<=n;i++) filter[i]=filter[i]/summe;

		filter[0]=(float)n;

/*		for(int i=0;i<=n;i++) cout << filter[i]<< " ";
		cout<< endl;*/
...
void bmphandler::convolute(float *mask,unsigned short direction)
{
	...
	case 0:
		...
		for(unsigned j=0;j<height;j++){
			for(unsigned k=0;k<=width-n;k++){
				dummy=0;
				for(unsigned l=0;l<n;l++) dummy+=mask[l+1]*bmp_bits[i+l];
	...
	case 1:
		...
		for(unsigned j=0;j<width*(height-n+1);j++){
			dummy=0;
			for(unsigned l=0;l<n;l++) dummy+=mask[l+1]*bmp_bits[i+l*width];
			work_bits[i+(n/2)*width]=dummy;
			i++;
		}

		for(i=0;i<(n/2)*width;i++) work_bits[i]=0;
		for(i=area-(n/2)*width;i<area;i++) work_bits[i]=0;
		break;
	case 2:
		n=(unsigned)mask[0];
...
void bmphandler::convolute_hist(float *mask)
{
	...
	for(int k=0;k<=256-n;k++){
		histo[k+n/2]=0;
		for(int l=0;l<n;l++) histo[k+n/2]+=mask[l+1]*histogram[k+l];
	}
...
void bmphandler::get_range(Pair *pp)
{
	...
	{
		pp->low=min(pp->low,work_bits[i]);
		pp->high=max(pp->high,work_bits[i]);
	}
}
...
void bmphandler::get_bmprange(Pair *pp)
{
	...
	for(unsigned int i=1;i<area;i++)
	{
		pp->low=min(pp->low,bmp_bits[i]);
		pp->high=max(pp->high,bmp_bits[i]);
	}
}
...
void bmphandler::scale_colors(Pair p)
{
	...
	for(unsigned int i=0;i<area;i++) work_bits[i]=(work_bits[i]-p.low)*step;
...
{
	for(unsigned int i=0;i<area;i++) work_bits[i]=min(max(work_bits[i],0.0f),255.0f);
...
void bmphandler::average(short unsigned n)
{
	...
	filter[0]=n;
	for(short unsigned int i=1;i<=n;i++)filter[i]=1.0f/n;
...
void bmphandler::laplacian1()
{
	...
	laplacianfilter[3]=laplacianfilter[5]=laplacianfilter[9]=laplacianfilter[11]=-1.0f/12;
	laplacianfilter[4]=laplacianfilter[6]=laplacianfilter[8]=laplacianfilter[10]=-2.0f/12;
...
{
//	float *sobelx=(float *)malloc(sizeof(float)*area);
//	float *sobely=(float *)malloc(sizeof(float)*area);
...
//	bmp_abs();
	...
	for(unsigned int m=0;m<area;m++) {
		work_bits[m]=abs(sobelx[m])+abs(sobely[m]);
//		if(work_bits[m]>255)work_bits[m]=255;
//		if(work_bits[m]<0)work_bits[m]=0;
	}
...
//			if(work_bits[i]>255)work_bits[i]=255;
//			if(work_bits[i]<0)work_bits[i]=0;

			if(sobelx[i]==0) {
				if(sobely[i]==0) help_bits[i]=0;
				else help_bits[i]=90;
			}
			else if((sobelx[i]<0&&sobely[i]>0)||(sobelx[i]>0&&sobely[i]<0)) {
				help_bits[i]=180.0f-(180.0f/3.141592f*atan(-sobely[i]/sobelx[i]));
			}
			else help_bits[i]=180.0f/3.141592f*atan(sobely[i]/sobelx[i]);

			float left_bit,right_bit;

			if(help_bits[i]<22.5||help_bits[i]>=157.5) {left_bit=work_bits[i-1]; right_bit=work_bits[i+1];}
			else if(help_bits[i]<67.5) {left_bit=work_bits[i-1-width]; right_bit=work_bits[i+1+width];}
			else if(help_bits[i]<112.5) {left_bit=work_bits[i+width]; right_bit=work_bits[i-width];}
			else if(help_bits[i]<157.5) {left_bit=work_bits[i+width-1]; right_bit=work_bits[i-width+1];}


			if(work_bits[i]<left_bit||work_bits[i]<right_bit) sobelx[i]=0;
//			else sobelx[i]=work_bits[i];
			else{
				if(work_bits[i]>=thresh_high) sobelx[i]=255;
				if(work_bits[i]<thresh_low) sobelx[i]=0;
				else{
					if(work_bits[i-1]>thresh_high||work_bits[i+1]>thresh_high||work_bits[i-width]>thresh_high||work_bits[i+width]>thresh_high||work_bits[i-width-1]>thresh_high||work_bits[i+width+1]>thresh_high||work_bits[i-width+1]>thresh_high||work_bits[i+width-1]>thresh_high) sobelx[i]=255;
					else sobelx[i]=0;
				}
			}
			i++;
...
float *bmphandler::direction_map(float *sobelx, float *sobely)
{
	...
	for(int j=1;j<height-1;j++) {
		for(int k=1;k<width-1;k++){
			...
			if(sobelx[i]==0) {
				if(sobely[i]==0) direct_map[i]=0;
				else direct_map[i]=90;
			}
			else if((sobelx[i]<0&&sobely[i]>0)||(sobelx[i]>0&&sobely[i]<0)) {
				direct_map[i]=180.0f-(180.0f/3.141592f*atan(-sobely[i]/sobelx[i]));
			}
			else direct_map[i]=180.0f/3.141592f*atan(sobely[i]/sobelx[i]);
...
void bmphandler::nonmaximum_supr(float *direct_map)
{
	...
	for(int j=1;j<height-1;j++) {
		for(int k=1;k<width-1;k++){
			...
			if(direct_map[i]<22.5||direct_map[i]>=157.5) {left_bit=work_bits[i-1]; right_bit=work_bits[i+1];}
			else if(direct_map[i]<67.5) {left_bit=work_bits[i-1-width]; right_bit=work_bits[i+1+width];}
			else if(direct_map[i]<112.5) {left_bit=work_bits[i+width]; right_bit=work_bits[i-width];}
			else if(direct_map[i]<157.5) {left_bit=work_bits[i+width-1]; right_bit=work_bits[i-width+1];}

			if(work_bits[i]<left_bit||work_bits[i]<right_bit) results[i]=0; 
			else results[i]=work_bits[i];
			i++;
		}
...
{
	float *results=(float *)malloc(area*sizeof(float));
	int i=width+1;
	for(int j=1;j<height-1;j++) {
		for(int k=1;k<width-1;k++){

			if(work_bits[i]!=0){
				if(work_bits[i]>=thresh_high) results[i]=255;
				else if(work_bits[i]<thresh_low) results[i]=0;
				else{
					if(work_bits[i-1]>thresh_high||work_bits[i+1]>thresh_high||work_bits[i-width]>thresh_high||work_bits[i+width]>thresh_high||work_bits[i-width-1]>thresh_high||work_bits[i+width+1]>thresh_high||work_bits[i-width+1]>thresh_high||work_bits[i+width-1]>thresh_high) results[i]=255;
					else results[i]=0;
				}
			}
			else results[i]=0;
...
/*void bmphandler::hysteretic(float thresh_low, float thresh_high, bool connectivity)
{
	stack<int> s;
	float *results=(float *)malloc(area*sizeof(float));
	...
	for(int j=1;j<height-1;j++) {
		for(int k=1;k<width-1;k++){
			...
			if(work_bits[i]!=0){
				if(work_bits[i]>=thresh_high) {results[i]=255; s.push(i);}
				else if(work_bits[i]<thresh_low) results[i]=0;
				else{
					results[i]=-1;
				}
	...
	while(!s.empty()){
		i=s.top();
		s.pop();
		if(results[i-1]==-1) {results[i-1]=255; s.push(i-1);}
		if(results[i+1]==-1) {results[i+1]=255; s.push(i+1);}
		if(results[i-width]==-1) {results[i-width]=255; s.push(i-width);}
		if(results[i+width]==-1) {results[i+width]=255; s.push(i+width);}
		if(connectivity){
			if(results[i-width-1]==-1) {results[i-width-1]=255; s.push(i-width-1);}
			if(results[i+width+1]==-1) {results[i+width+1]=255; s.push(i+width+1);}
			if(results[i-width+1]==-1) {results[i-width+1]=255; s.push(i-width+1);}
			if(results[i+width-1]==-1) {results[i+width-1]=255; s.push(i+width-1);}
		}
	...
	for(unsigned int i1=0;i1<area;i1++) if(results[i1]==-1) results[i1]=0;
...
//			if(work_bits[i1]!=0){
				if(bmp_bits[i1]>=thresh_high) {results[i]=set_to; s.push_back(i);}
				else if(bmp_bits[i1]<thresh_low) results[i]=0;
				else{
					results[i]=-1;
				}
//			}
//			else results[i]=0;
	...
	for(int j=0;j<width+2;j++) results[j]=results[j+((unsigned)width+2)*(height+1)]=0;
	for(int j=0;j<=((int)width+2)*(height+1);j+=width+2) results[j]=results[j+width+1]=0;

/*	while(!s.empty()){
		i=s.top();
		s.pop();
		if(results[i-1]==-1) {results[i-1]=set_to; s.push(i-1);}
		if(results[i+1]==-1) {results[i+1]=set_to; s.push(i+1);}
		if(results[i-width-2]==-1) {results[i-width-2]=set_to; s.push(i-width-2);}
		if(results[i+width+2]==-1) {results[i+width+2]=set_to; s.push(i+width+2);}
		if(connectivity){
			if(results[i-width-3]==-1) {results[i-width-3]=set_to; s.push(i-width-3);}
			if(results[i+width+3]==-1) {results[i+width+3]=set_to; s.push(i+width+3);}
			if(results[i-width-1]==-1) {results[i-width-1]=set_to; s.push(i-width-1);}
			if(results[i+width+1]==-1) {results[i+width+1]=set_to; s.push(i+width+1);}
		}
	}
...
void bmphandler::hysteretic(float thresh_low, float thresh_high, bool connectivity,float *mask, float f, float set_to)
{
	...
	for(int j=0;j<height;j++) {
		for(int k=0;k<width;k++){
			...
			if(work_bits[i1]!=0) results[i]=work_bits[i1];
			else {
...
void bmphandler::double_hysteretic(float thresh_low_l, float thresh_low_h, float thresh_high_l, float thresh_high_h, bool connectivity,float set_to)
{
	unsigned char dummymode=mode1;
	vector<int> s;
	float *results=(float *)malloc(sizeof(float)*(area+2*width+2*height+4));
	...
	for(int j=0;j<height;j++) {
		for(int k=0;k<width;k++){
			...
			if(bmp_bits[i1]>thresh_high_h) results[i]=0;
			else if(bmp_bits[i1]>thresh_high_l) results[i]=-1;
			else if(bmp_bits[i1]>=thresh_low_h) {results[i]=set_to; s.push_back(i);}
			else if(bmp_bits[i1]>=thresh_low_l) results[i]=-1;
			else results[i]=0;
...
void bmphandler::double_hysteretic(float thresh_low_l, float thresh_low_h, float thresh_high_l, float thresh_high_h, bool connectivity, float *mask, float f,float set_to)
{
	unsigned char dummymode=mode1;
	vector<int> s;
	float *results=(float *)malloc(sizeof(float)*(area+2*width+2*height+4));
...
void bmphandler::thresholded_growing(Point p, float thresh_low, float thresh_high, bool connectivity,float set_to)
{
	unsigned char dummymode=mode1;
	...
	for(int j=0;j<height;j++) {
		for(int k=0;k<width;k++){
			...
			if(bmp_bits[i1]>thresh_high||bmp_bits[i1]<thresh_low) results[i]=0;
			else results[i]=-1;
			i++;
			i1++;
		}
...
void bmphandler::thresholded_growing(Point p, float thresh_low, float thresh_high, bool connectivity,float set_to,vector<Point> *limits1)
{
	unsigned char dummymode=mode1;
	unsigned position=pt2coord(p);
	vector<int> s;
	...
	for(vector<Point>::iterator it=limits1->begin();it!=limits1->end();it++)
		results[(it->py+1)*w+it->px+1]=0;

	for(int j=0;j<width+2;j++) results[j]=results[j+w*(height+1)]=0;
	for(int j=0;j<=((int)w)*(height+1);j+=w) results[j]=results[j+width+1]=0;
...
void bmphandler::thresholded_growinglimit(Point p, float thresh_low, float thresh_high, bool connectivity,float set_to)
{
	unsigned char dummymode=mode1;
	unsigned position=pt2coord(p);
	vector<int> s;
	...
	unsigned w=(unsigned) width+2;
	for(vector<vector<Point> >::iterator vit=limits.begin();vit!=limits.end();vit++){
...
void bmphandler::thresholded_growing(Point p, float threshfactor_low, float threshfactor_high, bool connectivity,float set_to, Pair *tp)
{
	unsigned char dummymode=mode1;
	unsigned position=pt2coord(p);
...
void bmphandler::thresholded_growing(float thresh_low, float thresh_high, bool connectivity, float *mask, float f, float set_to)
{
	unsigned char dummymode=mode1;
	f=f-f_tol;
	vector<int> s;
...
void bmphandler::hysteretic_growth(float *results, vector<int> *s, unsigned short w, unsigned short h, bool connectivity, float set_to)
{
	unsigned char dummymode=mode1;
	int i;
	...
	while(!(*s).empty()){
		i=(*s).back();
		(*s).pop_back();
		if(results[i-1]==-1) {results[i-1]=set_to; (*s).push_back(i-1);}
		if(results[i+1]==-1) {results[i+1]=set_to; (*s).push_back(i+1);}
		if(results[i-w]==-1) {results[i-w]=set_to; (*s).push_back(i-w);}
		if(results[i+w]==-1) {results[i+w]=set_to; (*s).push_back(i+w);}
		if(connectivity){
			if(results[i-w-1]==-1) {results[i-w-1]=set_to; (*s).push_back(i-w-1);}
			if(results[i+w+1]==-1) {results[i+w+1]=set_to; (*s).push_back(i+w+1);}
			if(results[i-w+1]==-1) {results[i-w+1]=set_to; (*s).push_back(i-w+1);}
			if(results[i+w-1]==-1) {results[i+w-1]=set_to; (*s).push_back(i+w-1);}
		}
	...
	for(unsigned i1=0;i1<unsigned(w)*h;i1++) if(results[i1]==-1) results[i1]=0;
...
void bmphandler::hysteretic_growth(float *results, vector<int> *s, unsigned short w, unsigned short h, bool connectivity, float set_to, int nr)
{
	vector<int> sta;
	vector<int> *s1=&sta;
	unsigned char dummymode=mode1;
	...
	for(int i1=0;i1<nr;i1++) {
		while(!(*s).empty()){
			i=(*s).back();
			(*s).pop_back();
			if(results[i-1]==-1) {results[i-1]=set_to; (*s1).push_back(i-1);}
			if(results[i+1]==-1) {results[i+1]=set_to; (*s1).push_back(i+1);}
			if(results[i-w]==-1) {results[i-w]=set_to; (*s1).push_back(i-w);}
			if(results[i+w]==-1) {results[i+w]=set_to; (*s1).push_back(i+w);}
			if(connectivity){
				if(results[i-w-1]==-1) {results[i-w-1]=set_to; (*s1).push_back(i-w-1);}
				if(results[i+w+1]==-1) {results[i+w+1]=set_to; (*s1).push_back(i+w+1);}
				if(results[i-w+1]==-1) {results[i-w+1]=set_to; (*s1).push_back(i-w+1);}
				if(results[i+w-1]==-1) {results[i+w-1]=set_to; (*s1).push_back(i+w-1);}
			}
...
void bmphandler::sobel_finer()
{
	...
	sliceprovide->take_back(bmp_bits);
	bmp_bits=dummy;
	for(unsigned i=0;i<area;i++) work_bits[i]=sqrt(work_bits[i]*work_bits[i]+bmp_bits[i]*bmp_bits[i]);
	sliceprovide->take_back(bmp_bits);
	bmp_bits=tmp;
...
void bmphandler::median_interquartile(bool median)
{
	...
	if(median){
		for(unsigned k=0;k<width;k++) work_bits[k]=bmp_bits[k];
		for(unsigned k=area-width;k<area;k++) work_bits[k]=bmp_bits[k];
		for(unsigned k=0;k<area;k+=width) work_bits[k]=bmp_bits[k];
		for(unsigned k=width-1;k<area;k+=width) work_bits[k]=bmp_bits[k];
	}
	else{
		for(unsigned k=0;k<width;k++) work_bits[k]=0;
		for(unsigned k=area-width;k<area;k++) work_bits[k]=0;
		for(unsigned k=0;k<area;k+=width) work_bits[k]=0;
		for(unsigned k=width-1;k<area;k+=width) work_bits[k]=0;
	}
...
void bmphandler::median_interquartile(float *median, float *iq)
{
	...
	for(unsigned k=0;k<width;k++) median[k]=bmp_bits[k];
	for(unsigned k=area-width;k<area;k++) median[k]=bmp_bits[k];
	for(unsigned k=0;k<area;k+=width) median[k]=bmp_bits[k];
	for(unsigned k=width-1;k<area;k+=width) median[k]=bmp_bits[k];
	for(unsigned k=0;k<width;k++) iq[k]=0;
	for(unsigned k=area-width;k<area;k++) iq[k]=0;
	for(unsigned k=0;k<area;k+=width) iq[k]=0;
	for(unsigned k=width-1;k<area;k+=width) iq[k]=0;
...
void bmphandler::sigmafilter(float sigma, unsigned short nx, unsigned short ny)
{
	...
	for(int j=0;j<=height-ny;j++){
		for(int k=0;k<=width-nx;k++){
			...
			for(int l=0;l<nx;l++) {
				for(int o=0;o<ny;o++){
					if(bmp_bits[i+l+o*width]<dummy+sigma&&bmp_bits[i+l+o*width]>dummy-sigma){
...
//void bmphandler::interquartile()
//{
//	float smallest, secondsm, largest, secondlg, dummy;
//	unsigned k=0;
//	for(unsigned short i=0;i<height-2;i++){
//		for(unsigned short j=0;j<width-2;j++){
//			smallest=secondsm=largest=secondlg=bmp_bits[k];
//			for(unsigned short m=1;m<3;m++){
//				dummy=bmp_bits[k+m*width];
//				if(dummy<=smallest){
//					secondsm=smallest;
//					smallest=dummy;
//				}
//				else if(dummy<secondsm){
//					secondsm=dummy;
//				}
//				else if(dummy>=largest){
//					secondlg=largest;
//					largest=dummy;
//				}
//				else if(dummy>secondlg){
//					secondlg=dummy;
//				}
//			}
//			for(unsigned short l=1;l<3;l++){
//				for(unsigned short m=0;m<3;m++){
//					dummy=bmp_bits[k+l+m*width];
//					if(dummy<=smallest){
//						secondsm=smallest;
//						smallest=dummy;
//					}
...
//				}
//			}
//			work_bits[k+1+width]=secondlg-secondsm;
//			k++;
//		}
//		k+=2;
//	}
...
void bmphandler::sobelxy(float **sobelx,float **sobely)
{
	...
	for(unsigned int i=0;i<area;i++) work_bits[i]=abs((*sobelx)[i])+abs((*sobely)[i]);
...
void bmphandler::compacthist()
{
	...
	for(int i=1;i<255;i++) {
		if(histogram[i]==0&&histogram[i-1]!=0&&histogram[i+1]!=0) dummy+=1;
		else histmap[i]=dummy;
	}
	histmap[255]=dummy;

	for(unsigned int i=0;i<area;i++) work_bits[i]-=histmap[min((int)work_bits[i],255)];
...
float *bmphandler::find_modal(unsigned int thresh1,float thresh2)
{
	...
	lastmin=0; lastmax_h=lastmin_h=histogram[0];
	for(int i=1;i<256;i++){
		temp_hist=histogram[i];

		if(pushable){
			if(temp_hist<lastmin_h) {lastmin_h=temp_hist; lastmin=i;}
			...
			if(temp_hist>lastmax_h) {lastmax_h=temp_hist;}
...
void bmphandler::subthreshold(int n1, int n2,unsigned int thresh1,float thresh2, float sigma)
{
	...
	for(int i=0;i<n1;i++){
		for(int j=0;j<n2;j++){
			p.px=i*dx; p.py=j*dy;
//			dx1=min(dx,width-i*dx);
//			dy1=min(dy,height-j*dy);
...
void bmphandler::erosion1(int n, bool connectivity) // true for 8-, false for 4-connectivity
{
	...
	for(int l=0;l<n;l++){
		for(unsigned int i=0;i<area;i++) results[i]=work_bits[i];
...
void bmphandler::erosion(int n, bool connectivity) // true for 8-, false for 4-connectivity
{
	...
	for(int l=0;l<n;l++){
		...
		for(unsigned short i=0;i<(height-1);i++){
			for(unsigned short j=0;j<width;j++){
				if(work_bits[i1]!=work_bits[i1+width]) results[i1]=results[i1+width]=0;
		...
		for(unsigned short i=0;i<height;i++){
			for(unsigned short j=0;j<(width-1);j++){
				if(work_bits[i1]!=work_bits[i1+1]) results[i1]=results[i1+1]=0;
		...
		if(connectivity){
			...
			for(unsigned short i=0;i<(height-1);i++){
				for(unsigned short j=0;j<(width-1);j++){
					if(work_bits[i1]!=work_bits[i1+width+1]) results[i1]=results[i1+width+1]=0;
					if(work_bits[i1+1]!=work_bits[i1+width]) results[i1+1]=results[i1+width]=0;
...
void bmphandler::dilation(int n, bool connectivity) // true for 8-, false for 4-connectivity
{
	...
	for(int l=0;l<n;l++){
		...
		for(unsigned short i=0;i<(height-1);i++){
			for(unsigned short j=0;j<width;j++){
				if(work_bits[i1]!=work_bits[i1+width]) {
					if(work_bits[i1]==0) results[i1]=results[i1+width];
					if(work_bits[i1+width]==0) results[i1+width]=results[i1];
				}
		...
		for(unsigned short i=0;i<height;i++){
			for(unsigned short j=0;j<(width-1);j++){
				if(work_bits[i1]!=work_bits[i1+1]) {
					if(work_bits[i1]==0) results[i1]=results[i1+1];
					if(work_bits[i1+1]==0) results[i1+1]=results[i1];
				}
		...
		if(connectivity){
			i1=0;for(unsigned short i=0;i<(height-1);i++){
				for(unsigned short j=0;j<(width-1);j++){
					if(work_bits[i1]!=work_bits[i1+width+1]) {
						if(work_bits[i1]==0) results[i1]=results[i1+width+1];
						if(work_bits[i1+width+1]==0) results[i1+width+1]=results[i1];
					}
					if(work_bits[i1+1]!=work_bits[i1+width]) {
						if(work_bits[i1+1]==0) results[i1+1]=results[i1+width];
						if(work_bits[i1+width]==0) results[i1+width]=results[i1+1];
					}
...
void bmphandler::mark_border(bool connectivity) // true for 8-, false for 4-connectivity
{
	float *results=sliceprovide->give_me();
	...
	for(unsigned short i=0;i<(height-1);i++){
		for(unsigned short j=0;j<width;j++){
			if(work_bits[i1]>work_bits[i1+width]) results[i1]=-1;
			else if(work_bits[i1]<work_bits[i1+width]) results[i1+width]=-1;
	...
	for(unsigned short i=0;i<height;i++){
		for(unsigned short j=0;j<(width-1);j++){
			if(work_bits[i1]>work_bits[i1+1]) results[i1]=-1;
			else if(work_bits[i1]<work_bits[i1+1]) results[i1+1]=-1;
	...
	if(connectivity){
		...
		for(unsigned short i=0;i<(height-1);i++){
			for(unsigned short j=0;j<(width-1);j++){
				if(work_bits[i1]>work_bits[i1+width+1]) results[i1]=-1;
				else if(work_bits[i1]<work_bits[i1+width+1]) results[i1+width+1]=-1;
				if(work_bits[i1+1]>work_bits[i1+width]) results[i1+1]=-1;
				else if(work_bits[i1+1]<work_bits[i1+width]) results[i1+width]=-1;
...
void bmphandler::zero_crossings(bool connectivity) // true for 8-, false for 4-connectivity
{
	float *results=sliceprovide->give_me();

	for(unsigned int i=0;i<area;i++) results[i]=255;
	...
	for(unsigned short i=0;i<(height-1);i++){
		for(unsigned short j=0;j<width;j++){
			if(work_bits[i1]*work_bits[i1+width]<=0) 
				if(work_bits[i1]>0) results[i1+width]=-1; 
				else if(work_bits[i1+width]>0) results[i1]=-1;
	...
	for(unsigned short i=0;i<height;i++){
		for(unsigned short j=0;j<(width-1);j++){
			if(work_bits[i1]*work_bits[i1+1]<=0) 
				if(work_bits[i1]>0) results[i1+1]=-1; 
				else if(work_bits[i1+1]>0) results[i1]=-1;
		...
		for(unsigned short i=0;i<(height-1);i++){
			for(unsigned short j=0;j<(width-1);j++){
				if(work_bits[i1]*work_bits[i1+width+1]<=0) 
					if(work_bits[i1]>0) results[i1+width+1]=-1; 
					else if(work_bits[i1+width+1]>0) results[i1]=-1;

				if(work_bits[i1+1]*work_bits[i1+width]<=0) 
					if(work_bits[i1+1]>0) results[i1+width]=-1; 
					else if(work_bits[i1+width]>0) results[i1+1]=-1;
...
void bmphandler::zero_crossings(float thresh,bool connectivity) // true for 8-, false for 4-connectivity
{
	float *results=sliceprovide->give_me();
	...
	for(unsigned short i=0;i<(height-1);i++){
		for(unsigned short j=0;j<width;j++){
			if(work_bits[i1]*work_bits[i1+width]<0&&abs(work_bits[i1]-work_bits[i1+width])>thresh) 
				if(work_bits[i1]>0) results[i1]=-1; else results[i1+width]=-1;
	...
	for(unsigned short i=0;i<height;i++){
		for(unsigned short j=0;j<(width-1);j++){
			if(work_bits[i1]*work_bits[i1+1]<0&&abs(work_bits[i1]-work_bits[i1+width])>thresh) 
				if(work_bits[i1]>0) results[i1]=-1; else results[i1+1]=-1;
	...
	if(connectivity){
		...
		for(unsigned short i=0;i<(height-1);i++){
			for(unsigned short j=0;j<(width-1);j++){
				if(work_bits[i1]*work_bits[i1+width+1]<0&&abs(work_bits[i1]-work_bits[i1+width])>thresh) 
					if(work_bits[i1]>0) results[i1]=-1; else results[i1+width+1]=-1;

				if(work_bits[i1+1]*work_bits[i1+width]<0&&abs(work_bits[i1]-work_bits[i1+width])>thresh) 
					if(work_bits[i1+1]>0) results[i1+1]=-1; else results[i1+width]=-1;
...
//	for(unsigned i=0;i<area;i++) tmp2[i]-=thresh*work_bits[i];
	...
	for(unsigned i=0;i<area;i++) {
		if(work_bits[i]==-1&&tmp2[i]<thresh) work_bits[i]=255;
//		work_bits[i]=tmp2[i];
	}
...
void bmphandler::n_moment(short unsigned n, short unsigned p)
{
	...
	for(unsigned int i=0;i<area;i++)results[i]=0;

	for(unsigned short i=0;i<=height-n;i++){
		for(unsigned short j=0;j<=width-n;j++){
			for(unsigned short k=0;k<n;k++){
				for(unsigned short l=0;l<n;l++){
					results[(i+n/2)*width+j+n/2]+=pow(abs(bmp_bits[(i+k)*width+j+l]-work_bits[(i+n/2)*width+j+n/2]),p);
				}
			}
			results[(i+n/2)*width+j+n/2]=results[(i+n/2)*width+j+n/2]/(n*n);
		}
	}
...
void bmphandler::n_moment_sigma(short unsigned n, short unsigned p, float sigma)
{
	...
	for(unsigned short i=0;i<=height-n;i++){
		for(unsigned short j=0;j<=width-n;j++){
			...
			for(unsigned short k=0;k<n;k++){
				for(unsigned short l=0;l<n;l++){
					dummy=abs(bmp_bits[(i+k)*width+j+l]-work_bits[(i+n/2)*width+j+n/2]);
					if(dummy<sigma){
						count++;
			...
			results[(i+n/2)*width+j+n/2]=results[(i+n/2)*width+j+n/2]/(count);
		}
	}
...
void bmphandler::aniso_diff(float dt, int n, float (*f)(float,float), float k, float restraint)
{
	unsigned char dummymode=mode1;
	bmp2work();
...
void bmphandler::cont_anisodiff(float dt, int n, float (*f)(float,float), float k, float restraint)
{
	...
	for(int l=0;l<n;l++){
		...
		for(unsigned int i=0;i<area;i++) work_bits[i]+=dt*restraint*(bmp_bits[i]-work_bits[i]);
...
{
  return exp(-pow(dI/k,2));
}

float f2(float dI, float k)
{
  return 1/(1+pow(dI/k,2));
}

void bmphandler::to_bmpgrey(float *p_bits)
{
	for(unsigned int i=0;i<area;i++){
		p_bits[i]=floor(min(255.0f,max(0.0f,p_bits[i]+0.5f)));
	}
	return;
}
...
unsigned *bmphandler::watershed(bool connectivity)
{
	...
	for(unsigned i=0;i<area;i++) Y[i]=unvisited;

	vector<unsigned> sorted[256];
	vector<unsigned>::iterator it,it1;
	vector<unsigned> Bp;
	...
	for(unsigned i=0;i<256;i++){
		for(it=sorted[i].begin();it!=sorted[i].end();it++){
			...
			if(p%width!=0&&Y[p-1]!=unvisited) Bp.push_back(Y[p-1]);
			if((p+1)%width!=0&&Y[p+1]!=unvisited) Bp.push_back(Y[p+1]);
			if(p>=width&&Y[p-width]!=unvisited) Bp.push_back(Y[p-width]);
			if((p+width)<area&&Y[p+width]!=unvisited) Bp.push_back(Y[p+width]);
			if(connectivity){
				if(p%width!=0&&p>=width&&Y[p-1-width]!=unvisited) Bp.push_back(Y[p-1-width]);
				if(p%width!=0&&(p+width)<area&&Y[p-1+width]!=unvisited) Bp.push_back(Y[p+width-1]);
				if((p+1)%width!=0&&p>=width&&Y[p-width+1]!=unvisited) Bp.push_back(Y[p-width+1]);
				if((p+1)%width!=0&&(p+width)<area&&Y[p+width+1]!=unvisited) Bp.push_back(Y[p+width+1]);
			}
...
/*				it1=Bp.begin();
				minbase=deepest_con_bas(*it1);
				for(it1++;it1!=Bp.end();it1++) 
					minbase=min(minbase,deepest_con_bas(*it1));*/
				minbase_nr=Bp[0];
				minbase=deepest_con_bas(Bp[0]);
				for(unsigned j=1;j<Bp.size();j++)
					if(deepest_con_bas(Bp[j])<minbase) minbase_nr=Bp[j];
				Y[p]=minbase_nr;
...
bool bmphandler::remove_mark(Point p, unsigned radius)
{
	...
	while(it!=marks.end()&&(unsigned int)((it->p.px-p.px)*(it->p.px-p.px)+(it->p.py-p.py)*(it->p.py-p.py))>radius) it++;
...
{
	if(wshedobj.B[k].r!=k) return deepest_con_bas(wshedobj.B[k].r); 
	else return k;
}
...
void bmphandler::cond_merge(unsigned m, unsigned h)
{
	...
	if(h>=(wshedobj.M[m].g-wshedobj.B[k].g)){
		if(wshedobj.B[a].l==0||wshedobj.B[k].l==0||wshedobj.B[a].l==wshedobj.B[k].l){
			wshedobj.B[k].r=a;
...
void bmphandler::wshed2work(unsigned *Y)
{
	...
	for(unsigned i=0;i<area;i++)work_bits[i]=Y[i]*d;
...
/*void bmphandler::labels2work(unsigned *Y,unsigned lnr)
{
	float d=255.0f/lnr;
	for(unsigned i=0;i<area;i++) 
		work_bits[i]=label_lookup(i,Y)*d;
	return;
}*/
...
{
	unsigned k=wshed[i];
	if(wshedobj.B[k].l==0&&wshedobj.B[k].r!=k) wshedobj.B[k].l=wshedobj.B[deepest_con_bas(k)].l;
	...
	for(unsigned i=0;i<n-1;i++){
		...
		if(dx>dy){
			for(unsigned j=1;j<dx;j++){
				P.px=short(0.5+((float)p_vec[i+1].px-(float)p_vec[i].px)*j/dx+(float)p_vec[i].px);
				P.py=short(0.5+((float)p_vec[i+1].py-(float)p_vec[i].py)*j/dx+(float)p_vec[i].py);
				work_bits[pt2coord(P)]=255;
			}
		}
		else{
			for(unsigned j=1;j<dy;j++){
				P.px=short(0.5+((float)p_vec[i+1].px-(float)p_vec[i].px)*j/dy+(float)p_vec[i].px);
				P.py=short(0.5+((float)p_vec[i+1].py-(float)p_vec[i].py)*j/dy+(float)p_vec[i].py);
				work_bits[pt2coord(P)]=255;
			}
		}		
	}

	dx=max(p_vec[n-1].px,p_vec[0].px)-min(p_vec[n-1].px,p_vec[0].px);
	dy=max(p_vec[n-1].py,p_vec[0].py)-min(p_vec[n-1].py,p_vec[0].py);
	P.px=p_vec[n-1].px;
	P.py=p_vec[n-1].py;
	work_bits[pt2coord(P)]=0;
	if(dx>dy){
		for(unsigned j=1;j<dx;j++){
			P.px=short(0.5+((float)p_vec[0].px-(float)p_vec[n-1].px)*j/dx+(float)p_vec[n-1].px);
			P.py=short(0.5+((float)p_vec[0].py-(float)p_vec[n-1].py)*j/dx+(float)p_vec[n-1].py);
			work_bits[pt2coord(P)]=255;
		}
	}
	else{
		for(unsigned j=1;j<dy;j++){
			P.px=short(0.5+((float)p_vec[0].px-(float)p_vec[n-1].px)*j/dy+(float)p_vec[n-1].px);
			P.py=short(0.5+((float)p_vec[0].py-(float)p_vec[n-1].py)*j/dy+(float)p_vec[n-1].py);
			work_bits[pt2coord(P)]=255;
		}
	}	
...
void bmphandler::connected_components(bool connectivity){
	...
	for(unsigned i=0;i<area;i++) temp[i+i/width+width+2]=bmp_bits[i];
	for(unsigned i=width+1;i<=area+height;i+=width+1) temp[i]=-1e10f;
	for(unsigned i=0;i<=width;i++) temp[i]=-2e10f;
		...
		for(short k=0;k<width;k++){
			if(temp[i1]==temp[i1-1]) {
				...
				if(temp[i1]==temp[i1-width-1]) {
						maps[(int)base_connection(work_bits[i2-width],&maps)]=work_bits[i2];
				}
			}
			else {
				if(temp[i1]==temp[i1-width-1]) work_bits[i2]=base_connection(work_bits[i2-width],&maps);
				else if(connectivity&&temp[i1]==temp[i1-width-2]) work_bits[i2]=base_connection(work_bits[i2-width-1],&maps);
				else {
			...
			if(connectivity&&temp[i1-1]==temp[i1-width-1]) maps[(int)base_connection(work_bits[i2-1],&maps)]=base_connection(work_bits[i2-width],&maps);
	...
	for(unsigned i=0;i<area;i++) work_bits[i]=base_connection(work_bits[i],&maps);

	newest=0.0f;
	for(i1=0;i1<(unsigned)maps.size();i1++) if((unsigned)maps[i1]==i1){
		maps[i1]=newest;
		newest++;
	}

	for(unsigned i=0;i<area;i++) work_bits[i]=maps[(int)work_bits[i]];
...
{
	if(c==(*maps)[(int)c]) return c;
	else return (*maps)[(int)c]=base_connection((*maps)[(int)c],maps);
}
	...
	for(int l=0;l<(int) std::ceil(float(n+1)/2);l++){
		...
		for(unsigned short i=0;i<(height-1);i++){
			for(unsigned short j=0;j<width;j++){
				...
				if(isinterfaceold[i1]&&(bmp_bits[i1+width]==0)) isinterface[i1+width]=true;
				else if(isinterfaceold[i1+width]&&(bmp_bits[i1]==0)) isinterface[i1]=true;
		...
		for(unsigned short i=0;i<height;i++){
			for(unsigned short j=0;j<(width-1);j++){
				...
				if(isinterfaceold[i1]&&(bmp_bits[i1+1]==0)) isinterface[i1+1]=true;
				else if(isinterfaceold[i1+1]&&(bmp_bits[i1]==0)) isinterface[i1]=true;
		...
		if(connectivity){
			i1=0;for(unsigned short i=0;i<(height-1);i++){
				for(unsigned short j=0;j<(width-1);j++){
					...
					if(isinterfaceold[i1]&&(bmp_bits[i1+width+1]==0)) isinterface[i1+width+1]=true;
					else if(isinterfaceold[i1+width+1]&&(bmp_bits[i1]==0)) isinterface[i1]=true;
					...
					if(isinterfaceold[i1+1]&&(bmp_bits[i1+width]==0)) isinterface[i1+width]=true;
					else if(isinterfaceold[i1+width]&&(bmp_bits[i1+1]==0)) isinterface[i1+1]=true;
...
void bmphandler::fill_gapstissue(tissuelayers_size_t idx, short unsigned n, bool connectivity)
{
	unsigned char dummymode1=mode1;
	unsigned char dummymode2=mode2;
...
void bmphandler::get_tissuecontours(tissuelayers_size_t idx, tissues_size_t f, vector<vector<Point> > *outer_line, vector<vector<Point> > *inner_line,int minsize)
{
	minsize=2*minsize;
	float bubble_size;
	float linelength;
	...
	tissues_size_t *tmp_bits=(tissues_size_t *)malloc(sizeof(tissues_size_t)*(width+2)*(height+2));
	bool *visited=(bool *)malloc(sizeof(bool)*(width+2)*(height+2));
	for(unsigned i=0;i<unsigned(width+2)*(height+2);i++) visited[i]=false;
	...
	vector<Point> vec_pt;
	short offset[8]={1, width+3, width+2, width+1, -1, -width-3, -width-2, -width-1};
	...
	for(unsigned i=0;i<unsigned(width+2);i++) tmp_bits[i]=TISSUES_SIZE_MAX;
	for(unsigned i=unsigned(width+2)*(height+1);i<unsigned(width+2)*(height+2);i++) tmp_bits[i]=TISSUES_SIZE_MAX;
	for(unsigned i=0;i<unsigned(width+2)*(height+2);i+=(width+2)) tmp_bits[i]=TISSUES_SIZE_MAX;
	for(unsigned i=width+1;i<unsigned(width+2)*(height+2);i+=(width+2)) tmp_bits[i]=TISSUES_SIZE_MAX;

	pos=width+2;
	while(pos<unsigned(width+2)*(height+1))
	{

		while((tmp_bits[pos]!=f||tmp_bits[pos-1]==f||visited[pos])&&pos<unsigned(width+2)*(height+1)) pos++;
...
//			vec_pt.push_back(p);
			...
			if(tmp_bits[pos+1]!=f&&tmp_bits[pos+width+3]!=f&&tmp_bits[pos+width+2]!=f&&tmp_bits[pos+width+1]!=f&&tmp_bits[pos-width-1]!=f&&tmp_bits[pos-width-2]!=f&&tmp_bits[pos-width-3]!=f){
				vec_pt.push_back(p);
				if(1>=minsize)
				...
				if(tmp_bits[pos-width-3]==f){// tricky criteria
					inner=7; // inner line
...
//					if(direction<5||inner==1||tmp_bits[pos1-1]==f||directionold<4)
					if(tmp_bits[pos1-1]==f||
					 (inner==7&&!(((direction+6)%8>2&&directionold>3&&(directionchange+5)%8>2)||(direction==5&&directionold==3)))||
					 (inner==1&&!(((direction+4)%8>2&&(directionold+7)%8<4&&(directionchange+5)%8>2)||(direction==3&&directionold==5)))					 
...
//						cout << bubble_size+linelength <<", ";
						for(vector<Point>::iterator it1=vec_pt.begin();it1!=vec_pt.end();it1++)
						{
...
//						cout << endl;
					...
					if(bubble_size>=(float)minsize) (*inner_line).push_back(vec_pt);
					if(bubble_size<0) {
//						cout << bubble_size <<"; ";
						for(vector<Point>::iterator it1=vec_pt.begin();it1!=vec_pt.end();it1++)
						{
...
void bmphandler::get_tissuecontours2_xmirrored(tissuelayers_size_t idx, tissues_size_t f, vector<vector<Point> > *outer_line, vector<vector<Point> > *inner_line, int minsize)
{
	//int w=(int)width;
	//int h=(int)height;
	int movpos[4];
	...
	for(unsigned i=0;i<unsigned(width+2);i++) tmp_bits[i]=setto;
	for(unsigned i=unsigned(width+2)*(height+1);i<unsigned(width+2)*(height+2);i++) tmp_bits[i]=setto;
	for(unsigned i=0;i<unsigned(width+2)*(height+2);i+=(width+2)) tmp_bits[i]=setto;
	for(unsigned i=width+1;i<unsigned(width+2)*(height+2);i+=(width+2)) tmp_bits[i]=setto;
	...
	for(unsigned i=0;i<unsigned(width+2)*(height+2);i++) nrlines[i]=0;

	pos=0;
	for(unsigned short i=0;i<height+1;i++){
		for(unsigned short j=0;j<width+1;j++){
			if((tmp_bits[pos]!=tmp_bits[pos+1])&&(tmp_bits[pos]==f1||tmp_bits[pos+1]==f1)) nrlines[pos]++;
			if((tmp_bits[pos]!=tmp_bits[pos+width+2])&&(tmp_bits[pos]==f1||tmp_bits[pos+width+2]==f1)) nrlines[pos]++;
			if((tmp_bits[pos+width+2]!=tmp_bits[pos+width+3])&&(tmp_bits[pos+width+2]==f1||tmp_bits[pos+width+3]==f1)) nrlines[pos]++;
			if((tmp_bits[pos+1]!=tmp_bits[pos+width+3])&&(tmp_bits[pos+1]==f1||tmp_bits[pos+width+3]==f1)) nrlines[pos]++;
			pos++;
		}
		pos++;
	}
	...
	while(pos<int(width+2)*(height+2)){
		while(pos<int(width+2)*(height+2)&&nrlines[pos]==0) pos++;
		...
		if(pos!=int(width+2)*(height+2)){
			...
			do {
				count++;
				casenr=0;
				if(tmp_bits[pos]==f1) casenr+=1;
				if(tmp_bits[pos+1]==f1) casenr+=2;
				if(tmp_bits[pos+width+3]==f1) casenr+=4;
				if(tmp_bits[pos+width+2]==f1) casenr+=8;
				nrlines[pos]-=2;
				switch(casenr){
					case 1:
						if(direction==0){
					...
					case 4:
						...
						break;
					case 5:
						if(tmp_bits[pos+width+2]!=tmp_bits[pos+1]||tmp_bits[pos+1]>f1){
							if(direction==0){
								direction=1;
								p.px--;
								p.py++;
...
void bmphandler::get_tissuecontours2_xmirrored(tissuelayers_size_t idx, tissues_size_t f, vector<vector<Point> > *outer_line, vector<vector<Point> > *inner_line, int minsize, float disttol)
{
	//int w=(int)width;
	//int h=(int)height;
	int movpos[4];
...
void bmphandler::get_contours(float f, vector<vector<Point> > *outer_line, vector<vector<Point> > *inner_line,int minsize)
{
	minsize=2*minsize;
	float bubble_size;
	float linelength;
	...
	for(unsigned i=0;i<unsigned(width+2);i++) tmp_bits[i]=unvisited;
	for(unsigned i=unsigned(width+2)*(height+1);i<unsigned(width+2)*(height+2);i++) tmp_bits[i]=unvisited;
	for(unsigned i=0;i<unsigned(width+2)*(height+2);i+=(width+2)) tmp_bits[i]=unvisited;
	for(unsigned i=width+1;i<unsigned(width+2)*(height+2);i+=(width+2)) tmp_bits[i]=unvisited;
...
void bmphandler::get_contours(Point p, vector<vector<Point> > *outer_line, vector<vector<Point> > *inner_line,int minsize)
{
...
void bmphandler::distance_map(bool connectivity)
{
	...
	float wbp;
	unsigned p;
	for(unsigned i=0;i<area;i++) work_bits[i]=0;
	...
	for(unsigned i=255;i>0;i--){
		for(it=sorted[i].begin();it!=sorted[i].end();it++){
			...
			if((unsigned)floor(wbp+0.5f)==i){
				if(p%width!=0&&work_bits[p-1]<wbp-1) {work_bits[p-1]=wbp-1;sorted[i-1].push_back(p-1);}
				if((p+1)%width!=0&&work_bits[p+1]<wbp-1) {work_bits[p+1]=wbp-1;sorted[i-1].push_back(p+1);}
				if(p>=width&&work_bits[p-width]<wbp-1) {work_bits[p-width]=wbp-1;sorted[i-1].push_back(p-width);}
				if((p+width)<area&&work_bits[p+width]<wbp-1) {work_bits[p+width]=wbp-1;sorted[i-1].push_back(p+width);}
				if(connectivity){
					if(p%width!=0&&p>=width&&work_bits[p-1-width]<wbp-1) {work_bits[p-1-width]=wbp-1;sorted[i-1].push_back(p-1-width);}
					if(p%width!=0&&(p+width)<area&&work_bits[p-1+width]<wbp-1) {work_bits[p-1+width]=wbp-1;sorted[i-1].push_back(p-1+width);}
					if((p+1)%width!=0&&p>=width&&work_bits[p-width+1]<wbp-1) {work_bits[p-width+1]=wbp-1;sorted[i-1].push_back(p-width+1);}
					if((p+1)%width!=0&&(p+width)<area&&work_bits[p+width+1]<wbp-1) {work_bits[p+width+1]=wbp-1;sorted[i-1].push_back(p+width+1);}
				}
			}
...
void bmphandler::distance_map(bool connectivity, float f, short unsigned levlset)
{
	unsigned char dummymode=mode1;
	...
	vector<unsigned>::iterator vuit;
	vector<Point>::iterator vpit;
	...
	for(unsigned i=0;i<area;i++) work_bits[i]=background;
	for(vuit=v1.begin();vuit!=v1.end();vuit++) work_bits[*vuit]=f;

	if(levlset==0)
		for(unsigned i=0;i<area;i++) if(bmp_bits[i]==f) work_bits[i]=f;
	...
	while(!(*vp1).empty()){
		for(vuit=(*vp1).begin();vuit!=(*vp1).end();vuit++){
			...
			if(p%width!=0&&work_bits[p-1]==background) {
				if(bmp_bits[p-1]==f) {work_bits[p-1]=wbp+1; (*vp2).push_back(p-1);}
				else {work_bits[p-1]=wbp-1; (*vp2).push_back(p-1);}
			}
			...
			if(p>=width&&work_bits[p-width]==background) {
				if(bmp_bits[p-width]==f) {work_bits[p-width]=wbp+1; (*vp2).push_back(p-width);}
				else {work_bits[p-width]=wbp-1; (*vp2).push_back(p-width);}
			}
			...
			if(connectivity){
				if(p%width!=0&&p>=width&&work_bits[p-1-width]==background) {
					if(bmp_bits[p-1-width]==f) {work_bits[p-1-width]=wbp+1; (*vp2).push_back(p-1-width);}
					else {work_bits[p-1-width]=wbp-1; (*vp2).push_back(p-1-width);}
				}

				if(p%width!=0&&(p+width)<area&&work_bits[p-1+width]==background) {
					if(bmp_bits[p-1+width]==f) {work_bits[p-1+width]=wbp+1; (*vp2).push_back(p-1+width);}
					else {work_bits[p-1+width]=wbp-1; (*vp2).push_back(p-1+width);}
				}

				if((p+1)%width!=0&&p>=width&&work_bits[p-width+1]==background) {
					if(bmp_bits[p-width+1]==f) {work_bits[p-width+1]=wbp+1; (*vp2).push_back(p-width+1);}
					else {work_bits[p-width+1]=wbp-1; (*vp2).push_back(p-width+1);}					
				}

				if((p+1)%width!=0&&(p+width)<area&&work_bits[p+width+1]==background) {
					if(bmp_bits[p+width+1]==f) {work_bits[p+width+1]=wbp+1; (*vp2).push_back(p+width+1);}
					else {work_bits[p+width+1]=wbp-1; (*vp2).push_back(p+width+1);}
				}
...
//		cout << "."<<vi[i].size();
		...
		for(unsigned short l=0;l<width;l++){
			if(k>0){			
				if(l>0&&work_bits[j-1-width]+d2<work_bits[j]) {
					P[j]=P[j-1-width];
					work_bits[j]=sqrt(float((l-P[j]%width)*(l-P[j]%width)+(k-P[j]/width)*(k-P[j]/width)));
				}
	...
	for(unsigned i=0;i<area;i++) 
		if(bmp_bits[i]!=f) work_bits[i]=-work_bits[i];
	...
	for(unsigned short k=0;k<height;k++){
		for(unsigned short l=0;l<width;l++){
			if(k>0){			
				if(l>0&&work_bits[j-1-width]+d2<work_bits[j]) {
					P[j]=P[j-1-width];
					work_bits[j]=(float((l-P[j]%width)*(l-P[j]%width)+(k-P[j]/width)*(k-P[j]/width)));
				}
...
void bmphandler::stupidDT()
{
	...
	for(unsigned i=0;i<area;i++) work_bits[i]=f[i];
	vector<Point> Pt_vec;
	Point p;
	p.px=200;
	p.py=350;
...
//	IFTrg.rg_init(width,height,work_bits,lb_map);
	...
	for(unsigned i=0;i<area;i++) {
		if(f2[i]<thresh) work_bits[i]=f1[i];
		else work_bits[i]=0;
	}
...
/*void bmphandler::rgIFT(vector<mark> *vm,float thresh)
{
	IFT_regiongrowing IFTrg;

//	sobel();

	float *lb_map=(float *)malloc(sizeof(float)*area);
//	IFTrg.rg_init(width,height,work_bits,lb_map);
	IFTrg.rg_init(width,height,bmp_bits,lb_map);
...
/*void bmphandler::livewire(){
	float *sobelx=(float *)malloc(sizeof(float)*area);
	float *sobely=(float *)malloc(sizeof(float)*area);
	float *tmp=(float *)malloc(sizeof(float)*area);
	float *dummy;
	float *grad;
	...
	laplacian1();
	zero_crossings(0.2f,false);

	for(unsigned i=0;i<area;i++) work_bits[i]=(0.43f*(1-grad[i]/p.high)+0.43f*work_bits[i]/255)*100;
...
//	work_bits=sobelx;
	...
	if(p.high!=0)
		for(unsigned i=0;i<area;i++) sobelx[i]=(0.43f*(1-sobely[i]/p.high)+0.43f*((work_bits[i]+1)/256));
	else
		for(unsigned i=0;i<area;i++) sobelx[i]=0.43f*((work_bits[i]+1)/256);
...
void bmphandler::livewire_test()
{
	...
	for(unsigned i=0;i<area;i++) work_bits[i]=(0.43f*(1-grad[i]/p.high)+0.43f*((work_bits[i]+1)/256));
...
//	p2.py=410;
	...
	for(unsigned i=0;i<area;i++) work_bits[i]=tmp[i];
	for(vector<Point>::iterator it=Pt_vec.begin();it!=Pt_vec.end();it++)
		work_bits[(*it).py*width+(*it).px]=255;
...
void bmphandler::fill_contour(vector<Point> *vp, bool continuous)
{
	...
	if(continuous){
		...
		for(int j=width+3;j<2*width+3;j++) {
			if(results[j]==-1) {results[j]=255.0f; s.push_back(j);}
		}
		for(unsigned int j=area+2*height+1;j<area+width+2*height+1;j++) {
			if(results[j]==-1) {results[j]=255.0f; s.push_back(j);}
		}
		for(unsigned int j=2*width+5;j<=area+2*height+1;j+=width+2) {
			if(results[j]==-1) {results[j]=255.0f; s.push_back(j);}
		}
		for(unsigned int j=3*width+4;j<=area+width+2*height;j+=width+2) {
			if(results[j]==-1) {results[j]=255.0f; s.push_back(j);}
		}
...
void bmphandler::add_skin_outside(unsigned i4, float setto)
{
	...
	for(int j=0;j<=((int)width+2)*(height+2);j++) if(results[j]==255.0f) results[j]=-1;
...
void bmphandler::add_skintissue(tissuelayers_size_t idx, unsigned i4, tissues_size_t setto)
{
	vector<int> s;
	float *results=(float *)malloc(sizeof(float)*(area+2*width+2*height+4));
...
void bmphandler::add_skintissue_outside(tissuelayers_size_t idx, unsigned i4, tissues_size_t setto)
{
	vector<int> s;
	vector<int> s1;
	float *results=(float *)malloc(sizeof(float)*(area+2*width+2*height+4));
...
bool bmphandler::tissuevalue_at_boundary(tissuelayers_size_t idx, tissues_size_t value)
{
	// Top
	tissues_size_t *tissues = tissuelayers[idx];
...
void bmphandler::fill_skin( int thicknessX, int thicknessY, tissues_size_t backgroundID, tissues_size_t skinID )
{
	//BL recommendation
	...
	/*
	tissues_size_t *backgroundTissue = tissuelayers[backgroundID];
	tissues_size_t *skinTissue = tissuelayers[skinID];

	for( int j=skinThick; j+skinThick<dims[1]; j++ ) {
		for( int i=skinThick; i+skinThick<dims[0]; i++ ) {
			int pos = i + j*dims[0];
			if( backgroundTissue[pos] == backgroundID ) {
				//iterate through neighbors of pixel
				//if any of these are not
				for(int l=0; l<offsets.size(); l++) {
					int idx = pos + offsets[l];
					assert( idx >= 0 && idx < area );
					if( skinTissue[idx] != backgroundID && 
						skinTissue[idx] != skinID )
					{
						skinTissue[pos] = skinID;
						break;
					}
				}
			}
	...
	*/
	...
	for( int j=skinThick; j+skinThick<dims[1]; j++ ) {
		for( int i=skinThick; i+skinThick<dims[0]; i++ ) {
			int pos = i + j*dims[0];
			tissues_size_t value = tissuelayers[pos];
			if( value == backgroundID ) {
				//iterate through neighbors of pixel
				//if any of these are not
				for(int l=0; l<offsets.size(); l++) {
					tissuelayers_size_t idx = pos + offsets[l];
					assert( idx >= 0 && idx < area );
					if( tissuelayers[idx] != backgroundID && 
						tissuelayers[idx] != skinID )
					{
						tissuelayers[pos] = skinID;
						break;
					}
				}
			}
	...
	*/
	...
	for(int h=0;h<height;h++) {
		for(int w=0;w<width;w++) {
			...
			if(value==tissue) {
				int posAux;
				for( int xIt=(-1)*thicknessX; abs(xIt)<=thicknessX; xIt++ ) {
					for( int yIt=(-1)*thicknessY; abs(yIt)<=thicknessY; yIt++ ) {
						posAux = pos+(yIt*width)+xIt;
						if( posAux>=0 && posAux<=area )
							if( work_bits[posAux] == background )
								work_bits[posAux] = skin;
					}
				}
			}
			pos++;
...
void bmphandler::flood_exterior(float setto)
{
	...
	for(int j=0;j<height;j++) {
		for(int k=0;k<width;k++){
			if(results[i]==255.0f) work_bits[i3]=setto;
			i++;
			i3++;
		}
...
void bmphandler::flood_exteriortissue(tissuelayers_size_t idx, tissues_size_t setto)
{
	unsigned char dummymode1=mode1;
	unsigned char dummymode2=mode2;
	vector<int> s;
	...
	for(int j=0;j<height;j++) {
		for(int k=0;k<width;k++){
			if(results[i]==255.0f) tissues[i3]=setto;
			i++;
			i3++;
		}
...
void bmphandler::fill_unassigned(float setto)
{
	...
	for(unsigned short py=0;py<height;py++){
		for(unsigned short px=0;px<width;px++){
			if(results[pos]!=255.0f&&work_bits[pos1]==0) work_bits[pos1]=setto;
			pos++;
			pos1++;
		}
		pos+=2;
...
void bmphandler::fill_unassignedtissue(tissuelayers_size_t idx, tissues_size_t setto)
{
	vector<int> s;
	float *results=(float *)malloc(sizeof(float)*(area+2*width+2*height+4));
	...
	for(unsigned short py=0;py<height;py++){
		for(unsigned short px=0;px<width;px++){
			if(results[pos]!=255.0f&&tissues[pos1]==0) tissues[pos1]=setto;
			pos++;
			pos1++;
		}
		pos+=2;
...
void bmphandler::classify(short nrclasses, short dim, float **bits, float *weights, float *centers, float maxdist)
{
	...
	for(unsigned i=0;i<area;i++){
		...
		for(short m=0;m<dim;m++){
			distmin+=(bits[m][i]-centers[cindex])*(bits[m][i]-centers[cindex])*weights[m];
			cindex++;
		}
		for(short l=1;l<nrclasses;l++){
			dist=0;
			for(short m=0;m<dim;m++){
				dist+=(bits[m][i]-centers[cindex])*(bits[m][i]-centers[cindex])*weights[m];
				cindex++;
			}
			if(dist<distmin){distmin=dist; k=l;}
		}
		if(distmin<maxdist) work_bits[i]=255.0f*(k+1)/nrclasses;
		else work_bits[i]=0;
	}
...
//	float *centers= new float[nrclasses];
	...
	for(short i=0;i<nrclasses;i++) centers[i]=dummy[i];
	std::sort(centers, centers+nrclasses);
	for(short i=nrclasses-1;i>0;i--) centers[i]=(centers[i]+centers[i-1])/2;
...
{
	float w=0;
	for(short i=0;i<dim;i++) w+=weights[i];
	float *weightsnew=(float *)malloc(sizeof(float)*dim);
	if(w==0) for(short i=0;i<dim;i++) weightsnew[i]=1.0f/dim;
	else for(short i=0;i<dim;i++) weightsnew[i]=weights[i]/w;
	k_means kmeans;
...
void bmphandler::kmeans_mhd(short nrtissues,short dim,std::vector<std::string> mhdfiles,unsigned short slicenr,float *weights,unsigned int iternr,unsigned int converge)
{
	...
	for(unsigned short i=0;i+1<dim;i++) {
		if(!ImageReader::getSlice(mhdfiles[i].c_str(),bits[i+1],slicenr,width,height)) {
			for(unsigned short j=1;j<dim;j++) sliceprovide->take_back(bits[j]);
			delete[] bits;
			return;
		}
	}
...
void bmphandler::kmeans_png(short nrtissues,short dim,std::vector<std::string> pngfiles,std::vector<int> exctractChannel,unsigned short slicenr,float *weights,unsigned int iternr,unsigned int converge,const std::string initCentersFile)
{
	float **bits=new float*[dim];
	for(unsigned short j=0;j+1<dim;j++) {
		bits[j+1]=sliceprovide->give_me();
	...
	for(unsigned short i=0;i+1<dim;i++)
	{
		if(!ChannelExtractor::getSlice(pngfiles[0].c_str(),bits[i+1],exctractChannel[i],slicenr,width,height)) {
	...
	if(initCentersFile!="")
	{
		...
		int dimensions;
		int nrClasses;
		if (kmeans.get_centers_from_file(initCentersFile, centers, dimensions, nrClasses))
		{
			dim = dimensions;
			nrtissues = nrClasses;
...
void bmphandler::gamma_mhd(short nrtissues,short dim,std::vector<std::string> mhdfiles,unsigned short slicenr,float *weights,float **centers, float *tol_f, float *tol_d, Pair pixelsize)
{
	float **bits=new float*[dim];
	for(unsigned short j=0;j+1<dim;j++) {
		bits[j+1]=sliceprovide->give_me();
	...
	multidimgamma mdg;
	mdg.init(width,height,nrtissues,dim,bits,weights,centers,tol_f,tol_d,pixelsize.high,pixelsize.low);
	mdg.execute();
...
void bmphandler::levelsettest(float sigma, float epsilon, float alpha, float beta, float stepsize, unsigned nrsteps, unsigned reinitfreq)
{
	unsigned char dummymode=mode1;
	float *tmp=sliceprovide->give_me();
	float *dummy;
...
//	sobel_finer();
	...
	for(unsigned i=0;i<area;++i) bmp_bits[i]=1/(1+alpha*work_bits[i]);
//	SaveDIBitmap("D:\\Development\\segmentation\\sample images\\testt1.bmp");
//	for(unsigned i=0;i<area;++i) bmp_bits[i]=1;
//	cout << "muss raus"<<endl;
	for(unsigned i=0;i<area;++i) work_bits[i]=-beta*work_bits[i];
...
void bmphandler::levelsettest1(float sigma, float epsilon, float alpha, float beta, float stepsize, unsigned nrsteps, unsigned reinitfreq)
{
	unsigned char dummymode=mode1;
	float *tmp=sliceprovide->give_me();
	float *dummy;
...
/*	Pt.px=215;
	Pt.py=266;*/
	levset.init(height,width,dummy1,256.0f,bmp_bits,work_bits,1.0f,epsilon,stepsize);
	levset.iterate(nrsteps, reinitfreq);
	levset.return_levelset(bmp_bits);
	float thresh[2];
	thresh[0]=1;
...
//	for(unsigned i=0;i<area;i++) work_bits[i]=0.0f;
	for(unsigned i=0;i<area;i++) tmp[i]=1.0f;
	
	levelset levset;
	levset.init(height,width,initlev,f,tmp,work_bits,0.0f,epsilon,stepsize);
	levset.iterate(nrsteps, reinitfreq);
...
void bmphandler::cannylevelsettest(float sigma, float thresh_low, float thresh_high, float epsilon, float stepsize, unsigned nrsteps, unsigned reinitfreq)
{
	unsigned char dummymode=mode1;
...
void bmphandler::threshlevelset(float thresh_low, float thresh_high, float epsilon, float stepsize, unsigned nrsteps, unsigned reinitfreq)
{
	...
	for(unsigned i=0;i<area;++i) work_bits[i]=1-abs(bmp_bits[i]-mean)/halfdiff;
...
/*	Pt.px=215;
	Pt.py=266;*/
	float *tmp=sliceprovide->give_me();
	for(unsigned i=0;i<area;i++) tmp[i]=0;
...
unsigned bmphandler::loadstack(const char *filename)
{
	...
	if(fp==NULL) return 123456;

	short unsigned width1,height1;
	if (fread(&width1, 1, sizeof(short unsigned), fp) < sizeof(short unsigned) || width1!=width) {
		fclose(fp);
		return 123456;
	}		
	if (fread(&height1, 1, sizeof(short unsigned), fp) < sizeof(short unsigned) || height1!=height) {
		fclose(fp);
		return 123456;
	}		
	unsigned int bitsize=width*(unsigned)height*sizeof(float);
...
unsigned bmphandler::pushstack_tissue(tissuelayers_size_t idx, tissues_size_t tissuenr)
{
	float *bits=sliceprovide->give_me();
...
void bmphandler::getstack_tissue(tissuelayers_size_t idx, unsigned i,tissues_size_t tissuenr,bool override)
{
	//	sliceprovide->take_back(work_bits);
...
{
	tissues_size_t *tissues = tissuelayers[idx];
	for(unsigned int i=0;i<area;i++) tissues[i]=0;
}
...
void bmphandler::add2tissue(tissuelayers_size_t idx, tissues_size_t tissuetype, float f,bool override)
{
	tissues_size_t *tissues = tissuelayers[idx];
	if(override){
		for(unsigned int i=0;i<area;i++) if(work_bits[i]==f&&TissueInfos::GetTissueLocked(tissues[i])==false) {tissues[i]=tissuetype;}
	} else {
		for(unsigned int i=0;i<area;i++) if(work_bits[i]==f&&tissues[i]==0) {tissues[i]=tissuetype;}
...
void bmphandler::add2tissue(tissuelayers_size_t idx, tissues_size_t tissuetype, bool *mask,bool override)
{
	tissues_size_t *tissues = tissuelayers[idx];
	if(override){
		for(unsigned int i=0;i<area;i++) if(mask[i]&&TissueInfos::GetTissueLocked(tissues[i])==false) {tissues[i]=tissuetype;}
	} else {
		for(unsigned int i=0;i<area;i++) if(mask[i]&&tissues[i]==0) {tissues[i]=tissuetype;}
		//for(unsigned int i=0;i<area;i++) if(work_bits[i]==f&&tissuelocked[tissues[i]]==false) {tissues[i]=tissuetype;}
	}
...
void bmphandler::add2tissue_connected(tissuelayers_size_t idx, tissues_size_t tissuetype, Point p, bool override)
{
	...
	for(int j=0;j<height;j++) {
		for(int k=0;k<width;k++){
			...
			if(work_bits[i1]==f&&(tissues[i1]==0||(override&&TissueInfos::GetTissueLocked(tissues[i1])==false))) results[i]=-1;
			else results[i]=0;
			i++;
			i1++;
		}
	...
	s.push_back(position%width+1+(position/width+1)*(width+2));
	if(results[s.back()]==-1) results[s.back()]=255.0f;
	...
	while(!s.empty()){
		i=s.back();
		s.pop_back();
		if(results[i-1]==-1) {results[i-1]=255.0f; s.push_back(i-1);}
		if(results[i+1]==-1) {results[i+1]=255.0f; s.push_back(i+1);}
		if(results[i-w]==-1) {results[i-w]=255.0f; s.push_back(i-w);}
		if(results[i+w]==-1) {results[i+w]=255.0f; s.push_back(i+w);}
	}
	...
	for(int j=0;j<height;j++) {
		for(int k=0;k<width;k++){
			if(results[i]==255.0f) tissues[i2]=tissuetype;
...
void bmphandler::add2tissue_thresh(tissuelayers_size_t idx, tissues_size_t tissuetype, Point p)
{
	float f=work_pt(p);
	tissues_size_t *tissues = tissuelayers[idx];
	for(unsigned int i=0;i<area;i++) if(work_bits[i]>=f) tissues[i]=tissuetype;
	
	return;
}

void bmphandler::subtract_tissue(tissuelayers_size_t idx, tissues_size_t tissuetype, Point p)
{
	float f=work_pt(p);
...
void bmphandler::subtract_tissue_connected(tissuelayers_size_t idx, tissues_size_t tissuetype, Point p)
{
	unsigned position=pt2coord(p);
	vector<int> s;
	...
	for(int j=0;j<height;j++) {
		for(int k=0;k<width;k++){
			...
			if(work_bits[i1]==f&&tissues[i1]==tissuetype) results[i]=-1;
			else results[i]=0;
			i++;
			i1++;
		}
	...
	for(int j=0;j<height;j++) {
		for(int k=0;k<width;k++){
			if(results[i]==255.0f) tissues[i2]=0;
...
{
	tissues_size_t *tissues = tissuelayers[idx];
	for(unsigned int i=0;i<area;i++) if(work_bits[i]==f&&tissues[i]==tissuetype) tissues[i]=0;
...
void bmphandler::change2mask_connectedwork(bool *mask, Point p,bool addorsub)
{
	...
	for(int j=0;j<height;j++) {
		for(int k=0;k<width;k++){
			if(results[i]==255.0f) mask[i2]=addorsub;
...
void bmphandler::tissue2work(tissuelayers_size_t idx, tissues_size_t tissuetype)
{
	tissues_size_t *tissues = tissuelayers[idx];
	for(unsigned int i=0;i<area;i++){
		if(tissues[i]==tissuetype) work_bits[i]=255.0f;
		else work_bits[i]=0.0f;
	}
...
void bmphandler::setissue2work(tissuelayers_size_t idx, tissues_size_t tissuetype)
{
	tissues_size_t *tissues = tissuelayers[idx];
	for(unsigned int i=0;i<area;i++){
		if(tissues[i]==tissuetype) work_bits[i]=255.0f;
...
void bmphandler::tissue2mc(tissuelayers_size_t idx, tissues_size_t tissuetype,unsigned char ** voxels,int k)
{
	tissues_size_t *tissues = tissuelayers[idx];
	for(unsigned int i=0;i<area;i++){
		if(tissues[i]==tissuetype) voxels[0][i+k*area]=tissuetype;
	}
...
void bmphandler::cap_tissue(tissues_size_t maxval)
{
	for (tissuelayers_size_t idx = 0; idx < tissuelayers.size(); ++idx) {
		tissues_size_t *tissues = tissuelayers[idx];
		for(unsigned int i=0;i<area;i++){
				if(tissues[i]>maxval) tissues[i]=0;
		}
	}
}
...
void bmphandler::floodwork(bool *mask)
{
	...
	for(int j=0;j<width+2;j++) bigmask[j]=bigmask[j+((unsigned)width+2)*(height+1)]=false;
	for(int j=0;j<=((int)width+2)*(height+1);j+=width+2) bigmask[j]=bigmask[j+width+1]=false;
	...
	while(!s.empty()){
		i=s.front();
		s.pop();
		if(bigmask[i-1]) {bigmask[i-1]=false; values[i-1]=values[i]; s.push(i-1);}
		if(bigmask[i+1]) {bigmask[i+1]=false; values[i+1]=values[i]; s.push(i+1);}
		if(bigmask[i-w]) {bigmask[i-w]=false; values[i-w]=values[i]; s.push(i-w);}
		if(bigmask[i+w]) {bigmask[i+w]=false; values[i+w]=values[i]; s.push(i+w);}
	}
	...
	for(int j=0;j<height;j++) {
		for(int k=0;k<width;k++){
			if(mask[i2]) work_bits[i2]=values[i];
			i++;
			i2++;
		}
		i+=2;
...
void bmphandler::floodtissue(tissuelayers_size_t idx, bool *mask)
{
	...
	for(int j=0;j<height;j++) {
		for(int k=0;k<width;k++){
			if(mask[i2]) tissues[i2]=values[i];
			i++;
			i2++;
		}
		i+=2;
...
void bmphandler::correct_outline(float f,vector<Point> *newline)
{
	...
	vector<vector<Point> > vvPouter,vvPinner;
	vector<Point> limit1,limit2;
	vector<Point>::iterator it,itold,startP,endP;
	vector<vector<Point> >::iterator vvPit;
...
//			in=(work_bits[unsigned(width)*it->py+it->px]!=f);
			...
			bool dontdraw=false;
			if(order){
				for(vector<Point>::iterator it1=oldline1.begin();it1!=oldline1.end();it1++){
...
//						fprintf(fp3,"a%i %i\n",(int)it1->px,(int)it1->py);
					...
					if(!in1&&((it1->px==p2.px&&it1->py==p2.py)||(it1->px==p3.px&&it1->py==p3.py))){
						it1++;
...
//						fprintf(fp3,"c%i %i\n",(int)it1->px,(int)it1->py);
					...
					if(dontdraw) dontdraw=false;
					else if(in1) bkp[unsigned(width)*it1->py+it1->px]=f;
				}
			} else {
				for(vector<Point>::reverse_iterator it1=oldline1.rbegin();it1!=oldline1.rend();it1++){
					if(it1->px==p1.px&&it1->py==p1.py){
//						fprintf(fp3,"d%i %i\n",(int)it1->px,(int)it1->py);
						in1=!in1;
						p1=*it;
...
void bmphandler::correct_outlinetissue(tissuelayers_size_t idx, tissues_size_t f1,vector<Point> *newline)
{
	unsigned char dummymode1=mode1;
	unsigned char dummymode2=mode2;
	float f=float(f1);
	...
	pushstack_work();
	tissues_size_t *tissues = tissuelayers[idx];
	for(unsigned ineu=0;ineu<area;ineu++) work_bits[ineu]=(float)tissues[ineu];
...
//			fclose(fp3);
	...
	for(unsigned ineu=0;ineu<area;ineu++) tissues[ineu]=(tissues_size_t)(work_bits[ineu]+0.1f);
...
void bmphandler::_brush(T* data, T f, Point p, int radius, bool draw, T f1, F is_locked)
{
	unsigned short dist = radius*radius;

	int xmin, xmax, ymin, ymax, d;
	if (p.px < radius) xmin = 0;
	else xmin = int(p.px - radius);
	if (p.px + radius >= width) xmax = int(width - 1);
	else xmax = int(p.px + radius);

	for (int x = xmin; x <= xmax; x++)
	{
		d = int(floor(sqrt(float(dist - (x - p.px)*(x - p.px)))));
		ymin = max(0, int(p.py) - d);
		ymax = min(int(height - 1), d + p.py);
...
template<typename T, typename F>
void bmphandler::_brush(T* data, T f, Point p, float const radius, float dx, float dy, bool draw, T f1, F is_locked)
{
	float const radius_corrected = dx > dy
		? std::ceil(radius / dx + 0.5f)*dx
...
{
	_brush(work_bits, f, p, radius, dx, dy, draw, 0.f, [](float v) { return false; });
}
...
//				vec_pt.pop_back();
	...
	for(vector<vector<Point> >::iterator it1=inner_line.begin();it1!=inner_line.end();it1++){
		for(vector<Point>::iterator it=it1->begin();it!=it1->end();it++){
			tmp_bits[it->px+1+(it->py+1)*(width+2)]=1;
		}
	}

	for(int j=0;j<width+2;j++) tmp_bits[j]=tmp_bits[j+((unsigned)width+2)*(height+1)]=0;
	for(int j=0;j<=((int)width+2)*(height+1);j+=width+2) tmp_bits[j]=tmp_bits[j+width+1]=0;

	for(int j=width+3;j<2*width+3;j++) {
		if(tmp_bits[j]==-1) {tmp_bits[j]=255.0f; s.push_back(j);}
	}
	for(unsigned int j=area+2*height+1;j<area+width+2*height+1;j++) {
		if(tmp_bits[j]==-1) {tmp_bits[j]=255.0f; s.push_back(j);}
	}
...
void bmphandler::fill_holestissue(tissuelayers_size_t idx, tissues_size_t f, int minsize)
{
	vector<vector<Point> > inner_line;
	minsize=2*minsize;
	float bubble_size;
	...
	if(f==TISSUES_SIZE_MAX) unvis=0;
	for(unsigned i=0;i<unsigned(width+2);i++) tmp_bits[i]=unvis;
	for(unsigned i=unsigned(width+2)*(height+1);i<unsigned(width+2)*(height+2);i++) tmp_bits[i]=unvis;
	for(unsigned i=0;i<unsigned(width+2)*(height+2);i+=(width+2)) tmp_bits[i]=unvis;
	for(unsigned i=width+1;i<unsigned(width+2)*(height+2);i+=(width+2)) tmp_bits[i]=unvis;
...
//				vec_pt.pop_back();
	...
	for(int j=width+3;j<2*width+3;j++) {
		if(tmp_bits[j]==2) {tmp_bits[j]=TISSUES_SIZE_MAX; s.push_back(j);}
	}
	for(unsigned int j=area+2*height+1;j<area+width+2*height+1;j++) {
		if(tmp_bits[j]==2) {tmp_bits[j]=TISSUES_SIZE_MAX; s.push_back(j);}
	}
	...
	while(!s.empty()){
		i4=s.back();
		s.pop_back();
		if(tmp_bits[i4-1]==2) {tmp_bits[i4-1]=TISSUES_SIZE_MAX; s.push_back(i4-1);}
		if(tmp_bits[i4+1]==2) {tmp_bits[i4+1]=TISSUES_SIZE_MAX; s.push_back(i4+1);}
		if(tmp_bits[i4-width-2]==2) {tmp_bits[i4-width-2]=TISSUES_SIZE_MAX; s.push_back(i4-width-2);}
		if(tmp_bits[i4+width+2]==2) {tmp_bits[i4+width+2]=TISSUES_SIZE_MAX; s.push_back(i4+width+2);}
	}
...
//				vec_pt.pop_back();
				...
				if(inner==1) {
					if(bubble_size+linelength<(float)minsize) outer_line.push_back(vec_pt);
				}
	...
	for(vector<vector<Point> >::iterator it1=outer_line.begin();it1!=outer_line.end();it1++){
		for(vector<Point>::iterator it=it1->begin();it!=it1->end();it++){
			tmp_bits[it->px+1+(it->py+1)*(width+2)]=1;
		}
	}
...
void bmphandler::remove_islandstissue(tissuelayers_size_t idx, tissues_size_t f, int minsize)
{
	vector<vector<Point> > outer_line;
	minsize=2*minsize;
	float bubble_size;
...
bool bmphandler::del_vm(Point p, short radius)
{
	...
	while(!found&&vit!=vvm.end()){
		it=vit->begin();
		while(it!=vit->end()&&(it->p.px-p.px)*(it->p.px-p.px)+(it->p.py-p.py)*(it->p.py-p.py)>radius2) it++;
		if(it!=vit->end()&&(it->p.px-p.px)*(it->p.px-p.px)+(it->p.py-p.py)*(it->p.py-p.py)<=radius2) found=true;
		else vit++;
	}
		...
		if(vvm.size()==1){
			...
			if(maxim_store==maxim1&&!vvm.empty()){
				...
				for(vit=vvm.begin();vit!=vvm.end();vit++)
					maxim_store=max(maxim_store,vit->begin()->mark);
			}
		}
		return true;
	} else return false;
...
vector<vector<mark> > *bmphandler::return_vvm()
{
	return &vvm;
}

unsigned bmphandler::return_vvmmaxim()
{
	return maxim_store;
}
...
bool bmphandler::del_limit(Point p, short radius)
{
	...
	while(!found&&vit!=limits.end()){
		it=vit->begin();
		while(it!=vit->end()&&(it->px-p.px)*(it->px-p.px)+(it->py-p.py)*(it->py-p.py)>radius2) it++;
		if(it!=vit->end()&&(it->px-p.px)*(it->px-p.px)+(it->py-p.py)*(it->py-p.py)<=radius2) found=true;
		else vit++;
	}
	...
	if(found){
		if(limits.size()==1){
			...
			vector<vector<Point> >::iterator vit1;
			vit1=limits.begin();
...
vector<vector<Point> > *bmphandler::return_limits()
{
	return &limits;
}
...
void bmphandler::remove_tissue(tissues_size_t tissuenr,tissues_size_t tissuecount1) //assumes tissue[tissuecount] has not been erased yet
{
	for (tissuelayers_size_t idx = 0; idx < tissuelayers.size(); ++idx) {
		tissues_size_t *tissues = tissuelayers[idx];
		for(unsigned int i = 0; i < area; ++i) {
...
void bmphandler::group_tissues(tissuelayers_size_t idx, vector<tissues_size_t> &olds, vector<tissues_size_t> &news)
{
	tissues_size_t crossref[TISSUES_SIZE_MAX+1];
	for(int i=0;i<TISSUES_SIZE_MAX+1;i++) crossref[i]=(tissues_size_t)i;
	unsigned count=min(olds.size(),news.size());

	for(unsigned int i=0;i<count;i++) crossref[olds[i]]=news[i];
...
/*void bmphandler::locktissue(tissues_size_t nr)
{
	lockedtissues[nr]=true;
}

void bmphandler::unlocktissue(tissues_size_t nr)
{
	lockedtissues[nr]=false;
}

bool bmphandler::islockedtissue(tissues_size_t nr)
{
	return lockedtissues[nr];
}

void bmphandler::set_tissuelock(tissues_size_t nr, bool setval)
{
	lockedtissues[nr]=setval;
}*/

unsigned char bmphandler::return_mode(bool bmporwork)
{
	if(bmporwork) return mode1;
	else return mode2;
}

void bmphandler::set_mode(unsigned char mode, bool bmporwork)
{
	if(bmporwork) mode1=mode;
	else mode2=mode;
}

bool bmphandler::print_amascii_slice(tissuelayers_size_t idx, std::ofstream &streamname)
{
	tissues_size_t *tissues = tissuelayers[idx];
	for(unsigned i=0;i<area;i++) {
...
/*	unsigned count=0;
	...
	if(count==area) return true;
	else return false;*/
}

bool bmphandler::print_vtkascii_slice(tissuelayers_size_t idx, std::ofstream &streamname)
{
	tissues_size_t *tissues = tissuelayers[idx];
	for(unsigned i=0;i<area;i++){
...
bool bmphandler::print_vtkbinary_slice(tissuelayers_size_t idx, std::ofstream &streamname)
{
	tissues_size_t *tissues = tissuelayers[idx];
...
/*void bmphandler::adaptwork2bmp(float f)
{
	...
	if(!outerline.empty()) {
		...
		for(size_t i=0;i<outerline.size();i++) {
			std::vector<unsigned> newline;
			if(outerline[i].size()<=n) {
				newline.resize(outerline[i].size());
				for(size_t j=0;j<outerline[i].size();j++){
					newline[j]=outerline[i][j].px+unsigned(width)*outerline[i][j].py;
				}
				outerlinenew.push_back(newline);
			} else {
				std::vector<unsigned> pts(2);
				for(int ni=0;ni<n;ni+=2) {
					unsigned p1=outerline[i][(ni*outerline[i].size())/n].px+unsigned(width)*outerline[i][(ni*outerline[i].size())/n].py;
					if(ni==0)
						pts[0]=outerline[i][((n-1)*outerline[i].size())/n].px+outerline[i][((n-1)*outerline[i].size())/n].py*width;
					else
						pts[0]=outerline[i][((ni-1)*outerline[i].size())/n].px+outerline[i][((ni-1)*outerline[i].size())/n].py*width;
					pts[1]=outerline[i][((ni+1)*outerline[i].size())/n].px+outerline[i][((ni+1)*outerline[i].size())/n].py*width;
					lw->change_pt(p1,pts);
					lw->append_path(pts[0],&newline);
					lw->append_path(pts[1],&newline);
				}
				outerlinenew.push_back(newline);
			}
		}
	...
	if(!innerline.empty()) {
		if(lw==NULL) lw=livewireinit(innerline[0][0]);
	}
...
void bmphandler::adaptwork2bmp(float f)
{
		...
		for(size_t i=0;i<outerline.size();i++) {
			...
			if((int)outerline[i].size()<=n) {
				...
				for(int ni=0;ni<n;ni+=2) {
					...
					if(newline1.size()>2)
						for(size_t j=newline1.size()-2;j>0;j--) newline.push_back(newline1[j]);
				}
				outerlinenew.push_back(newline);
			}
			...
			if(newline.size()>2) {
				...
				for(size_t j=0;j<newline.size();j++) newlinex[j]=newline[j]%width;
				bool forward=true;
				...
				if(newlinex[newline.size()-1]!=newlinex[0]) {
					...
					if((newlinex[0]>newlinex[newline.size()-1])!=forwardnew) {
						forwardnew=!forwardnew;
						counterarray[newline[newline.size()-1]]=!counterarray[newline[newline.size()-1]];
					}
				}
			}
		}
		...
		for(size_t i=0;i<innerline.size();i++) {
			...
			if((int)innerline[i].size()<=n) {
				...
				for(size_t j=0;j<innerline[i].size();j++){
					newline[j]=innerline[i][j].px+unsigned(width)*innerline[i][j].py;
				}
				innerlinenew.push_back(newline);
			} else {
				std::vector<unsigned> pts(2);
				for(int ni=0;ni<n;ni+=2) {
					unsigned p1=innerline[i][(ni*innerline[i].size())/n].px+unsigned(width)*innerline[i][(ni*innerline[i].size())/n].py;
					if(ni==0)
						pts[0]=innerline[i][((n-1)*innerline[i].size())/n].px+innerline[i][((n-1)*innerline[i].size())/n].py*width;
					else
						pts[0]=innerline[i][((ni-1)*innerline[i].size())/n].px+innerline[i][((ni-1)*innerline[i].size())/n].py*width;
					pts[1]=innerline[i][((ni+1)*innerline[i].size())/n].px+innerline[i][((ni+1)*innerline[i].size())/n].py*width;
					lw->change_pt(p1,pts);
					lw->append_path(pts[0],&newline);
					lw->return_path(pts[1],&newline1);
					if(newline1.size()>2)
	...
	for(unsigned short i=0;i<width;i++) {
		...
		for(unsigned short j=0;j<width;j++) {
			if(counterarray[pos]) draw=!draw;
			if(draw) work_bits[pos]=255;
			pos+=width;
		}
	}
...
unsigned long bmphandler::return_tissuepixelcount(tissuelayers_size_t idx, tissues_size_t c)
{
	unsigned long pos=0;
	unsigned long counter=0;
	tissues_size_t *tissues = tissuelayers[idx];
...
bool bmphandler::get_extent(tissuelayers_size_t idx, tissues_size_t tissuenr,unsigned short extent[2][2])
{
	...
	while(!found&&pos<area) {
		if(tissues[pos]==tissuenr) found=true;
		pos++;
	}
	...
	while(pos<(unsigned long)(extent[1][0]+1)*width) {
		if(tissues[pos]==tissuenr) extent[0][1]=pos%width;
		pos++;
	}
	found=false;
	pos=area;
...
void bmphandler::swap(bmphandler &bmph)
{
		...
		list<unsigned char> mode_stackd;
...
bool bmphandler::unwrap(float jumpratio, float range, float shift)
{
	...
	for(unsigned short i=0;i<height;i++) {
		if(i!=0) {
			if(bmp_bits[pos-width]-bmp_bits[pos]>jumpabs) shiftbegin+=range;
			else if(bmp_bits[pos]-bmp_bits[pos-width]>jumpabs) shiftbegin-=range;
		}
		shift=shiftbegin;
		work_bits[pos]=bmp_bits[pos]+shift;
		for(unsigned short j=1;j<width;j++) {
			if(bmp_bits[pos]-bmp_bits[pos+1]>jumpabs) shift+=range;
			else if(bmp_bits[pos+1]-bmp_bits[pos]>jumpabs) shift-=range;
			pos++;
			work_bits[pos]=bmp_bits[pos]+shift;
		}
		pos++;
	...
	for(pos=width-1;pos+width<area;pos+=width) {
		if(work_bits[pos+width]-work_bits[pos]>jumpabs) return false;
		if(work_bits[pos]-work_bits[pos+width]>jumpabs) return false;
	}
	return true;
}
