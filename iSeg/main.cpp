...
int main(int argc, char **argv)
{
	...
	if (!Tools::interceptOutput(tmpdir.absolutePath().toStdString() + "/iSeg.log"))
	{
		Tools::error("intercepting output failed");
	}

	QDir fileDirectory = fileinfo.dir();
	cerr << "fileDirectory = " << fileDirectory.absolutePath().toAscii().data() << endl;
	...
	QString settingspath = tmpdir.absFilePath(QString("settings.bin"));
		...
		if (argc > 1)
		{
			...
			if (qstr.compare(QString("S4Llink")) == 0)
				mainWindow = new MainWindow(&h3Ds2, QString(locationpath.toAscii().data()), picpath, tmpdir, true, 0, "new window", Qt::WDestructiveClose | Qt::WResizeNoErase, argv);
		}
		if (mainWindow == NULL)
			mainWindow = new MainWindow(&h3Ds2, QString(locationpath.toAscii().data()), picpath, tmpdir, false, 0, "new window", Qt::WDestructiveClose | Qt::WResizeNoErase, argv);
		...
		{
			QString qstr(argv[1]);
			if (qstr.compare(QString("S4Llink")) != 0)	mainWindow->loadproj(qstr);
			else {
				s4llink = true;
				if (argc > 2) mainWindow->loadS4Llink(QString(argv[2]));
			}
	...
	MainWindow *tbw2 =
		new MainWindow(&h3Ds1, locationpath, picpath, tmpdir, false, 0, "new window", Qt::WDestructiveClose | Qt::WResizeNoErase);
	tbw2->show();
