...
measure_widget::measure_widget(SlicesHandler *hand3D, QWidget *parent, const char *name, Qt::WindowFlags wFlags)
...
// 	hboxoverall->setFixedSize(hboxoverall->sizeHint());//	
	...
	QObject::connect(inputgroup,SIGNAL(buttonClicked(int)),this,SLOT(inputtype_changed(int)));
	QObject::connect(cbb_lb1,SIGNAL(activated(int)),this,SLOT(cbb_changed(int)));
	QObject::connect(cbb_lb2,SIGNAL(activated(int)),this,SLOT(cbb_changed(int)));
	QObject::connect(cbb_lb3,SIGNAL(activated(int)),this,SLOT(cbb_changed(int)));
	QObject::connect(cbb_lb4,SIGNAL(activated(int)),this,SLOT(cbb_changed(int)));
}
...
QSize measure_widget::sizeHint() const
{
...
FILE *measure_widget::LoadParams(FILE *fp, int version)
{
	if(version>=3) {
		...
		QObject::disconnect(inputgroup,SIGNAL(buttonClicked(int)),this,SLOT(inputtype_changed(int)));
	...
	if(rb_pts->isOn()){
		if(rb_vector->isOn()){ 
			if(state==0) {
				...
				dynamic.clear();
				emit vp1dyn_changed(&established,&dynamic);
				txt_displayer->setText(QString("(")+QString::number(calculatevec(0),'g',3)+QString(",")
					+QString::number(calculatevec(1),'g',3)+QString(",")+
					QString::number(calculatevec(2),'g',3)+QString(") mm (Mark new start point.)"));
			}
			...
			if(state==0) {
				...
				dynamic.clear();
				emit vp1dyn_changed(&established,&dynamic, true);
				txt_displayer->setText(QString::number(calculate(),'g',3)+QString(" mm (Mark new start point.)"));
			}
			...
			if(state==0) {
				...
				dynamic.clear();
				emit vp1dyn_changed(&established,&dynamic);
				txt_displayer->setText(QString::number(calculate(),'g',3)+QString(" deg (Mark new first point.)"));
			}
			...
			emit vp1dyn_changed(&established,&dynamic);
			QString tissuename1;
			tissues_size_t tnr=handler3D->get_tissue_pt(p,handler3D->get_activeslice());
			if(tnr==0)
				tissuename1=QString("-");
			else
				tissuename1=TissueInfos::GetTissueName(tnr);
			QString note = QString("");
			if (!(handler3D->return_startslice() == 0 && handler3D->return_endslice() == handler3D->return_nrslices())) {
				note = QString("\nCalculated for active slices only.");
			}
			txt_displayer->setText(QString("Tissue '")+tissuename1+QString("': ")+QString::number(handler3D->calculate_tissuevolume(p,handler3D->get_activeslice()),'g',3)+QString(" mm^3\n")+
				QString("'Target': ")+QString::number(handler3D->calculate_volume(p,handler3D->get_activeslice()),'g',3)+QString(" mm^3")+note+QString("\n(Select new object.)"));
		}
...
void measure_widget::set_coord(unsigned short posit,Point p,unsigned short slicenr)
{
	pt[posit][0]=(int)p.px;
	pt[posit][1]=(int)p.py;
...
void measure_widget::cbb_changed(int)
{
	if(rb_lbls->isOn()&&!rb_vol->isOn()){
		...
		if(rb_dist->isOn()){
			...
			Point p1,p2;
			if(cbb_lb1->currentItem()==0) p1=pc;
			else p1=labels[cbb_lb1->currentItem()-1].p;
			if(cbb_lb2->currentItem()==0) p2=pc;
			else p2=labels[cbb_lb2->currentItem()-1].p;
			...
			if(cbb_lb3->currentItem()==0) p3=pc;
			else p3=labels[cbb_lb3->currentItem()-1].p;
			addLine(&established,p1,p2);
			...
			if(cbb_lb4->currentItem()==0) p4=pc;
			else p4=labels[cbb_lb4->currentItem()-1].p;
			addLine(&established,p1,p2);
			addLine(&established,p2,p3);
...
void measure_widget::method_changed(int)
{
	update_visualization();
}

void measure_widget::inputtype_changed(int)
{
	update_visualization();
}
...
void measure_widget::getlabels()
{
	...
	QObject::disconnect(cbb_lb1,SIGNAL(activated(int)),this,SLOT(cbb_changed(int)));
	QObject::disconnect(cbb_lb2,SIGNAL(activated(int)),this,SLOT(cbb_changed(int)));
	QObject::disconnect(cbb_lb3,SIGNAL(activated(int)),this,SLOT(cbb_changed(int)));
	QObject::disconnect(cbb_lb4,SIGNAL(activated(int)),this,SLOT(cbb_changed(int)));
...
void measure_widget::marks_changed()
{
	getlabels();
}
...
float measure_widget::calculate()
{
	...
	if(rb_dist->isOn()){ 
		value=sqrt((pt[0][0]-pt[1][0])*(pt[0][0]-pt[1][0])*p1.high*p1.high+(pt[0][1]-pt[1][1])*(pt[0][1]-pt[1][1])*p1.low*p1.low+
		...
		if(zDist!=0) zDist+=1;
		if(!(xDist||yDist||zDist)) xDist=1;
		value=sqrt((xDist)*(xDist)*p1.high*p1.high
				  +(yDist)*(yDist)*p1.low*p1.low
				  +(zDist)*(zDist)*thick*thick);
	} else if(rb_angle->isOn()){
		float l1square=(pt[0][0]-pt[1][0])*(pt[0][0]-pt[1][0])*p1.high*p1.high+(pt[0][1]-pt[1][1])*(pt[0][1]-pt[1][1])*p1.low*p1.low+
			(pt[0][2]-pt[1][2])*(pt[0][2]-pt[1][2])*thick*thick;
		float l2square=(pt[2][0]-pt[1][0])*(pt[2][0]-pt[1][0])*p1.high*p1.high+(pt[2][1]-pt[1][1])*(pt[2][1]-pt[1][1])*p1.low*p1.low+
			(pt[2][2]-pt[1][2])*(pt[2][2]-pt[1][2])*thick*thick;
		if(l1square*l2square>0)
			value=acos(((pt[0][0]-pt[1][0])*(pt[2][0]-pt[1][0])*p1.high*p1.high+(pt[0][1]-pt[1][1])*(pt[2][1]-pt[1][1])*p1.low*p1.low+
				(pt[0][2]-pt[1][2])*(pt[2][2]-pt[1][2])*thick*thick)/sqrt(l1square*l2square))*180/3.141592f;
	} else if(rb_4ptangle->isOn()){ 
		float d1[3];
		float d2[3];
		d1[0]=(pt[0][1]-pt[1][1])*(pt[1][2]-pt[2][2])*thick*p1.low-
			(pt[0][2]-pt[1][2])*(pt[1][1]-pt[2][1])*thick*p1.low;
		d1[1]=(pt[0][2]-pt[1][2])*(pt[1][0]-pt[2][0])*thick*p1.high-
			(pt[0][0]-pt[1][0])*(pt[1][2]-pt[2][2])*thick*p1.high;
		d1[2]=(pt[0][0]-pt[1][0])*(pt[1][1]-pt[2][1])*p1.high*p1.low-
		...
		d2[1]=(pt[1][2]-pt[2][2])*(pt[2][0]-pt[3][0])*thick*p1.high-
			(pt[1][0]-pt[2][0])*(pt[2][2]-pt[3][2])*thick*p1.high;
		d2[2]=(pt[1][0]-pt[2][0])*(pt[2][1]-pt[3][1])*p1.high*p1.low-
		...
		if(l1square*l2square>0)
			value=acos((d1[0]*d2[0]+d1[1]*d2[1]+d1[2]*d2[2])/sqrt(l1square*l2square))*180/3.141592f;
	}
