	...
	if (rb_pts->isOn())
	{
		...
		else if (rb_vol->isOn())
		{
			...
			txt_displayer->setText(
					...
					QString(" mm^3\n") + QString("'Target': ") +
					QString::number(
							handler3D->calculate_volume(p, handler3D->get_activeslice()), 'g',
							3) +
...
float measure_widget::calculate()
{
	...
	{
		value =
				sqrt((pt[0][0] - pt[1][0]) * (pt[0][0] - pt[1][0]) * p1.high * p1.high +
	...
	else if (rb_angle->isOn())
	{
		...
		if (l1square * l2square > 0)
			value =
					acos(
							((pt[0][0] - pt[1][0]) * (pt[2][0] - pt[1][0]) * p1.high *
									 p1.high +
							 (pt[0][1] - pt[1][1]) * (pt[2][1] - pt[1][1]) * p1.low * p1.low +
							 (pt[0][2] - pt[1][2]) * (pt[2][2] - pt[1][2]) * thick * thick) /
