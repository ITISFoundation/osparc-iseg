...
void hyster_widget::execute1()
{
	...
	else
	{
		...
		if (allslices->isChecked())
		{
			handler3D->thresholded_growing(handler3D->get_activeslice(), p1, ll, uu,
																		 255.f);
		}
		else
		{
...
void hyster_widget::getrange_sub(float ll, float uu, float ul, float lu)
{
	...
	else
	{
		if (upper_limit == lower_limit)
			...
			sl_lower->setValue(
					int(0.5f + (ll - lower_limit) / (upper_limit - lower_limit) * 200));
	}
	...
	else
	{
		if (upper_limit == lower_limit)
			...
			sl_upper->setValue(
					int(0.5f + (uu - lower_limit) / (upper_limit - lower_limit) * 200));
	}
