...
hyster_widget::hyster_widget(SlicesHandler *hand3D, QWidget *parent, const char *name, Qt::WindowFlags wFlags)
  : QWidget1( parent, name, wFlags ),handler3D(hand3D)
{
	setToolTip(Format("Segment a tissue by picking seed points and adding neighboring pixels with similar intensities."));
	...
	if(autoseed->isOn()) autoseed->toggle();
...
//	setFixedSize(vbox1->size());
	...
	QObject::connect(sl_lower,SIGNAL(sliderMoved(int)),this,SLOT(slider_changed()));
	QObject::connect(sl_upper,SIGNAL(sliderMoved(int)),this,SLOT(slider_changed()));
	QObject::connect(sl_lowerhyster,SIGNAL(sliderMoved(int)),this,SLOT(slider_changed()));
	QObject::connect(sl_upperhyster,SIGNAL(sliderMoved(int)),this,SLOT(slider_changed()));
	QObject::connect(sl_lower,SIGNAL(sliderPressed()),this,SLOT(slider_pressed()));
	QObject::connect(sl_upper,SIGNAL(sliderPressed()),this,SLOT(slider_pressed()));
	QObject::connect(sl_lowerhyster,SIGNAL(sliderPressed()),this,SLOT(slider_pressed()));
	QObject::connect(sl_upperhyster,SIGNAL(sliderPressed()),this,SLOT(slider_pressed()));
	QObject::connect(sl_lower,SIGNAL(sliderReleased()),this,SLOT(slider_released()));
	QObject::connect(sl_upper,SIGNAL(sliderReleased()),this,SLOT(slider_released()));
	QObject::connect(sl_lowerhyster,SIGNAL(sliderReleased()),this,SLOT(slider_released()));
	QObject::connect(sl_upperhyster,SIGNAL(sliderReleased()),this,SLOT(slider_released()));

	QObject::connect(le_bordervall,SIGNAL(returnPressed()),this,SLOT(le_bordervall_returnpressed()));
	QObject::connect(le_bordervalu,SIGNAL(returnPressed()),this,SLOT(le_bordervalu_returnpressed()));
	QObject::connect(le_bordervallh,SIGNAL(returnPressed()),this,SLOT(le_bordervallh_returnpressed()));
	QObject::connect(le_bordervaluh,SIGNAL(returnPressed()),this,SLOT(le_bordervaluh_returnpressed()));
...
{
	if(autoseed->isOn()){
		float ll=lower_limit+(upper_limit-lower_limit)*0.005f*sl_lower->value();
		float uu=lower_limit+(upper_limit-lower_limit)*0.005f*sl_upper->value();
		float ul=ll+(uu-ll)*0.005f*sl_lowerhyster->value();
		float lu=ll+(uu-ll)*0.005f*sl_upperhyster->value();
		if(allslices->isChecked()) 
			handler3D->double_hysteretic_allslices(ll,ul,lu,uu,false,255.0f);
		else 
		...
		le_bordervallh->setText(QString::number(int(0.4f+(ul-ll)/(uu-ll)*100),'g',3));
		le_bordervaluh->setText(QString::number(int(0.4f+(lu-ll)/(uu-ll)*100),'g',3));
	} else {
		float ll=lower_limit+(upper_limit-lower_limit)*0.005f*sl_lower->value();
		float uu=lower_limit+(upper_limit-lower_limit)*0.005f*sl_upper->value();
//		bmphand->thresholded_growing(p1,ll,uu,false,255.f);
		if(allslices->isChecked()) {
			handler3D->thresholded_growing(handler3D->get_activeslice(), p1, ll, uu,255.f);
		} else {
...
void hyster_widget::getrange_sub(float ll,float uu,float ul,float lu)
{
	if(ll<lower_limit){
		...
		if(upper_limit==lower_limit)
			...
			sl_lower->setValue(int(0.5f+(ll-lower_limit)/(upper_limit-lower_limit)*200));
	}
	...
	if(uu<lower_limit){
		...
		if(upper_limit==lower_limit)
			...
			sl_upper->setValue(int(0.5f+(uu-lower_limit)/(upper_limit-lower_limit)*200));
	}
...
void hyster_widget::slider_changed()
{
	execute1();
}
...
QSize hyster_widget::sizeHint() const
{
...
void hyster_widget::newloaded()
{
	...
	vp1.clear();
	for(vector<vector<Point> >::iterator it=vvp->begin();it!=vvp->end();it++){
...
void hyster_widget::slider_released()
{
	emit end_datachange(this);
}
...
{
	QString loadfilename = Q3FileDialog::getOpenFileName(QString::null,"Borders (*.txt)\n" "All(*.*)",this);
	...
	if(b1){
		if(val<lower_limit){
			...
			if(upper_limit==lower_limit) {
				...
				sl_lower->setValue(int(0.5f+(val-lower_limit)/(upper_limit-lower_limit)*200));
		}
	...
	if(b1){
		if(val<lower_limit){
			...
			if(upper_limit==lower_limit) {
				...
				sl_upper->setValue(int(0.5f+(val-lower_limit)/(upper_limit-lower_limit)*200));
		}
...
void hyster_widget::le_bordervallh_returnpressed()
{
	...
	if(b1){
		if(val<0){
			ul=ll;
			sl_lowerhyster->setValue(0);
			le_bordervallh->setText(QString::number(int((ul-ll)/(uu-ll)*100),'g',3));
		} else if(val>100) {
			ul=uu;
			sl_lowerhyster->setValue(200);
			le_bordervallh->setText(QString::number(int((ul-ll)/(uu-ll)*100),'g',3));
...
void hyster_widget::le_bordervaluh_returnpressed()
{
	...
	if(b1){
		if(val<0){
			lu=ll;
			sl_upperhyster->setValue(0);
			le_bordervaluh->setText(QString::number(int((lu-ll)/(uu-ll)*100),'g',3));
		} else if(val>100) {
			lu=uu;
			sl_upperhyster->setValue(200);
			le_bordervaluh->setText(QString::number(int((lu-ll)/(uu-ll)*100),'g',3));
...
{
	if(version>=2) {
		QObject::disconnect(autoseed,SIGNAL(clicked()),this,SLOT(auto_toggled()));
		QObject::disconnect(sl_lower,SIGNAL(sliderMoved(int)),this,SLOT(slider_changed()));
		QObject::disconnect(sl_upper,SIGNAL(sliderMoved(int)),this,SLOT(slider_changed()));
		QObject::disconnect(sl_lowerhyster,SIGNAL(sliderMoved(int)),this,SLOT(slider_changed()));
		QObject::disconnect(sl_upperhyster,SIGNAL(sliderMoved(int)),this,SLOT(slider_changed()));
