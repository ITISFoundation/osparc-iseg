...
#endif
		if (result) {
			cerr << "bmphandler::ReadRawOverlay() : error, file operation failed"
					 << endl;
			free(bits_tmp);
			fclose(fp);
			return (NULL);
...
int SlicesHandler::ReadRTdose(const char *filename)
{
	...
	cerr << "SlicesHandler::ReadRTdose() : Extent = " << dims[0] << " " << dims[1]
			 << " " << dims[2] << endl;
	...
	for (unsigned i = startslice; i < endslice; i++) {
		...
		for (auto cur_mark : marks) {
			...
			if (mark_name.isEmpty()) {
				mark_name =
						QString::fromStdString("Mark_" + std::to_string(mark_counter));
				mark_counter++;
			}
														 ...
														 unsigned short slicenr)
{
	...
	for (unsigned short i = startslice; i < endslice; i++)
		j += (image_slices[i])
						 .ReloadRaw(filename, bitdepth, (unsigned)slicenr + i - startslice);
																	...
																	Point p)
{
	...
	for (unsigned short i = startslice; i < endslice; i++)
		j += (image_slices[i])
						 .ReloadRawFloat(filename, w, h, (unsigned)slicenr + i - startslice,
														 p);
																...
																Transform transform_to_write)
{
	...
	int combinedVersion =
			common::CombineTissuesVersion(project_version, tissue_version);
...
bool SlicesHandler::SaveCommunicationFile(const char *filename)
{
	...
	SaveAllXdmf(QFileInfo(filename).dir().absFilePath(filename).toAscii().data(),
...
int SlicesHandler::SaveRaw(const char *filename, bool work)
{
	...
	for (unsigned short j = 0; j < nrslices; j++) {
		...
		for (unsigned int i = 0; i < area; i++) {
			bits_tmp[i] =
					(unsigned char)(std::min(255.0, std::max(0.0, p_bits[i] + 0.5)));
		}
	...
	if (slicenr >= startslice && slicenr < endslice) {
		...
		for (unsigned short i = startslice; i < slicenr; i++) {
			...
			for (unsigned short k = 0; k + 1 < dim; k++) {
				if (!ImageReader::getSlice(mhdfiles[k].c_str(), bits[k + 1], i, width,
																	 height)) {
					for (unsigned short j = 1; j < dim; j++)
						delete[] bits[j];
					delete[] bits;
															 ...
															 const std::string initCentersFile)
{
	...
	if (slicenr >= startslice && slicenr < endslice) {
		...
		for (unsigned short i = startslice; i < slicenr; i++) {
			...
			for (unsigned short k = 0; k + 1 < dim; k++) {
				if (!ChannelExtractor::getSlice(pngfiles[0].c_str(), bits[i + 1],
																				exctractChannel[i], i, width, height)) {
					for (unsigned short j = 1; j < dim; j++)
						delete[] bits[j];
					delete[] bits;
					return;
		...
		bool dp, float epsilon, const char *filename)
{
	...
	for (unsigned short j = 0; j + 1 < nrslices; j++) {
		...
		dummy3D.copy2tissue(between + 1,
												image_slices[j + 1].return_tissues(active_tissuelayer));
																		 ...
																		 std::vector<tissues_size_t> &tissuevec)
{
	...
	for (std::vector<tissues_size_t>::iterator it1 = tissuevec.begin();
		...
		for (unsigned short i = 0; i < nrslices; i++) {
			...
			v2.clear();
			(image_slices[i])
					.get_tissuecontours(active_tissuelayer, *it1, &v1, &v2, minsize);
		...
		int minsize, std::vector<tissues_size_t> &tissuevec)
{
	...
	for (std::vector<tissues_size_t>::iterator it1 = tissuevec.begin();
		...
		for (unsigned short i = 0; i < nrslices; i++) {
			...
			v2.clear();
			(image_slices[i])
					.get_tissuecontours2_xmirrored(active_tissuelayer, *it1, &v1, &v2,
																				 minsize);
			for (std::vector<std::vector<Point>>::iterator it = v1.begin();
					 it != v1.end(); it++) {
				//			Pointconvert(*it,vP);
															 ...
															 bool connectivity, unsigned short nrpasses)
{
	...
	for (unsigned short i = 0; i < nrpasses; i++) {
		while (++slicenr < endslice) {
			image_slices[slicenr].hysteretic(thresh_low, thresh_high, connectivity,
																			 image_slices[slicenr - 1].return_work(),
																			 setvalue - 1, setvalue);
		}
		setvalue++;
		slicenr--;
																				...
																				unsigned short nrpasses)
{
	if (slicenr < endslice && slicenr >= startslice) {
		...
		image_slices[slicenr].thresholded_growing(
				p, threshfactor_low, threshfactor_high, connectivity, setvalue, &tp);

		for (unsigned short i = 0; i < nrpasses; i++) {
			while (++slicenr < endslice) {
				image_slices[slicenr].thresholded_growing(
						tp.low, tp.high, connectivity,
						image_slices[slicenr - 1].return_work(), setvalue - 1, setvalue);
			}
			setvalue++;
			slicenr--;
			while (slicenr-- > startslice) {
		...
		while (!s.empty()) {
			...
			if (i.pxy >= width && work[i.pxy - width] == f &&
					(tissue[i.pxy - width] == 0 ||
					 (override &&
						TissueInfos::GetTissueLocked(tissue[i.pxy - width]) == false))) {
				work[i.pxy - width] = set_to;
			...
			if (i.pz > startslice) {
				...
				if (work[i.pxy] == f &&
						(tissue[i.pxy] == 0 || (override && TissueInfos::GetTissueLocked(
																										tissue[i.pxy]) == false))) {
					work[i.pxy] = set_to;
	...
	for (unsigned short i = 0; i < nrpasses; i++) {
		while (++slicenr < endslice) {
			image_slices[slicenr].double_hysteretic(
					...
					connectivity, image_slices[slicenr - 1].return_work(), setvalue - 1,
					setvalue);
		}
		setvalue++;
		slicenr--;
	...
	if (n > 0) {
		...
		for (p.py = 0; p.py < height; p.py++) {
			for (p.px = 0; p.px < width; p.px++) {
				...
				n1 = (unsigned short)n * prop;
				for (unsigned short j = 1; j <= n1 && j < n; j++) {
					image_slices[slice1 + j].set_tissue_pt(active_tissuelayer, p,
																								 bmp1[i1]);
				}
				for (unsigned short j = n1 + 1; j < n; j++) {
					image_slices[slice1 + j].set_tissue_pt(active_tissuelayer, p,
...
{
	tissues_size_t *result = (tissues_size_t *)malloc(sizeof(tissues_size_t) *
																										unsigned(width) * nrslices);
		...
		float passBand, float featureAngle)
{
	...
	labelField->SetExtent(0, (int)return_width() + 1, 0, (int)return_height() + 1,
												0, (int)(endslice - startslice) + 1);
	...
	for (unsigned long long i = (endslice - startslice + 1) *
			 ...
			 i < (endslice - startslice + 2) *
							 (unsigned long long)(return_width() + 2) * (return_height() + 2);
			 i++)
		field[i] = 0;
																...
																std::vector<rgbs> &colorvec)
{
	...
	for (std::vector<tissues_size_t>::iterator it = tissuevec.begin();
			 it != tissuevec.end(); it++) {
		vstring.push_back(
				(std::string)TissueInfos::GetTissueName(*it + 1)); // TODO: why +1???
	}
...
void SlicesHandler::add2tissue_connected(tissues_size_t tissuetype, Point p,
																				 bool override)
{
	image_slices[activeslice].add2tissue_connected(active_tissuelayer, tissuetype,
																								 p, override);
}
...
void SlicesHandler::start_undo(common::DataSelection &dataSelection)
{
	if (uelem == NULL) {
		...
		if (dataSelection.tissues)
			uelem->tissue_old =
					image_slices[dataSelection.sliceNr].copy_tissue(active_tissuelayer);
		else
...
bool SlicesHandler::start_undo(common::DataSelection &dataSelection,
															 std::vector<unsigned> vslicenr1)
{
	if (uelem == NULL) {
		...
		if (uelem->multi) {
			...
			if (uelem1 != NULL) {
				...
				for (unsigned i = 0; i < uelem1->vslicenr.size(); i++) {
					...
					if (dataSelection.bmp) {
						...
						image_slices[current_slice].copy2bmp(uelem1->vbmp_old[i],
																								 uelem1->vmode1_old[i]);
					...
					if (dataSelection.work) {
						...
						image_slices[current_slice].copy2work(uelem1->vwork_old[i],
																									uelem1->vmode2_old[i]);
						free(uelem1->vwork_old[i]);
					}
					if (dataSelection.tissues) {
						uelem1->vtissue_new.push_back(
								image_slices[current_slice].copy_tissue(active_tissuelayer));
						image_slices[current_slice].copy2tissue(active_tissuelayer,
																										uelem1->vtissue_old[i]);
			...
			if (uelem != NULL) {
				...
				if (dataSelection.bmp) {
					...
					image_slices[dataSelection.sliceNr].copy2bmp(uelem->bmp_old,
																											 uelem->mode1_old);
					free(uelem->bmp_old);
					uelem->bmp_old = NULL;
				}
				...
				if (dataSelection.work) {
					...
					image_slices[dataSelection.sliceNr].copy2work(uelem->work_old,
																												uelem->mode2_old);
					free(uelem->work_old);
					uelem->work_old = NULL;
				}
				...
				if (dataSelection.tissues) {
					uelem->tissue_new = image_slices[dataSelection.sliceNr].copy_tissue(
							active_tissuelayer);
					image_slices[dataSelection.sliceNr].copy2tissue(active_tissuelayer,
																													uelem->tissue_old);
		...
		if (uelem->multi) {
				...
				for (unsigned i = 0; i < uelem1->vslicenr.size(); i++) {
					...
					if (dataSelection.bmp) {
						...
						uelem1->vmode1_old.push_back(
								image_slices[current_slice].return_mode(true));
						image_slices[current_slice].copy2bmp(uelem1->vbmp_new[i],
																								 uelem1->vmode1_new[i]);
					...
					if (dataSelection.work) {
						...
						uelem1->vmode2_old.push_back(
								image_slices[current_slice].return_mode(false));
						image_slices[current_slice].copy2work(uelem1->vwork_new[i],
																									uelem1->vmode2_new[i]);
			...
			if (uelem != NULL) {
				...
				if (dataSelection.bmp) {
					...
					uelem->mode1_old =
							image_slices[dataSelection.sliceNr].return_mode(true);
					image_slices[dataSelection.sliceNr].copy2bmp(uelem->bmp_new,
																											 uelem->mode1_new);
					free(uelem->bmp_new);
					uelem->bmp_new = NULL;
				}
				...
				if (dataSelection.work) {
					...
					uelem->mode2_old =
							image_slices[dataSelection.sliceNr].return_mode(false);
					image_slices[dataSelection.sliceNr].copy2work(uelem->work_new,
																												uelem->mode2_new);
					free(uelem->work_new);
					uelem->work_new = NULL;
				}
...
int SlicesHandler::LoadDICOM(std::vector<const char *> lfilename)
{
	if (lfilename.size() > 0) {
		...
		if (gdcmvtk_rtstruct::GetSizeUsingGDCM(lfilename[0], a, b, c, d, e, thick1,
																					 disp1, dc1)) {
		...
		for (int i = 0; i < lfilename.size(); i++) {
			if (gdcmvtk_rtstruct::GetSizeUsingGDCM(lfilename[i], a, b, c, d, e,
				...
				if (c >= 1) {
					...
					bool canload =
							gdcmvtk_rtstruct::GetDicomUsingGDCM(lfilename[i], bits, a, b, c);
					if (!canload) {
						free(bits);
						return 0;
					}
	...
	if (lfilename.size() == 1) {
		...
		if (c > 1) {
			...
			if (bits == NULL)
				return 0;
			bool canload =
					gdcmvtk_rtstruct::GetDicomUsingGDCM(lfilename[0], bits, a, b, c);
			if (!canload) {
				free(bits);
				return 0;
			}
			...
			for (unsigned short i = 0; i < nrslices; i++) {
				if ((image_slices[i])
								.LoadArray(&(bits[(unsigned long)(a)*b * i]), a, b, p, dx, dy))
					j++;
			}
...
int SlicesHandler::ReloadDICOM(std::vector<const char *> lfilename, Point p)
{
	if ((endslice - startslice) == (unsigned short)lfilename.size()) {
		...
		if (nrslices == c) {
			...
			for (unsigned short i = startslice; i < endslice; i++) {
				if ((image_slices[i])
								.LoadArray(&(bits[(unsigned long)(a)*b * i]), a, b, p, width,
													 height))
					j++;
			}
...
bool SlicesHandler::print_atlas(const char *filename)
{
	...
	for (unsigned short i = 0; i < nrslices; i++) {
		...
		out.writeRawData((char *)image_slices[i].return_tissues(active_tissuelayer),
...
bool SlicesHandler::print_amascii(const char *filename)
{
	...
	if (streamname.good()) {
		...
		streamname << "# CreationDate: Fri Jun 16 14:24:32 2006" << endl
							 << endl
							 << endl;
		streamname << "define Lattice " << width << " " << height << " " << nrslices
							 << endl
							 << endl;
...
bool SlicesHandler::export_tissue(const char *filename, bool binary) const
{
	...
	float spacing[3] = {dx, dy, thickness};
	return ImageWriter(binary).writeVolume(filename, slices.data(), width, height,
																				 nrslices, spacing, transform);
}
																			...
																			const char *projname)
{
	...
	if (streamname.good()) {
		...
		for (tissues_size_t tissuenr = 1; tissuenr <= TissueInfos::GetTissueCount();
				 tissuenr++) {
								...
								if (value == backgroundID) {
									for (int y = 0; y < offsetsSlices.size(); y++) {
										...
										for (int m = 0; m < offsetsSlices[y].size(); m++) {
											...
											if (k + neighborSlice + neighborSlice2 >= 0 &&
													k + neighborSlice + neighborSlice2 < dims[2]) {
												tissues_size_t value2 =
														tissuesVector[k + neighborSlice + neighborSlice2]
																				 [idx2];
												if (value2 != backgroundID && value2 != skinID) {
													...
													if (std::find(partialChanges.begin(),
																				partialChanges.end(),
																				changes) == partialChanges.end())
				...
				for (short i = -n; i <= n; i++) {
					for (short j = -n; j <= n; j++) {
						...
						if (tissuecount.find(currtissue) == tissuecount.end()) {
							tissuecount.insert(
									std::pair<tissues_size_t, unsigned short>(currtissue, 1));
						}
						else {
...
std::vector<float> SlicesHandler::GetBoundingBox()
{
	...
	bbox[4] = (float)std::min<double>(origin[2],
																		origin[2] + d3[2] * dimensionZ * thickness);
	bbox[5] = (float)std::max<double>(origin[2],
																		origin[2] + d3[2] * dimensionZ * thickness);
																		...
																		int end)
{
	...
	for (unsigned z = 0; z < output->GetLargestPossibleRegion().GetSize(2); z++) {
		...
		for (unsigned y = 0; y < output->GetLargestPossibleRegion().GetSize(1);
				 y++) {
			pi.SetElement(1, y);
			for (unsigned x = 0; x < output->GetLargestPossibleRegion().GetSize(0);
					 x++) {
				pi.SetElement(0, x);
				field[x + y * size[0]] = output->GetPixel(pi);
			}
