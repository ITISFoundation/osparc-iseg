...
#include "Core/matexport.h"
#include "Core/marchingcubeprint.h"
...
struct posit
{
	unsigned pxy;
	unsigned short pz;
	inline bool operator<(const posit& a) const {if(pz<a.pz||((pz==a.pz)&&(pxy < a.pxy))) return true; else return false;}
	inline bool operator==(const posit& a) const {return (pz==a.pz)&&(pxy == a.pxy);}
};
...
std::vector<const tissues_size_t*> SlicesHandler::get_tissues(tissuelayers_size_t layeridx) const
{
...
std::vector<tissues_size_t*> SlicesHandler::get_tissues(tissuelayers_size_t layeridx)
{
...
tissues_size_t *SlicesHandler::return_tissues(tissuelayers_size_t layeridx, unsigned short slicenr1)
{
	return image_slices[slicenr1].return_tissues(layeridx);
}

float *SlicesHandler::return_overlay()
{
	return overlay;
}

int SlicesHandler::LoadDIBitmap(const char *filename, unsigned short slicenr, unsigned short nrofslices)
{
...
//		fprintf(fp,"%s %u.bmp",filename,i+slicenr);
	...
	if (overlay != NULL) free(overlay);
	overlay = (float *)malloc(sizeof(float)*area);
	clear_overlay();

	for(unsigned short i=1;i<nrofslices;i++){
		if(width!=image_slices[i].return_width()||height!=image_slices[i].return_height())
			j=nrofslices+1;
	}
...
int SlicesHandler::LoadDIBitmap(std::vector<const char *> filenames)
{
	...
	if( image_slices[0].CheckBMPDepth(filenames[0]) > 8 )
	{
		...
		for(unsigned short i=0;i<nrslices;i++)
		{	
			(image_slices[i]).SetConverterFactors(redFactor,greenFactor,blueFactor);
		}
	}
...
int SlicesHandler::LoadDIBitmap(std::vector<const char *> filenames, double refFactor, double blueFactor, double greenFactor )
{
	endslice=nrslices=(unsigned short)filenames.size();
	os.set_sizenr(nrslices);
	...
	for(unsigned short i=0;i<nrslices;i++)
	{
		(image_slices[i]).SetRGBtoGrayScaleFactors(refFactor,blueFactor,greenFactor);
	}
	return LoadDIBitmap(filenames);
}

int SlicesHandler::LoadDIBitmap(const char *filename, unsigned short slicenr, unsigned short nrofslices, Point p, unsigned short dx, unsigned short dy)
{
	UpdateColorLookupTable(nullptr);
...
int SlicesHandler::LoadPng(std::vector<const char *> filenames, double refFactor, double blueFactor, double greenFactor )
{
	UpdateColorLookupTable(nullptr);
...
int SlicesHandler::LoadDIJpg(const char *filename, unsigned short slicenr, unsigned short nrofslices)
{
	UpdateColorLookupTable(nullptr);
...
int SlicesHandler::LoadDIJpg(const char *filename, unsigned short slicenr, unsigned short nrofslices, Point p, unsigned short dx, unsigned short dy)
{
	UpdateColorLookupTable(nullptr);
  ...
  if ((fp = fopen(filename, "rb")) == NULL)
...
#endif
		if( result ){
			cerr << "bmphandler::ReadRawOverlay() : error, file operation failed" << endl;
			free(bits_tmp);
			fclose(fp);
			return (NULL);
		}
	...
	else if(bytedepth==2)
	{
	  ...
	  if ((bits_tmp = (unsigned short *) malloc(bitsize*2)) == NULL)
		{
			cerr << "bmphandler::ReadRawOverlay() : error, allocation failed" << endl;
			fclose(fp);
			return (NULL);
		}
	...
	{
		cerr << "bmphandler::ReadRawOverlay() : error, unsupported depth..." << endl;
		fclose(fp);
		return (NULL);
	}
...
int SlicesHandler::ReadImage(const char *filename)
{
	...
	if (ImageReader::getInfo(filename, w, h, nrofslices, spacing1, transform1)) 
	{
		...
		ImageReader::getVolume(filename,slices.data(),nrslices,width,height);
...
int SlicesHandler::ReadOverlay(const char *filename, unsigned short slicenr)
{
	...
	if (ImageReader::getInfo(filename, w, h, nrofslices, spacing1, transform1))
	{
		return ImageReader::getVolume(filename, &overlay, slicenr, 1, width, height);
	}
	return 0;
}
...
int SlicesHandler::ReadRTdose(const char *filename)
{
	...
	cerr << "SlicesHandler::ReadRTdose() : Extent = " << dims[0] << " " << dims[1] << " " << dims[2] << endl;
	cerr << "SlicesHandler::ReadRTdose() : Origin = " << origin[0] << " " << origin[1] << " " << origin[2] << endl;
	cerr << "SlicesHandler::ReadRTdose() : Spacing = " << spacing[0] << " " << spacing[1] << " " << spacing[2] << endl;
	cerr << "SlicesHandler::ReadRTdose() : Direction Cosines Row = " << dc[0] << " " << dc[1] << " " << dc[2] << endl;
	cerr << "SlicesHandler::ReadRTdose() : Direction Cosines Column = " << dc[3] << " " << dc[4] << " " << dc[5] << endl;
...
bool SlicesHandler::LoadSurface(const char *filename, const bool overwrite_working)
{
	...
	auto ret = voxeler.Run(filename, dims, spacing, transform, slices.data(), startslice, endslice);
...
int SlicesHandler::LoadAllHDF(const char *filename)
{
	...
	cerr << "SlicesHandler::LoadAllHDF() : Extent = " << width << " " << height << " " << nrofslices << endl;
	cerr << "SlicesHandler::LoadAllHDF() : Origin = " << offset[0] << " " << offset[1] << " " << offset[2] << endl;
	cerr << "SlicesHandler::LoadAllHDF() : Spacing = " << pixsize[0] << " " << pixsize[1] << " " << pixsize[2] << endl;
	const int NPA = arrayNames.size();
	cerr << "SlicesHandler::LoadAllHDF() : number of point arrays: " << NPA << endl;
	for(int i=0; i<NPA; ++i)
	{
...
int SlicesHandler::LoadAllXdmf(const char *filename)
{
	...
	cerr << "SlicesHandler::loadAllXdmf() : Extent = " << width << " " << height << " " << nrofslices << endl;
...
int SlicesHandler::SaveAllXdmf(const char *filename, int compression, bool naked)
{
	...
	for(unsigned i=startslice; i<endslice; i++)
	{
		...
		tissueslices[i-startslice] = image_slices[i].return_tissues(0); // TODO
	}
	...
	ok &= TissueInfos::SaveTissuesHDF(filename, tissue_hierachy->selected_hierarchy(), naked, 0);
...
bool SlicesHandler::SaveMarkersHDF(const char *filename, bool naked, unsigned short version)
{
	...
	cerr << "storing current folder " << oldcwd.absolutePath().toAscii().data() << endl;

	// enter the xmf file folder so relative names for hdf5 files work
	QDir::setCurrent(fileInfo.absolutePath());
	cerr << "changing current folder to " << fileInfo.absolutePath().toAscii().data() << endl;
	...
	for (unsigned i = startslice; i < endslice; i++)
	{
		...
		for (auto cur_mark : marks)
		{
			...
			std::string mystring = std::string("/Markers/") + mark_name.toLocal8Bit().constData();
...
int SlicesHandler::ReadRaw(const char *filename, short unsigned w, short unsigned h, unsigned bitdepth, unsigned short slicenr, unsigned short nrofslices, Point p, unsigned short dx, unsigned short dy)
{
	...
	int j=0;
	for(unsigned short i=0;i<nrofslices;i++)
		j+=(image_slices[i]).ReadRaw(filename,w,h,bitdepth,slicenr+i,p,dx,dy);
...
int SlicesHandler::ReadRawFloat(const char *filename, short unsigned w, short unsigned h, unsigned short slicenr, unsigned short nrofslices, Point p, unsigned short dx, unsigned short dy)
{
	...
	int j=0;
	for(unsigned short i=0;i<nrofslices;i++)
		j+=(image_slices[i]).ReadRawFloat(filename,w,h,slicenr+i,p,dx,dy);
...
int SlicesHandler::ReloadDIBitmap(const char *filename, unsigned short slicenr)
{
	...
	if(j==(endslice-startslice)) return 1;
	else  {
		newbmp(width,height,nrslices);
		return 0;
	}
...
int SlicesHandler::ReloadDIBitmap(const char *filename, Point p, unsigned short slicenr)
{
	UpdateColorLookupTable(nullptr);
...
int SlicesHandler::ReloadRaw(const char *filename, unsigned bitdepth, unsigned short slicenr)
{
	...
	int j=0;
	for(unsigned short i=startslice;i<endslice;i++)
		j+=(image_slices[i]).ReloadRaw(filename, bitdepth, (unsigned)slicenr+i-startslice);
...
int SlicesHandler::ReloadImage(const char *filename, unsigned short slicenr)
{
	...
	if (ImageReader::getInfo(filename, w, h, nrofslices, spacing1, transform1))
	{
		if((w!=width)||(h!=height)||(endslice-startslice+slicenr>nrofslices)) return 0;
		std::vector<float*> slices(endslice - startslice);
		for(unsigned i=startslice;i<endslice;i++)
		{
			slices[i-startslice]=image_slices[i].return_bmp();
		}
		ImageReader::getVolume(filename,slices.data(),slicenr,endslice-startslice,width,height);
		return 1;
	}
	return 0;
}
...
int SlicesHandler::ReloadRTdose(const char *filename, unsigned short slicenr)
{
	...
	if ((dims[0] != width) || (dims[1] != height) || (endslice - startslice + slicenr > dims[2])) return 0;
...
int SlicesHandler::ReloadAVW(const char *filename, unsigned short slicenr)
{
	...
	float dx1,dy1,thickness1;
	avw::ReadHeader(filename,w,h,nrofslices,dx1,dy1,thickness1,type);
	if((w!=width)||(h!=height)||(endslice>nrofslices)) return 0;
...
int SlicesHandler::ReloadRaw(const char *filename, short unsigned w, short unsigned h, unsigned bitdepth, unsigned  short slicenr, Point p)
{
	...
	int j=0;
	for(unsigned short i=startslice;i<endslice;i++)
		j+=(image_slices[i]).ReloadRaw(filename, w, h, bitdepth,(unsigned)slicenr+i-startslice, p);
...
int SlicesHandler::ReloadRawFloat(const char *filename, unsigned short slicenr)
{
	...
	int j=0;
	for(unsigned short i=startslice;i<endslice;i++)
		j+=(image_slices[i]).ReloadRawFloat(filename, (unsigned)slicenr+i-startslice);
...
int SlicesHandler::ReloadRawFloat(const char *filename, short unsigned w, short unsigned h, unsigned  short slicenr, Point p)
{
	...
	int j=0;
	for(unsigned short i=startslice;i<endslice;i++)
		j+=(image_slices[i]).ReloadRawFloat(filename, w, h,(unsigned)slicenr+i-startslice, p);
...
int SlicesHandler::ReloadRawTissues(const char *filename, unsigned bitdepth, unsigned short slicenr)
{
	...
	int j=0;
	for(unsigned short i=startslice;i<endslice;i++)
		j+=(image_slices[i]).ReloadRawTissues(filename, bitdepth,(unsigned)slicenr+i-startslice);
...
int SlicesHandler::ReloadRawTissues(const char *filename, short unsigned w, short unsigned h, unsigned bitdepth, unsigned  short slicenr, Point p)
{
	...
	int j=0;
	for(unsigned short i=startslice;i<endslice;i++)
		j+=(image_slices[i]).ReloadRawTissues(filename, w, h, bitdepth,(unsigned)slicenr+i-startslice, p);
...
FILE *SlicesHandler::SaveHeader(FILE *fp, short unsigned nr_slices_to_write, Transform transform_to_write)
{
	...
	if (thick > 0) thick = -thick;
	fwrite(&thick, 1, sizeof(float), fp);
	fwrite(&dx, 1, sizeof(float), fp);
	fwrite(&dy, 1, sizeof(float), fp);
	int combinedVersion = common::CombineTissuesVersion(project_version, tissue_version);
...
FILE *SlicesHandler::SaveProject(const char *filename, const char *imageFileExtension)
{
	...
	if((fp = fopen(filename, "wb")) == NULL) return NULL;
	...
	while(imageFileExtension[length1]!='\0') length1++;
	length1++;
	...
	SaveAllXdmf(QFileInfo(filename).dir().absFilePath(imageFileName).toAscii().data(), this->Compression, false);
...
bool SlicesHandler::SaveCommunicationFile(const char *filename)
{
	...
	SaveAllXdmf(QFileInfo(filename).dir().absFilePath(filename).toAscii().data(), this->Compression, true);
...
FILE *SlicesHandler::MergeProjects(const char *savefilename,std::vector<QString> &mergeFilenames)
{
	...
	for(unsigned short i=0;i<mergeFilenames.size();i++) {
		...
		if((fpMerge = fopen(mergeFilenames[i].toAscii().data(), "rb")) == NULL) return NULL;
	...
	// Save merged project
	FILE *fp;
	if((fp = fopen(savefilename, "wb")) == NULL) return NULL;
	...
	int afterDot = imageFileName.lastIndexOf('.')+1;
	imageFileName = imageFileName.remove(afterDot, imageFileName.length()-afterDot) + imageFileExtension;
	if (!SaveMergeAllXdmf(QFileInfo(savefilename).dir().absFilePath(imageFileName).toAscii().data(), mergeFilenames, nrslicesTotal, this->Compression)) return NULL;
	startslice=startslice1;
	endslice=endslice1;
...
void SlicesHandler::LoadHeader(FILE *fp, int &tissuesVersion, int &version)
{
	...
	if (thickness < 0) {
		...
		common::ExtractTissuesVersion(combinedVersion, version, tissuesVersion);
	}
...
FILE *SlicesHandler::LoadProject(const char *filename, int &tissuesVersion)
{
	...
	if (version > 1) 
	{
		...
		if (version>2) 
		{
			...
			int afterDot = imageFileName.lastIndexOf('.')+1;
			imageFileName = imageFileName.remove(afterDot, imageFileName.length()-afterDot) + QString(imageFileExtension);
		} 
		else 
		{
		...
		} else cerr << "error, unsupported format..." << endl;
	}
...
bool SlicesHandler::LoadS4Llink(const char *filename, int &tissuesVersion)
{
	...
	cerr << "SlicesHandler::LoadS4Llink() : Extent = " << width << " " << height << " " << nrofslices << endl;
	cerr << "SlicesHandler::LoadS4Llink() : Spacing = " << pixsize[0] << " " << pixsize[1] << " " << pixsize[2] << endl;
	const int NPA = arrayNames.size();
	cerr << "SlicesHandler::LoadS4Llink() : number of point arrays: " << NPA << endl;
	for (int i = 0; i < NPA; ++i)
	{
		cerr << "\tarray id = " << i << ", name = " << arrayNames[i].toAscii().data() << endl;
	}
...
int SlicesHandler::SaveTissueRaw(const char *filename)
{
	...
	if ((TissueInfos::GetTissueCount() <= 255) && (sizeof(tissues_size_t) > sizeof(unsigned char))) {
		...
		for(unsigned short j=0;j<nrslices;j++){
			...
			if (fwrite(ucharBuffer, sizeof(unsigned char), bitsize, fp) < (unsigned int)bitsize) {
				fclose(fp);
				return (-1);
			}
		}
		...
		for(unsigned short j=0;j<nrslices;j++){
			bits_tmp=(image_slices[j]).return_tissues(active_tissuelayer);
			if (fwrite(bits_tmp, sizeof(tissues_size_t), bitsize, fp) < (unsigned int)bitsize) {
				fclose(fp);
				return (-1);
			}
		}
...
int SlicesHandler::SaveRaw_resized(const char *filename,int dxm,int dxp,int dym,int dyp,int dzm,int dzp, bool work)
{
	if((-(dxm+dxp)>=width)||(-(dym+dyp)>=height)||(-(dzm+dzp)>=nrslices)) return (-1);
	FILE             *fp;   
	float *	 bits_tmp;
	float *p_bits;
	...
	unsigned newarea=(unsigned)(width+dxm+dxp)*(unsigned)(height+dym+dyp);
	bits_tmp=(float *)malloc(sizeof(float)*newarea);
	if(bits_tmp==NULL) return -1;

	if ((fp = fopen(filename, "wb")) == NULL) return (-1);

	for(unsigned i=0;i<newarea;i++) bits_tmp[i]=0;
	for(int i=0;i<dzm;i++) {
		if (fwrite(bits_tmp, 1, sizeof(float)*newarea, fp) < sizeof(float)*newarea) {
			free(bits_tmp);
			fclose(fp);
			return (-1);
		}
	...
	for(unsigned short j=(unsigned short)std::max(0,-dzm);j<nrslices-(unsigned short)std::max(0,-dzp);j++){
		if(work)
			p_bits=(image_slices[j]).return_work();
		else
			p_bits=(image_slices[j]).return_bmp();
...
int SlicesHandler::SaveTissuesRaw_resized(const char *filename,int dxm,int dxp,int dym,int dyp,int dzm,int dzp)
{
	if((-(dxm+dxp)>=width)||(-(dym+dyp)>=height)||(-(dzm+dzp)>=nrslices)) return (-1);
	FILE             *fp;   
	tissues_size_t *	 bits_tmp;
	...
	for(unsigned short j=(unsigned short)std::max(0,-dzm);j<nrslices-(unsigned short)std::max(0,-dzp);j++){
		tissues_size_t *p_bits=(image_slices[j]).return_tissues(active_tissuelayer);
		unsigned pos1,pos2;
		pos1=posstart1;
		pos2=posstart2;
		for(int y=0;y<h1;y++){
...
bool SlicesHandler::SwapXY()
{
	...
	if (SaveRaw_xy_swapped(str1.ascii(), false) != 0) ok = false;
	str1 = QDir::temp().absFilePath(QString("work_float.raw"));
	if (SaveRaw_xy_swapped(str1.ascii(), true) != 0) ok = false;
	str1 = QDir::temp().absFilePath(QString("tissues.raw"));
	if (SaveTissuesRaw_xy_swapped(str1.ascii()) != 0) ok = false;

	if (ok)
	{
		str1 = QDir::temp().absFilePath(QString("work_float.raw"));
		if (ReadRawFloat(str1.ascii(), w, h, 0, nrslices) != 1) ok = false;
		set_modeall(mode1, true);
	}
	if (ok)
	{
		str1 = QDir::temp().absFilePath(QString("bmp_float.raw"));
		if (ReloadRawFloat(str1.ascii(), 0) != 1) ok = false;
		set_modeall(mode2, false);
	}
	if (ok)
	{
		str1 = QDir::temp().absFilePath(QString("tissues.raw"));
		if (ReloadRawTissues(str1.ascii(), sizeof(tissues_size_t) * 8, 0) != 1) ok = false;
	}
	...
	sliceprovider_installer::getinst()->report();
...
bool SlicesHandler::SwapYZ()
{
	...
	bool ok = true;
	str1 = QDir::temp().absFilePath(QString("bmp_float.raw"));
	if (SaveRaw_yz_swapped(str1.ascii(), false) != 0) ok = false;
	str1 = QDir::temp().absFilePath(QString("work_float.raw"));
	if (SaveRaw_yz_swapped(str1.ascii(), true) != 0) ok = false;
	str1 = QDir::temp().absFilePath(QString("tissues.raw"));
	if (SaveTissuesRaw_yz_swapped(str1.ascii()) != 0) ok = false;
	...
	common::Cross(&dc[0], &dc[3], cross);
	common::Normalize(cross);
...
bool SlicesHandler::SwapXZ()
{
	...
	bool ok = true;
	str1 = QDir::temp().absFilePath(QString("bmp_float.raw"));
	if (SaveRaw_xz_swapped(str1.ascii(), false) != 0) ok = false;
	str1 = QDir::temp().absFilePath(QString("work_float.raw"));
	if (SaveRaw_xz_swapped(str1.ascii(), true) != 0) ok = false;
	str1 = QDir::temp().absFilePath(QString("tissues.raw"));
	if (SaveTissuesRaw_xz_swapped(str1.ascii()) != 0) ok = false;
...
int SlicesHandler::SaveRaw_xy_swapped(const char *filename, bool work)
{
	...
	for(unsigned short j=0;j<nrslices;j++){
		...
		if (fwrite(bits_tmp, 1, sizeof(float)*bitsize, fp) < sizeof(float)*(unsigned int)bitsize) {
			fclose(fp);
			return (-1);
		}
	}
...
int SlicesHandler::SaveRaw_xy_swapped(const char *filename, std::vector<float*> bits_to_swap, short unsigned width, short unsigned height, short unsigned nrslices )
{
	FILE  *fp;   
	float *bits_tmp;
	float *p_bits;
...
int SlicesHandler::SaveRaw_xz_swapped(const char *filename, std::vector<float*> bits_to_swap, short unsigned width, short unsigned height, short unsigned nrslices )
{
	FILE             *fp;   
	float *	 bits_tmp;
	float *p_bits;
...
int SlicesHandler::SaveRaw_yz_swapped(const char *filename, std::vector<float*> bits_to_swap, short unsigned width, short unsigned height, short unsigned nrslices )
{
	FILE             *fp;   
	float *	 bits_tmp;
	float *p_bits;
...
int SlicesHandler::SaveTissuesRaw_xz_swapped(const char *filename)
{
	...
	for(unsigned short x=0;x<width;x++){
		...
		pos1=0;
		for(unsigned short z=0;z<nrslices;z++){
			p_bits=(image_slices[z]).return_tissues(active_tissuelayer); // TODO
			pos2=z;
			pos1=x;
			for(unsigned short y=0;y<height;y++){
				bits_tmp[pos2]=p_bits[pos1];
...
void SlicesHandler::work2bmp()
{
	(image_slices[activeslice]).work2bmp();
}

void SlicesHandler::bmp2work()
{
	(image_slices[activeslice]).bmp2work();
}
...
{
	(image_slices[activeslice]).clear_marks();
}
...
void SlicesHandler::add_mark(Point p, unsigned label, std::string str, unsigned short slicenr)
{
	if(slicenr<nrslices&&slicenr>=0)
...
bool SlicesHandler::remove_limit(Point p, unsigned radius, unsigned short slicenr)
{
	if(slicenr<nrslices&&slicenr>=0)
...
void SlicesHandler::newbmp(unsigned short width1,unsigned short height1,unsigned short nrofslices)
{
	activeslice=0;
	startslice=0;
	endslice=nrslices=nrofslices;
...
{
	for (unsigned int i = 0; i < area; i++) overlay[i] = 0.0f;
}

void SlicesHandler::set_bmp(unsigned short slicenr, float *bits, unsigned char mode)
{
	(image_slices[slicenr]).set_bmp(bits,mode);
}

void SlicesHandler::set_work(unsigned short slicenr, float *bits, unsigned char mode)
{
...
{
	(image_slices[slicenr]).copyfromtissuepadded(active_tissuelayer,bits,padding);
}
...
unsigned int SlicesHandler::make_histogram(bool includeoutofrange)
{
	...
	for(unsigned short i=0;i<255;i++) histogram[i]=dummy[i];
...
unsigned int *SlicesHandler::return_histogram()
{
	return histogram;
}
...
unsigned short SlicesHandler::return_width()
{
	return width;
}

unsigned short SlicesHandler::return_height()
{
	return height;
}

unsigned short SlicesHandler::return_nrslices()
{
	return nrslices;
}

unsigned short SlicesHandler::return_startslice()
{
	return startslice;
}

unsigned short SlicesHandler::return_endslice()
{
	return endslice;
}
...
void SlicesHandler::fill_gapstissue(int minsize,bool connectivity)
{
	...
	for (int i = startslice; i < iN; i++)
	{
		(image_slices[i]).fill_gapstissue(active_tissuelayer, minsize, connectivity);
	}
}
...
{
	// Top
	tissues_size_t *tmp = &(image_slices[startslice].return_tissues(active_tissuelayer)[0]);
	for(unsigned pos=0;pos<area;pos++,tmp++){
		if(*tmp==value) {
			return true;
		}
	...
	// Sides
	for(unsigned short i=startslice+1;i<endslice-1;i++) {
		if ((image_slices[i]).tissuevalue_at_boundary(active_tissuelayer, value)) {
			return true;
		}
	}
...
void SlicesHandler::kmeans_mhd(unsigned short slicenr,short nrtissues,short dim,std::vector<std::string> mhdfiles,float *weights,unsigned int iternr,unsigned int converge)
{
	if(mhdfiles.size()+1<dim) return;
	if(slicenr>=startslice&&slicenr<endslice){
		k_means kmeans;
		...
		for(unsigned short j=0;j+1<dim;j++) {
			...
			if(bits[j+1]==NULL) {
				for(unsigned short i=1;i<j;i++) delete[] bits[i];
				delete[] bits;
				return;
			}
		}
		...
		for(unsigned short i=0;i+1<dim;i++) {
			if(!ImageReader::getSlice(mhdfiles[i].c_str(),bits[i+1],slicenr,width,height)) {
				for(unsigned short j=1;j<dim;j++) delete[] bits[j];
				delete[] bits;
				return;
			}
		}
		...
		for(unsigned short i=startslice;i<slicenr;i++){
			...
			for(unsigned short k=0;k+1<dim;k++) {
				if(!ImageReader::getSlice(mhdfiles[k].c_str(),bits[k+1],i,width,height)) {
					for(unsigned short j=1;j<dim;j++) delete[] bits[j];
					delete[] bits;
					return;
				}
...
void SlicesHandler::kmeans_png(unsigned short slicenr,short nrtissues,short dim,std::vector<std::string> pngfiles,std::vector<int> exctractChannel,float *weights,unsigned int iternr,unsigned int converge,const std::string initCentersFile)
{
	if(pngfiles.size()+1<dim) return;
	if(slicenr>=startslice&&slicenr<endslice){
		float **bits=new float*[dim];
		for(unsigned short j=0;j+1<dim;j++) {
			bits[j+1]=new float[area];
		...
		if(initCentersFile!="")
		{
			...
			if (kmeans.get_centers_from_file(initCentersFile, centers, dimensions, nrClasses))
			{
				dim = dimensions;
				nrtissues = nrClasses;
				kmeans.init(width,height,nrtissues,dim,bits,weights,centers);
			}
			else
			{
				QMessageBox msgBox;
		...
		for(unsigned short i=startslice;i<slicenr;i++){
			bits[0]=image_slices[i].return_bmp();
			for(unsigned short k=0;k+1<dim;k++) {
				if(!ChannelExtractor::getSlice(pngfiles[0].c_str(),bits[i+1],exctractChannel[i],i,width,height)) {
					for(unsigned short j=1;j<dim;j++) delete[] bits[j];
					delete[] bits;
					return;
				}
...
void SlicesHandler::aniso_diff(float dt, int n,float (*f)(float,float), float k, float restraint)
{
	for(unsigned short i=startslice;i<endslice;i++)
...
void SlicesHandler::cont_anisodiff(float dt, int n,float (*f)(float,float), float k, float restraint)
{
	for(unsigned short i=startslice;i<endslice;i++)
...
void SlicesHandler::extractinterpolatesave_contours(int minsize, std::vector<tissues_size_t> &tissuevec,unsigned short between,bool dp,float epsilon,const char *filename)
{
/*	os.clear();
	...
	dummy3D.newbmp((unsigned short)(image_slices[0].return_width()),(unsigned short)(image_slices[0].return_height()),between+2);
	dummy3D.set_slicethickness(thickness/(between+1));
	Pair pair1=get_pixelsize();
	dummy3D.set_pixelsize(pair1.high,pair1.low);

	FILE *fp=dummy3D.save_contourprologue(filename,(between+1)*nrslices-between);

	for(unsigned short j=0;j+1<nrslices;j++){
		dummy3D.copy2tissue(0,image_slices[j].return_tissues(active_tissuelayer));
		dummy3D.copy2tissue(between+1,image_slices[j+1].return_tissues(active_tissuelayer));
		...
		if(dp) dummy3D.dougpeuck_line(epsilon);
...
void SlicesHandler::extractinterpolatesave_contours2_xmirrored(int minsize, std::vector<tissues_size_t> &tissuevec,unsigned short between,bool dp,float epsilon,const char *filename)
{
	/*	os.clear();
	os.set_sizenr((between+1)*nrslices-between);
	os.set_thickness(thickness/(between+1));*/
...
//			dummy3D.dougpeuck_line(epsilon*2);
		...
		dummy3D.shift_contours(-(int)dummy3D.return_width(),-(int)dummy3D.return_height());
		fp=dummy3D.save_contoursection(fp,0,between,(between+1)*j);
	}
...
void SlicesHandler::extract_contours(int minsize, std::vector<tissues_size_t> &tissuevec)
{
	...
	for(std::vector<tissues_size_t>::iterator it1=tissuevec.begin();it1!=tissuevec.end();it1++)
	{
		for(unsigned short i=0;i<nrslices;i++){
			v1.clear();
			v2.clear();
			(image_slices[i]).get_tissuecontours(active_tissuelayer,*it1,&v1,&v2,minsize);
			for(std::vector<std::vector<Point> >::iterator it=v1.begin();it!=v1.end();it++){
	//			Pointconvert(*it,vP);
	//			os.add_line(i,tissuetype,&vP,true);
				os.add_line(i,*it1,&(*it),true);
			}
			for(std::vector<std::vector<Point> >::iterator it=v2.begin();it!=v2.end();it++){
	//			Pointconvert(*it,vP);
...
void SlicesHandler::extract_contours2_xmirrored(int minsize, std::vector<tissues_size_t> &tissuevec)
{
	...
	for(std::vector<tissues_size_t>::iterator it1=tissuevec.begin();it1!=tissuevec.end();it1++)
	{
		for(unsigned short i=0;i<nrslices;i++){
			v1.clear();
			v2.clear();
			(image_slices[i]).get_tissuecontours2_xmirrored(active_tissuelayer,*it1,&v1,&v2,minsize);
			for(std::vector<std::vector<Point> >::iterator it=v1.begin();it!=v1.end();it++){
				//			Pointconvert(*it,vP);
				//			os.add_line(i,tissuetype,&vP,true);
				os.add_line(i,*it1,&(*it),true);
...
void SlicesHandler::extract_contours2_xmirrored(int minsize, std::vector<tissues_size_t> &tissuevec,float epsilon)
{
	...
	for(std::vector<tissues_size_t>::iterator it1=tissuevec.begin();it1!=tissuevec.end();it1++)
	{
		for(unsigned short i=0;i<nrslices;i++){
			v1.clear();
			v2.clear();
			(image_slices[i]).get_tissuecontours2_xmirrored(active_tissuelayer,*it1,&v1,&v2,minsize,epsilon);
			for(std::vector<std::vector<Point> >::iterator it=v1.begin();it!=v1.end();it++){
				//			Pointconvert(*it,vP);
				//			os.add_line(i,tissuetype,&vP,true);
				os.add_line(i,*it1,&(*it),true);
...
void SlicesHandler::extract_contours(float f,int minsize, tissues_size_t tissuetype)
{
//	os.clear();
	std::vector<std::vector<Point> > v1,v2;
	std::vector<Point_type> vP;
...
void SlicesHandler::get_range(Pair *pp)
{
	...
	for(unsigned short i=startslice+1;i<endslice;i++){
		...
		if((*pp).high<p.high) (*pp).high=p.high;
		if((*pp).low>p.low) (*pp).low=p.low;
	}
	...
	#pragma omp parallel
	{
		...
		for (int i = 0; i < iN; i++)
		{
			if (image_slices[i].return_mode(false) == 1)
			{
				(image_slices[i]).get_range(&p);
				slice_ranges[i] = p;
				if (high < p.high) high = p.high;
				if (low > p.low) low = p.low;
			}
		}

		#pragma omp critical
		{
			if (low < pp->low) pp->low = low;
			if (high > pp->high) pp->high = high;
		}
	}
...
void SlicesHandler::compute_range_mode1(unsigned short updateSlicenr, Pair *pp)
{
	...
	pp->low = FLT_MAX;
	for (unsigned short i = 0; i < nrslices; ++i) {
		if (image_slices[i].return_mode(false) != 1) continue;
...
void SlicesHandler::compute_bmprange_mode1(unsigned short updateSlicenr, Pair *pp)
{
	...
	if (image_slices[updateSlicenr].return_mode(true) == 1) {
		(image_slices[updateSlicenr]).get_bmprange(&slice_bmpranges[updateSlicenr]);
	}
	...
	pp->low = FLT_MAX;
	for (unsigned short i = 0; i < nrslices; ++i) {
		if (image_slices[i].return_mode(true) != 1) continue;
...
{
	os.printsection(fp, startslice1, endslice1, offset, TissueInfos::GetTissueCount());
	return fp;
}
...
FILE *SlicesHandler::save_tissuenamescolors(FILE *fp)
{
	...
	if (tissueCount > 255) { // Only print tissue indices which contain outlines
		...
		for (idxIt=tissueIndices.begin();idxIt!=tissueIndices.end();++idxIt) {
			tissueInfo = TissueInfos::GetTissueInfo(*idxIt);
			fprintf(fp,"T%i %f %f %f %s\n",(int)*idxIt,tissueInfo->color[0],tissueInfo->color[1],tissueInfo->color[2],tissueInfo->name.ascii());
		}
	} else {
		// Print infos of all tissues
		for(unsigned i=1;i<=tissueCount;i++){
			tissueInfo = TissueInfos::GetTissueInfo(i);
			fprintf(fp,"T%i %f %f %f %s\n",(int)i,tissueInfo->color[0],tissueInfo->color[1],tissueInfo->color[2],tissueInfo->name.ascii());
		}
	}
...
void SlicesHandler::hysteretic(float thresh_low, float thresh_high, bool connectivity, unsigned short nrpasses)
{
	...
	image_slices[slicenr].hysteretic(thresh_low,thresh_high,connectivity,setvalue);
	for (unsigned short i = 0; i < nrpasses; i++) 
	{
		while (++slicenr < endslice) {
			image_slices[slicenr].hysteretic(thresh_low, thresh_high, connectivity, image_slices[slicenr - 1].return_work(), setvalue - 1, setvalue);
		}
		setvalue++;
		slicenr--;
...
void SlicesHandler::thresholded_growing(short unsigned slicenr, Point p, float threshfactor_low, float threshfactor_high, bool connectivity, unsigned short nrpasses)
{
	if(slicenr<endslice&&slicenr>=startslice){
		...
		image_slices[slicenr].thresholded_growing(p,threshfactor_low,threshfactor_high,connectivity,setvalue,&tp);

		for(unsigned short i=0;i<nrpasses;i++)
		{
			while(++slicenr<endslice){
				image_slices[slicenr].thresholded_growing(tp.low,tp.high,connectivity,image_slices[slicenr-1].return_work(),setvalue-1,setvalue);
			}
			setvalue++;
			slicenr--;
			while(slicenr-->startslice){
...
void SlicesHandler::thresholded_growing(short unsigned slicenr, Point p, float thresh_low, float thresh_high,float set_to) //bool connectivity,float set_to)
{
	if(slicenr>=startslice&&slicenr<endslice){
		...
		for(unsigned short z=startslice;z<endslice;z++){
			...
			for(unsigned short j=0;j<height;j++) {
				for(unsigned short k=0;k<width;k++){
					if(bmp[i]>thresh_high||bmp[i]<thresh_low) work[i]=0;
					else work[i]=-1;
					i++;
				}
			}
		...
		for(unsigned short z=startslice;z<endslice;z++){
			...
			for(unsigned i1=0;i1<area;i1++) if(work[i1]==-1) work[i1]=0;
...
void SlicesHandler::add2tissueall_connected(tissues_size_t tissuetype, Point p,bool override)
{
	if(activeslice>=startslice&&activeslice<endslice){
		...
		tissues_size_t *tissue=image_slices[activeslice].return_tissues(active_tissuelayer);
		bool tissueLocked = TissueInfos::GetTissueLocked(tissue[position]);
		if(tissue[position]==0||(override&&tissueLocked==false)) tissue[position]=tissuetype;
		if(tissue[position]==0||(override&&tissueLocked==false)) work[position]=set_to;
		...
		while(!s.empty()){
			...
			if(i.pxy%width!=0&&work[i.pxy-1]==f&&(tissue[i.pxy-1]==0||(override&&TissueInfos::GetTissueLocked(tissue[i.pxy-1])==false))) {
				work[i.pxy-1]=set_to; 
			...
			if((i.pxy+1)%width!=0&&work[i.pxy+1]==f&&(tissue[i.pxy+1]==0||(override&&TissueInfos::GetTissueLocked(tissue[i.pxy+1])==false))) {
				work[i.pxy+1]=set_to; 
				tissue[i.pxy+1]=tissuetype;
				j.pxy=i.pxy+1;
				j.pz=i.pz;
			...
			if(i.pxy>=width&&work[i.pxy-width]==f&&(tissue[i.pxy-width]==0||(override&&TissueInfos::GetTissueLocked(tissue[i.pxy-width])==false))) {
				work[i.pxy-width]=set_to; 
			...
			if(i.pxy<area-width&&work[i.pxy+width]==f&&(tissue[i.pxy+width]==0||(override&&TissueInfos::GetTissueLocked(tissue[i.pxy+width])==false))) {
				work[i.pxy+width]=set_to; 
				tissue[i.pxy+width]=tissuetype;
				j.pxy=i.pxy+width;
				j.pz=i.pz;
			...
			if(i.pz>startslice){
				...
				if(work[i.pxy]==f&&(tissue[i.pxy]==0||(override&&TissueInfos::GetTissueLocked(tissue[i.pxy])==false))) {
					work[i.pxy]=set_to; 
		...
		for(unsigned short z=startslice;z<endslice;z++){
			work=image_slices[z].return_work();
			for(unsigned i1=0;i1<area;i1++) if(work[i1]==set_to) work[i1]=f;
		}
	}
...
void SlicesHandler::subtract_tissueall_connected(tissues_size_t tissuetype, Point p)
{
	if(activeslice<endslice&&activeslice>=startslice){
		...
		float f=work[position];
		tissues_size_t *tissue=image_slices[activeslice].return_tissues(active_tissuelayer);
		if(tissue[position]==tissuetype) tissue[position]=tissuetype;
		if(tissue[position]==tissuetype) work[position]=set_to;
		...
		while(!s.empty()){
			...
			work=image_slices[i.pz].return_work();
			tissue=image_slices[i.pz].return_tissues(active_tissuelayer);
			if(i.pxy%width!=0&&work[i.pxy-1]==f&&tissue[i.pxy-1]==tissuetype) {
				work[i.pxy-1]=set_to; 
				tissue[i.pxy-1]=0;
				j.pxy=i.pxy-1;
				j.pz=i.pz;
			...
			if((i.pxy+1)%width!=0&&work[i.pxy+1]==f&&tissue[i.pxy+1]==tissuetype) {
				work[i.pxy+1]=set_to; 
				tissue[i.pxy+1]=0;
				j.pxy=i.pxy+1;
				j.pz=i.pz;
			...
			if(i.pxy>=width&&work[i.pxy-width]==f&&tissue[i.pxy-width]==tissuetype) {
				work[i.pxy-width]=set_to; 
			...
			if(i.pxy<area-width&&work[i.pxy+width]==f&&tissue[i.pxy+width]==tissuetype) {
				work[i.pxy+width]=set_to; 
				tissue[i.pxy+width]=0;
				j.pxy=i.pxy+width;
				j.pz=i.pz;
...
void SlicesHandler::double_hysteretic(float thresh_low_l, float thresh_low_h, float thresh_high_l, float thresh_high_h, bool connectivity, unsigned short nrpasses)
{
	float setvalue=255;
	unsigned short slicenr=startslice;
	...
	image_slices[slicenr].double_hysteretic(thresh_low_l,thresh_low_h,thresh_high_l,thresh_high_h,connectivity,setvalue);
//	if(nrslices>1) {
		for(unsigned short i=0;i<nrpasses;i++){
			while(++slicenr<endslice){
				image_slices[slicenr].double_hysteretic(thresh_low_l,thresh_low_h,thresh_high_l,thresh_high_h,connectivity,image_slices[slicenr-1].return_work(),setvalue-1,setvalue);
			}
			setvalue++;
			slicenr--;
			while(slicenr-->startslice){
...
void SlicesHandler::double_hysteretic_allslices(float thresh_low_l, float thresh_low_h, float thresh_high_l, float thresh_high_h, bool connectivity,float set_to)
{
	for(unsigned short i=startslice;i<endslice;i++){
...
void SlicesHandler::erosion(int n, bool connectivity)
{
	...
	auto output = morpho::MorphologicalOperation<float>(all_slices, ball, morpho::kErode, startslice, endslice);
...
void SlicesHandler::dilation(int n, bool connectivity)
{
	...
	auto output = morpho::MorphologicalOperation<float>(all_slices, ball, morpho::kDilate, startslice, endslice);
...
void SlicesHandler::closure(int n,bool connectivity)
{
	...
	auto output = morpho::MorphologicalOperation<float>(all_slices, ball, morpho::kClose, startslice, endslice);
...
void SlicesHandler::open(int n,bool connectivity)
{
	...
	auto output = morpho::MorphologicalOperation<float>(all_slices, ball, morpho::kOpen, startslice, endslice);
...
void SlicesHandler::interpolateworkgrey_medianset(unsigned short slice1, unsigned short slice2, bool connectivity, bool handleVanishingComp)
{
	...
	if(n>1){
		...
		if (handleVanishingComp) {
			...
			float *connected_comp_forward=(float *)malloc(sizeof(float)*area);
			float *connected_comp_backward=(float *)malloc(sizeof(float)*area);

			image_slices[slice1].connected_components(false, vanishing_comp_forward); // TODO: connectivity?
			image_slices[slice1].copyfromwork(connected_comp_forward);
			
			image_slices[slice2].connected_components(false, vanishing_comp_backward); // TODO: connectivity?
			image_slices[slice2].copyfromwork(connected_comp_backward);

			for (unsigned int i = 0; i < area; ++i) {
				if (f_1[i] == f_2[i]) {
					std::set<float>::iterator iter = vanishing_comp_forward.find(connected_comp_forward[i]);
		...
		if (handleVanishingComp) {
			...
			// Recursion
			interpolateworkgrey_medianset(slice1, slicehalf, connectivity, false);
			interpolateworkgrey_medianset(slicehalf, slice2, connectivity, false);
...
void SlicesHandler::interpolatetissuegrey(unsigned short slice1, unsigned short slice2)
{
	...
	if(n>0){
		...
		tissues_size_t *bmp1=image_slices[slice1].return_tissues(active_tissuelayer);
		tissues_size_t *bmp2=image_slices[slice2].return_tissues(active_tissuelayer);
		float *work1=image_slices[slice1].return_work();
		float *work2=image_slices[slice2].return_work();
		...
		for(p.py=0;p.py<height;p.py++){
			for(p.px=0;p.px<width;p.px++){
				...
				n1=(unsigned short)n*prop;
				for(unsigned short j=1;j<=n1&&j<n;j++) {
					image_slices[slice1+j].set_tissue_pt(active_tissuelayer,p,bmp1[i1]);
				}
				for(unsigned short j=n1+1;j<n;j++) {
					image_slices[slice1+j].set_tissue_pt(active_tissuelayer,p,bmp2[i1]);
				}
				i1++;
			}
		}
...
void SlicesHandler::interpolatetissuegrey_medianset(unsigned short slice1, unsigned short slice2, bool connectivity, bool handleVanishingComp)
{
	// Beucher et al.: "Sets, Partitions and Functions Interpolations"
	// Modified algorithm: Do not interplate vanishing components
	...
	if(n>1){
		...
		if (handleVanishingComp) {
			tissue1_copy=(tissues_size_t *)malloc(sizeof(tissues_size_t)*area);
			image_slices[slice1].copyfromtissue(active_tissuelayer, tissue1_copy);
			tissue2_copy=(tissues_size_t *)malloc(sizeof(tissues_size_t)*area);
			image_slices[slice2].copyfromtissue(active_tissuelayer, tissue2_copy);
		}
...
void SlicesHandler::interpolatetissue(unsigned short slice1, unsigned short slice2, tissues_size_t tissuetype)
{
	if(slice2<slice1){
		unsigned short dummy=slice1;
		slice1=slice2;
	...
	if(n!=0){
		for(p.py=0;p.py<height;p.py++){
			for(p.px=0;p.px<width;p.px++){
				...
				for(unsigned short j=1;j<n;j++) {
					if(bmp1[i1]+delta*j>=0)image_slices[slice1+j].set_work_pt(p,255.0f);
					else image_slices[slice1+j].set_work_pt(p,0.0f);
				}
				i1++;
			}
		}
...
void SlicesHandler::interpolatetissue_medianset(unsigned short slice1, unsigned short slice2, tissues_size_t tissuetype, bool connectivity, bool handleVanishingComp)
{
...
void SlicesHandler::extrapolatetissue(unsigned short origin1, unsigned short origin2, unsigned short target, tissues_size_t tissuetype)
{
	...
	if(n!=0){
		for(p.py=0;p.py<height;p.py++){
			for(p.px=0;p.px<width;p.px++){
				delta=(bmp2[i1]-bmp1[i1])/n;
				if(bmp1[i1]+delta*(target-origin1)>=0)image_slices[target].set_work_pt(p,255.0f);
				else image_slices[target].set_work_pt(p,0.0f);
				i1++;
			}
		}
	}
...
void SlicesHandler::interpolatework(unsigned short slice1, unsigned short slice2)
{
	...
	for(unsigned int i=0;i<area;i++){
		if(work1[i]!=0) bmp1[i]=255.0f;
		else bmp1[i]=0.0f;
		if(work2[i]!=0) bmp2[i]=255.0f;
		else bmp2[i]=0.0f;
	}
	...
	if(n!=0){
		for(p.py=0;p.py<height;p.py++){
			for(p.px=0;p.px<width;p.px++){
				...
				for(unsigned short j=1;j<n;j++) image_slices[slice1+j].set_work_pt(p,bmp1[i]+delta*j);
				i++;
			}
		}
	}
...
void SlicesHandler::extrapolate(unsigned short origin1, unsigned short origin2, unsigned short target)
{
	...
	for(p.py=0;p.py<height;p.py++){
		for(p.px=0;p.px<width;p.px++){
			delta=(bmp2[i]-bmp1[i])/n;
			image_slices[target].set_work_pt(p,bmp1[i]+delta*(target-origin1));
			i++;
		}
	}
...
void SlicesHandler::interpolate(unsigned short slice1, unsigned short slice2, float *bmp1, float *bmp2)
{
	...
	for(p.py=0;p.py<height;p.py++){
		for(p.px=0;p.px<width;p.px++){
			delta=(bmp2[i]-bmp1[i])/n;
			for(unsigned short j=0;j<=n;j++) image_slices[slice1+j].set_work_pt(p,bmp1[i]+delta*j);
			i++;
		}
	}
...
float SlicesHandler::get_slicethickness()
{
	return thickness;
}
...
Transform SlicesHandler::get_transform() const
{
	return transform;
}
...
void SlicesHandler::set_transform(const Transform& tr)
{
	transform = tr;
}
...
void SlicesHandler::slicetissue_x(tissues_size_t *return_bits, unsigned short xcoord)
{
	unsigned n=0;
...
void SlicesHandler::slicetissue_y(tissues_size_t *return_bits, unsigned short ycoord)
{	
	unsigned n=0;
	tissues_size_t *dummy;
...
{
	tissues_size_t *result=(tissues_size_t *)malloc(sizeof(tissues_size_t)*unsigned(height)*nrslices);
...
{	
	tissues_size_t *result=(tissues_size_t *)malloc(sizeof(tissues_size_t)*unsigned(width)*nrslices);
...
int SlicesHandler::extract_tissue_surfaces(const QString& filename, std::vector<tissues_size_t> &tissuevec, 
	bool usediscretemc, float ratio, unsigned smoothingiterations, float passBand, float featureAngle)
{
	int error_counter = 0;
	...
	vtkSmartPointer<vtkImageData> labelField = vtkSmartPointer<vtkImageData>::New();
	labelField->SetExtent(0,(int)return_width()+1,0,(int)return_height()+1,0,(int)(endslice-startslice)+1);
	...
	vtkSmartPointer<vtkStringArray> names_array = vtkSmartPointer<vtkStringArray>::New();
	...
	{
		copyfromtissuepadded(i, &(field[(nrSlice+1)*(unsigned long long)(return_width()+2)*(return_height()+2)]), padding);
	}
	for(unsigned long long i=0; i<(unsigned long long)(return_width()+2)*(return_height()+2); i++)
		field[i]=0;
	for(unsigned long long i=(endslice-startslice+1)*(unsigned long long)(return_width()+2)*(return_height()+2); i<(endslice-startslice+2)*(unsigned long long)(return_width()+2)*(return_height()+2); i++)
		field[i]=0;
	...
	vtkSmartPointer<vtkImageExtractCompatibleMesher> contour = vtkSmartPointer<vtkImageExtractCompatibleMesher>::New();
	vtkSmartPointer<vtkDiscreteMarchingCubes> cubes = vtkSmartPointer<vtkDiscreteMarchingCubes>::New();
	vtkSmartPointer<vtkWindowedSincPolyDataFilter> smoother = vtkSmartPointer<vtkWindowedSincPolyDataFilter>::New();
	vtkSmartPointer<vtkEdgeCollapse> simplify = vtkSmartPointer<vtkEdgeCollapse>::New();
	...
	if(usediscretemc)
	{
		...
		for(size_t i=0;i<tissuevec.size();i++) cubes->SetValue((int)i,tissuevec[i]);
		cubes->Update();
	...
	if(targetReduction > 0.05)
	{
		...
		double cellSize   = 0.5 * std::sqrt( vtkMath::Distance2BetweenPoints(x0,x1) );
		...
		if( tissuevec.size() == 1 )
		{
			vtkSmartPointer<vtkStringArray> names_array_1 = vtkSmartPointer<vtkStringArray>::New();
			names_array_1->SetNumberOfTuples(1);
			names_array_1->SetName(tissueNameArrayName);

			vtkSmartPointer<vtkFloatArray> color_array_1 = vtkSmartPointer<vtkFloatArray>::New();
			...
			for (tissues_size_t i=1; i<num_tissues; i++)
			{
				...
				if( i == tissuevec[0] )
				{
					names_array_1->SetValue(0, TissueInfos::GetTissueName(i).ascii());
	...
	elems.back() = 1.0;
	double padding_displacement[3] = { -padding*ps.high, -padding*ps.low, -padding*thickness + thickness*startslice };
	for (int i=0; i<3; i++)
	{
...
void SlicesHandler::triangulate(const char *filename, std::vector<tissues_size_t> &tissuevec)
{
	tissues_size_t **tissuebits=(tissues_size_t **)malloc(sizeof(tissues_size_t *)*(endslice-startslice));
	for(unsigned short i=startslice;i<endslice;i++)
		tissuebits[i-startslice]=image_slices[i].return_tissues(active_tissuelayer);

	std::vector<std::string> vstring;
	std::vector<rgbs> colorvec;
	TissueInfoStruct *tissueInfo;
	for(std::vector<tissues_size_t>::iterator it=tissuevec.begin();it!=tissuevec.end();it++){
	...
	marchingcube mc;
	mc.init(tissuebits,width, height, endslice-startslice,thickness,dx,dy);
...
void SlicesHandler::triangulate(const char *filename, std::vector<tissues_size_t> &tissuevec, std::vector<rgbs> &colorvec)
{
	tissues_size_t **tissuebits=(tissues_size_t **)malloc(sizeof(tissues_size_t *)*(endslice-startslice));
	for(unsigned short i=startslice;i<endslice;i++)
		tissuebits[i-startslice]=image_slices[i].return_tissues(active_tissuelayer);
	...
	std::vector<std::string> vstring;
	for(std::vector<tissues_size_t>::iterator it=tissuevec.begin();it!=tissuevec.end();it++){
		vstring.push_back((std::string)TissueInfos::GetTissueName(*it+1)); // TODO: why +1???
	}
...
void SlicesHandler::triangulatesimpl(const char *filename, std::vector<tissues_size_t> &tissuevec, float ratio)
{
...
{
	image_slices[activeslice].add2tissue(active_tissuelayer,tissuetype,p,override);
}

void SlicesHandler::add2tissue(tissues_size_t tissuetype, bool *mask, unsigned short slicenr,bool override)
{
	image_slices[slicenr].add2tissue(active_tissuelayer,tissuetype,mask,override);
}
...
{
	image_slices[activeslice].add2tissue_connected(active_tissuelayer,tissuetype,p,override);
}

void SlicesHandler::add2tissue_thresh(tissues_size_t tissuetype, Point p)
{
	image_slices[activeslice].add2tissue_thresh(active_tissuelayer,tissuetype,p);
}

void SlicesHandler::subtract_tissue(tissues_size_t tissuetype, Point p)
{
	image_slices[activeslice].subtract_tissue(active_tissuelayer,tissuetype,p);
}
...
void SlicesHandler::subtract_tissueall(tissues_size_t tissuetype, Point p, unsigned short slicenr)
{
...
void SlicesHandler::subtract_tissue_connected(tissues_size_t tissuetype, Point p)
{
	image_slices[activeslice].subtract_tissue_connected(active_tissuelayer,tissuetype,p);
}
...
void SlicesHandler::selectedtissue2mc(tissues_size_t tissuetype,unsigned char ** voxels)
{
	for(unsigned short i=startslice;i<endslice;i++){
		int k=i-startslice;
...
unsigned short SlicesHandler::get_next_featuring_slice(tissues_size_t type, bool &found)
{
	found = true;
...
unsigned short SlicesHandler::get_activeslice()
{
	return activeslice;
}
...
void SlicesHandler::clear_stack()
{
	get_activebmphandler()->clear_stack();
}
...
void SlicesHandler::getstack_tissue(unsigned i,tissues_size_t tissuenr,bool override)
{
	get_activebmphandler()->getstack_tissue(active_tissuelayer,i,tissuenr,override);
}

void SlicesHandler::getstack_tissue(unsigned int slice, unsigned i,tissues_size_t tissuenr,bool override)
{
	image_slices[slice].getstack_tissue(active_tissuelayer,i,tissuenr,override);
}
...
void SlicesHandler::popstack_bmp()
{
	get_activebmphandler()->popstack_bmp();
}

void SlicesHandler::popstack_work()
{
	get_activebmphandler()->popstack_work();
}

void SlicesHandler::popstack_help()
{
	get_activebmphandler()->popstack_help();
}
...
void SlicesHandler::start_undo(common::DataSelection &dataSelection)
{
	if(uelem==NULL){
		uelem=new UndoElem;
		...
		if(dataSelection.bmp) {
			...
			uelem->mode1_old=image_slices[dataSelection.sliceNr].return_mode(true);
		} else uelem->bmp_old=NULL;

		if(dataSelection.work) {
			uelem->work_old=image_slices[dataSelection.sliceNr].copy_work();
			uelem->mode2_old=image_slices[dataSelection.sliceNr].return_mode(false);
		} else uelem->work_old=NULL;

		if(dataSelection.tissues) uelem->tissue_old=image_slices[dataSelection.sliceNr].copy_tissue(active_tissuelayer);
		else uelem->tissue_old=NULL;

		uelem->vvm_old.clear();
		if(dataSelection.vvm) uelem->vvm_old=*(image_slices[dataSelection.sliceNr].return_vvm());

		uelem->limits_old.clear();
		if(dataSelection.limits) uelem->limits_old=*(image_slices[dataSelection.sliceNr].return_limits());

		uelem->marks_old.clear();
		if(dataSelection.marks) uelem->marks_old=*(image_slices[dataSelection.sliceNr].return_marks());
	}

	return;
}

bool SlicesHandler::start_undoall(common::DataSelection &dataSelection)
{
...
bool SlicesHandler::start_undo(common::DataSelection &dataSelection,std::vector<unsigned> vslicenr1)
{
	if(uelem==NULL){
		...
		if(uelem->arraynr()<this->undoQueue.return_nrundoarraysmax()){
			...
			if(dataSelection.bmp)
				for(it=vslicenr1.begin();it!=vslicenr1.end();it++) {
					...
					uelem1->vmode1_old.push_back(image_slices[*it].return_mode(true));
				}
			...
			if(dataSelection.work)
				for(it=vslicenr1.begin();it!=vslicenr1.end();it++) {
					...
					uelem1->vmode2_old.push_back(image_slices[*it].return_mode(false));
				}
			uelem1->vtissue_old.clear();
			if(dataSelection.tissues)
				for(it=vslicenr1.begin();it!=vslicenr1.end();it++)
					uelem1->vtissue_old.push_back(image_slices[*it].copy_tissue(active_tissuelayer));
			uelem1->vvvm_old.clear();
			if(dataSelection.vvm)
				for(it=vslicenr1.begin();it!=vslicenr1.end();it++)
					uelem1->vvvm_old.push_back(*(image_slices[*it].return_vvm()));
			uelem1->vlimits_old.clear();
			if(dataSelection.limits)
				for(it=vslicenr1.begin();it!=vslicenr1.end();it++)
					uelem1->vlimits_old.push_back(*(image_slices[*it].return_limits()));
			uelem1->marks_old.clear();
			if(dataSelection.marks)
				for(it=vslicenr1.begin();it!=vslicenr1.end();it++)
					uelem1->vmarks_old.push_back(*(image_slices[*it].return_marks()));
...
void SlicesHandler::merge_undo()
{
	if(uelem!=NULL&&!uelem->multi){
		common::DataSelection dataSelection=uelem->dataSelection;
/*		unsigned short slicenr1=uelem->slicenr;
...
common::DataSelection SlicesHandler::undo()
{
	if(uelem==NULL){
		...
		if(uelem->multi){
			...
			if(uelem1!=NULL){
				...
				for(unsigned i=0;i<uelem1->vslicenr.size();i++) {
					...
					if(dataSelection.bmp) {
						uelem1->vbmp_new.push_back(image_slices[current_slice].copy_bmp());
						uelem1->vmode1_new.push_back(image_slices[current_slice].return_mode(true));
						image_slices[current_slice].copy2bmp(uelem1->vbmp_old[i],uelem1->vmode1_old[i]);
						free(uelem1->vbmp_old[i]);
					}
					if(dataSelection.work) {
						uelem1->vwork_new.push_back(image_slices[current_slice].copy_work());
						uelem1->vmode2_new.push_back(image_slices[current_slice].return_mode(false));
						image_slices[current_slice].copy2work(uelem1->vwork_old[i],uelem1->vmode2_old[i]);
						free(uelem1->vwork_old[i]);
					}
					if(dataSelection.tissues) {
						uelem1->vtissue_new.push_back(image_slices[current_slice].copy_tissue(active_tissuelayer));
						image_slices[current_slice].copy2tissue(active_tissuelayer, uelem1->vtissue_old[i]);
						free(uelem1->vtissue_old[i]);
					}
					if(dataSelection.vvm) {
						uelem1->vvvm_new.push_back(*(image_slices[current_slice].return_vvm()));
						image_slices[current_slice].copy2vvm(&(uelem1->vvvm_old[i]));
					}
					if(dataSelection.limits) {
						uelem1->vlimits_new.push_back(*(image_slices[current_slice].return_limits()));
						image_slices[current_slice].copy2limits(&(uelem1->vlimits_old[i]));
					}
					if(dataSelection.marks) {
						uelem1->vmarks_new.push_back(*(image_slices[current_slice].return_marks()));
						image_slices[current_slice].copy2marks(&(uelem1->vmarks_old[i]));
					}
				}
				uelem1->vbmp_old.clear();
				uelem1->vmode1_old.clear();
			...
			} else return common::DataSelection();
		} else {
			if(uelem!=NULL){
				common::DataSelection dataSelection=uelem->dataSelection;

				if(dataSelection.bmp) {
					uelem->bmp_new=image_slices[dataSelection.sliceNr].copy_bmp();
					uelem->mode1_new=image_slices[dataSelection.sliceNr].return_mode(true);
					image_slices[dataSelection.sliceNr].copy2bmp(uelem->bmp_old,uelem->mode1_old);
					free(uelem->bmp_old);
					uelem->bmp_old=NULL;
				...
				if(dataSelection.work) {
					uelem->work_new=image_slices[dataSelection.sliceNr].copy_work();
					uelem->mode2_new=image_slices[dataSelection.sliceNr].return_mode(false);
					image_slices[dataSelection.sliceNr].copy2work(uelem->work_old,uelem->mode2_old);
					free(uelem->work_old);
					uelem->work_old=NULL;
				}

				if(dataSelection.tissues) {
					uelem->tissue_new=image_slices[dataSelection.sliceNr].copy_tissue(active_tissuelayer);
					image_slices[dataSelection.sliceNr].copy2tissue(active_tissuelayer, uelem->tissue_old);
				...
				if(dataSelection.vvm) {
					uelem->vvm_new.clear();
					uelem->vvm_new=*(image_slices[dataSelection.sliceNr].return_vvm());
					image_slices[dataSelection.sliceNr].copy2vvm(&uelem->vvm_old);
					uelem->vvm_old.clear();
				}

				if(dataSelection.limits) {
					uelem->limits_new.clear();
					uelem->limits_new=*(image_slices[dataSelection.sliceNr].return_limits());
					image_slices[dataSelection.sliceNr].copy2limits(&uelem->limits_old);
					uelem->limits_old.clear();
				}

				if(dataSelection.marks) {
					uelem->marks_new.clear();
					uelem->marks_new=*(image_slices[dataSelection.sliceNr].return_marks());
					image_slices[dataSelection.sliceNr].copy2marks(&uelem->marks_old);
					uelem->marks_old.clear();
				}
...
common::DataSelection SlicesHandler::redo()
{
	if(uelem==NULL){
		uelem=this->undoQueue.redo();
		if(uelem==NULL) return common::DataSelection();
		if(uelem->multi){
			MultiUndoElem *uelem1=dynamic_cast<MultiUndoElem *>(uelem);
			...
			if(uelem1!=NULL){
				...
				for(unsigned i=0;i<uelem1->vslicenr.size();i++) {
					current_slice=uelem1->vslicenr[i];
					if(dataSelection.bmp) {
						uelem1->vbmp_old.push_back(image_slices[current_slice].copy_bmp());
						uelem1->vmode1_old.push_back(image_slices[current_slice].return_mode(true));
						image_slices[current_slice].copy2bmp(uelem1->vbmp_new[i],uelem1->vmode1_new[i]);
						free(uelem1->vbmp_new[i]);
					}
					if(dataSelection.work) {
						uelem1->vwork_old.push_back(image_slices[current_slice].copy_work());
						uelem1->vmode2_old.push_back(image_slices[current_slice].return_mode(false));
						image_slices[current_slice].copy2work(uelem1->vwork_new[i],uelem1->vmode2_new[i]);
						free(uelem1->vwork_new[i]);
					}
					if(dataSelection.tissues) {
						uelem1->vtissue_old.push_back(image_slices[current_slice].copy_tissue(active_tissuelayer));
						image_slices[current_slice].copy2tissue(active_tissuelayer, uelem1->vtissue_new[i]);
						free(uelem1->vtissue_new[i]);
					}
					if(dataSelection.vvm) {
						uelem1->vvvm_old.push_back(*(image_slices[current_slice].return_vvm()));
						image_slices[current_slice].copy2vvm(&(uelem1->vvvm_new[i]));
					}
					if(dataSelection.limits) {
						uelem1->vlimits_old.push_back(*(image_slices[current_slice].return_limits()));
						image_slices[current_slice].copy2limits(&(uelem1->vlimits_new[i]));
					}
					if(dataSelection.marks) {
						uelem1->vmarks_old.push_back(*(image_slices[current_slice].return_marks()));
						image_slices[current_slice].copy2marks(&(uelem1->vmarks_new[i]));
					}
				}
				uelem1->vbmp_new.clear();
				uelem1->vwork_new.clear();
			...
			if(uelem!=NULL){
				...
				if(dataSelection.bmp) {
					uelem->bmp_old=image_slices[dataSelection.sliceNr].copy_bmp();
					uelem->mode1_old=image_slices[dataSelection.sliceNr].return_mode(true);
					image_slices[dataSelection.sliceNr].copy2bmp(uelem->bmp_new,uelem->mode1_new);
					free(uelem->bmp_new);
					uelem->bmp_new=NULL;
				...
				if(dataSelection.work) {
					uelem->work_old=image_slices[dataSelection.sliceNr].copy_work();
					uelem->mode2_old=image_slices[dataSelection.sliceNr].return_mode(false);
					image_slices[dataSelection.sliceNr].copy2work(uelem->work_new,uelem->mode2_new);
					free(uelem->work_new);
					uelem->work_new=NULL;
				}

				if(dataSelection.tissues) {
					uelem->tissue_old=image_slices[dataSelection.sliceNr].copy_tissue(active_tissuelayer);
					image_slices[dataSelection.sliceNr].copy2tissue(active_tissuelayer, uelem->tissue_new);
				...
				if(dataSelection.vvm) {
					uelem->vvm_old.clear();
					uelem->vvm_old=*(image_slices[dataSelection.sliceNr].return_vvm());
					image_slices[dataSelection.sliceNr].copy2vvm(&uelem->vvm_new);
					uelem->vvm_new.clear();
				}

				if(dataSelection.limits) {
					uelem->limits_old.clear();
					uelem->limits_old=*(image_slices[dataSelection.sliceNr].return_limits());
					image_slices[dataSelection.sliceNr].copy2limits(&uelem->limits_new);
					uelem->limits_new.clear();
				}

				if(dataSelection.marks) {
					uelem->marks_old.clear();
					uelem->marks_old=*(image_slices[dataSelection.sliceNr].return_marks());
					image_slices[dataSelection.sliceNr].copy2marks(&uelem->marks_new);
					uelem->marks_new.clear();
				}
...
void SlicesHandler::reverse_undosliceorder()
{
	...
	if(uelem!=NULL) {
		uelem->dataSelection.sliceNr=nrslices-1-uelem->dataSelection.sliceNr;
	}
...
bool SlicesHandler::return_undo3D()
{
	return undo3D;
}
...
void SlicesHandler::set_undo3D(bool undo3D1)
{
	undo3D=undo3D1;
}

void SlicesHandler::set_undonr(unsigned nr)
{
	this->undoQueue.set_nrundo(nr);
}
		...
		for( int i=0; i<lfilename.size(); i++ )
		{
			if(gdcmvtk_rtstruct::GetSizeUsingGDCM(lfilename[i],a,b,c,d,e,thick1,disp1,dc1))
			{
				if(c>=1)
				{
					...
					(image_slices[i]).LoadArray(&(bits[(unsigned long)(a)*b*0]),a,b);
...
int SlicesHandler::LoadDICOM(std::vector<const char *> lfilename, Point p, unsigned short dx, unsigned short dy)
{
	...
	if(lfilename.size()==1) {
		...
		if(c>1) 
		{
			...
			for(unsigned short i=0;i<nrslices;i++)
			{	
				if((image_slices[i]).LoadArray(&(bits[(unsigned long)(a)*b*i]),a,b,p,dx,dy)) j++;
			}

			free(bits);

			if(j<nrslices) return 0;
	...
	for(unsigned short i=0;i<nrslices;i++)
	{	
		if((image_slices[i]).LoadDICOM(lfilename[i],p,dx,dy)) j++;
	}
...
int SlicesHandler::ReloadDICOM(std::vector<const char *> lfilename)
{
	if((endslice-startslice)==(unsigned short)lfilename.size()){
		...
		DICOMsort(&lfilename);
		for(unsigned short i=startslice;i<endslice;i++) {
			if((image_slices[i]).ReloadDICOM(lfilename[i-startslice])) j++;
		}

		if(j==(startslice-endslice)) return 1;
		else return 0;
		...
		DICOMsort(&lfilename);
		for(unsigned short i=0;i<nrslices;i++) {
			if((image_slices[i]).ReloadDICOM(lfilename[i])) j++;
		}
		...
		if(nrslices==c) {
			...
			for(unsigned short i=startslice;i<endslice;i++)
			{	
				if((image_slices[i]).LoadArray(&(bits[(unsigned long)(a)*b*i]),a,b)) j++;
			}

			free(bits);
			if(j<endslice-startslice) return 0;
			return 1;
		}
   } return 0;
}
...
int SlicesHandler::ReloadDICOM(std::vector<const char *> lfilename, Point p)
{
	if((endslice-startslice)==(unsigned short)lfilename.size()){
		...
		DICOMsort(&lfilename);
		for(unsigned short i=startslice;i<endslice;i++) {
			if((image_slices[i]).ReloadDICOM(lfilename[i-startslice],p)) j++;
		}
		...
		DICOMsort(&lfilename);
		for(unsigned short i=0;i<nrslices;i++) {
			if((image_slices[i]).ReloadDICOM(lfilename[i],p)) j++;
		}
		...
		if(nrslices==c) {
			...
			for(unsigned short i=startslice;i<endslice;i++)
			{	
				if((image_slices[i]).LoadArray(&(bits[(unsigned long)(a)*b*i]),a,b,p,width,height)) j++;
			}
...
float SlicesHandler::DICOMsort(std::vector<const char *> *lfilename)
{
	...
	for(std::vector<const char *>::iterator it=lfilename->begin();it!=lfilename->end();it++){
...
void SlicesHandler::remove_tissue(tissues_size_t tissuenr,tissues_size_t tissuecount1)
{
	for(short unsigned i=0;i<nrslices;i++){
...
bool SlicesHandler::print_tissuemat(const char *filename)
{
	...
	bool ok=matexport::print_matslices(filename,matrix.data(),int(width),int(height),int(nrslices),"iSeg tissue data v1.0",21,"tissuedistrib",13);
	return ok;
}
...
bool SlicesHandler::print_bmpmat(const char *filename)
{
	...
	bool ok=matexport::print_matslices(filename,matrix.data(),int(width),int(height),int(nrslices),"iSeg source data v1.0",21,"sourcedistrib",13);
	return ok;
}
...
bool SlicesHandler::print_workmat(const char *filename)
{
	...
	bool ok=matexport::print_matslices(filename,matrix.data(),int(width),int(height),int(nrslices),"iSeg target data v1.0",21,"targetdistrib",13);
	return ok;
}
...
bool SlicesHandler::print_atlas(const char *filename)
{
	...
	out << (qint32)common::CombineTissuesVersion(1, 1);
	...
	for(tissues_size_t tissuenr=1;tissuenr<=tissueCount;tissuenr++) {
		...
		out << tissueInfo->name << tissueInfo->color[0] << tissueInfo->color[1] << tissueInfo->color[2];
	}
	for(unsigned short i=0;i<nrslices;i++) {
		out.writeRawData((char *)image_slices[i].return_bmp(),(int)area*sizeof(float));
		out.writeRawData((char *)image_slices[i].return_tissues(active_tissuelayer),(int)area*sizeof(tissues_size_t));
	}
...
bool SlicesHandler::print_amascii(const char *filename)
{
	...
	if(streamname.good()){
		...
		streamname << "# CreationDate: Fri Jun 16 14:24:32 2006" << endl << endl << endl;
		streamname << "define Lattice " << width << " " << height << " "  << nrslices << endl << endl;
		...
		for(tissues_size_t tc=0;tc<tissueCount;tc++){
			...
			streamname << "            Color " << tissueInfo->color[0] << " " << tissueInfo->color[1] << " " << tissueInfo->color[2] << "," << endl;
			streamname << "            Id " << tc+2 <<endl;
			streamname << "        }" << endl;
		}
		streamname << "    }" << endl;
		if (tissueCount <= 255) {
			streamname << "    Content \"" << width << "x" << height<< "x" << nrslices<< " byte, uniform coordinates\"," << endl;
		} else {
			streamname << "    Content \"" << width << "x" << height<< "x" << nrslices<< " ushort, uniform coordinates\"," << endl;
		}
		streamname << "    BoundingBox 0 " << width*dx << " 0 " << height*dy << " 0 " << nrslices*thickness << "," << endl;
		...
		streamname << "@1" << endl;
		for(unsigned short i=0;i<nrslices;i++)
			ok&=image_slices[i].print_amascii_slice(active_tissuelayer, streamname);
		streamname << endl;
...
vtkImageData* SlicesHandler::make_vtktissueimage()
{
	...
	labelField->SetOrigin(offset[0], offset[1], offset[2] + thickness*startslice);
	if (TissueInfos::GetTissueCount() <= 255)
	{
		labelField->AllocateScalars(VTK_UNSIGNED_CHAR,1);
		unsigned char *field=(unsigned char *)labelField->GetScalarPointer(0,0,0);
		for(unsigned short i=startslice;i<endslice;i++)
		{
	...
	else if (sizeof(tissues_size_t) == sizeof(unsigned short))
	{
		...
		cerr << "SlicesHandler::make_vtktissueimage: Error, tissues_size_t not implemented!" << endl;
		labelField->Delete();
		return NULL;
	}
	...
	for (tissues_size_t i=1; i<num_tissues; i++)
	{
		...
		names_array->SetValue(i, TissueInfos::GetTissueName(i).ascii());
		float* color = TissueInfos::GetTissueColor(i);
		std::cerr << TissueInfos::GetTissueName(i).ascii() << " " << color[0] << "," << color[1] << "," << color[2] << std::endl;
		color_array->SetTuple(i, color);
	}

	std::cerr << "vtkImageData::GetActualMemorySize = " << labelField->GetActualMemorySize() << " KB" << std::endl;
	return labelField;
}
...
bool SlicesHandler::export_tissue(const char *filename, bool binary) const
{
	...
	float spacing[3] = { dx, dy, thickness };
	return ImageWriter(binary).writeVolume(filename, slices.data(), width, height, nrslices, spacing, transform);
}
...
bool SlicesHandler::print_xmlregionextent(const char *filename,bool onlyactiveslices,const char *projname)
{
	float offset[3];
	get_displacement(offset);
	...
	if(streamname.good()){
		...
		for(tissues_size_t tissuenr=1;tissuenr<=tissueCount;tissuenr++) {
			if(get_extent(tissuenr,onlyactiveslices,extent)) {
				tissueInfo = TissueInfos::GetTissueInfo(tissuenr);
				streamname << "\t<label id=\"" << (int)tissuenr << "\" name=\"" << tissueInfo->name.ascii() << "\" color=\"" << tissueInfo->color[0] << " " << tissueInfo->color[1] << " " << tissueInfo->color[2] <<"\">" << endl;
				streamname << "\t\t<dataset filename=\"";
				if(projname!=NULL) {
					streamname << projname ;
				}
				streamname << "\" extent=\""<<extent[0][0] << " " << extent[0][1] << " " << extent[1][0] << " " << extent[1][1] << " " << extent[2][0] << " " << extent[2][1];
				streamname << "\" global_bounds=\""<<extent[0][0]*dx+ offset[0] << " " << extent[0][1]*dx+ offset[0] << " " << extent[1][0]*dy+ offset[1] << " " << extent[1][1]*dy+ offset[1] << " " << extent[2][0]*thickness+ offset[2] << " " << extent[2][1]*thickness+ offset[2] << "\">"  << endl;
...
bool SlicesHandler::print_tissueindex(const char *filename,bool onlyactiveslices,const char *projname)
{
	std::ofstream streamname;
	streamname.open(filename,std::ios_base::binary);
	if(streamname.good()){
		...
		for(tissues_size_t tissuenr=1;tissuenr<=TissueInfos::GetTissueCount();tissuenr++) {
			streamname << (int)tissuenr<<" \"" << TissueInfos::GetTissueName(tissuenr).ascii() << "\"\n";
		}
		streamname.close();
		return ok;
	} else return false;
...
bool SlicesHandler::get_extent(tissues_size_t tissuenr, bool onlyactiveslices ,unsigned short extent[3][2])
{
	bool found=false;
	...
	for(unsigned short i=startslice1;i<endslice1;i++) {
		if(!found) {
			found=image_slices[i].get_extent(active_tissuelayer,tissuenr,extent1);
			if(found) {
				extent[2][0]=i;
				extent[2][1]=i;
			...
			if(image_slices[i].get_extent(active_tissuelayer,tissuenr,extent1)) {
				if(extent1[0][0]<extent[0][0]) extent[0][0]=extent1[0][0];
				if(extent1[1][0]<extent[1][0]) extent[1][0]=extent1[1][0];
				if(extent1[0][1]>extent[0][1]) extent[0][1]=extent1[0][1];
				if(extent1[1][1]>extent[1][1]) extent[1][1]=extent1[1][1];
				extent[2][1]=i;
			}
		}
	}
...
void SlicesHandler::add_skin3D(int ix,int iy,int iz,float setto)
{
	...
	for(unsigned short z=endslice-1;z>startslice+checkSliceDistance;z--) {
		float *work1;
		float *work2;
		work1=image_slices[z].return_work();
		work2=image_slices[z-checkSliceDistance].return_work();
			...
			for(unsigned long i=0;i<area;i++) {
				if(work1[i]==set_to) {
					...
					if((work2[i]!=set_to)&&(work2[i]!=set_to2)) {
						...
						if(n1->getPriority()>subz) treap1.update_priority(n1,subz);
					}
				} else {
					if(work2[i]==set_to2) {
						p1.pxy=i;
		...
		for(unsigned short z=startslice;z<endslice;z++) {
			...
			for(unsigned short y=0;y<height;y++) {
				for(unsigned short x=0;x+1<width;x++) {
					if(work1[i]==set_to) {
						if((work1[i+1]!=set_to)&&(work1[i+1]!=set_to2)) {
							p1.pxy=i;
							Treap<posit,unsigned int>::Node *n = NULL;
						...
						if((work1[i+1]!=set_to)&&(work1[i+1]!=set_to2)) {
							p1.pxy=i;
							Treap<posit,unsigned int>::Node *n1=treap1.lookup(p1);
							if(n1->getPriority()>subx) treap1.update_priority(n1,subx);
						}
					} else {
	...
	if(iy>0) {
		for(unsigned short z=startslice;z<endslice;z++) {
			...
			for(unsigned short y=0;y+1<height;y++) {
				for(unsigned short x=0;x<width;x++) {
					if(work1[i]==set_to) {
						if((work1[i+width]!=set_to)&&(work1[i+width]!=set_to2)) {
							p1.pxy=i;
							Treap<posit,unsigned int>::Node *n = NULL;
						...
						if((work1[i+width]!=set_to)&&(work1[i+width]!=set_to2)) {
							p1.pxy=i;
							Treap<posit,unsigned int>::Node *n1=treap1.lookup(p1);
							if(n1->getPriority()>suby) treap1.update_priority(n1,suby);
						}
					} else {
	...
	while((n1=treap1.get_top())!=NULL) {
		...
		if(p1.pxy%width!=0) {
			if(work1[p1.pxy-1]==set_to) {
				...
				if(n2->getPriority()>prior+subx) treap1.update_priority(n2,prior+subx);
			}
		}
		if((p1.pxy+1)%width!=0) {
			if(work1[p1.pxy+1]==set_to) {
		...
		if(p1.pxy>=width) {
			if(work1[p1.pxy-width]==set_to) {
				...
				p2.pz=p1.pz;
				Treap<posit,unsigned int>::Node *n2=treap1.lookup(p2);
				if(n2->getPriority()>prior+suby) treap1.update_priority(n2,prior+suby);
			}
		}
		...
		if(p1.pz>startslice){
			...
			if(work2[p1.pxy]==set_to) {
				...
				p2.pz=p1.pz-1;
				Treap<posit,unsigned int>::Node *n2=treap1.lookup(p2);
				if(n2->getPriority()>prior+subz) treap1.update_priority(n2,prior+subz);
			}
		}
		if(p1.pz+1<endslice){
			work2=image_slices[p1.pz+1].return_work();
...
//	p1.pxy=position;
		...
		for(y=0;y<height;y++){
			...
			while(pos>(unsigned long)y*width){
				if(tissue[pos]==set_to) i1=ix;
					...
					if(i1>0) {
						if(!TissueInfos::GetTissueLocked(tissue[pos])) tissue[pos]=f;
						i1--;
					}
				}
				pos--;
	...
	for(unsigned short z=startslice;z<endslice;z++){
		tissue=image_slices[z].return_tissues(active_tissuelayer);
		for(pos=0;pos<area;pos++){
			if(tissue[pos]==set_to) counter[pos]=iz;
			else {
				if(counter[pos]>0) {
					if(!TissueInfos::GetTissueLocked(tissue[pos])) tissue[pos]=f;
					counter[pos]--;
	...
	for(unsigned short z=startslice;z<endslice;z++){
		tissue=image_slices[z].return_tissues(active_tissuelayer);
		for(unsigned i1=0;i1<area;i1++) if(tissue[i1]==set_to) tissue[i1]=0;
	}
...
void SlicesHandler::fill_skin_3d( int thicknessX, int thicknessY, int thicknessZ, tissues_size_t backgroundID, tissues_size_t skinID )
{
	...
	struct changesToMakeStruct
	{
		...
		bool operator==(const changesToMakeStruct& m) const {
			return ((m.sliceNumber == sliceNumber)&&(m.positionConvert==positionConvert));
		}
	};
								...
								if( value == backgroundID )
								{
									for( int y=0; y<offsetsSlices.size(); y++ ) {
										...
										for(int m=0; m<offsetsSlices[y].size(); m++) {
											...
											if( k+neighborSlice+neighborSlice2 >= 0 && k+neighborSlice+neighborSlice2 < dims[2] )
											{
...
float SlicesHandler::calculate_volume(Point p, unsigned short slicenr)
{
	...
	for(unsigned short j=startslice;j<endslice;j++) count+=image_slices[j].return_workpixelcount(f);
...
float SlicesHandler::calculate_tissuevolume(Point p, unsigned short slicenr)
{
	...
	for(unsigned short j=startslice;j<endslice;j++) count+=image_slices[j].return_tissuepixelcount(active_tissuelayer, c);
	return get_slicethickness()*p1.high*p1.low*count;
}
...
void SlicesHandler::gamma_mhd(unsigned short slicenr, short nrtissues, short dim, std::vector<std::string>mhdfiles, float *weights, float **centers, float *tol_f, float *tol_d)
{
	if (mhdfiles.size() + 1 < dim) return;
	//	if(slicenr>=startslice&&slicenr<endslice){
	float **bits = new float*[dim];
	...
	for (unsigned short i = startslice; i < endslice; i++) {
		...
		multidimgamma mdg;
		Pair pair1 = get_pixelsize();
		mdg.init(width, height, nrtissues, dim, bits, weights, centers, tol_f, tol_d, pair1.high, pair1.low);
		mdg.execute();
...
{
	if(n>(endslice-startslice)) return;
	stepsmooth stepsm;
...
void SlicesHandler::smooth_tissues(unsigned short n)
{
	...
	tissues_size_t *tissuesnew=(tissues_size_t *)malloc(sizeof(tissues_size_t)*area);
	if (n%2==0) n++;
	...
	for(unsigned short j=startslice;j<endslice;j++){
		...
		tissues_size_t *tissuesold=image_slices[j].return_tissues(active_tissuelayer);
		for(unsigned short y=n;y<height-n;y++){
			for(unsigned short x=n;x<width-n;x++){
				...
				for (short i=-n;i<=n;i++) {
					for (short j=-n;j<=n;j++) {
						tissues_size_t currtissue = tissuesold[(y+i)*width + (x+j)];
				...
				unsigned short halffiltersize = (unsigned short)(0.5f*filtersize + 0.5f);
				tissues_size_t tissuemajor = tissuesold[y*width+x];
				unsigned short tissuemajorcount = tissuecount[tissuemajor];
				for (std::map<tissues_size_t, unsigned short>::iterator iter = tissuecount.begin();
					iter != tissuecount.end(); ++iter) {
					if (iter->second > tissuemajorcount && iter->second >= halffiltersize) {
						tissuemajor = iter->first;
						tissuemajorcount = iter->second;
					}
				}
...
void SlicesHandler::regrow(unsigned short sourceslicenr, unsigned short targetslicenr, int n)
{
	...
	for(unsigned i=0;i<area;i++)lbmap[i]=0;
	
	tissues_size_t*dummy;
	tissues_size_t*results1=(tissues_size_t *)malloc(sizeof(tissues_size_t)*area);
	tissues_size_t*results2=(tissues_size_t *)malloc(sizeof(tissues_size_t)*area);
	tissues_size_t *tissues1=image_slices[sourceslicenr].return_tissues(active_tissuelayer);
	for(unsigned int i=0;i<area;i++) {
		if(tissues1[i]==0)
	...
	for(int l=0;l<n;l++){
		for(unsigned int i=0;i<area;i++) results1[i]=results2[i];

		int i1=0;

		for(unsigned short i=0;i<(height-1);i++){
			for(unsigned short j=0;j<width;j++){
				if(results2[i1]!=results2[i1+width]) results1[i1]=results1[i1+width]=0;
		...
		for(unsigned short i=0;i<height;i++){
			for(unsigned short j=0;j<(width-1);j++){
				if(results2[i1]!=results2[i1+1]) results1[i1]=results1[i1+1]=0;
	...
	for(unsigned int i=0;i<area;i++) lbmap[i]=float(results2[i]);
	...
	tissues_size_t*tissue_bits=image_slices[targetslicenr].return_tissues(active_tissuelayer);

	for(unsigned i=0;i<area;i++) {
		// if(f2[i]<thresh&&f1[i]<255.1f) tissue_bits[i]=(tissues_size_t)(f1[i]+0.1f);
		if(f2[i]<thresh&&f1[i]<(float)TISSUES_SIZE_MAX+0.1f) tissue_bits[i]=(tissues_size_t)(f1[i]+0.1f); // TODO: Correct?
		else tissue_bits[i]=0;
	}
...
bool SlicesHandler::unwrap(float jumpratio, float shift)
{
	...
	for(unsigned short i=startslice;i<endslice;i++) {
		if(i>startslice) {
			if((image_slices[i-1]).return_bmp()[0]-(image_slices[i]).return_bmp()[0]>jumpabs) shift+=range;
			else if((image_slices[i]).return_bmp()[0]-(image_slices[i-1]).return_bmp()[0]>jumpabs) shift-=range;
		}
		ok&=(image_slices[i]).unwrap(jumpratio,range,shift);;
	}
	if(area>0) {
		for(unsigned short i=startslice+1;i<endslice;i++) {
			if((image_slices[i-1]).return_work()[area-1]-(image_slices[i]).return_work()[area-1]>jumpabs) return false;
			if((image_slices[i]).return_work()[area-1]-(image_slices[i-1]).return_work()[area-1]>jumpabs) return false;
		}
	}
	return ok;
}
...
std::vector<float> SlicesHandler::GetBoundingBox()
{
	...
	bbox.resize(6);
	bbox[0] = (float)std::min<double>( origin[0], origin[0] + d1[0]*dimensionX*dx);
	bbox[1] = (float)std::max<double>( origin[0], origin[0] + d1[0]*dimensionX*dx);
	bbox[2] = (float)std::min<double>( origin[1], origin[1] + d2[1]*dimensionY*dy);
	bbox[3] = (float)std::max<double>( origin[1], origin[1] + d2[1]*dimensionY*dy);
	bbox[4] = (float)std::min<double>( origin[2], origin[2] + d3[2]*dimensionZ*thickness);
	bbox[5] = (float)std::max<double>( origin[2], origin[2] + d3[2]*dimensionZ*thickness);
...
void SlicesHandler::GetITKImage(itk::Image<float, 3>* inputim, int begin, int end)
{
	const SlicesHandler* self = this; // for const slices
	auto slices = self->get_bmp();
	float spacing[3] = { dx, dy, thickness };
	ImageToITK::copy(slices.data(), width, height, (unsigned)begin, (unsigned)(end - begin), spacing, transform, inputim);
}

void SlicesHandler::GetITKImageGM(itk::Image<float, 3>* inputim, int begin, int end)
{
	float spacing[3] = { dx, dy, thickness };
	ImageToITK::setup<float>(width, height, begin, end, spacing, transform, inputim);
...
void SlicesHandler::GetITKImageFB(itk::Image<float, 3>* inputim, int begin, int end)
{
	float spacing[3] = { dx, dy, thickness };
	ImageToITK::setup<float>(width, height, begin, end, spacing, transform, inputim);
	...
	for (unsigned z = 0; z < size[2]; z++)
	{
		...
		std::vector<float> fieldin((int)size[0] * (int)size[1]);
		float *field = &fieldin[0];
		copyfromwork(begin + z, field);//&([z*(unsigned long long)return_area()])
		for (unsigned y = begin; y < size[1]; y++)
		{
			pi.SetElement(1, y);
			for (unsigned x = 0; x < size[0]; x++)
...
void SlicesHandler::ModifyWork(itk::Image<unsigned int, 3>* output, int begin, int end)
{
...
void SlicesHandler::ModifyWorkFloat(itk::Image<float, 3>* output, int begin, int end)
{
	typedef itk::Image<unsigned int, 3> TInput;
	TInput::SizeType size;
	size[0] = width; // size along X
	...
	for (unsigned z = 0; z < output->GetLargestPossibleRegion().GetSize(2); z++)
	{
		...
		std::vector<float> fieldin((int)size[0] * (int)size[1]);
		float *field = &fieldin[0];
		for (unsigned y = 0; y < output->GetLargestPossibleRegion().GetSize(1); y++)
		{
			pi.SetElement(1, y);
			for (unsigned x = 0; x < output->GetLargestPossibleRegion().GetSize(0); x++)
			{
				pi.SetElement(0, x);
				field[x + y*size[0]] = output->GetPixel(pi);
			}
...
template<typename T>
typename itk::SliceContiguousImage<T>::Pointer _GetITKView(std::vector<T*>& slices, size_t dims[3], double spacing[3])
{
...
itk::SliceContiguousImage<float>::Pointer SlicesHandler::GetImage(eImageType type, bool active_slices)
{
	size_t dims[3] = { width, height, static_cast<size_t>(active_slices ? endslice - startslice : nrslices) };
...
itk::SliceContiguousImage<unsigned short>::Pointer SlicesHandler::GetTissues(bool active_slices)
{
	size_t dims[3] = { width, height, static_cast<size_t>(active_slices ? endslice - startslice : nrslices) };
	double spacing[3] = { dx, dy, thickness };
