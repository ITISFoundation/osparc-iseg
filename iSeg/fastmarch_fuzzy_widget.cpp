		...
		: QWidget1(parent, name, wFlags), handler3D(hand3D)
{
	...
	sl_s1->setToolTip(Format("A measure of how much the gray values are expected "
													 "to deviate from m1 (standard deviation)."));

	sl_s2 = new QSlider(Qt::Horizontal, hbox5);
	sl_s2->setRange(0, 100);
	sl_s2->setToolTip(Format(
			"Sudden changes larger than s2 are considered to indicate boundaries."));
...
void FastMarchFuzzy_widget::mouse_clicked(Point p)
{
	...
	else
	{
		if (IFTfuzzy == NULL)
		{
			...
			IFTfuzzy->fuzzy_init(bmphand->return_width(), bmphand->return_height(),
