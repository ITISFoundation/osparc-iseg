...
: QWidget( parent, name, wFlags )
{
	...
	lb_contrast->setPixmap(QIcon(m_picpath.absFilePath(QString("icon-contrast.png")).ascii()).pixmap());
	lb_brightness=new QLabel("B:",this);
	lb_brightness->setPixmap(QIcon(m_picpath.absFilePath(QString("icon-brightness.png")).ascii()).pixmap());
	...
	atlasViewer = new AtlasViewer(image,tissue,2,dimx,dimy,dimz,dx,dy,dz,&color_r,&color_g,&color_b,this);
...
//	imgorval->hide();
	...
	zoomer=new zoomer_widget(1.0,m_picpath,this);
	...
	QObject::connect(sl_transp,SIGNAL(valueChanged(int)),this,SLOT(sl_transp_changed()));
	QObject::connect(bg_orient,SIGNAL(buttonClicked(int)),this,SLOT(xyz_changed()));
	QObject::connect(zoomer,SIGNAL(set_zoom(double)),atlasViewer,SLOT(set_zoom(double)));
	QObject::connect(sl_brightness,SIGNAL(valueChanged(int)),this,SLOT(sl_brightcontr_moved()));
	QObject::connect(sl_contrast,SIGNAL(valueChanged(int)),this,SLOT(sl_brightcontr_moved()));

	QObject::connect(atlasViewer,SIGNAL(mousemoved_sign(tissues_size_t)),this,SLOT(pt_moved(tissues_size_t)));
...
bool AtlasWidget::loadfile(const char *filename)
{
	...
	if (tissuesVersion > 0) {
		for(unsigned short i=0;i<dimz;i++) {
			...
			in.readRawData((char *)&(tissue[area*i]),area*sizeof(tissues_size_t));
		}
	} else {
	...
	for(unsigned i=1;i<dimtot;i++) {
		if(minval>image[i]) minval=image[i];
		if(maxval<image[i]) maxval=image[i];
	}
...
void AtlasWidget::sl_brightcontr_moved()
{
	...
	offset=(127.5f-maxval*factor)*(1.0f-sl_brightness->value()*0.01f)+0.01f*(127.5f-minval*factor)*sl_brightness->value();
