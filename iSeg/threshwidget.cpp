...
thresh_widget::thresh_widget(SlicesHandler *hand3D, QWidget *parent, const char *name, Qt::WindowFlags wFlags)
...
//	QObject::connect(pushrange,SIGNAL(clicked()),this,SLOT(getrange()));
	...
	QObject::connect(pb_saveborders,SIGNAL(clicked()),this,SLOT(saveborders_execute()));
	QObject::connect(pb_loadborders,SIGNAL(clicked()),this,SLOT(loadborders_execute()));
	QObject::connect(sb_nrtissues,SIGNAL(valueChanged(int)),this,SLOT(nrtissues_changed(int)));
	QObject::connect(sb_dim,SIGNAL(valueChanged(int)),this,SLOT(dim_changed(int)));
	QObject::connect(sb_tissuenr,SIGNAL(valueChanged(int)),this,SLOT(tissuenr_changed(int)));
	QObject::connect(slider,SIGNAL(sliderMoved(int)),this,SLOT(slider_changed(int)));
	QObject::connect(slider,SIGNAL(sliderPressed()),this,SLOT(slider_pressed()));
	QObject::connect(slider,SIGNAL(sliderReleased()),this,SLOT(slider_released()));
	QObject::connect(le_borderval,SIGNAL(editingFinished()),this,SLOT(le_borderval_returnpressed()));
	QObject::connect(pushfilename,SIGNAL(clicked()),this,SLOT(select_pushed()));
	QObject::connect(buttonR, SIGNAL(stateChanged(int)), this, SLOT(RGBA_changed(int)));
	QObject::connect(buttonG, SIGNAL(stateChanged(int)), this, SLOT(RGBA_changed(int)));
	QObject::connect(buttonB, SIGNAL(stateChanged(int)), this, SLOT(RGBA_changed(int)));
	QObject::connect(buttonA, SIGNAL(stateChanged(int)), this, SLOT(RGBA_changed(int)));
	QObject::connect(cb_useCenterFile, SIGNAL(stateChanged(int)), this, SLOT(useCenterFile_changed(int)));
	QObject::connect(pushcenterFilename,SIGNAL(clicked()),this,SLOT(selectCenterFile_pushed()));
...
void thresh_widget::execute()
{
	...
	if(rb_manual->isOn()){
		...
		float *thresh1=bmphand->find_modal((unsigned)sb_minpix->value(),0.005f*ratio->value());
		if(allslices->isChecked())
		...
		if(fp!=NULL) {
			...
			for(int i=0;i<sb_nrtissues->value();i++) centers[i]=new float[sb_dim->value()];
			float *tol_f=new float[sb_dim->value()];
			float *tol_d=new float[sb_dim->value()];
			for(int i=0;i<sb_nrtissues->value();i++) {
				for(int j=0;j<sb_dim->value();j++) fscanf(fp,"%f",&(centers[i][j]));
			}
			for(int j=0;j<sb_dim->value();j++) fscanf(fp,"%f",&(tol_f[j]));
			for(int j=1;j<sb_dim->value();j++) fscanf(fp,"%f",&(tol_d[j]));
			tol_d[0]=0.0f;
			fclose(fp);
			std::vector<std::string> mhdfiles;
			for(int i=0;i+1<sb_dim->value();i++) mhdfiles.push_back(std::string(filenames[i].ascii()));
			if(allslices->isChecked())
				handler3D->gamma_mhd(handler3D->get_activeslice(),(short)sb_nrtissues->value(),(short)sb_dim->value(),
				mhdfiles,weights,centers,tol_f,tol_d);
			else 
				bmphand->gamma_mhd((short)sb_nrtissues->value(),(short)sb_dim->value(),mhdfiles,handler3D->get_activeslice(),weights,centers,tol_f,tol_d,handler3D->get_pixelsize());
			delete[] tol_d;
			delete[] tol_f;
			for(int i=0;i<sb_nrtissues->value();i++) delete[] centers[i];
			delete[] centers;
		} else {
			...
			{
				if(kmeansfiles[0].substr(kmeansfiles[0].find_last_of(".") + 1) == "png")
				{
					std::vector<int> extractChannels;
					if( buttonR->isChecked() ) extractChannels.push_back(0);
					if( buttonG->isChecked() ) extractChannels.push_back(1);
					if( buttonB->isChecked() ) extractChannels.push_back(2);
					if( buttonA->isChecked() ) extractChannels.push_back(3);
					if( sb_dim->value() != extractChannels.size()+1 )
						return;
					if(allslices->isChecked())
						handler3D->kmeans_png(handler3D->get_activeslice(),(short)sb_nrtissues->value(),(short)sb_dim->value(),
											  kmeansfiles,extractChannels,weights,(unsigned int)sb_iternr->value(),(unsigned int)sb_converge->value(),centerFilename.toStdString());
					else 
						bmphand->kmeans_png((short)sb_nrtissues->value(),(short)sb_dim->value(),
											kmeansfiles,extractChannels,handler3D->get_activeslice(),weights,(unsigned int)sb_iternr->value(),(unsigned int)sb_converge->value(),centerFilename.toStdString());
				}
				else
				{
					if(allslices->isChecked())
						handler3D->kmeans_mhd(handler3D->get_activeslice(),(short)sb_nrtissues->value(),(short)sb_dim->value(),
						kmeansfiles,weights,(unsigned int)sb_iternr->value(),(unsigned int)sb_converge->value());
					else 
						bmphand->kmeans_mhd((short)sb_nrtissues->value(),(short)sb_dim->value(),kmeansfiles,handler3D->get_activeslice(),weights,(unsigned int)sb_iternr->value(),(unsigned int)sb_converge->value());
				}
			}
			else
			{
...
//		unsigned w=bmphand->return_width();
//		unsigned h=bmphand->return_height();
//		float *slice1=new float[(unsigned long)124*164];
//		MHDreader::getSclice("C:\\Documents and Settings\\michael\\Desktop\\example3d.mhd",slice1,1,124,164);
//		delete[] slice1;
//		return;
//
////		k_means kmeans;
//		for(int i=0;i<sb_dim->value();i++){
//			if(i==0) bits[i]=bmphand->return_bmp();
////			else bits[i]=bmphand->getstack(i,modedummy);
//			else bits[i]=bmphand->getstack(bmphand->getfirststackindexxxxxxxxx(),modedummy);
////xxx			//if(bits1[i]==0)
//			//	bits[i]=bmphand->return_bmp();
//			//else
//			//	bits[i]=bmphand->getstack(bits1[i],modedummy);
//		}
////		kmeans.init(bmphand->return_width(),bmphand->return_height(),sb_nrtissues->value(),sb_dim->value(),bits,weights);
////		kmeans.make_iter(sb_iternr->value(),sb_converge->value());
////		kmeans.return_m(bmphand->return_work());
//		if(allslices->isChecked())
//			handler3D->kmeans(handler3D->get_activeslice(),(short)sb_nrtissues->value(),(unsigned int)sb_iternr->value(),(unsigned int)sb_converge->value());
//		else
//			bmphand->kmeans((short)sb_nrtissues->value(),(short)sb_dim->value(),bits,weights,(unsigned int)sb_iternr->value(),(unsigned int)sb_converge->value());
	} else {
//		EM em;
		for(int i=0;i<sb_dim->value();i++){
			if(bits1[i]==0)
...
//		em.init(bmphand->return_width(),bmphand->return_height(),sb_nrtissues->value(),sb_dim->value(),bits,weights);
//		em.make_iter(sb_iternr->value(),sb_converge->value());
//		em.classify(bmphand->return_work());
		if(allslices->isChecked())
			handler3D->em(handler3D->get_activeslice(),(short)sb_nrtissues->value(),(unsigned int)sb_iternr->value(),(unsigned int)sb_converge->value());
		else
			bmphand->em((short)sb_nrtissues->value(),(short)sb_dim->value(),bits,weights,(unsigned int)sb_iternr->value(),(unsigned int)sb_converge->value());
	}
...
{
	if(hideparams) {
		if(!rb_histo->isOn()) rb_histo->hide();
		if(!rb_EM->isOn()) rb_EM->hide();
	} else {
		rb_histo->show();
		rb_EM->show();
	}
	if(rb_manual->isOn()){
		...
		le_borderval->setText(QString::number(threshs[sb_tissuenr->value()],'g',3));
		le_borderval->show();
		txt_lower->show();
		txt_upper->show();
		hbox2a->show();
...
void thresh_widget::getrange()
{
	...
	for(unsigned i=0;i<20;i++){
		if(threshs[i+1]>upper) threshs[i+1]=upper;
		if(threshs[i+1]<lower) threshs[i+1]=lower;
	}

	if(rb_manual->isOn()){
		int i=int((threshs[sb_tissuenr->value()]-lower)*200/(upper-lower));
		slider->setValue(i);
		le_borderval->setText(QString::number(threshs[sb_tissuenr->value()],'g',3));
	}
...
void thresh_widget::dim_changed(int newval)
{
	if(rb_kmeans->isOn()||rb_EM->isOn()){
		...
		if(newval>cursize+1) {
			...
			for(size_t i=cursize;i+1<newval;i++) filenames[i]=QString("");
		}
...
QSize thresh_widget::sizeHint() const
{
...
void thresh_widget::le_borderval_returnpressed()
{
	...
	if(b1){
		if(rb_manual->isOn()){
			if(val>upper) threshs[sb_tissuenr->value()]=upper;
			else if(val<lower) threshs[sb_tissuenr->value()]=lower;
			else threshs[sb_tissuenr->value()]=val;
...
{
	QString savefilename = Q3FileDialog::getSaveFileName(QString::null, "Boarders (*.txt)\n", this);//, filename);
...
{
	if(rb_manual->isOn()){
		QString loadfilename = Q3FileDialog::getOpenFileName(QString::null,"Boarders (*.txt)\n" "All(*.*)",this);
		...
		if(!loadfilename.isEmpty()) {
			...
			if(fvec.size()>0&&fvec.size()<20){
				...
				for(unsigned i=0;i<(unsigned)fvec.size();i++){
					if(fvec[i]>upper) f=upper;
					else if(fvec[i]<lower) f=lower;
					else f=fvec[i];
					threshs[i+1]=f;
				}
				sb_tissuenr->setValue(1);
...
{
	if(version>=2) {
		QObject::disconnect(sb_nrtissues,SIGNAL(valueChanged(int)),this,SLOT(nrtissues_changed(int)));
		QObject::disconnect(sb_dim,SIGNAL(valueChanged(int)),this,SLOT(dim_changed(int)));
		QObject::disconnect(sb_tissuenr,SIGNAL(valueChanged(int)),this,SLOT(tissuenr_changed(int)));
		QObject::disconnect(slider,SIGNAL(sliderMoved(int)),this,SLOT(slider_changed(int)));
		QObject::disconnect(le_borderval,SIGNAL(returnPressed()),this,SLOT(le_borderval_returnpressed()));
		QObject::disconnect(subsect,SIGNAL(clicked()),this,SLOT(subsect_toggled()));
		...
		QObject::connect(sb_tissuenr,SIGNAL(valueChanged(int)),this,SLOT(tissuenr_changed(int)));
		QObject::connect(slider,SIGNAL(sliderMoved(int)),this,SLOT(slider_changed(int)));
		QObject::connect(le_borderval,SIGNAL(returnPressed()),this,SLOT(le_borderval_returnpressed()));

	}
	return fp;
}

void thresh_widget::hideparams_changed()
{
	method_changed(0);
}
...
{
	QString loadfilename = Q3FileDialog::getOpenFileName(QString::null, "Images (*.png)\n" "Images (*.mhd)\n" "All(*.*)",//"Images (*.bmp)\n" "All(*.*)", QString::null,
		this);//, filename);
...
{
	centerFilename = Q3FileDialog::getOpenFileName(QString::null, "Text File (*.txt*)", this);
...
void thresh_widget::RGBA_changed( int change )
{
	...
	buttonsChecked = buttonR->isChecked() + buttonG->isChecked() + buttonB->isChecked() + buttonA->isChecked();
