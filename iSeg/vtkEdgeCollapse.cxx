																 ...
																 vtkInformationVector *outputVector)
{
	...
	if (DomainLabelName != 0 && strlen(DomainLabelName) > 0 &&
		...
		if (Loud)
			std::cerr << "Time (find duplicate triangles): " << timer.runningTime()
								<< std::endl;
		timer.start();
	}
	else if (DomainLabelName != 0 && strlen(DomainLabelName) > 0) {
		std::cerr << "WARNING: could not locate cell data array " << DomainLabelName
							<< std::endl;
	}
	...
	if (Loud)
		std::cerr << "Time (create edge table): " << timer.runningTime()
							<< std::endl;
	timer.start();
	...
	if (Loud)
		std::cerr << "Time (compute normals/cell locator): " << timer.runningTime()
							<< std::endl;
	timer.start();
	...
	if (Loud)
		std::cerr << "Time (edge collapse): " << timer.runningTime() << std::endl;
	timer.start();
	...
	if (this->FlipEdges && !abort) {
		...
		if (Loud)
			std::cerr << "Time (triangle flip): " << timer.runningTime() << std::endl;
		timer.start();
	}
	...
	if (this->UseMaximumEdgeLength) {
		...
		if (Loud)
			std::cerr << "Time (edge divide): " << timer.runningTime() << std::endl;
		timer.start();
	}
	...
	if (DomainLabelName != 0 && input->GetCellData()->HasArray(DomainLabelName)) {
		...
		if (Loud)
			std::cerr << "Time (recreate duplicate triangles): "
								<< timer.runningTime() << std::endl;
		timer.start();
	}
	else {
...
int vtkEdgeCollapse::DelaunayFlipEdges()
{
	...
	while (edgeId >= 0) {
		...
		if (this->IntersectionCheckLevel > 0) {
			...
			if (!NoSelfIntersection(this->Mesh->GetPoints(), triangles_after_flip,
															triangles_nearby))
				continue;
		}
...
{ // Assumption is that later CollapseEdge will remove pt1Id
	...
	for (i = 0; i < changedEdges->GetNumberOfIds(); i++) {
		...
		if (edge[0] == pt1Id) {
			if (this->Edges->IsEdge(edge[1], pt0Id) ==
					-1) { // The edge will be completely new, add it.
				edgeId = this->Edges->GetNumberOfEdges();
...
{ // Assumption is that i2 will be removed (moved to i1)
	...
	for (int k = 0; k < this->Neighbors->GetNumberOfIds(); k++) {
		...
		if (this->IsDegenerateTriangle(ids[0], ids[1], ids[2])) {
			...
			vtkPolygon::ComputeNormal(this->Mesh->GetPoints(), 3,
																this->PointIds->GetPointer(0), normal_new);
	...
	*/
	...
	if (IntersectionCheckLevel > 0) {
		...
		for (std::set<vtkIdType>::iterator sit = triangles_after_collapse.begin();
