...
namespace MESH
{
	...
	struct SortElement {
		...
		bool operator<(SortElement const&rhs) const
		{
...
//----------------------------------------------------------------------------
int vtkEdgeCollapse::RequestData(
	...
	vtkInformationVector *outputVector)
{
	...
	NormalDotProductThreshold = std::cos(this->MaximumNormalAngleDeviation * 3.141592654f / 180.0);
	...
	if(DomainLabelName != 0
		&& strlen(DomainLabelName) > 0
		&& input->GetCellData()->HasArray(DomainLabelName))
	{
		...
		assert(this->Mesh->GetNumberOfCells() == this->Labels->GetNumberOfTuples());

		if(Loud) std::cerr << "Time (find duplicate triangles): " << timer.runningTime() << std::endl;
		timer.start();
	}
	else if(DomainLabelName != 0 && strlen(DomainLabelName) > 0)
	{
		std::cerr << "WARNING: could not locate cell data array " << DomainLabelName << std::endl;
	}
	...
	{
		if(this->Mesh->GetCellType(i) != VTK_TRIANGLE) continue;
	...
	if(Loud) std::cerr << "Time (create edge table): " << timer.runningTime() << std::endl;
	timer.start();
	...
	if(Loud) std::cerr << "Time (compute normals/cell locator): " << timer.runningTime() << std::endl;
	timer.start();


	// OK collapse edges until desired reduction is reached
	if(Loud) cout << "Starting edge collapse" << endl;
	...
	while ( !abort && edgeId >= 0 )
	{
		if ( ! (processed++ % 1000) )
		{
			double myprogress = std::min(1.0, 0.25 + 0.75 * processed / numEdgesToCollapse);
	...
	if(Loud) std::cerr << "Time (edge collapse): " << timer.runningTime() << std::endl;
	timer.start();
	...
	{
		if(Loud) cout << "Started flipping edges" << endl;
		this->DelaunayFlipEdges();

		if(Loud) std::cerr << "Time (triangle flip): " << timer.runningTime() << std::endl;
		timer.start();
	}
	...
	{
		if(Loud) cout << "Started dividing edges" << endl;
		this->SubdivideEdges();

		if(Loud) std::cerr << "Time (edge divide): " << timer.runningTime() << std::endl;
		timer.start();
	}
	...
	if(DomainLabelName != 0 && input->GetCellData()->HasArray(DomainLabelName))
	{
		...
		if(Loud) std::cerr << "Time (recreate duplicate triangles): " << timer.runningTime() << std::endl;
		timer.start();
	}
	else
	{
...
double vtkEdgeCollapse::ComputeAngleAtFirstPoint(vtkIdType i1, vtkIdType i2, vtkIdType i3)
{
	...
	double cosa = vtkMath::Dot(v12, v13) / vtkMath::Norm(v12) * vtkMath::Norm(v13);
...
int vtkEdgeCollapse::DelaunayFlipEdges()
{
	...
	while (edgeId >= 0)
	{
		...
		if(this->IntersectionCheckLevel > 0)
		{
			...
			for (std::set<vtkIdType>::iterator it = neighbors.begin(); it != neighbors.end(); ++it)
			{
...
void vtkEdgeCollapse::ComputeManifoldedness()
{
	...
	for (cellId=0, polys->InitTraversal(); 
		polys->GetNextCell(npts,pts); cellId++)
	{
		// skip this cell if is e.g. VTK_EMPTY_CELL
		if(Mesh->GetCellType(cellId) != VTK_TRIANGLE) continue;
...
{// Assumption is that i2 will be removed (moved to i1)
	...
	if(this->Neighbors->GetNumberOfIds() == 0) return false;
	...
	for (int k=0; k<this->Neighbors->GetNumberOfIds(); k++)
	{
		...
		else
		{
			...
			*/
			...
			{
				cout << "Could not find closest point on original surface" << endl;
				return false;
			}
		}
	...
	*/
	...
	if(IntersectionCheckLevel > 0)
	{
		...
		default:
			{
				...
				{
					if(this->CellLocator->GetDataSet()->GetCellType(i) == VTK_TRIANGLE)
					{
		...
		for(std::set<vtkIdType>::iterator sit = triangles_after_collapse.begin(); sit != triangles_after_collapse.end(); ++sit)
		{
...
int vtkEdgeCollapse::SubdivideEdges()
{
	...
	while (edgeId >= 0)
	{
		...
		vtkIdType newPtId = this->Mesh->InsertNextLinkedPoint(newX, 2 * this->Neighbors->GetNumberOfIds());
		...
		for (i = 0; i<Neighbors->GetNumberOfIds(); i++)
		{
			...
			this->Mesh->GetCellPoints(cellId, this->PointIds);
			vtkIdType newCellId = this->Mesh->InsertNextLinkedCell(VTK_TRIANGLE, 3, this->PointIds->GetPointer(0));
			if(this->Labels)
			{
				this->Labels->InsertNextTuple1(this->Labels->GetTuple1(cellId));
				assert(this->Labels->GetNumberOfTuples() == this->Mesh->GetNumberOfCells());
			}
...
{   // Assumes Mesh has been created, now just need to find duplicates
	...
	// the DuplicateLabel
	double* range = labels->GetRange();
	if(Loud) std::cout << "[" << range[0] << "," << range[1] << "]" << std::endl;
	int newLabel = range[1]+2;
	vtkIdType numTris = this->Mesh->GetNumberOfPolys();
	...
	for(vtkIdType i=0 ; i<numTris; ++i)
	{
		...
		else
		{
			...
			if(labelit != labelmap.end())
			{
				assert(this->Neighbors->GetId(0) >= 0 && this->Neighbors->GetId(0) < numTris);
				...
				if(label_i<label_other) isduplicate[this->Neighbors->GetId(0)] = true;
				else isduplicate[i] = true;
			}
...
void vtkEdgeCollapse::CreateDuplicateTriangles(vtkPolyData *output)
{
	...
	vtkDataArray *inlabels = this->Mesh->GetCellData()->GetArray(DomainLabelName);
	assert(inlabels);
	...
	for (i = 0; i < this->Mesh->GetNumberOfCells(); i++)
	{
		if (this->Mesh->GetCell(i)->GetCellType() == VTK_TRIANGLE)
		{
			...
			if(it != ilabelmap.end())
			{
				...
				outputCellList->InsertNextId(i);
				outputCellList->InsertNextId(i);
				flipCellList->InsertNextId(outputCellList->GetNumberOfIds()-1);
			}
			else
			{
...
enum interresult edge_vert_col_inter(REAL* A, REAL* B,
	REAL* P)
{
	int i = 0;
	do {
...
enum interresult  edge_edge_cop_inter(REAL* A, REAL* B,
...
enum interresult tri_vert_cop_inter(REAL* A, REAL* B,
...
enum interresult tri_edge_cop_inter(REAL* A, REAL* B,
...
enum interresult tri_edge_inter_tail(REAL* A, REAL* B,
...
enum interresult tri_edge_inter(REAL* A, REAL* B,
...
enum interresult tri_tri_inter(REAL* A, REAL* B,
