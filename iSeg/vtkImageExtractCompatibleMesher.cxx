...
public:
	...
	~Timer()
	{
		total_clocks += (clock()-c0);
	}
	static double EllapsedTime()
	{
		return total_clocks / CLOCKS_PER_SEC;
	}
	static void Reset()
	{
		total_clocks = 0;
...
public:
	TetNeighbors()
	{
		cell0 = cell1 = cell2 = cell3 = VTK_UNSIGNED_INT_MAX;
	};
	void SetNeighbor(int k, IDType id)
	{
		assert(k>=0 && k<4);
		if (k==0)       cell0 = id;
		else if (k==1)  cell1 = id;
		else if (k==2)  cell2 = id;
		else /*(k==3)*/cell3 = id;
	}
	IDType GetNeighbor(int k) const
	{
		assert(k>=0 && k<4);
		if (k==0)       return cell0;
		else if (k==1)  return cell1;
		else if (k==2)  return cell2;
		else /*(k==3)*/return cell3;
	}
	bool IsAssigned(IDType id) const
	{
		return (id != VTK_UNSIGNED_INT_MAX);
	}
	...
	bool BuildNeighbors()
	{
		...
		for (IDType cellId = 0; cellId<NC; cellId++)
		{
			...
			for(int j=0; j<4; j++)
			{
				...
				if ((tmap_it = tmap.find(*faces[j])) == tmap.end()) continue;
...
public:
	...
	/// Override
	virtual void AddTetrahedron(vtkIdType v1,vtkIdType v2,vtkIdType v3,vtkIdType v4, 
		int domain)
	{
		assert(Tetrahedra);
		assert(CellDomainArray);
		assert(Tetrahedra->GetNumberOfTetrahedra() == CellDomainArray->GetNumberOfTuples());
...
vtkImageExtractCompatibleMesher::~vtkImageExtractCompatibleMesher()
{
	if (this->OutputScalarName) delete [] this->OutputScalarName;
}
...
int vtkImageExtractCompatibleMesher::ContourSurface( vtkInformationVector **inputVector,
	vtkInformationVector *outputVector )
{
	vtkInformation *inInfo = inputVector[0]->GetInformationObject(0);
	this->Input = vtkImageData::SafeDownCast(
	...
	{
		vtkIncrementalOctreePointLocator *loc = vtkIncrementalOctreePointLocator::New();
	...
	vtkIdType extOffset = this->Input->GetExtent()[0] + this->Input->GetExtent()[2] + this->Input->GetExtent()[4];
	...
	for (vtkIdType k=0; k < numKCells && !abort; k++)
	{
		...
		if (k%10==0)
		{
			std::cerr << "Time for full voxels: " << Timer<1>::EllapsedTime() << std::endl;
			std::cerr << "Time for clipped voxels: " << Timer<2>::EllapsedTime() << std::endl;
		}
	}
	std::cerr << "Final Time for full voxels: " << Timer<1>::EllapsedTime() << std::endl;
	std::cerr << "Final Time for clipped voxels: " << Timer<2>::EllapsedTime() << std::endl;
	...
	std::cerr << "Number of tetra: " << this->Tetrahedra->GetNumberOfTetrahedra() << std::endl;
	std::cerr << "Number of points: " << this->Points->GetNumberOfPoints() << std::endl;
...
// diagonals.
void vtkImageExtractCompatibleMesher::ClipVoxel_not_used(vtkShortArray *cellScalars, 
	int flip, double spacing[3], vtkIdList *cellIds, vtkPoints *cellPts)
{
	Timer<2> timer;
	...
	static int order_flip[2][8] = {
		{0,3,5,6,1,2,4,7},
		{1,2,4,7,0,3,5,6}
	};//injection order based on flip
	static int order_noflip[8] = {
		0,1,2,3,4,5,6,7
	};//injection order without flip
	...
	for (int numPts=0; numPts < 8; numPts++)
	{
		...
		if (type == 4) continue; // TODO: this might be wrong, in vtkClipVolume all points are added to locator
	...
	for (int edgeNum=0; edgeNum < 12; edgeNum++)
	{
		...
		if ( s1 != s2 )
		{
			...
			{
				this->PointDomainArray->InsertValue(ptId, SURFACE_DOMAIN); // reuse background for interface/surface label
			}
...
void vtkImageExtractCompatibleMesher::ClipVoxel( vtkShortArray *cellScalars, int flip, double spacing[3], vtkIdList *cellIds, vtkPoints *cellPts )
{
	...
	for (vtkIdType i=0; i<numNew; i++)
	{
		...
		int doms[4] = {
			PointDomainArray->GetValue(pts[0]),
...
void vtkImageExtractCompatibleMesher::TriangulateVoxel( int cellScalar, 
	int flip, double spacing[3], vtkIdList *cellIds, vtkPoints *cellPts )
{
	Timer<1> timer;
...
int vtkImageExtractCompatibleMesher::LabelTetra_not_used()
{
	...
	assert(this->PointDomainArray->GetNumberOfTuples() == this->Points->GetNumberOfPoints());
	int OUTSIDE_DOMAIN = this->BackgroundLabel;

	double x[3];

	this->CellDomainArray->SetNumberOfTuples(this->Tetrahedra->GetNumberOfTetrahedra());
	...
	for (vtkIdType i = 0; i < numberOfCells; i++)
	{
		...
		if (numberSurface < 4)
		{
			...
			{
				std::cout << "Tetrahedron with both inside and outside nodes (" << numberInside << "," << numberOutside << ")." << std::endl;
			}
		}
		...
		else
		{
			...
			for (int k=0; k<4; k++)
			{
				...
				if ((hist_it = hist.find(dom[k])) == hist.end())	hist[dom[k]] = 1;
				else hist_it->second++;
			}
			if (hist.size() == 1)
			{
			...
			if (domainCopy->GetValue(cellId) == UNSURE_DOMAIN)
			{
				...
				for(int k=0; k<4; k++)
				{
					if (this->Tetrahedra->HasNeighbor(cellId, k))
					{
						...
						{
							if ((hist_it = hist.find(dom)) == hist.end()) hist[dom] = 1;
							else hist_it->second++;
						}
					}
				}
				if (hist.size() > 0)
				{
					...
					if (maxhist == 0)
						...
						this->CellDomainArray->SetValue(cellId, cell_domain);
					if (this->CellDomainArray->GetValue(cellId) != UNSURE_DOMAIN) assigned++;
				}
			}
		}
	...
	for (vtkIdType cellId = 0; cellId < numberOfCells && NumberOfUnassignedLabels > 0; cellId++)
	{
		if (this->CellDomainArray->GetValue(cellId) == UNSURE_DOMAIN)
		{
			...
			for(int k=0; k<4; k++)
			{
				...
				{
					vtkIdType neighborId = this->Tetrahedra->GetNeighbor(cellId, k);
					if ((dom=this->CellDomainArray->GetValue(neighborId)) != UNSURE_DOMAIN)
					{
						if (hist.find(dom) == hist.end()) hist[dom] = 1;
						else hist[dom]++;
					}
				}
			}
			...
			this->CellDomainArray->SetValue(cellId, cell_domain);
			if (this->CellDomainArray->GetValue(cellId) != UNSURE_DOMAIN) NumberOfUnassignedLabels--;
		}
	}

	std::cout << "Number of cells with 4 points on surface: " << number4PointsOnSurface << std::endl;
	std::cout << "Number of unassigned cells: " << NumberOfUnassignedLabels << std::endl;
	return 1;
}
...
int vtkImageExtractCompatibleMesher::EvaluateLabel( double x[3] )
{
	...
	std::cout << "Warning: probing outside image " << i << " " << j << " " << k << std::endl;
...
void vtkImageExtractCompatibleMesher::ExtractMeshDomainInterfaces( vtkInformationVector *outputVector )
{
	...
	{
		if (static_cast<int>(this->CellDomainArray->GetTuple1(cellId)) == BackgroundLabel) continue;
		...
		for (int k=0; k<4; k++)
		{
			...
			else if (visited[neighborId=this->Tetrahedra->GetNeighbor(cellId, k)] == false)
			{
				// neighbor has not been visited yet
				if (this->CellDomainArray->GetTuple1(cellId) != this->CellDomainArray->GetTuple1(neighborId))
				{
					...
					if (static_cast<int>(this->CellDomainArray->GetTuple1(neighborId)) != BackgroundLabel)
					{
						...
						domainArray->InsertNextValue(this->CellDomainArray->GetTuple1(neighborId));
					}
				}
			}
		}
	...
	std::cerr << "Number of triangles: " << cells->GetNumberOfCells() << std::endl;
}
