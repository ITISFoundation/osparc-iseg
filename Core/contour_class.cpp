...
{
	for(unsigned int i=0;i<n;i++) cout << plist[i].px <<":"<< plist[i].py <<" ";
	cout << endl;
	return;
}
...
unsigned int contour_class::return_n() 
{
	return n;
}
...
{
	(*Pt_vec).clear();
	for(unsigned int i=0;i<n;i++) (*Pt_vec).push_back(plist[i]);
	return;
}


void contour_class::doug_peuck_sub(float epsilon,const unsigned int p1,const unsigned int p2, vector<bool> *v1_p)
{
//	cout << p1<<" "<<p2<<endl;
	if(p2<=p1+1) return;
	float dv,l;
...
void contour_class2::doug_peuck(float epsilon,vector<Point> *Pt_vec,vector<unsigned> *Meetings_vec,vector<Point> *Result_vec)
{
	n=Pt_vec->size();
	m=Meetings_vec->size();
	if(m==0) {
	...
	if(n>m){
		...
		for(unsigned int i=0;i<n;i++) v1[i]=false;
		for(unsigned int i=0;i<m;i++) v1[(*Meetings_vec)[i]]=true;

		for(unsigned int i=0;i+1<m;i++){
			doug_peuck_sub(epsilon,Pt_vec,(*Meetings_vec)[i],(*Meetings_vec)[i+1],&v1);
		}
		doug_peuck_sub2(epsilon,Pt_vec,(*Meetings_vec)[m-1],(*Meetings_vec)[0],&v1);

		Result_vec->clear();
		for(unsigned int i=0;i<n;i++) {
			if(v1[i]) Result_vec->push_back((*Pt_vec)[i]);
		}
	}
...
void contour_class2::doug_peuck_sub(float epsilon, vector<Point> *Pt_vec, unsigned short p1,unsigned short p2, vector<bool> *v1_p)
{
	if(p2<=p1+1) return;
	float dv,l;
	float max_dist=0;
...
void contour_class2::doug_peuck_sub2(float epsilon, vector<Point> *Pt_vec, unsigned short p1,unsigned short p2, vector<bool> *v1_p)
{
	if((p2==0&&p1+1==n)||p1<p2) return;
	...
	if(p1==p2){
		if(n>2){
			...
			if((unsigned int)(p1+1)<n) p3=p1+1;
			else p3=0;
			if(p1==0) p4=n-1; 
			else p4=p1-1;
			unsigned int max_pos=p1;
			...
			if(SIJ.px>0||(SIJ.px==0&&SIJ.py>0)){
				...
				if(p2>0){
					...
					dv=float(SIV.px*SIV.px+SIV.py*SIV.py);
					//			if(dv>=max_dist-0.001) {max_dist=dv; max_pos=i;}
					if(dv>max_dist+0.001) {max_dist=dv; max_pos=0;}
				}
