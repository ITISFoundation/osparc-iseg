	...
	public:
		...
		void Write()
		{
			...
			BOOST_REQUIRE(ImageWriter().writeVolume(_file_name.string().c_str(),
				slices.data(), _dims[0], _dims[1], _dims[2], _spacing, _transform) );
		}
		...
		void Read()
		{
			...
			BOOST_REQUIRE(ImageReader::getInfo(_file_name.string().c_str(),
				width, height, nrslices, spacing1, transform1));
...

			{
				...
				BOOST_REQUIRE(ImageReader::getVolume(_file_name.string().c_str(), slices.data(), 0, nrslices, width, height));
...

			{
				...
				BOOST_REQUIRE(ImageReader::getSlice(_file_name.string().c_str(), data.data(), 2, width, height));
