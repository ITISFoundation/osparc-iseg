...
unsigned outline_line::return_length()
{
...
//	for(unsigned i=0;i<l;i++) fprintf(fp,"%f,%f ",line[i].px,line[i].py);
	for(unsigned i=0;i<l;i++) fprintf(fp,"%i,%i ",line[i].px,line[i].py);
...
void outline_line::doug_peuck(float epsilon,bool closed)
{
	...
	if(n>2){
		...
		for(unsigned int i=0;i<n;i++) v1[i]=false;
		v1[0]=true;
		v1[n-1]=true;
...
void outline_line::doug_peuck_sub(float epsilon,const unsigned int p1,const unsigned int p2, vector<bool> *v1_p)
{
//	cout << p1<<" "<<p2<<endl;
	if(p2<=p1+1) return;
	float dv,l;
...
//	cout << l << endl;
	...
	for(unsigned int i=p1+1;i<p2;i++){
		...
		dv=cross(&SIV,&SIJ)/l;
//		cout << dv << ":";
		if(dv>max_dist) {max_dist=dv; max_pos=i;}
	}
//	cout << endl;
...
outline_slice::outline_slice(float thick)
{
	thickness=thick;
}

void outline_slice::add_line(tissues_size_t tissuetype, vector<Point_type> *P_vec, bool outer)
{
	outline_line ol;
//	ol.add_points(P_vec);
	if(outer) {
...
void outline_slice::add_points(tissues_size_t tissuetype, unsigned short linenr, vector<Point_type> *P_vec, bool outer)
{
	if(outer) {
...
void outline_slice::add_point(tissues_size_t tissuetype, unsigned short linenr, Point_type P, bool outer)
{
	if(outer) {
...
void outline_slice::clear(tissues_size_t tissuetype, unsigned short linenr, bool outer)
{
...
unsigned short outline_slice::return_nrlines(tissues_size_t tissuetype, bool outer)
{
	if (outer) {
		if (outer_lines.find(tissuetype) != outer_lines.end()) {
...
unsigned outline_slice::return_length(tissues_size_t tissuetype, unsigned short linenr, bool outer)
{
	if (outer) {
		if (outer_lines.find(tissuetype) != outer_lines.end()) {
...
Point_type *outline_slice::return_line(tissues_size_t tissuetype, unsigned short linenr, bool outer)
{
	if (outer) {
		if (outer_lines.find(tissuetype) != outer_lines.end()) {
...
FILE *outline_slice::print(FILE *fp, tissues_size_t nr_tissues)
{
	...
	if (nr_tissues > 255) { // Only print tissue indices which contain outlines
			...
			itTissueIdx != tissueIndices.end(); ++itTissueIdx)
		{
			...
			if (outer_lines.find(currTissueIdx) != outer_lines.end()) {
				fprintf(fp,"T%u O%u\n", currTissueIdx, (unsigned)outer_lines[currTissueIdx].size());
				for(itVec = outer_lines[currTissueIdx].begin(); itVec != outer_lines[currTissueIdx].end(); ++itVec)
					(*itVec).print(fp);
			} else {
				fprintf(fp,"T%u O%u\n", currTissueIdx, 0);
			}
			...
			if (inner_lines.find(currTissueIdx) != inner_lines.end()) {
				fprintf(fp,"T%u I%u\n", currTissueIdx, (unsigned)inner_lines[currTissueIdx].size());
				for(itVec = inner_lines[currTissueIdx].begin(); itVec != inner_lines[currTissueIdx].end(); ++itVec)
					(*itVec).print(fp);
			} else {
				fprintf(fp,"T%u I%u\n", currTissueIdx, 0);
			}
	...
	} else { // Print all tissue indices within range of unsigned char
		...
		for(unsigned short i = 0; i < 256; ++i) {
			...
			if (outer_lines.find(i) != outer_lines.end()) {
				...
				for(itVec = outer_lines[i].begin(); itVec != outer_lines[i].end(); ++itVec)
					(*itVec).print(fp);
			} else {
				fprintf(fp,"T%u O%u\n",i,0);
			}
			...
			if (inner_lines.find(i) != inner_lines.end()) {
				...
				for(itVec = inner_lines[i].begin(); itVec != inner_lines[i].end(); ++itVec)
					(*itVec).print(fp);
			} else {
				fprintf(fp,"T%u I%u\n",i,0);
			}
...
void outline_slice::set_thickness(float thick)
{
	thickness=thick;
}

float outline_slice::get_thickness()
{
	return thickness;
}
...
void outline_slice::doug_peuck(float epsilon,bool closed)
{
	...
	for (itTissue = outer_lines.begin(); itTissue != outer_lines.end(); ++itTissue) {
		for (itOutline = itTissue->second.begin(); itOutline != itTissue->second.end(); ++itOutline) {
			itOutline->doug_peuck(epsilon,closed);
		}
	}
	for (itTissue = inner_lines.begin(); itTissue != inner_lines.end(); ++itTissue) {
		for (itOutline = itTissue->second.begin(); itOutline != itTissue->second.end(); ++itOutline) {
			itOutline->doug_peuck(epsilon,closed);
		}
	}
...
{
	slices.resize(nrslices);
	for(unsigned i=0;i<nrslices;i++) slices[i].set_thickness(thick);
	return;
}
...
{
	for(unsigned i=0;i<(unsigned)slices.size();i++) slices[i].clear();
	return;
}

void outline_slices::clear(tissues_size_t tissuetype)
{
	for(unsigned i=0;i<(unsigned)slices.size();i++) slices[i].clear(tissuetype);
	return;
}
...
void outline_slices::clear_slice(unsigned slicenr, tissues_size_t tissuetype, unsigned short linenr, bool outer)
{
	slices[slicenr].clear(tissuetype,linenr,outer);
	return;
}

void outline_slices::add_line(unsigned slicenr,tissues_size_t tissuetype, vector<Point_type> *P_vec, bool outer)
{
	slices[slicenr].add_line(tissuetype,P_vec,outer);
	return;
}

void outline_slices::add_points(unsigned slicenr, tissues_size_t tissuetype, unsigned short linenr, vector<Point_type> *P_vec, bool outer)
{
	slices[slicenr].add_points(tissuetype,linenr,P_vec,outer);
	return;
}

void outline_slices::add_point(unsigned slicenr, tissues_size_t tissuetype, unsigned short linenr, Point_type P, bool outer)
{
	slices[slicenr].add_point(tissuetype,linenr,P,outer);
	return;
}

unsigned outline_slices::return_nrslices()
{
	return (unsigned)slices.size();
}

unsigned short outline_slices::return_nrlines(unsigned slicenr, tissues_size_t tissuetype, bool outer)
{
	return slices[slicenr].return_nrlines(tissuetype, outer);
}

unsigned outline_slices::return_length(unsigned slicenr, tissues_size_t tissuetype, unsigned short linenr, bool outer)
{
	return slices[slicenr].return_length(tissuetype, linenr, outer);
}

Point_type *outline_slices::return_line(unsigned slicenr, tissues_size_t tissuetype, unsigned short linenr, bool outer)
{
...
FILE *outline_slices::printprologue(const char *filename, unsigned nr_slices, tissues_size_t nr_tissues)
{
	FILE *fp;
...
FILE *outline_slices::printsection(FILE *fp, unsigned startslice, unsigned endslice, unsigned offset, tissues_size_t nr_tissues)
{
