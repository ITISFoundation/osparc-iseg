...
bool VTIreader::getSlice(const char *filename, float *slice, unsigned slicenr, unsigned width, unsigned height)
{
	...
	if(ext[3]-ext[2]+1!=(int)height) return false;
	if(ext[1]-ext[0]+1!=(int)width) return false;
	if(slicenr<0||slicenr>ext[5]-ext[4]) return false;
...
bool VTIreader::getVolume(const char *filename, float **slices, unsigned nrslices, unsigned width, unsigned height, const std::string& arrayName)
{
	vtkXMLImageDataReader *reader = vtkXMLImageDataReader::New();
   if(reader->CanReadFile(filename)==0){
      cerr << "VTIreader::getVolume() : can not read file " << filename << endl;
		return false;
	}
	reader->SetFileName(filename);
	reader->Update();
	...
	if(ext[3]+1-ext[2]!=(int)height) return false;
	if(ext[1]+1-ext[0]!=(int)width) return false;
	if(ext[5]+1-ext[4]!=(int)nrslices) return false;
	...
	for(int k=0;k<nrslices;k++) {
		...
		if(className=="vtkUnsignedCharArray") {
			...
			float *ptr = (float*)img->GetScalarPointer(ext[0], ext[2], k+ext[4]);
			for(int j=ext[2];j<=ext[3];j++) {
				for(int i=ext[0];i<=ext[1];i++,pos++) {
			...
			double *ptr = (double*)img->GetScalarPointer(ext[0], ext[2], k+ext[4]);
			for(int j=ext[2];j<=ext[3];j++) {
				for(int i=ext[0];i<=ext[1];i++,pos++) {
					slices[k][pos]=(float) ptr[pos];
				}
			...
			for(int j=ext[2];j<=ext[3];j++) {
				for(int i=ext[0];i<=ext[1];i++,pos++) {
					slices[k][pos]=img->GetScalarComponentAsFloat(i, j, k+ext[4], 0);
				}
			}
		}
	}
...
bool VTIreader::getVolumeAll(const char *filename, float **slicesbmp,float **sliceswork,tissues_size_t **slicestissue, unsigned nrslices, unsigned width, unsigned height)
{
	...
	bool tissueUnsignedChar = (classNametissue.compare("vtkUnsignedCharArray") == 0);
	bool tissueUnsignedShort = (classNametissue.compare("vtkUnsignedShortArray") == 0);
	if (!(tissueUnsignedChar || tissueUnsignedShort)
		|| classNamebmp.compare("vtkFloatArray") != 0
		|| classNamework.compare("vtkFloatArray") != 0)
	{
		reader->Delete();
      cerr << "VTIreader::getVolumeAll() : unsupported array types for the arrays" << endl;
      return false;
	}
	...
	for(int k=0;k<nrslices;k++) {
		unsigned long long pos=0;
		float *ptrfloat = (float*)img->GetScalarPointer(ext[0], ext[2], k+ext[4]);
		for(int j=ext[2];j<=ext[3];j++) {
			for(int i=ext[0];i<=ext[1];i++,pos++) {
...
bool VTIreader::getVolume(const char *filename, float **slices, unsigned startslice, unsigned nrslices, unsigned width, unsigned height, const std::string& arrayName)
{
	...
	if(ext[3]+1-ext[2]!=(int)height) return false;
	if(ext[1]+1-ext[0]!=(int)width) return false;
	if(ext[5]+1-ext[4]<(int)nrslices+startslice) return false;
...
bool VTIwriter::writeVolumeAll(const char *filename, float **slicesbmp,float **sliceswork,tissues_size_t **slicestissue, tissues_size_t nrtissues, unsigned nrslices, unsigned width, unsigned height,float *pixelsize,float *offset,bool binary,bool compress)
{
	...
	vtkSmartPointer<vtkFloatArray> bmparray = vtkSmartPointer<vtkFloatArray>::New();
	bmparray->SetNumberOfValues((vtkIdType)width*(vtkIdType)height*(vtkIdType)nrslices);
	bmparray->SetNumberOfComponents(1);
	bmparray->SetName("Source");
	//bmparray->SetNumberOfTuples(xxx);
	vtkSmartPointer<vtkFloatArray> workarray = vtkSmartPointer<vtkFloatArray>::New();
	workarray->SetNumberOfValues((vtkIdType)width*(vtkIdType)height*(vtkIdType)nrslices);
	...
	if (nrtissues <= 255) {
		vtkSmartPointer<vtkUnsignedCharArray> tissuearray = vtkSmartPointer<vtkUnsignedCharArray>::New();
		tissuearray->SetNumberOfValues((vtkIdType)width*(vtkIdType)height*(vtkIdType)nrslices);
		...
		vtkSmartPointer<vtkUnsignedShortArray> tissuearray = vtkSmartPointer<vtkUnsignedShortArray>::New();
		tissuearray->SetNumberOfValues((vtkIdType)width*(vtkIdType)height*(vtkIdType)nrslices);
		tissuearray->SetNumberOfComponents(1);
		tissuearray->SetName("Tissue");
		input->GetPointData()->AddArray(tissuearray);
	} else {
		cerr << "VTIwriter::writeVolumeAll: Error, tissues_size_t not implemented." << endl;
		return false;
	}
	//BL? input->Update();
	...
	if (nrtissues <= 255) {
		for(int k=0;k<nrslices;k++) {
			unsigned long long pos=0;
			unsigned char *ptrchar = (unsigned char*)input->GetScalarPointer(0, 0, k);
			for(int j=0;j<height;j++) {
				for(int i=0;i<width;i++,pos++) {
		...
		for(int k=0;k<nrslices;k++) {
			unsigned long long pos=0;
			unsigned short *ptrshort = (unsigned short*)input->GetScalarPointer(0, 0, k);
			for(int j=0;j<height;j++) {
				for(int i=0;i<width;i++,pos++) {
