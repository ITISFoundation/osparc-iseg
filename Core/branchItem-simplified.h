...
public:
	...
	BranchItem(BranchItem* parent) { Initialize(); _parent = parent; };
	...
	//! Destructor
	~BranchItem(void) {
		if (_parent != NULL) _parent->removeChild(getLabel());
		_centerList.clear();
		for (std::list<BranchItem*>::iterator it = _children.begin(); it != _children.end(); it++) {
	...
	void addChild(BranchItem* child) {
		...
		if (it == _children.end()) _children.push_back(child);
		if (child->getParent() != this) child->setParent(this);
	};
	void removeChild(unsigned label) {
		std::list<BranchItem*>::iterator it = _children.begin();
		while ((it != _children.end()) && ((*it)->getLabel() != label)) it++;
		if (it != _children.end()) {
			(*it)->setParentDirty(NULL);
			//xxxa delete *it?
			_children.erase(it);
	...
	void doug_peuck(float epsilon, float dx, float dy, float dz)
	{
		...
		if (n > 2) {
			...
			for (unsigned int i = 0; i < n; i++) {
				if (!v1[i]) it = _centerList.erase(it);
				else it++;
			}
		}
		return;
	...
	void doug_peuck(float epsilon, float dx, float dy, float dz, std::vector<vec3> &vp)
	{
		vp.clear();
		unsigned int n = (unsigned int)_centerList.size();
		if (n > 2) {
	...
	void doug_peuck_inclchildren(float epsilon, float dx, float dy, float dz, std::vector<std::vector<vec3> > &vp)
	{
		std::vector<vec3> vp2;
	...
	{
		Point p;
		for (std::vector<vec3>::iterator it = _centerList.begin(); it != _centerList.end(); it++) {
	...
	void getCenterListSlice_inclchildren(unsigned short slicenr, std::vector<Point> &vp)
	{
...
private:
	...
	vec3 _endVox;        // voxel (in voxel coord) where branch ends (end of vessel or begin of other branches)
	vec3 _endVoxWorld; // voxel (in world coord) where branch ends (end of vessel or begin of other branches)
	void doug_peuck_sub(float epsilon, const unsigned int p1, const unsigned int p2, std::vector<bool> *v1_p, float dx, float dy, float dz)
	{
		//	cout << p1<<" "<<p2<<endl;
		if (p2 <= p1 + 1) return;
		float dv, l;
