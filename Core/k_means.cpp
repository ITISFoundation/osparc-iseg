...
void k_means::init(short unsigned w, short unsigned h, short nrclass, short dimension, float **bit, float *weight)
{
	width=w;
	height=h;
	area=unsigned(w)*h;
...
void k_means::init(short unsigned w, short unsigned h, short nrclass, short dimension, float **bit, float *weight, float *center)
{
	width=w;
	height=h;
	area=unsigned(w)*h;
	...
	m=(short *)malloc(sizeof(unsigned)*area);
	centers=(float *)malloc(sizeof(float)*dim*nrclass);
	for(short i=0;i<nrclass*dimension;i++) centers[i]=center[i];
	for(unsigned i=0;i<area;i++)m[i]=-1;
...
{
	for(unsigned i=0;i<area;i++) result_bits[i]=255.0f/(nrclasses-1)*m[i];
	return;
}
...
void k_means::apply_to(float **sources, float *result_bits)
{
	...
	for(unsigned i=0;i<area;i++){
		...
		for(short n=0;n<dim;n++){
			distmin+=(sources[n][i]-centers[cindex])*(sources[n][i]-centers[cindex])*weights[n];
			cindex++;
		}
		for(short l=1;l<nrclasses;l++){
			dist=0;
			for(short n=0;n<dim;n++){
				dist+=(sources[n][i]-centers[cindex])*(sources[n][i]-centers[cindex])*weights[n];
				cindex++;
			}
			if(dist<distmin){distmin=dist; dummy=l;}
		}
...
unsigned k_means::recompute_membership()
{
	...
	for(unsigned i=0;i<area;i++){
		...
		cindex=0;
		for(short n=0;n<dim;n++){
			distmin+=(bits[n][i]-centers[cindex])*(bits[n][i]-centers[cindex])*weights[n];
			cindex++;
		}
		for(short l=1;l<nrclasses;l++){
			dist=0;
			for(short n=0;n<dim;n++){
				dist+=(bits[n][i]-centers[cindex])*(bits[n][i]-centers[cindex])*weights[n];
				cindex++;
			}
			if(dist<distmin){distmin=dist; dummy=l;}
		}
...
void k_means::init_centers()
{
	...
	{
		for(short l=0;l<nrclasses;l++){ //equally spaced for 3D, 27 classes
			centers[l*3+0] = maxVals[0] -(l%3+.5f)*(maxVals[0] - minVals[0])/3;
			centers[l*3+1] = maxVals[1] -((l/3)%3+.5f)*(maxVals[1] - minVals[1])/3;
			centers[l*3+2] = maxVals[2] -(l/9+.5f)*(maxVals[2] - minVals[2])/3;
		}
	}
	else
	{
		...
		for(short l=0;l<nrclasses;l++){
			for(short i=0;i<dim;i++){
				centers[l*dim+i] = maxVals[i] - (float)l/(nrclasses-1.0f) * (maxVals[i] - minVals[i]);
			}
		}
...
{
	for(short i=0;i<nrclasses*dim;i++) centers[i]=center[i];
	return;
}

bool k_means::get_centers_from_file(const std::string &fileName, float* &centers, int &dimensions, int &classes)
{
