...
//			fseek(fp,ftell(fp),SEEK_SET);
			...
			if(str[0]=='n'&&str[1]=='o'&&str[2]=='d'&&str[3]=='e'){
				fscanf(fp,"(%u,%f,%f,%f",&dummy,&vertex.v[0],&vertex.v[1],&vertex.v[2]);
...
//				fprintf(fp1,"%u %u %u; ",corner1,corner2,corner3);
				...
				while(itinner!=tissdescvec->vtd.end()&&itinner->name!=string1) itinner++;
				if(itinner==tissdescvec->vtd.end()){
					if(string1==exteriorstring) exterior1=true;
					else {
						exterior1=false;
				...
				while(itouter!=tissdescvec->vtd.end()&&itouter->name!=string2) itouter++;
				if(itouter==tissdescvec->vtd.end()){
					if(string2==exteriorstring) exterior2=true;
					else {
						exterior2=false;
						tissdescvec->vtd.push_back(ts);
						...
						itinner=tissdescvec->vtd.begin();
						while(itinner->name!=string1) itinner++;
					}
				}
