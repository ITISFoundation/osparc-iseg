...
bool ImageReader::getVolume(const char *filename, float **slices, unsigned startslice, unsigned nrslices, unsigned width, unsigned height)
{
	...
	if (extension == ".vti")
	{
		...
		if (VTIreader::getInfo(filename, w, h, n, s, o, arrayNames) && !arrayNames.empty())
		{
			return VTIreader::getVolume(filename, slices, startslice, nrslices, width, height, arrayNames[0]);
		}
	}
...
bool ImageReader::getInfo(const char *filename, unsigned &width, unsigned &height, unsigned &nrslices, float spacing[3], Transform& transform)
{
	...
	else
	{
		...
		if (extension == ".vti")
		{
			float offset[3];
			std::vector<std::string> arrayNames;
			VTIreader::getInfo(filename, width, height, nrslices, spacing, offset, arrayNames);
