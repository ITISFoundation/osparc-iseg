...
void UndoQueue::add_undo(UndoElem *ue)
{
	sub_add_undo(ue);
}
...
unsigned UndoQueue::return_nrundo()
{
	return nrnow;
}

unsigned UndoQueue::return_nrredo()
{
	return nrin-nrnow;
}

unsigned UndoQueue::return_nrundoarraysmax()
{
	return nrundoarraysmax;
}

unsigned UndoQueue::return_nrundomax()
{
	return nrundo;
}
...
{
	for(unsigned i=0;i<nrin;i++)
		undos[i]->dataSelection.sliceNr=nrslices-1-undos[i]->dataSelection.sliceNr;
}
