...
unsigned sliceprovider::return_area()
{
	return area;
}
...
bool sliceprovider_installer::unused()
{
	return counter==0;
}
...
/*	fp1=fopen("D:\\Development\\segmentation\\sample images\\test100.txt","w");
	...
	while(it!=splist.end()&&(it->area!=area1)) it++;
...
{
	list<spobj>::iterator it=splist.begin();
	while(it!=splist.end()&&(it->area!=sp->return_area())) it++;
