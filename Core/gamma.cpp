...
multidimgamma::multidimgamma() { m = NULL; }

void multidimgamma::init(short unsigned w, short unsigned h, short nrclass,
												 short dimension, float **bit, float *weight,
												 float **centers1, float *tol_f1, float *tol_d1,
												 float dx1, float dy1)
{
	width = w;
	height = h;
		...
		for (unsigned i = 0; i < area; i++)
		{
			...
			for (short dimnr = 1; dimnr < dim; dimnr++)
			{
				...
				for (int ny = nymin; ny < nymax; ny++)
				{
					...
					{
						float valdim = (bits[dimnr][j] - centers[classnr][dimnr]) *
