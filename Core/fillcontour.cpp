...
{
	if (nrpoints < 3) return 0;
	float area = 0;
	for (unsigned int pos = 0; pos + 5 < nrpoints * 3; pos += 3) {
		area += (points[pos + 1] + points[pos + 4])*(points[pos + 3] - points[pos]);
	}
	area += (points[nrpoints * 3 - 2] + points[1])*(points[0] - points[nrpoints * 3 - 3]);
	return area / 2;
}
...
bool fillcontours::pointinpoly(float *pt, unsigned int cnt, float *polypts)
{
	...
	for (unsigned int i = 0; i < cnt; i++) { /* for each point in the polygon */
		thisptidx = 3 * i;
...
int fillcontours::whichquad(float *pt, float *orig)
{
	int quad;
	if (pt[0] < orig[0]) {
		if (pt[1] < orig[1]) quad = 2;
		else quad = 1;
	}
	else {
		if (pt[1] < orig[1]) quad = 3;
		else quad = 0;
	}
	return quad;
}

bool fillcontours::is_hole(float **points, unsigned int *nrpoints, unsigned int nrcontours, unsigned int contournr)
{
	int nrinside = 0;
	for (unsigned int i = 0; i < nrcontours; i++) {
		if (i != contournr) {
			if (pointinpoly(points[contournr], nrpoints[i], points[i])) nrinside++;
		}
	}
	return nrinside % 2;
}

void fillcontours::fill_contour(bool *array, unsigned short* pixel_extents, float* origin, float* pixel_size, float* direction_cosines, float **points, unsigned int *nrpoints, unsigned int nrcontours, bool clockwisefill)
{
	...
	for (unsigned int i = 0; i < nrcontours; i++) {
		...
		if (swap_x*swap_y < 0) clockwisefill = !clockwisefill;
		if (clockwisefill) {
			for (unsigned long pos = 0; pos + 3 < 3 * nrpoints[i]; pos += 3) {
				...
				if (y0 != y1) {
					...
					if (y0 > y1) {
						...
						if (end_h >= pixel_extents[1]) end_h = pixel_extents[1] - 1;
						for (unsigned short h = begin_h; h <= end_h; h++) {
							trans.pos = (x1 - swap_x*origin[0]) + tol + (pixel_size[1] * h + (swap_y*origin[1] - y1)) * slope;
						...
						if (end_h >= pixel_extents[1]) end_h = pixel_extents[1] - 1;
						for (unsigned short h = begin_h; h <= end_h; h++) {
							trans.pos = (x0 - swap_x*origin[0]) - tol + (pixel_size[1] * h + (swap_y*origin[1] - y0)) * slope;
							inouts.at(h).push_back(trans);
						}
					}
				}
		...
		for (unsigned short w = 0; w < pixel_extents[0]; w++) {
			while (it != inouts.at(h).end() && it->pos < w*pixel_size[0]) {
				...
				if (status && !it->to_inside) {
					it++;
					if (it == inouts.at(h).end() || it->to_inside) status = false;
				}
				else {
					status = it->to_inside;
					it++;
