...
void EM::init(short unsigned wi, short unsigned h, short nrclass, short dimension, float **bit, float *weight)
{
	width=wi;
	height=h;
	area=unsigned(wi)*h;
...
void EM::init(short unsigned wi, short unsigned h, short nrclass, short dimension, float **bit, float *weight, float *center, float *dev, float *ampl)
{
	width=wi;
	height=h;
	area=unsigned(wi)*h;
	...
	devs=(float *)malloc(sizeof(float)*nrclass);
	for(short i=0;i<nrclass;i++) devs[i]=dev[i];
	ampls=(float *)malloc(sizeof(float)*nrclass);
	for(short i=0;i<nrclass;i++) ampls[i]=ampl[i];
...
void EM::apply_to(float **sources, float *result_bits)
{
	...
	for(unsigned i=0;i<area;i++){
		...
		for(short n=0;n<dim;n++){
			dist+=(bits[n][i]-centers[n])*(bits[n][i]-centers[n])*weights[n];
		}
		...
		for(short l=1;l<nrclasses;l++){
			...
			if(wdummy>wmax) {wmax=wdummy; dummy=l;}
		}
...
//				cout << "y"<<centers[k+i*dim];
			...
			for(unsigned j=0;j<area;j++){
				for(short k=0;k<dim;k++){
					devs[i]+=w[j+area*i]*(bits[k][j]-centers[k+i*dim])*(bits[k][j]-centers[k+i*dim])*weights[k];
				}
			}
			devs[i]/=sw[i];
			if(devs[i]==0) devs[i]=devold;
//			cout << "z"<<devs[i]<<" ";
		}
	}
//	cout << endl;
...
{
	for(short j=0;j<nrclasses;j++) sw[j]=0;
	for(short j=0;j<nrclasses;j++){
		for(unsigned i=0;i<area;i++)w[i+area*j]=0;
...
//	for(short i=0;i<nrclasses;i++) devs[i]=float(width)/nrclasses;
	for(short i=0;i<nrclasses;i++) ampls[i]=1.0f/nrclasses;
...
//			cout << j << "-"<<centers[i+k*dim] << ":";
	...
	for(short i=0;i<nrclasses;i++) devs[i]=dist/(nrclasses*nrclasses);
...
void EM::init_centers(float *center,float *dev, float *ampl)
{
	...
	for(short i=0;i<nrclasses;i++) devs[i]=dev[i];
	for(short i=0;i<nrclasses;i++) ampls[i]=ampl[i];
	return;
}

float *EM::return_centers()
{
	return centers;
}

float *EM::return_devs()
{
	return devs;
}

float *EM::return_ampls()
{
	return ampls;
}
