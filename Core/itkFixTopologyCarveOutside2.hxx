...
template<class TInputImage, class TOutputImage>
void FixTopologyCarveOutside2<TInputImage, TOutputImage>::ComputeThinImage2()
{
	...
	public:
		...
		NeighborAccessor(value_type* buffer, const itk::Size<3>& dims)
				: m_Data(buffer)
		{
			std::ptrdiff_t stride_j = dims[0], stride_k = dims[0] * dims[1];
			// assume non-boundary 'id'
			m_Offsets[0] = -1 - stride_j - stride_k;
			m_Offsets[1] = -stride_j - stride_k;
			m_Offsets[2] = +1 - stride_j - stride_k;
			m_Offsets[3] = -1 - stride_k;
			m_Offsets[4] = -stride_k;
			m_Offsets[5] = +1 - stride_k;
			m_Offsets[6] = -1 + stride_j - stride_k;
			m_Offsets[7] = +stride_j - stride_k;
			m_Offsets[8] = +1 + stride_j - stride_k;

			m_Offsets[9] = -1 - stride_j;
			m_Offsets[10] = -stride_j;
			m_Offsets[11] = +1 - stride_j;
			m_Offsets[12] = -1;
			m_Offsets[13] = 0; // center is not neighbor
			m_Offsets[14] = +1;
			m_Offsets[15] = -1 + stride_j;
			m_Offsets[16] = +stride_j;
			m_Offsets[17] = +1 + stride_j;

			m_Offsets[18] = -1 - stride_j + stride_k;
			m_Offsets[19] = -stride_j + stride_k;
			m_Offsets[20] = +1 - stride_j + stride_k;
			m_Offsets[21] = -1 + stride_k;
			m_Offsets[22] = +stride_k;
			m_Offsets[23] = +1 + stride_k;
			m_Offsets[24] = -1 + stride_j + stride_k;
			m_Offsets[25] = +stride_j + stride_k;
			m_Offsets[26] = +1 + stride_j + stride_k;
		}

		// TODO: add test to make sure order is same as from a NeighborhoodIterator
		void GetNeighbors(size_t id, std::array<value_type, 27>& vals) const
		{
			for (size_t k = 0; k < 27; ++k)
			{
				vals[k] = m_Data[id + m_Offsets[k]];
			}
		}

		const std::array<std::ptrdiff_t, 27>& Offsets() { return m_Offsets; }

	private:
		std::array<std::ptrdiff_t, 27> m_Offsets;
	...
	// copy to output
	itk::ImageAlgorithm::Copy<TOutputImage, TOutputImage>(m_PaddedMask, thin_image, region, region);
}

template<class TInputImage, class TOutputImage>
void FixTopologyCarveOutside2<TInputImage, TOutputImage>::ComputeThinImage()
{
	InputImagePointer inputImage = dynamic_cast<const TInputImage*>(ProcessObject::GetInput(0));
	OutputImagePointer thinImage = GetThinning();
...
template<class TInputImage, class TOutputImage>
void FixTopologyCarveOutside2<TInputImage, TOutputImage>::GenerateData()
{
	this->PrepareData();

	this->ComputeThinImage2();
}

template<class TInputImage, class TOutputImage>
void FixTopologyCarveOutside2<TInputImage, TOutputImage>::FillEulerLUT(int* LUT)
{
	LUT[1] = 1;
	LUT[3] = -1;
	LUT[5] = -1;
...
template<class TInputImage, class TOutputImage>
bool FixTopologyCarveOutside2<TInputImage, TOutputImage>::IsEulerInvariant(NeighborhoodType neighbors, const int* LUT)
{
...
template<class TInputImage, class TOutputImage>
void FixTopologyCarveOutside2<TInputImage, TOutputImage>::OctreeLabeling(int octant, int label, int* cube)
{
...
template<class TInputImage, class TOutputImage>
void FixTopologyCarveOutside2<TInputImage, TOutputImage>::PrintSelf(std::ostream& os, Indent indent) const
{
