...
namespace GeometricPredicates
{
	...
	struct Segment
	{
		...
		// not exact.
		static int intersect(double a[3], double b[3], 
			double c[3], double d[3], double *x1 = 0, double *x2 = 0);
	};
	...
	struct Triangle
	{
		...
		static int intersect(double a[3], double b[3], double c[3], 
			double xa[3], double xe[3], double *pcoords = 0, double *x = 0);

		// Compute if a point is inside a 3d triangle (exact)
		static int inside(double a[3], double b[3], double c[3],
			double x[3]);

		// Compute if a 2d point is inside a 2d triangle (not exact)
		static int inside2d(double a[2], double b[2], double c[2],
			double x[2]);
		...
		static void project2d(double a[2], double b[2], double c[2], 
