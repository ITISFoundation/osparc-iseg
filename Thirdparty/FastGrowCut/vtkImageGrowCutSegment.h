...
#if defined(WIN32)
#if defined(FastGrowCut_EXPORTS)
#define FAST_GROW_CUT_API __declspec(dllexport)
#else
#define FAST_GROW_CUT_API __declspec(dllimport)
#endif
#else
#define FAST_GROW_CUT_API
#endif
...
class FAST_GROW_CUT_API vtkImageGrowCutSegment : public vtkImageAlgorithm
{
public:
  ...
  /// If this volume is specified then only those regions outside the mask (where mask has zero
  /// value) will be included in the segmentation result. Regions outside the mask will not be used
  ...
  /// This method has to be called if intensity volume changes or if seeds are deleted after initial
  /// computation.
  void Reset();
  ...
  /// By default = 0, which means spatial distance does not play a role in the region growing, only
