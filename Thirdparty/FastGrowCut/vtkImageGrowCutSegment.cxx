...
//----------------------------------------------------------------------------
// must match NodeKeyValueType, stores "distance" (difference in voxels)
static_assert(std::is_same<float, NodeKeyValueType>::value, "NodeKeyValueType changed");
const int NodeKeyValueTypeID = VTK_FLOAT;
...
public:
  ...
  bool ExecuteGrowCut(vtkImageData* intensityVolume, vtkImageData* seedLabelVolume,
  ...
  // If a point is set to DIST_INF then that point will modified, as a shorter distance path will be
  // found. If a point is set to DIST_EPSILON, then the distance is so small that a shorter path
  // will not be found and so the point will not be relabeled.
...
bool vtkImageGrowCutSegment::vtkInternal::InitializationAHP(
  vtkImageData* vtkNotUsed(intensityVolume), vtkImageData* seedLabelVolume,
  vtkImageData* maskLabelVolume, double distancePenalty)
{
  ...
  if ((m_HeapNodes = new FibHeapNode[dimXYZ + 1]) ==
    nullptr) // size is +1 for storing the zeroValueElement
  {
    vtkGenericWarningMacro(
      "Memory allocation failed. Dimensions: " << m_DimX << "x" << m_DimY << "x" << m_DimZ);
    return false;
  }
  ...
  if (!m_bSegInitialized)
  {
    ...
    LabelPixelType* resultLabelVolumePtr =
      static_cast<LabelPixelType*>(m_ResultLabelVolume->GetScalarPointer());
    NodeKeyValueType* distanceVolumePtr =
      static_cast<NodeKeyValueType*>(m_DistanceVolume->GetScalarPointer());
      ...
      for (long iy = -1; iy <= 1; iy++)
      {
        for (long iz = -1; iz <= 1; iz++)
        {
          ...
          m_NeighborDistancePenalties.push_back(this->m_DistancePenalty *
            sqrt((spacing[0] * ix) * (spacing[0] * ix) + (spacing[1] * iy) * (spacing[1] * iy) +
    ...
    const unsigned char numberOfNeighbors =
      static_cast<unsigned char>(m_NeighborIndexOffsets.size());
    ...
    for (NodeIndexType z = 0; z < m_DimZ; z++)
    {
      ...
      for (NodeIndexType y = 0; y < m_DimY; y++)
      {
        ...
        *(nbSizePtr++) =
          0; // x == 0 (there is always padding, so we don't need to check if m_DimX>0)
        unsigned char nbSize = (zEdge || yEdge) ? 0 : numberOfNeighbors;
        for (NodeIndexType x = m_DimX - 2; x > 0; x--)
        {
          *(nbSizePtr++) = nbSize;
        }
        *(nbSizePtr++) = 0; // x == m_DimX-1 (there is always padding, so we don'neighborNewDistance
                            // need to check if m_DimX>1)
      }
    }
  ...
  else
  {
    ...
    for (NodeIndexType index = 0; index < dimXYZ; index++)
    {
      if (seedLabelVolumePtr[index] != 0)
      {
        ...
        // Old seeds will be completely ignored in updates, as their labels have been already
        // propagated and their value cannot changed (because their value is prescribed).
      }
      else
      {
        m_HeapNodes[index] = DIST_INF;
...
void vtkImageGrowCutSegment::vtkInternal::DijkstraBasedClassificationAHP(
  vtkImageData* intensityVolume, vtkImageData* vtkNotUsed(seedLabelVolume),
  vtkImageData* vtkNotUsed(maskLabelVolume))
{
  ...
  else
  {
    ...
    while (!m_Heap->IsEmpty())
    {
      ...
      // Stop if minimum value is infinite (it means there are no more voxels to propagate labels
      // from)
...
bool vtkImageGrowCutSegment::vtkInternal::ExecuteGrowCut2(vtkImageData* intensityVolume,
  vtkImageData* seedLabelVolume, vtkImageData* maskLabelVolume, double distancePenalty)
{
  vtkIdType numberOfVoxels = intensityVolume->GetNumberOfPoints();
  vtkIdType maxNumberOfVoxels = std::numeric_limits<NodeIndexType>::max();
  if (numberOfVoxels >= maxNumberOfVoxels)
  {
    // we use unsigned int as index type to reduce memory usage, which limits number of voxels to
    // 2^32, but this is not a practical limitation, as images containing more than 2^32 voxels
    // would take too much time an memory to grow-cut anyway
    vtkGenericWarningMacro("vtkImageGrowCutSegment: image size is too large ("
  ...
  int imSize[3];
  intensityVolume->GetDimensions(imSize);
  ...
  if (m_DimX <= 2 || m_DimY <= 2 || m_DimZ <= 2)
  {
    ...
    vtkGenericWarningMacro(
      "vtkImageGrowCutSegment: image size is too small. Minimum size along each dimension is 3.");
    return false;
  }
  ...
  DijkstraBasedClassificationAHP<IntensityPixelType, LabelPixelType>(
    intensityVolume, seedLabelVolume, maskLabelVolume);
  return true;
}
...
bool vtkImageGrowCutSegment::vtkInternal::ExecuteGrowCut(vtkImageData* intensityVolume,
  vtkImageData* seedLabelVolume, vtkImageData* maskLabelVolume, vtkImageData* resultLabelVolume,
  double distancePenalty)
{
  ...
  {
    vtkGenericWarningMacro("vtkImageGrowCutSegment: Seed label volume geometry does not match "
                           "intensity volume geometry");
    return false;
  }
  ...
  if (maskLabelVolume)
  {
    ...
    {
      vtkGenericWarningMacro("vtkImageGrowCutSegment: Mask label volume geometry does not match "
                             "intensity volume geometry");
      return false;
    }
    if (maskLabelVolume->GetScalarType() != MaskPixelTypeID ||
      maskLabelVolume->GetNumberOfScalarComponents() != 1)
    {
      vtkGenericWarningMacro(
        "vtkImageGrowCutSegment: Mask label volume scalar must be single-component unsigned char");
      return false;
    }
  }
