...
/** @cond */
template Size GC_DLL_EXPORT InitialEstimate(const System::Collection::Array<2, Float32> & im, Size max_iter,
                                            Params<Float32> & pm);
template Size GC_DLL_EXPORT InitialEstimate(const System::Collection::Array<2, Float64> & im, Size max_iter,
                                            Params<Float64> & pm);
template Size GC_DLL_EXPORT InitialEstimate(const System::Collection::Array<3, Float32> & im, Size max_iter,
                                            Params<Float32> & pm);
template Size GC_DLL_EXPORT InitialEstimate(const System::Collection::Array<3, Float64> & im, Size max_iter,
                                            Params<Float64> & pm);
/** @endcond */
          ...
          Flow::IGridMaxFlow<N, T, T, T> & mf, System::Collection::Array<N, bool> & seg)
{
    ...
    while (iter < max_iter)
    {
        ...
        if (Math::Abs(opm.m_c1 - pm.m_c1) + Math::Abs(opm.m_c2 - pm.m_c2) +
                Math::Abs(opm.m_v1 - pm.m_v1) + Math::Abs(opm.m_v2 - pm.m_v2) <=
            conv_crit)
        {
            break;
        }
...
} // namespace Gc::Algo::Segmentation::RoussonDeriche
