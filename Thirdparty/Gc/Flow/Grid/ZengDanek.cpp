...
template <Size N, class TFLOW, class TTCAP, class TCAP, bool MASK>
void ZengDanek<N, TFLOW, TTCAP, TCAP, MASK>::SetInitialLabelingRef(const System::Collection::Array<N, bool> & ilab)
{
    m_ilab = &ilab;
    ...
    else if (N == 3)
    {
        ...
        if (!p[0] || !p[1] || !p[2] || p[0] == sz[0] - 1 || p[1] == sz[1] - 1 || p[2] == sz[2] - 1)
        {
            ...
            if (p[2] > 0)
            {
                ...
                {
                    if (p[0] > 0)
                        val += (*(lp - srd[2] + srd[1] - 1)) << 6;
                    val += (*(lp - srd[2] + srd[1])) << 7;
                    if (p[0] < sz[0] - 1)
                        val += (*(lp - srd[2] + srd[1] + 1)) << 8;
                }
            }
            ...
            if (p[2] < sz[2] - 1)
            {
                ...
                {
                    if (p[0] > 0)
                        val += (*(lp + srd[2] - srd[1] - 1)) << 17;
                    val += (*(lp + srd[2] - srd[1])) << 18;
                    if (p[0] < sz[0] - 1)
                        val += (*(lp + srd[2] - srd[1] + 1)) << 19;
                }
                ...
                {
                    if (p[0] > 0)
                        val += (*(lp + srd[2] + srd[1] - 1)) << 23;
                    val += (*(lp + srd[2] + srd[1])) << 24;
                    if (p[0] < sz[0] - 1)
                        val += (*(lp + srd[2] + srd[1] + 1)) << 25;
                }
            }
        }
        else
