...
TFLOW HighestLevel<N, TFLOW, TCAP, MASK>::FindMaxFlow()
{
    ...
    while (m_max_active_rank >= m_min_active_rank)
    {
        ...
        while (node->m_rank < m_nodes)
        {
            ...
            if (lay->m_first_active == nullptr && lay->m_first_inactive == nullptr)
            {
                // Gap relabeling
...
void HighestLevel<N, TFLOW, TCAP, MASK>::GapRelabeling(Layer * empty_lay, Size next_layer_rank)
{
    ...
    {
        for (node = lay->m_first_inactive; node != nullptr; node = node->m_lay_next)
        {
            node->m_rank = m_nodes;
        }

        lay->m_first_inactive = nullptr;
    }
...
void HighestLevel<N, TFLOW, TCAP, MASK>::GlobalUpdate()
{
    ...
    {
        m_layer_list[i].m_first_active = nullptr;
        m_layer_list[i].m_first_inactive = nullptr;
    }
    ...
    for (; lay->m_first_active != nullptr || lay->m_first_inactive != nullptr; lay++)
    {
        m_max_rank++;

        // Check list of active nodes on this layer
        for (node = lay->m_first_active; node != nullptr; node = node->m_lay_next)
        {
