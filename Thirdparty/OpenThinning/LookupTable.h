...
public:
  ...
  inline bool
  readFile(const std::string & _filename);
  inline bool
  writeFile(const std::string & _filename) const;

  // Get the stored lookup table entry. The value of the middle voxel is ignored.
  template <typename Voxel>
  Entry
  getEntry(const Voxel _neighborhood[27]) const;
...
bool
LookupTable::readFile(const std::string & _filename)
{
  // Open the file
...
bool
LookupTable::writeFile(const std::string & _filename) const
{
  // Open the file
  ...
  for (int byteIdx = 0; byteIdx < (1 << 23); ++byteIdx)
  {
    ...
    unsigned char byte = (m_entries[8 * byteIdx + 0] << 7) | (m_entries[8 * byteIdx + 1] << 6) |
                         (m_entries[8 * byteIdx + 2] << 5) | (m_entries[8 * byteIdx + 3] << 4) |
                         (m_entries[8 * byteIdx + 4] << 3) | (m_entries[8 * byteIdx + 5] << 2) |
                         (m_entries[8 * byteIdx + 6] << 1) | (m_entries[8 * byteIdx + 7] << 0);
...
template <typename Voxel>
LookupTable::Entry
LookupTable::getEntry(const Voxel _neighborhood[27]) const
{
  // Get the index in the lookup table
  int entryIdx = ((_neighborhood[0] << 0) | (_neighborhood[1] << 1) | (_neighborhood[2] << 2) |
                  ...
                  (_neighborhood[12] << 12) | /* ignored */ (_neighborhood[14] << 13) | (_neighborhood[15] << 14) |
                  (_neighborhood[16] << 15) | (_neighborhood[17] << 16) |
