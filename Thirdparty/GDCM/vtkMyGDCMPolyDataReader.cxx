...
//----------------------------------------------------------------------------
int vtkMyGDCMPolyDataReader::RequestData_RTStructureSetStorage(
  gdcm::Reader const& reader, vtkInformationVector* outputVector)
{
  ...
  for (unsigned int pd = 0; pd < sqi->GetNumberOfItems(); ++pd)
  {
    ...
    for (unsigned int i = 0; i < nitems; ++i)
    {
      ...
      // (3006,0050) DS [43.57636\65.52504\-10.0\46.043102\62.564945\-10.0\49.126537\60.714... #
      // 398,48 ContourData
      ...
      scalars->InsertTuple3(
        cellId, (double)color[0] / 255.0, (double)color[1] / 255.0, (double)color[2] / 255.0);
    }
...
//----------------------------------------------------------------------------
int vtkMyGDCMPolyDataReader::RequestData_HemodynamicWaveformStorage(
  gdcm::Reader const& reader, vtkInformationVector* outputVector)
{
  const gdcm::DataSet& ds = reader.GetFile().GetDataSet();
  ...
  // (5400,1010) OW 00ba\0030\ff76\ff8b\00a2\ffd3\ffae\ff50\0062\00c4\011e\00c2\00ba... # 57600, 1
  // WaveformData
...
//----------------------------------------------------------------------------
int vtkMyGDCMPolyDataReader::RequestData(vtkInformation* vtkNotUsed(request),
...
//----------------------------------------------------------------------------
int vtkMyGDCMPolyDataReader::RequestInformation_HemodynamicWaveformStorage(
...
//----------------------------------------------------------------------------
int vtkMyGDCMPolyDataReader::RequestInformation(vtkInformation* vtkNotUsed(request),
  vtkInformationVector** vtkNotUsed(inputVector), vtkInformationVector* outputVector)
{
  gdcm::Reader reader;
  reader.SetFileName(this->FileName);
...
int gdcmvtk_rtstruct::RequestData_RTStructureSetStorage(const char* filename, tissuevec& tissues)
{
  ...
  for (unsigned int pd = 0; pd < sqi->GetNumberOfItems(); ++pd)
  {
    ...
    unsigned int nitems = (sqi2 == nullptr) ? 0 : static_cast<unsigned>(sqi2->GetNumberOfItems());
  ...
  // Sort the list of tissues giving to the smallest tissue the highest priority and to the biggest
  // tissue the lowest
...
//----------------------------------------------------------------------------
bool gdcmvtk_rtstruct::GetDicomUsingGDCM(
...
//----------------------------------------------------------------------------
bool gdcmvtk_rtstruct::GetDicomUsingGDCM(
  const char* filename, float* bits, unsigned short& w, unsigned short& h, unsigned short& nrslices)
{
  auto reader = vtkSmartPointer<vtkGDCMImageReader>::New();
  if (reader->CanReadFile(filename) == 0)
  {
...
//----------------------------------------------------------------------------
bool gdcmvtk_rtstruct::GetSizeUsingGDCM(const char* filename, unsigned short& w, unsigned short& h,
...
//----------------------------------------------------------------------------
bool gdcmvtk_rtstruct::GetSizeUsingGDCM(const std::vector<std::string>& filenames,
  unsigned short& w, unsigned short& h, unsigned short& nrslices, float& dx, float& dy, float& dz,
  float* disp, float* c1, float* c2, float* c3)
{
  auto reader = vtkSmartPointer<vtkGDCMImageReader>::New();
