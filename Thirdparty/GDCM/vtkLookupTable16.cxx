...
inline unsigned short* vtkLinearLookup16(
  double v, unsigned short* table, double maxIndex, double shift, double scale)
{
...
inline double vtkApplyLogScale16(double, double[2], double[2])
{
  assert(0);
  return 0;
}
...
template <class T>
void vtkLookupTable16MapData(
  vtkLookupTable16* self, T* input, unsigned short* output, int length, int inIncr, int outFormat)
{
  int i = length;
    ...
    if (self->GetScale() == VTK_SCALE_LOG10)
    {
      ...
      else if (outFormat == VTK_LUMINANCE_ALPHA)
      {
        while (--i >= 0)
        {
          ...
          cptr = vtkLinearLookup16(val, table, maxIndex, shift, scale);
          *output++ =
            static_cast<unsigned short>(cptr[0] * 0.30 + cptr[1] * 0.59 + cptr[2] * 0.11 + 0.5);
          *output++ = cptr[3];
          input += inIncr;
        }
      }
    ...
    if (self->GetScale() == VTK_SCALE_LOG10)
    {
      ...
      if (outFormat == VTK_RGBA)
      {
        while (--i >= 0)
        {
          ...
          *output++ = *cptr++;
          *output++ = *cptr++;
          *output++ = static_cast<unsigned short>((*cptr) * alpha);
          cptr++;
          input += inIncr;
        }
      }
...
//----------------------------------------------------------------------------
void vtkLookupTable16::MapScalarsThroughTable2(void* input, unsigned char* output,
  int inputDataType, int numberOfValues, int inputIncrement, int outputFormat)
{
  ...
  switch (inputDataType)
  {
    ...
    break;
      ...
      vtkTemplateMacro(vtkLookupTable16MapData(this, static_cast<VTK_TT*>(input),
        (unsigned short*)output, numberOfValues, inputIncrement, outputFormat));
    default:
      vtkErrorMacro(<< "MapImageThroughTable: Unknown input ScalarType");
      return;
  }
