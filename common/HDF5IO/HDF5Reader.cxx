...
void HDF5Reader::decompose(string &filename, string &dataname, const string &pathname)
{
...
vector<string> HDF5Reader::getGroupInfo(const string &name)
{
	...
	if(loud) cerr << "HDF5Reader::groupInfo : idx_f = " << idx_f << endl;
	return names;
}
...
bool HDF5Reader::existsValidHdf5(const string &fname)
{
	...
	if(!exists)
	{
		cerr << "HDF5Reader::existsValidHdf5() : error, '" << fname << "' does not exist" << endl;
		return 0;
	}
#else
...
int HDF5Reader::open(const string &fname)
{
	...
	{
		cerr << "HDF5Reader::open() : " << fname << " does not exist or is not a valid hdf5 file" << endl;
		return 0;
	}
...
int HDF5Reader::open(const char *fn)
{
	return this->open(string(fn));
}
...
int HDF5Reader::getDatasetInfo(string &type, vector<size_type> &extents, const string &name)
{
	if(file < 0)
	{
	...
	if(loud) cerr << "HDF5Reader::getDatasetInfo() : checking dataset " << name << endl;
	hid_t dataset = H5Dopen2(file, name.c_str(), H5P_DEFAULT);
	if(dataset < 0)
	{
		if(loud) cerr << "HDF5Reader::getDatasetInfo() : no such dataset: " << name << endl;
		type = "none";
		return 0;
	}
...
// 	}
	...
	{
		type = "double";
		if(loud) cerr << "HDF5Reader::getDatasetInfo() : found double" << endl;
	}
	else if(H5Tequal(native_type, H5T_NATIVE_FLOAT) > 0)
	{
		type = "float";
		if(loud) cerr << "HDF5Reader::getDatasetInfo() : found float" << endl;
	}
	else if(H5Tequal(native_type, H5T_NATIVE_INT) > 0)
	{
		type = "int";
		if(loud) cerr << "HDF5Reader::getDatasetInfo() : found int" << endl;
	}
	else if(H5Tequal(native_type, H5T_NATIVE_UINT) > 0)
	{
		type = "unsigned int";
		if(loud) cerr << "HDF5Reader::getDatasetInfo() : found unsigned int" << endl;
	}
	...
	{
		type = "unsigned char";
		if(loud) cerr << "HDF5Reader::getDatasetInfo() : found unsigned char" << endl;
	}
	else if(H5Tequal(native_type, H5T_NATIVE_USHORT) > 0)
	{
		type = "unsigned short";
		if(loud) cerr << "HDF5Reader::getDatasetInfo() : found unsigned short" << endl;
	}
	else
	{
		cerr << "HDF5Reader::getDatasetInfo() : warning, unsupported native type" << endl;
	...
	if(loud) cerr << "HDF5Reader::getDatasetInfo() : rank = " << rank << endl;
	assert(rank > 0);
...
//	hsize_t dims[rank];
	...
	{
		extents[i] = dims[i];
		if(loud) cerr << "HDF5Reader::getDatasetInfo() : extents = " << extents[i] << endl;
	}
...
int HDF5::HDF5Reader::read(float* data, size_type offset, size_type length, const std::string& name)
{
	return HDF5IO().readData(file, name, offset, length, data) ? 1 : 0;
}

int HDF5::HDF5Reader::read(unsigned short* data, size_type offset, size_type length, const std::string& name)
{
	return HDF5IO().readData(file, name, offset, length, data) ? 1 : 0;
}
...
int HDF5Reader::readData(const string &name)
{
	...
	if(loud) cerr << "HDF5Reader::readData() : loading dataset " << name << endl;
	...
	int rank = H5Sget_simple_extent_ndims(dataspace);
	std::vector<hsize_t> dims(rank);
	if(loud) cerr << "HDF5Reader::readData() : rank = " << rank << endl;
	if(rank <= 0)
		error("invalid rank");
	status = H5Sget_simple_extent_dims(dataspace, dims.data(), NULL);
...
// 	if(elclass == H5T_FLOAT && elsize==8)
	if(H5Tequal(native_type, H5T_NATIVE_DOUBLE) > 0)
	{
		if(loud) cerr << "HDF5Reader::readData() : double format detected" << endl;
		status = H5Dread(dataset, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, (double *)data);
	}
// 	else if(elclass == H5T_INTEGER && elsize==4)
	else if(H5Tequal(native_type, H5T_NATIVE_FLOAT) > 0)
	{
		if(loud) cerr << "HDF5Reader::readData() : float format detected" << endl;
		status = H5Dread(dataset, H5T_NATIVE_FLOAT, H5S_ALL, H5S_ALL, H5P_DEFAULT, (float *)data);
	}
	else if(H5Tequal(native_type, H5T_NATIVE_INT) > 0)
	{
		if(loud) cerr << "HDF5Reader::readData() : int format detected" << endl;
		status = H5Dread(dataset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, (int *)data);
	}
	else if(H5Tequal(native_type, H5T_NATIVE_UINT) > 0)
	{
		if(loud) cerr << "HDF5Reader::readData() : unsigned int format detected" << endl;
		status = H5Dread(dataset, H5T_NATIVE_UINT, H5S_ALL, H5S_ALL, H5P_DEFAULT, (unsigned int *)data);
	}
// 	else if(elclass == H5T_INTEGER && elsize==1)
	else if(H5Tequal(native_type, H5T_NATIVE_LONG) > 0)
	{
		if(loud) cerr << "HDF5Reader::readData() : long format detected" << endl;
		status = H5Dread(dataset, H5T_NATIVE_LONG, H5S_ALL, H5S_ALL, H5P_DEFAULT, (long *)data);
	}
	else if(H5Tequal(native_type, H5T_NATIVE_UCHAR) > 0)
	{
		if(loud) cerr << "HDF5Reader::readData() : unsigned char format detected" << endl;
		status = H5Dread(dataset, H5T_NATIVE_UCHAR, H5S_ALL, H5S_ALL, H5P_DEFAULT, (unsigned char *)data);
	}
	else if(H5Tequal(native_type, H5T_NATIVE_USHORT) > 0)
	{
		if(loud) cerr << "HDF5Reader::readData() : unsigned short format detected" << endl;
		status = H5Dread(dataset, H5T_NATIVE_USHORT, H5S_ALL, H5S_ALL, H5P_DEFAULT, (unsigned short *)data);
	}
