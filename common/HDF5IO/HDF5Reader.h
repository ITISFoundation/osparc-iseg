...
public:
	...
	int getDatasetInfo(std::string&, std::vector<size_type>&, const std::string&);
	...
	int read(float* data, size_type offset, size_type length, const std::string& name);
	int read(unsigned short* data, size_type offset, size_type length, const std::string& name);

	template <class T> static int read2(std::vector<T>& array, const std::string& path)
	{
		std::string filename;
		std::string dataset;
