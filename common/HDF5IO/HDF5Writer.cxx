...
int HDF5Writer::open(const std::string &fname, const std::string &fileMode)
{
	if (loud)
	{
		std::cerr << "HDF5Writer::open() : opening file " << fname << " in " << fileMode << " mode" << std::endl;
	}
	if (file >= 0)
	{
		std::cerr << "HDF5Writer::open() : error, file " << fname << " is already open, close first" << std::endl;
		return 0;
	}
	...
	if (file < 0)
	{
		std::cerr << "HDF5Writer::write() : error, no files were opened" << std::endl;
		return 0;
	}
	else
	{
		if (loud)
		{
			std::cerr << "HDF5Writer::write() : created file id = " << file << std::endl;
		}
		return 1;
	}
}
...
int HDF5Writer::close()
{
	if (file >= 0)
	{
		if (loud)
		{
			std::cerr << "HDF5Writer::close() : closing file id " << file << std::endl;
		}
		herr_t status = H5Fclose(file);
		if (status < 0)
		{
			std::cerr << "HDF5Writer::close() : error, closing file failed" << std::endl;
			return 0;
		}
		file = -1;
	}
...
int HDF5Writer::createGroup(const std::string &gname)
{
	...
	hid_t group = H5Gcreate2(file, gname.c_str(), H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
	if (group < 0)
	{
		std::cerr << "HDF5Writer::createGroup() : creating group failed" << std::endl;
		return 0;
	}
	herr_t status = H5Gclose(group);
	if (status < 0)
	{
		std::cerr << "HDF5Writer::createGroup() : closing group failed" << std::endl;
		return 0;
	}
	return 1;
}
...
int HDF5::HDF5Writer::write(float** const slice_data, size_type num_slices, size_type slice_size, const std::string& name, size_t offset)
{
	return HDF5IO(compression).writeData(file, name, slice_data, num_slices, slice_size, offset) ? 1 : 0;
}

int HDF5::HDF5Writer::write(unsigned short** const slice_data, size_type num_slices, size_type slice_size, const std::string& name, size_t offset)
{
	return HDF5IO(compression).writeData(file, name, slice_data, num_slices, slice_size, offset) ? 1 : 0;
}
...
int HDF5Writer::write_attribute(const std::string &value, const std::string &name)
{
	int result = 0;
	...
	if (group_andle >= 0)
	{
		...
		if (dataspace_id >= 0)
		{
			...
			herr_t ret = H5Tset_size(tid1, value.length() + 1); &ret;
...
int HDF5Writer::writeData(const void *data, const std::string &type, const std::vector<size_type> &extents, const std::string &name)const
{
	...
	{
		std::cerr << "HDF5Writer::write() : error, unsupported data type" << std::endl;
	...
	for (int j = 0; j < rank; j++)
	{
		if (loud)
		{
			std::cerr << "HDF5Writer::write() extents[" << j << "] = " << extents[j] << std::endl;
		}
	...
	if (compression)
	{
		...
		if (!chunkSize.empty())
		{
			...
			for (int j = 0; j < rank; j++)
			{
				...
				{
					if (loud) std::cerr << "HDF5Writer::write() : invalid chunk size" << std::endl;
					chunk_ok = false;
				}
				cdims[j] = chunkSize[j];
			}
		...
		if (!chunk_ok)
		{
			if (loud)
			{
				std::cerr << "HDF5Writer::write() : using automatic chunk size with buffer size " << bufsize << std::endl;
			}
			for (int j = 0; j < rank; j++)
			{
	...
	if (dataspace >= 0)
	{
		...
		else if (dataset >= 0 && data != 0)
		{
			if (loud)
			{
				std::cerr << "HDF5Writer : writing " << name << " of type " << type << std::endl;
			}
			...
			if (status < 0)
			{
				std::cerr << "HDF5Writer::write() : writing dataset failed" << std::endl;
				ok = 0;
			}
			else if (loud)
			{
				std::cerr << "HDF5Writer::write() : " << name << " writen succesfully" << std::endl;
			}
