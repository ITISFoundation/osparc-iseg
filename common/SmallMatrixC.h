...
#ifndef MATH_SMALL_MATRIXC_H
#define MATH_SMALL_MATRIXC_H
...
namespace Math
{
	template <typename T, size_t NR, size_t NC> class SmallMatrixC
	{
		public:
			// this assumes row-wise ordering
			...
			template <typename SmallMatrixC1, typename SmallMatrixC2> void matrixProduct(const SmallMatrixC1& A, const SmallMatrixC2& B, const double factor = 1.0)
			{
...
*/
			template <typename SmallMatrixC1, typename SmallMatrixC2> void addScaledMatrixProduct(const SmallMatrixC1& A, const SmallMatrixC2& B, const double factor=1.0)
			{
				//assert(A.rows()==B.columns());
				assert(A.columns()==B.rows());
				assert(NR==A.rows());
...
//			}
			...
			void addScaled(const SmallMatrixC& M, const double factor=1.0)
			{
				assert(M.rows()==NR);
				assert(M.columns()==NC);
...
/*
			template <typename SmallMatrixC> void insert(const SmallMatrixC& M1, const int posrow, const int poscol)
			{
				assert(M1.rows()<=NR);
...
*/
			template <typename SmallMatrixC> void transposeTo(SmallMatrixC& M)
			{
				assert(M.rows()==NC);
				assert(M.columns()==NR);
				for(int c=1; c<=NC; ++c)
			...
			friend std::ostream& operator << (std::ostream& os, const SmallMatrixC& x)
			{
			...
			// row-wise
			friend std::istream& operator >> (std::istream& is, SmallMatrixC& x)
			{
				for(int i=1; i<=x.rows(); i++)
				{
					for(int j=1; j<=x.columns(); j++)
