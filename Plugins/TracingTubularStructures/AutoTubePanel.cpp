...
void AutoTubePanel::add_to_tissues()
{
    ...
    {
        if (k_filters.size() > 0 and max_active_slice_reached == _handler3D->num_slices() - 1)
        {
...
{
    if (!label_maps.empty() and label_maps[_handler3D->active_slice()])
        visualize_label_map(label_maps[_handler3D->active_slice()]);
    else
    {
        QMessageBox mBox;
...
void AutoTubePanel::extrapolate_results()
{
    ...
    if (limit_slice <= _handler3D->active_slice() or limit_slice >= _handler3D->num_slices())
    ...
    if (k_filters.size() > 0)
    {
        ...
        for (unsigned int index(_handler3D->active_slice()); index < limit_slice -1 ; index ++)
        {
            ...
            if (centroids->empty() or extrapolated_labelMap->GetNumberOfLabelObjects() == 0)
            {
                quit = true;
            ...
            for(unsigned int i = 0; i <   extrapolated_labelMap->GetNumberOfLabelObjects(); i++)
            {
                ...
                for(unsigned int j = 0; j <  labelMap->GetNumberOfLabelObjects(); j++)
                {
                    ...
                    if (!_extrapolate_only_matches->isChecked() and !set)
...
bool AutoTubePanel::is_number(const std::string& s)
{
...
void AutoTubePanel::refresh_object_list()
{
    object_list->clear();
    std::map<LabelType, std::string> l_to_t;
    if (!objects.empty() and objects[_handler3D->active_slice()].size() > 0)
    {
        for (unsigned int i(0) ; i < objects[_handler3D->active_slice()].size(); i++ )
        {
...
void AutoTubePanel::refresh_probability_list()
{
    ...
    if (_handler3D->active_slice() != 0)
    {
        if (!_probabilities.empty() and _probabilities[_handler3D->active_slice()].size() > 0)
        {
...
void AutoTubePanel::on_slicenr_changed()
{
    ...
    if (objects.empty() or objects.size() != _handler3D->num_slices())

        objects = std::vector<std::vector<std::string>>(_handler3D->num_slices());
    
    // initiating label maps
    if (label_maps.empty() or label_maps.size() != _handler3D->num_slices())
        label_maps = std::vector<LabelMapType::Pointer>(_handler3D->num_slices());
    
    if (label_to_text.empty() or label_to_text.size() != _handler3D->num_slices())
        label_to_text = std::vector<std::map<LabelType, std::string>>(_handler3D->num_slices());
    
     if (_probabilities.empty() or _probabilities.size() != _handler3D->num_slices())
     {
